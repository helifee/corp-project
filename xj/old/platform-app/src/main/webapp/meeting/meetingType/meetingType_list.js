...
function getProjectTypeData(){
	...
	// jQuery("#list2").jqGrid('clearGridData');
	   jQuery("#list2").jqGrid({
		    url: hostUrl+'oa/meeting/meetingType/getTypetree',
...
var prev=	$("#"+ids).prev().attr("id");
	if(ids&&ids!='') {
		  pop_text_open("blue",'确认要删除该会议类型及该类型下面的所有类型吗？',function(){
				$.ajax({
					url:hostUrl+"oa/meeting/meetingType/deleteBatch/"+ids,
					type:'DELETE',
					dataType:'JSON',
					success:function (resultData ) {
...
function  updateChildStatusByParentId(e){
    ...
    paramData = JSON.stringify({'parentId':ids,'all':'all'});
		$.ajax({
			type:'post',
			url:hostUrl+'oa/meeting/meetingType/queryList'+"?time="+Math.random(),
			dataType:'json',
			contentType:'application/json',
			data:paramData,
			async: false,
						...
						if(childs.length > 0){
							...
							if(ids&&ids!='') {
								  pop_text_open("blue",'确认要启用该子级下面的会议类别吗吗？',function(){
									  //如果是，则要启用下面的子级会议类别
									  $.ajax({
											url:hostUrl+"oa/meeting/meetingType/updateChildStatusByParentId/"+ids,
											type:'PUT',
											dataType:'JSON',
											async:false,
											contentType:"application/json",
...
function  updateStatus(e){
	...
	$.ajax({
		url:hostUrl+"oa/meeting/meetingType/updateStatus/"+rowid,
		type:'PUT',
		dataType:'JSON',
		async:false,
		contentType:"application/json",
