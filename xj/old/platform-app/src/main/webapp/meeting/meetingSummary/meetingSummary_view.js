...
function chnageStatusOfMsg(){
    ...
    var fullUrl = baseUrl+"flow/sysNoticeMsg/updateStatusOfNoticeMsgByCurrentUser";
    $.ajax({ //发送更新的ajax请求
        type: "post",
        url: fullUrl,
        dataType: "json",
...
function submitForm(type, flag) {
	...
	if(type == 'update') {
    	...
    	meetingDto.id=id;
    	url = baseUrl + "oa/meeting/meetingSummary/update/"+id;
	}else {
		meetingDto.delflag=false;
...
function saveMeetingReploy(flag) {
	...
	$.ajax({
		type: "POST",
		contentType: "application/json",
		url: baseUrl+"oa/meeting/meetingReply/save",
...
function operationMeeting(flag) {
	...
	$.ajax({
		type: "POST",
		contentType: "application/json",
		url: baseUrl+"oa/meeting/meeting/operationMeeting",
		data:JSON.stringify(meetingReloyDto),
		dataType:"JSON",
		success: function (result) {
			if(result.success) {	//新增
