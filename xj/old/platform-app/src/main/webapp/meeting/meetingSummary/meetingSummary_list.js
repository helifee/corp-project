...
$(function(){
	...
	jQuery('#allListRowgrid').jqGrid({
	      url: hostUrl + "oa/meeting/meetingSummary/page"+"?time="+Math.random(),
	      ajaxGridOptions: { 
	...
	$(".deleteMeeting").click(function() {
        ...
        if (gr == "" || gr == null) {
            ...
            $.xljUtils.confirm("blue", tipText, function () {
                if (gr && gr != '') {
                    $.ajax({
                        url: baseUrl + "oa/meeting/meetingSummary/deleteBatch/" + gr.join(','),
                        type: 'DELETE',
                        dataType: 'JSON',
...
function trackFlow(){
	...
	if(row.status=='草稿'){
		...
		var pcUrl = "/platform-app/meeting/meetingSummary/meetingSummary_flow.html";
