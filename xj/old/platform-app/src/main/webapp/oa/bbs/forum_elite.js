...
function getCustomTree() {
    $.ajax({
        type:'POST',
        url:baseUrl + "oa/bbs/forumType/getHomePageTree",
        dataType:'json',
...
function toAddFavorite(){
	...
	if(ids&&ids!='') {
		$.ajax({
			url:baseUrl+"oa/bbs/topic/addFavorite/"+ids,
			type:'post',
			dataType:'json',
	        contentType:'application/json',
...
function toDelete(){
	...
	if(ids&&ids!='') {
		pop_text_open("blue", "确认要删除这"+delCount+"条数据吗？",function(){
            $.ajax({
                url:baseUrl+"oa/bbs/topic/deletePseudoBatch/"+ids,
                type:'DELETE',
                dataType:'JSON',
                contentType:'application/json',
                data:JSON.stringify({}),
...
function getOperationAuthorition() {
	...
	$.ajax({
		type: 'GET',
		url: hostUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=jhq',
		dataType: 'json',
		//contentType: 'application/json',
		async: false,
