...
function serverCreateEdit(serveyCreateFormType){
	...
	if((serveyCreateFormType=='directConclusion'||serveyCreateFormType=='over')&&data.length==0){
	...
	var type = '';
	if(urlParamJson.act=='create'){
		url = hostUrl+'oa/servey/serveyCreate/createServey';
		type = 'POST';
	}else if(urlParamJson.act=='update'){
		var idVal = $("#id").val();
		url = hostUrl+'oa/servey/serveyCreate/update/'+idVal;
		type = 'put';
	}
...
function initserveyQuestionType(){
	$.ajax({
		url : hostUrl+'oa/servey/serveyQuestionType/queryList',
		data : "{}",
		type : "POST",
		contentType : 'application/json',
		dataType : 'JSON',
...
function initUUID() {
	$.ajax({
        type:"GET",
        url:hostUrl+"oa/content/contentChild/getGuuid?time="+Math.random(),
        dataType:"json",
...
function initServeyTableData(idVal){
	var initServeyGrid = jQuery("#serveyCreateAdd").jqGrid({
        url: hostUrl + 'oa/servey/serveyCreate/queryServeyQuestionList',
...
function questionList(id){
	 ...
	 var categoryGrid = jQuery("#serveyCreatePageTable").jqGrid({
	        url: hostUrl + 'oa/servey/serveyQuestion/page',
	        ajaxGridOptions: {contentType: 'application/json'},
	        mtype: "POST",
	        contentType: "application/json",
	        datatype: "json",
...
function initCategoryList() {
    ...
    var categoryGrid = jQuery("#serveyCreateShowTable").jqGrid({
        url: hostUrl + 'oa/servey/page?time='+Math.random(),
        ajaxGridOptions: {contentType: 'application/json'},
        mtype: "POST",
        contentType: "application/json",
        datatype: "json",
...
function getQuestionById1(idVal){
	var result = "";
	$.ajax({
	    url:hostUrl+'oa/servey/serveyQuestion/get/'+idVal,
	    type:'GET',
	    dataType:'JSON',
	    async: false,
	    success:function (resultData) {
