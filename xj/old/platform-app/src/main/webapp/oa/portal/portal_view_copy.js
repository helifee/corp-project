...
function initNewsRead() {
    $.ajax({
        url:hostUrl + 'oa/content/contentRowType/getNewsReadForPersonal?_t='+new Date().getTime(),
        type:'GET',
        dataType: 'json',
        success:function (resultData) {
...
function recordNewsReadForPerson(ele) {
    ...
    if($(ele).siblings('.newicon').length>0){
        $.ajax({
            url:hostUrl + 'oa/content/contentRowType/getNewsReadForPersonal?contentRowTypeId='+contentRowTypeId+'&_t='+new Date().getTime(),
            type:'GET',
            dataType: 'json',
            success:function (resultData) {
                if(resultData.success){
...
function checkLogin(url){
    ...
    $.when(currentUserDef,def).done(function (currentUserInfo,url) {
        ...
        var flag = true;
        var ajaxUrl = hostUrl + 'sys/thirdPartyAuthentication/checkLogin?_time='+new Date().getTime();
