...
$(function () {
    ...
    function initFormData() {
       ...
       /*     $("#componentForm").attr('action', baseUrl + 'portal/component/save');
            ...
            $.ajax({
                url: baseUrl + 'portal/component/get/' + componentId + '?_t=' + new Date().getTime(),
                type: 'GET',
    ...
    $("#saveCategoryBtn").on('click', function () {
        $("#categoryForm").attr('action', baseUrl + 'portal/componentCategory/save');
        $("#categoryForm").submit();
    });
...
function saveForm(op){
	...
	if(act=="create"){
		   $.ajax({
		       url:baseUrl + 'portal/component/save',
		   ...
		   $.ajax({
		       url:baseUrl + 'portal/component/update',
		       data:new FormData($( "#componentForm" )[0]),  
		       type:'POST',
			   	processData:false,
				contentType:false,
...
function initCategory() {
    //获取分类列表
    $.ajax({
        url: baseUrl + "portal/componentCategory/queryList",
        data: JSON.stringify({}),
        type: 'POST',
        async: false,
