...
//                	str = "<p><span>"+dataList[i].name+":&nbsp;</span>  <span>"+valueText+"</span></p>";
			...
			if (flowList) {
				...
				for (var i = 0; i<flowList.length; i++) {
					...
					//q取群聊数组的值，值为null的，不进入数组中
					if(flowList[i].approverId !==null){
                        arrLst.push(flowList[i].approverId)
					}
					var taskStatus = itemObj.taskStatus;
					instanceStatus = itemObj.instanceStatus;
					var postName = itemObj.postName;
                    var approverId = itemObj.approverId;
					...
					// 	"</span>");
					var tempTable = '';
                    	tempTable += '<table class="table_detail_approve">';
                    tempTable += '<tr>';
                    tempTable += '<td class="td_w1 pass_name">'+ approverName +'</td>';
                    tempTable += '<td class="td_w2 info_s">'+ postName +'</td>';
                    if(approverId != null){
                    tempTable += '<td><img src="../myimg/startchat@2x.png" class="chat chat_phone_zx" data-userId="'+ approverId +'"/></td>';
                    }else{
                        tempTable += '<td></td>';
					}
                    tempTable += '</tr>';
                    tempTable += '<tr>';
                    tempTable += '<td class="td_w1 pass_acname"></td>';
                    tempTable += '<td class="td_w2 info_s">'+ taskEndTime +'</td>';
                    tempTable +=  '</tr>';
                    tempTable +=  '<tr>';
                    tempTable +=  '<td class="td_w1 pass_acname">'+acName +'</td>';
                    tempTable +=  '<td class="td_w2 info_text">'+ taskComments +'</td>';
                    tempTable +=  '</tr>';
                    tempTable += '</table>';
            ...
            if(msgOpType=="DB" && dataDto.nextList &&  dataDto.nextList.length>0){
                $("#spcz").show();
...
//群聊
//必须调用，两端可以封装在一个方法里,根据所用的手机是ios/Android,走不同的方法
...
setupWebViewJavascriptBridge(function(bridge) {
    document.getElementById("groupChat").onclick = function(e) {
        e.preventDefault()

        //js调用objc
        bridge.callHandler(
            'jzyJsHandlers',
            {'method':1,'params':arrLst},
            function(response){
                log('此处不用处理，可为空',response);
                alert("good")
            }
        );
    };
    $(document).on("click",".chat_phone_zx",function(e){
        e.preventDefault();
        bridge.callHandler(
            'jzyJsHandlers',
            {'method':2,'params':{'thirdUserId':$(this).attr("data-userid")}},
            function(response){
                log('此处不用处理，可为空',response);
                alert("good")
            }
...
//发起群聊按钮后面的标签效果，点击弹出下面的标签，再点击隐藏
$(function(){
    $('#cli').click(function(){//点击a标签
        if($('#divMui').is(':hidden')){//如果当前隐藏
            $('#divMui').show();//那么就显示div
			$("#imgIcon").attr("src","../myimg/ejectIcon/fan.png");
        }else{//否则
            $('#divMui').hide();//就隐藏div
            $("#imgIcon").attr("src","../myimg/ejectIcon/zhen.png");
        }
    })
})
