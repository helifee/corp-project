...
function getVoucherBill(){
	if(voucherBillId){
		$.ajax({
	        type:'get',
	        url:hostUrl+'finance/voucherBill/get/'+voucherBillId+'?time='+Math.random(),
...
function getVoucherEntrys(){
	jQuery("#voucherEntryList").jqGrid(
            {
                url:hostUrl+'finance/voucherBillEntry/queryList',
...
function getVoucherBillRelation(){
	jQuery("#voucherBillRelationList").jqGrid(
            {
                url:hostUrl+'finance/voucherBillRelation/queryList',
                ajaxGridOptions: { contentType: 'application/json' },
                mtype : "POST",  
                contentType : "application/json",  
                datatype : "json", 
...
function changeDate(){
	...
	$.ajax({
	       url:hostUrl+'finance/voucherBill/update/'+voucherBillId+'?time='+Math.random(),
...
function deleteVhoucherEntry(){
	...
	if(!id) {
	  ...
	  pop_text_open("blue",'确认要删除这条数据吗？',function(){
			$.ajax({
				url:hostUrl+"finance/voucherBillEntry/deletePseudo/"+id,
				type:'DELETE',
				dataType:'JSON',
				success:function (resultData ) {
...
function editVoucher(){
	...
	$('#entryModalBody').load("voucherbillentry_modal.html",function(){
		...
		$.ajax({
	        type:'get',
	        url:hostUrl+'finance/voucherBillEntry/get/'+id+'?time='+Math.random(),
	        success: function(data) {
...
function entrySelEvent(){
	...
	$('#voucherBillEntryFrom').find('input[name="assCompent"],a[id="selectAssName"]').on('click', function() {
		...
		//$("#add").modal("show");
    		var urlAll = hostUrl+'finance/assType/queryTree';
...
function save_form(){
	//映射后台保存方法
	var url=hostUrl+"finance/voucherBillEntry/save";
	//请求方式
	var type = 'POST';
	if(isup){
		//映射后台修改方法
		url=hostUrl+"finance/voucherBillEntry/update/"+$('#voucherBillEntryFrom').find("input[name='id']").val();
		type = 'PUT';
	}
