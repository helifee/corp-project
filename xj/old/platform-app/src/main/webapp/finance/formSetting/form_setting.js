...
function getFormSettingList(){
	...
	jQuery("#formSettingGrid").jqGrid(
            {
                url: hostUrl+'finance/voucherFormSetting/getFormSettingPage',
...
function editFormSetting(rowid){
	...
	window.open("formsetting_edit.html?id="+rowid);
	 /*$("#add").modal("show");
		$.ajax({
	        type:'get',
	        url:hostUrl+'finance/voucherFormSetting/get/'+id+'?time='+Math.random(),
...
function delFormSetting(){
	...
	var prvid=$("#"+ids).prev().attr("id");
	  pop_text_open("blue",'确认要删除这1条数据吗？',function(){
			$.ajax({
				url:hostUrl+"finance/voucherFormSetting/deletePseudo/"+ids,
				type:'DELETE',
				dataType:'JSON',
