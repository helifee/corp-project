    ...
    var _assList = {
    	...
    	loadPageData:function(){
    		...
    		jQuery("#assType").jqGrid({
    	        url: hostUrl+'finance/assType/queryList'+'?time='+Math.random(),
...
//                    }
    	...
    	toUpAssType:function(){
    		...
    		$.ajax({
    	        type:'get',
    	        url:hostUrl+'finance/assType/get/'+my.dataPar.rowData.id+'?time='+Math.random(),
    	...
    	toDelAssType:function(){
    		...
    		pop_text_open("blue",'确认要删除这条数据吗？',function(){
	    		$.ajax({
	    			url:hostUrl+"finance/assType/deletePseudoBatch/"+ids+'?time='+Math.random(),
	    			type:'DELETE',
	    			dataType:'JSON',
    	...
    	toSaveAssType:function(){
    		...
    		var type = 'POST';
			//映射后台保存方法
    		var url=hostUrl+"finance/assType/save";
    		if(my.dataPar.isup){
    			//映射后台修改方法
				url=hostUrl+"finance/assType/update/"+$('#addAuxiliaryId').val();
				type = 'PUT';
    		}
			//请求方式
			//获取form表单元素并遍历赋值
    	...
    	loadMapingData:function(){
    		...
    		if(!my.dataPar.isOne){
	    		jQuery("#assMapping").jqGrid({
	    	        url: hostUrl+'finance/assMapping/queryListByBusinessObj'+'?time='+Math.random(),
	    	        ajaxGridOptions: { contentType: 'application/json' },
	    	        mtype : "POST",  
	    	        contentType : "application/json",  
	    	        datatype : "json",
    	...
    	toSaveAssMap:function(){
    		...
    		var rowData=null;
    		//映射后台保存方法
    		var url=hostUrl+"finance/assMapping/saveAllAssMapp";
    		//请求方式
    		var type = 'POST';
		...
		sendAssMappingNC:function(){
			...
			$.ajax({
				url:hostUrl+"finance/assMapping/sendAssMappingNC"+'?time='+Math.random(),
		...
		sendAssMappingNCRe:function(){
			...
			$.ajax({
				url:hostUrl+"finance/assMapping/sendAssMappingNCRe/"+senId+'?time='+Math.random(),
				type:'GET',
				dataType:'JSON',
				success:function (resultData ) {
					if (resultData&&resultData.success) {
		...
		getCoContract:function(){
			...
			$.ajax({
				url:hostUrl+"finance/assMapping/getCoContract"+'?time='+Math.random(),
				data:JSON.stringify(postData),
				type:'POST',
				contentType:'application/json',
				dataType:'JSON',
