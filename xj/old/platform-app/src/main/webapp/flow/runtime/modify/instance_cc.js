...
$(function () {
    ...
    var initPartnerList = function () {
        ...
        for (var i = 0; i < approvePartnerArr.length; i++) {
            ...
            /!*if(!(approvePartner.status=='1'||approvePartner.status=='2')){
                approvePartnerArr.splice(i,1);
            }*!/
            //根据审批人状态判断审批人是否已经审批完成
            if(!(approvePartner.status=='1'||approvePartner.status=='2')){
                approvePartnerArr.splice(i,1);
            }

            //根据任务判断审批人是否已经审批完成
            if(approvePartner.taskStatus&&!(approvePartner.taskStatus=='1'||approvePartner.taskStatus=='2')){
                approvePartnerArr.splice(i,1);
            }
    ...
    window.saveAcFormData = function(){
        ...
        /*if(acBaseInfo.acType=='2'){
            ...
            for (var i = 0; i < originApprovePartnerArr.length; i++) {
                ...
                if((originApprover.taskStatus=='1'||originApprover.taskStatus=='2')&&originApprover.postStatus!='3'){
            ...
            for(var item in newApproverMap){
                ...
                var acStatus = selCell.acStatus?selCell.acStatus+'':'1';
                newApprover.taskStatus = acStatus;
                newApprover.status = acStatus;
                newApprover.postStatus = acStatus;
        ...
        var originCopyPersonArr = selCell.ccPerson&&selCell.ccPerson!=''?JSON.parse(selCell.ccPerson):[];
        var newCopyForPartnerMap = {};
        for (var i = 0; i < copyForPartnerArr.length; i++) {
            var obj = copyForPartnerArr[i];
            var participantName = obj.participantName;
            if(obj.participantType!='4'&&(!participantName||participantName=='')){
                $.xljUtils.tip('blue','流程节点抄送人中含有空值！');
                return;
            }
            var approverId = obj.approverId;
            if(!approverId||approverId==''){
            ...
            obj.id = approverId;
            delete obj['approverId'];
            newCopyForPartnerMap[approverId] = obj;
        }
