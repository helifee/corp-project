...
$(function() {
	...
	setInterval(function () {
		var refreshTaskList = window.location.hash.slice(1);
		if(refreshTaskList!=''&&refreshTaskList.indexOf('refreshTaskList=')!=-1){
			refreshTaskList = refreshTaskList.replace('refreshTaskList=','');
			if(refreshTaskList=='true'){
				refreshMyTaskData();
				var winSrc = window.location.href;
				if(winSrc.indexOf('#')!=-1){
					winSrc = winSrc.substring(0,winSrc.indexOf('#'));
				}
				window.top.location = winSrc + '#';
			}
		}
	},100);*/
});
...
function initTaskGridList(){
	...
	if(dataType=="HAVE_DONE"){
		searchSortName="dealDate";
		searchSortOrder="desc";
	}else{
		searchSortName="sendDate";
		searchSortOrder="desc";
	}
//	if(dataType=="HAVE_DONE"){
//    	postData.sidx="dealDate";
//    	postData.sord="desc";
//	}else{
//		postData.sidx="sendDate";
//		postData.sord="desc";
//	}
	if (firstType) {
...
//            	repeatitems: false 
            ...
            colModel : [
                ...
                {name:'title',   label:'标题', width:300, align:"left", formatter: titleFormatter},
                {name:'sendDate',label:'到达时间',width:100, align:"center"},
                {name:'dealDate',label:'完成时间',width:100, align:"center"},
            ...
            sortname : searchSortName,//默认的排序列
            sortorder : searchSortOrder,//排序方式,可选desc,asc
...
function taskView(urlText, opType){
	...
	var url = initText;
	/*var parentWinSrc = window.location.href;
	if(parentWinSrc.indexOf('#')!=-1){
		parentWinSrc = parentWinSrc.substring(0,parentWinSrc.indexOf('#'));
	}
	url += '&parentWinSrc='+ encodeURIComponent(parentWinSrc);*/

	var newWin = openWin(url);
	if(urlText && urlText.indexOf("http")==0){
		var intervalIndex = setInterval(function () {
			if(typeof newWin.closed!='undefined'){
				if(newWin.closed){
					//console.info('子页面已关闭');
					refreshMyTaskData();
				...
				if(!newWin.window||newWin.window.closed){
					//console.info('子页面已关闭');
					refreshMyTaskData();
					window.clearInterval(intervalIndex);
				}
	...
	if(rowId&& rowId != ''){
		...
		if(rowData.opType=='DB'||rowData.opType=='DY'){
			$.ajax({
				...
				url: hostUrl + "flow/sysNoticeMsg/updateStatusOfNoticeMsgByCurrentUser?_t="+new Date().getTime() ,
        ...
        for(var i in ids){
            ...
            if(rowData.opType=='DB'||rowData.opType=='DY'){
                $.ajax({
                    ...
                    contentType: "application/json",
                    url: hostUrl + "flow/sysNoticeMsg/updateStatusOfNoticeMsgByCurrentUser?_t="+new Date().getTime() ,
                    data:JSON.stringify({'oldStatus':rowData.opType,'newStatus':rowData.opType=='DB'?'YB':'YY','id':rowData.id,'manualSet':1}),
                    dataType: "json",
                    async: false,
                    success: function (result) {
                        if(result&&result.success){
