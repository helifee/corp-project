        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success) {
                    ...
                    //当前登录用户id
                    $("#loginUserId").val(flowInstance.currentUserId);
                    //登录用户tendId
                    $("#loginUserTendId").val(flowInstance.loginUserTendId);
                    instanceDataDef.resolve(flowInstance);
                    //设置流程标题
                    $('.flow-content-title span').text(flowInstance.instanceName);
                    $("#processTitle").val(flowInstance.instanceName);
                    $("#processLink").val(window.location.href);
        ...
        switch(btnCode){
            ...
            case 'modifyAc':
                btnObj.on('click',function () {
                    ...
                    var url = encodeURI(hostUrl+"flow/runtime/modify/modifyInstance.html?instanceId=" + instanceId + "&requestSource=adjust");
                    openWin(url);
                });
                break;
            //审结
...
// ();
                        ...
                        $.when(commonAjaxDef).done(function (success) {
                            if(success){
                               ...
                               // $('#passReadGrid').jqGrid().trigger("reloadGrid");
                                $('#passReadGrid').jqGrid('setGridParam', { search: true
            ...
            case 'print':
                btnObj.on('click',function () {
                    ...
                    // var scrollHeight = $('#flowContent')[0].scrollHeight + 500;
                    $(window).off('resize').on('resize',function () {
                        // $('#flowContent').height(scrollHeight);
                        $('#flowContent').removeAttr("style");
                        $('#flowContent').width(printW);
                        $('#approvalList').width(printW);
                    });
    ...
    var drawApproveList = function (approvalTable, approvalList) {
        ...
        var theader = $('<tr class="form-tr form-header" >' +
            ...
            '<td style="width: 10%; text-align: left; font-weight: bold;white-space: nowrap">责任人<span class="groupChatIcon" onclick="onLine(this,null,\'group\');"></span></td>' +
        ...
        for (var i = 0; i < approvalList.length; i++) {
            ...
            var approverNameTd;
            if(obj.acType != '3' && approverId){
                approverNameTd = $('<td style="width: 10%; text-align: left;white-space: nowrap">'+ approverName +"<span title=\"发起聊天\" class=\"startChartIcon\"   onclick=\"onLine(this,'"+ approverId +"','single');\" ></span></td>");
                if(toUsersIds.indexOf(approverId) == -1){
                    toUsersIds.push(approverId);
                }
            }else{
                approverNameTd = $('<td style="width: 10%; text-align: left;white-space: nowrap">'+ approverName + "</td>");
            }
        ...
        $("#toUserIds").val(JSON.stringify(toUsersIds));
        approvalTable.empty();
 ...
 * @param toUserId 聊天对象
 */
var onLine = function (Obj, toUserId,type) {
    // String processLink = window.location.href;
    $(Obj).attr("onclick", "null");
    var loginUserTendId = $("#loginUserTendId").val();
    var loginUserId = $("#loginUserId").val();
    var processTitle = $("#processTitle").val();
    var processLink = $("#processLink").val();
    var submitData = {
        "tendId": loginUserTendId,
    ...
    if (type == 'group') {
        var toUserIdsJson = $("#toUserIds").val();
        var toUserIds = JSON.parse(toUserIdsJson);
        toUserIds.splice($.inArray(loginUserId, toUserIds), 1);
        submitData.toUserIds = toUserIds;
    } else {
        submitData.toUserIds = [toUserId];
    }
    $.ajax({
        type: "post",
        url: $.xljUtils.serverAddr + 'flow/instance/imOnLine',
        timeout: 60000,
        ...
        success: function (data) {
            ...
            $(Obj).attr("onclick", "onLine(this,'" + toUserId + "','" + type + "');");
        }, error: function (data) {
            if (data.msg) {
                $.xljUtils.tip("red", data.msg);
            } else {
                $.xljUtils.tip("red", "操作失败！");
            }
            $(Obj).attr("onclick", "onLine(this,'" + toUserId + "','" + type + "');");
        }
    });
