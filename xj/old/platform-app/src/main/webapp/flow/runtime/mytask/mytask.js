...
$(function() {
    //var hostUrl = 'http://10.17.4.88:8080/platform-app/';
	//hostUrl = 'http://127.0.0.1:8080/platform-app/';
            ...
            switch (i){
                ...
                case 0:
                    $(tab).on('click',function () {
                        ...
                        if(moreAObj[0]){
                            $(moreAObj[0]).attr('href', baseUrl+'flow/runtime/mytask/task_list.html?dataType=DB');//待办
                ...
                case 1:
                    $(tab).on('click',function () {
                        ...
                        var moreAObj = $('#moreMsgListBtn');//$(this).parents('.portlet-container').find('.news_title').find('.glyphicon-th-list').parent();
                        if(moreAObj[0]){
                            $(moreAObj[0]).attr('href', baseUrl+'flow/runtime/mytask/task_list.html?dataType=DY');//待阅
                            $(moreAObj[0]).attr('target', '_blank');
                        }
                        $('#moreMsgListBtn').show();
                    });
                ...
                case 2:
                    $(tab).on('click',function () {
                        ...
                        var moreAObj = $('#moreMsgListBtn');//$(this).parents('.portlet-container').find('.news_title').find('.glyphicon-th-list').parent();
                        if(moreAObj[0]){
                            $(moreAObj[0]).attr('href', baseUrl+'flow/runtime/mytask/task_list.html?dataType=HAVE_DONE');//已办已阅
                            $(moreAObj[0]).attr('target','_blank');
                        }
                        $('#moreMsgListBtn').show();
                    });
                ...
                case 3:
                    $(tab).on('click',function () {
                        ...
                        var moreAObj = $('#moreMsgListBtn');//$(this).parents('.portlet-container').find('.news_title').find('.glyphicon-th-list').parent();
                        if(moreAObj[0]){
                            $(moreAObj[0]).attr('href', baseUrl+'flow/runtime/mytask/task_list.html?dataType=FQ');//我的发起
                            $(moreAObj[0]).attr('target','_blank');
                        }
                        $('#moreMsgListBtn').show();
                    });
    ...
    function moreMsgList(){
    	...
    	var url = baseUrl+'flow/runtime/mytask/task_list.html?dataType='+dataType;
    	openWin(url);
    }
    ...
    function chooseTab(tabName) {//tabName=toApprove | toRead | haveDone | myStart| 
        ...
        var fullUrl = hostUrl+"flow/sysNoticeMsg/queryDBDYList?_t="+new Date().getTime();
    ...
    function getEmailInfo(){
    	$.ajax({
            type: 'GET',
            url: hostUrl + 'oa/email/userEmail/getUserEmail?_time='+new Date().getTime(),
            dataType:'JSON',
...
function switchTaskTab(tabType) {
    ...
    $.ajax({ //发送更新的ajax请求
        type: "POST",
        url: hostUrl + "flow/sysNoticeMsg/querySysNoticeMsgByPage",
        dataType: "JSON",
        data: JSON.stringify( paramData ),
        contentType: 'application/json;charset=utf-8', //设置请求头信息
        success: function (data) {
