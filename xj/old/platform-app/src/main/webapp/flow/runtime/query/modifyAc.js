...
function approveHistoryInit() {
	jqGrid = $("#_approveHistory").jqGrid(
		{
			url : hostUrl + "flow/instance/queryApprovalListAdmin",
			postData : postData,
			datatype : "json",
			ajaxGridOptions : {
...
function addAcPrefix(){
	...
	if(ids.length == 0){
		...
		openWin(hostUrl + "flow/runtime/query/instance_ac.html?instanceId="+$.getUrlParam("instanceId")+"&srcRowIndex="+srcRowIndex+"&interval="+interval+"&position=before&code=QJQHJ&name=前加签环节&time=" + Math.random());
	}	
...
function addAcSuffix(){
	...
	if(ids.length == 0){
		...
		var interval = $("#sequence" + ids[0]).attr('rowspan');
		if(typeof (interval) == "undefined"){interval = 0;}
		openWin(hostUrl + "flow/runtime/query/instance_ac.html?instanceId="+$.getUrlParam("instanceId")+"&srcRowIndex="+srcRowIndex+"&interval="+interval+"&position=after&code=HJQHJ&name=后加签环节&time=" + Math.random());
	}	
}
/**
 * 删除
...
function save(){
	$.ajax({
	    url: hostUrl + "flow/instance/adjustAc",
	    ...
	    success:function (data) {
	        if(data) {
	            ...
	            if(success) {
					...
					if(window.opener&&window.opener.jqGrid){
