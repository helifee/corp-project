...
function getOperationTypePage(){
    ...
    $("#list").jqGrid(
        {
            url : hostUrl+"flow/operationType/queryList",
            datatype : "json",
...
function save(){
	if(lastSel){
		...
		if(!verify(rowData.name.trim())){
			...
			$("#list").setGridParam({url:hostUrl+"flow/operationType/queryList"}).trigger("reloadGrid");
			return;
		}
		$.ajax({  
		    type: "put",  
		    url: hostUrl+"flow/operationType/update/" + lastSel,  
