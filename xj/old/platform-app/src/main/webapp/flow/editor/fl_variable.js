...
$(function(){
	...
	initPageParam();
    var isDeafultObject = $.getUrlParam("isDeafultObject");
    if(!isDeafultObject){
    ...
    resizeGrid();
    initGridTable();
	queryVariableList(isDeafultObject);
 ...
 * 查询相关的业务变量数据列表
 */
function queryVariableList(isDeafultObject){
	var paramData = {businessObjectId: objectId, delflag:false};
	if(isDeafultObject){
		paramData.isDeafultObject = isDeafultObject;
	}
	$.ajax({ //发送更新的ajax请求
	    type : "post",  
