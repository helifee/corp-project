...
//            postData:{"businessObjectId":""},
            ...
            colModel : [
                ...
                {name:'businessObjectName',label:'业务对象',align:"center",hidden:true},
                ...
                {name:'retract',label:'允许撤回流程',align:"center",formatter: 'isDefualtFormatter',width:80},
...
function newFl(){
	...
	//openWin(url);
	searchAppIdBybusinessObjectId();
	window.open(hostUrl+'flow/builder/fl_template.html?appId='+v_appId+'&businessObjectId='+v_businessObjectId+'&businessObjectName='+encodeURIComponent(v_businessObjectName)+"&act=create");
}
/**
 * 查看流程模板
 */
...
function setDefaultFl(){
	...
	if(ids.length == 0){
		...
		$.ajax({
		    type: "POST",
		    url: hostUrl + "flow/fl/setDefaultFl",
...
function deleteFl(){
	...
	pop_text_open("blue", popText, function() {
		$.ajax({
			type : "delete",
			url : hostUrl + "flow/fl/deleteFlowsByCodeText/" + codeText,
			dataType : "json",
			contentType : 'application/json;charset=utf-8', //设置请求头信息
			success : function(data) {
...
function gotoReplace(){
	var url = encodeURI(hostUrl+"/flow/runtime/replace/replace_list.html?titName=flow");
	openWin(url);
}
...
function resizeHeight(){
  $(".slide-left .ztree-box").height(($(window).height()-145)+"px");
	$(".grid-container").height(($(window).height()-$(".con-tit").height()-$("#userList").height()-45)+"px");
}
function resizeGrid(){
