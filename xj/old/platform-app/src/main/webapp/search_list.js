...
;(function($, window, document, undefined) {
	...
	function initSearchCategoryTab() {
		...
		// 查询检索分类
		$.xljUtils.xljAjax({
			url : hostUrl + 'univ/search/searchCategory/queryList',
			data:'{}'
		}, function(resultData) {
	...
	function doSearch(pageNo) {
		...
		$.xljUtils.xljAjax({
			url : hostUrl + 'univ/search/searchIndex/pageFullTextQuery',
...
//		});
		...
		$('.list-box .cont-box').on('click','a.a-all-download', function(e){
			var form1 = $('<form action="' + hostUrl +'univ/attachment/attachment/downloadAll" method="post"/>')
