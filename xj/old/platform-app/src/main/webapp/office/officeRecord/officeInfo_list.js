 ...
 $("#officeList").keypress(function(e){
	...
	$("#btnDelete").click(function() {
	    ...
	    var prvid="";
	        ...
	        if(gr.length > 1) {	//批量删除
	        	url = hostUrl + "oa/office/officeInfo/deleteBatch/"+grStr
	        }else {	//单个删除
	        	 prvid=$("#"+grStr).prev().attr("id");
	        	url = hostUrl + "oa/office/officeInfo/delete/"+grStr
	        }
...
function getLeftTree(){
    ...
    $.ajax({
        type: "GET",
        url: hostUrl + "oa/office/officeHouse/getOfficeHouseTreeById/null?t=" + (+new Date()),
        dataType: "json",
...
function onClick(e,treeId,treeNode){
	...
	jQuery("#officeInfoList").jqGrid({
    	url : hostUrl + "oa/office/officeInfo/getOfficeInfopage",
...
//	var officeInfoFrid=jQuery("#officeInfoList");
	...
	$.ajax({
		url : hostUrl + "oa/office/officeInfo/updateCount",
		type : 'PUT',
		dataType : 'JSON',
		contentType : "application/json",
...
function getOperationAuthorition() {
	...
	$.ajax({
		type: 'GET',
		url: hostUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=YPDA',
		dataType: 'json',
		//contentType: 'application/json',
		async: false,
