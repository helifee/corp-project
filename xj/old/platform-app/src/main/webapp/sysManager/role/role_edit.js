...
function addSaveForm(sign){
   ...
   var uBody = "sys/org/roleCatalog/saveByMold";
   var uAll = hostUrl + uBody;
   $.ajax({
       url:uAll,
       data:JSON.stringify(roleAndCataDto),
...
function roleCallback(data) {
	if(data.prefixId.indexOf($("#id").val())>=0){
		...
		$("#roleFrom").find("input[id='catalogIdName']").val(data.name);
...
//    var urlAll = hostUrl + urlBody;
	...
	//上级目录
	var selNodeId=window.opener.selNodeId;
	var selNodeName=window.opener.selNodeName;
	var selNodeType=window.opener.selNodeType;
	if(selNodeId != "" && selNodeName != "" && selNodeType !=""){
		$("#catalogId").val(selNodeId);
		selNodeName=$.xljUtils.htmlDecode(selNodeName);
