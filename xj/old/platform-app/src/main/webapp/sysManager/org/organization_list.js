...
//	console.log(nodes[0].children);
	if(nodes.length != 1 ){
		...
		if(status == 1){
            pop_text_open("blue",'是否同时启用下级？',function(){
                data.ifUnLockDown = 1;
                lockAjax(status,nodes,data);
            },function(){
                lockAjax(status,nodes,data);
            });
        }else{
...
function lockAjax(status,nodes,data){
    ...
    $.ajax({
        ...
        success: function(json) {
            if(json.success == true){
                lockOrUnNodes(status, nodes[0],data.ifUnLockDown);
 ...
 * @param node 节点
 */
function lockOrUnNodes(status,node,ifUnlockDown) {
	var zTree=$.fn.zTree.getZTreeObj("treeDemo");
	if(status == 0){
		lockAllChildrenNodes(node,zTree);
	}else{
		unLockAllParentNodes(node,zTree,ifUnlockDown);
	}
}
/**
 * 禁用所有子节点
 ...
 * @returns
 */
function unLockAllParentNodes(treeNode,zTree,ifUnlockDown){
	treeNode.status=1;
	zTree.updateNode(treeNode);
	var pNode = treeNode.getParentNode();
	if(pNode!=null){
	...
	if(ifUnlockDown && ifUnlockDown == 1){
		//启用下级
        var cNodes = treeNode.children;
        if(cNodes && cNodes.length >0){
			for(var i = 0 ;i < cNodes.length; i++){
                unLockAllParentNodes(cNodes[i],zTree,ifUnlockDown);
			}
		}
	}
}
