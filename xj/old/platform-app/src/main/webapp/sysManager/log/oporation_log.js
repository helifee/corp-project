	...
	opeLogGridObj = jQuery("#opeLogList").jqGrid(
			{
				...
				colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
			                 ...
			                 {name : 'note',label : '操作记录内容',width : 180,align : "center",formatter:function (cellvalue, options, rowObject) {
								 ...
								 return '<a href="javascript:void(0);" onclick="showNote(\''+rowObject.id+'\',\''+ rowObject.ifLong +'\')">'+$.xljUtils.escapeHtml(cellvalue)+'</a>';
							 }}
	             ],
	             rownumWidth:55,
	             rowNum : 20,//一页显示多少条
...
function showNote(rowId,ifLong) {
	var rowData = $('#opeLogList').jqGrid('getRowData',rowId);
	var aObjHtml = rowData.note;
	var aLinkObj = $(aObjHtml);
	if(ifLong && ifLong == 1){
        var ubody = "sys/log/logOperation/get/"+ rowId;
        var uall = hostUrl+ubody;
        $.ajax({
            type:'get',
            url:uall,
            ...
            success: function(data) {
                if(data.success){
                    if(data.result){
                        $('#showContentModal').find('.modal-body').html(data.result.note);
            ...
            error:function(XMLHttpRequest, textStatus, errorThrown){
                pop_tip_open("red","请求失败");
            }
        })
	}else{
        $('#showContentModal').find('.modal-body').html(aLinkObj.text());
        $('#showContentModal').modal({
            backdrop:'static',
            show:true
        });
