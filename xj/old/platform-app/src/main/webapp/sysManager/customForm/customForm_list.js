...
function initJqGrid2(){
    jqGrid2 = jQuery("#list2").jqGrid(
        {
        	url: baseUrl+"/sys/base/customForm/page",
...
//  	    }
//  	});
//    $("#customFormGroupForm").attr('action',baseUrl+'/sys/base/customFormGroup/save');
...
//        $.ajax({
//            url:baseUrl+'/sys/base/customFormGroup/get/'+nodes[0].id+'?time='+Math.random(),
//            type:'GET',
...
//                        }
//                    }
//                    $("#customFormGroupForm").attr('action',baseUrl+'/sys/base/customFormGroup/update/'+result['id']);
...
function delGroup(){
	...
	if(nodes.length>0){
		...
		if(curNode.pId==null || curNode.pId=="0"){
			...
			$.ajax({
		        type:'get',
		        url:baseUrl+'/sys/base/customFormGroup/getGroupCount/'+curNode.id+'?time='+Math.random(),
		        success: function(data) {
		        	...
		        	if(count>0){
		        		...
		        		pop_text_open("blue","确认删除该分类吗？",function(){
		    				$.ajax({
		    					url:baseUrl+"/sys/base/customFormGroup/deletePseudo/"+curNode.id,
		    					type:'DELETE',
		    					dataType:'JSON',
		    					success:function (resultData ) {
		    						if (resultData&&resultData.success) {
			...
			$.ajax({
		        type:'get',
		        url:baseUrl+'/sys/base/customFormGroup/getCount/'+curNode.id+'?time='+Math.random(),
		        success: function(data) {
		        	var count=data.result;
		        	if(count>0){
...
function formDel(){
	...
	if(ids&&ids!='') {
		$.ajax({
	        type:'get',
			url : baseUrl + '/sys/base/customForm/getCount/'+ids+'?time='+Math.random(),
	        success: function(data) {
	        	...
	        	if(resultData){
	        		if(resultData.hasInstance){
	        			...
	        			pop_text_open("blue","确认删除【"+delCount+"】张表单吗？",function(){
							$.ajax({
								url:baseUrl+"/sys/base/customForm/deletePseudoBatch/"+ids,
								type:'DELETE',
								dataType:'JSON',
								success:function (resultData ) {
									if (resultData && resultData.success) {
	...
	if(sign == 1){
		...
		if(index == 0){
			...
			if(node==null){
				...
				$.ajax({
					url:hostUrl+"sys/base/customFormGroup/updateSort/"+nodes[0].id,
					type:'put',
					dataType:'JSON',
					contentType:"application/json",
					data:'{"sortType":'+sign+'}',
...
function updateSort(type){
	...
	$.ajax({
		url:hostUrl+"sys/base/customForm/updateSort/"+ids,
		type:'put',
		dataType:'JSON',
		contentType:"application/json",
		data:'{"sortType":'+type+'}',
...
function enableOrNot(st){
	...
	if(id!=null && id!=""){
		...
		if(status!=st){
			$.ajax({
		       url:hostUrl+"/sys/base/customForm/enableOrNot",
