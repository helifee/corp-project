    ...
    var switchTab = function () {
        $('.flow-tab button').on('click', function () {
            ...
            if (btnId == 'lanuchForm') {
                ...
                $('#participateDiv').hide();
                $('#approveHistoryDiv').find('.ui-jqgrid-bdiv').getNiceScroll().show().resize();
                $('#participateDiv').find('.ui-jqgrid-bdiv').getNiceScroll().hide().resize();
                resizeCustomformGridList();
            } else if (btnId == 'participateForm') {
                $('#approveHistoryDiv').hide();
                $('#participateDiv').show();
                resizeInstanceGridList();
                initQueryGridList();
                $('#participateDiv').find('.ui-jqgrid-bdiv').getNiceScroll().show().resize();
                $('#approveHistoryDiv').find('.ui-jqgrid-bdiv').getNiceScroll().hide().resize();
 ...
 * 重置页面高度
 */
function resizeCustomformGridList(){
	//页面加载完毕后更改grid宽高
//	$.xljUtils.resizeNestedGrid();
	$('#list2').jqGrid('setGridHeight',$(window).height()-$(".xj-main-dimsearch").height()
	-$(".xj-main-advanced").height()-$("#lanuchForm").height()-$(".flow-tab").height()
	-170, true);
}
