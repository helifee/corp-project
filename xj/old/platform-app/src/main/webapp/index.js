...
//    $('.menu_search').css({display:"none"});
    ...
    /**
     * 添加方法帮助文档图标过滤,过滤掉li-app、hr-app、bi-app目前保留platform-app
     * @constructor
     * @param {string} url - 代表需要判定链接.
     */
    function filterUrl(url){
        if(url && url.match('/platform-app/')){
            $('.helpMement').removeClass('dispN');
        }else{
            if(url && (url.match('/li-app/') || url.match('/hr-app/') || url.match('/bi-app/') || url.match('http://bireport.xyre.com:30007'))){
                $('.helpMement').addClass('dispN');
            }else if(url=='null'){
                $('.helpMement').addClass('dispN');
            }else{
                $('.helpMement').removeClass('dispN');
