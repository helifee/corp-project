...
function saveAndAdd() {
	submitForm(function(data){
        ...
        $('.title-part').text('新增');
        $('title').text('检索分类-新增');
        $mainForm.attr('action', hostUrl + 'univ/search/searchCategory/save');
...
function initPropertyModal() {
	$.xljUtils.initJqGrid({
		...
		url:hostUrl + 'univ/search/searchProperty/queryList?random='+Date.now(),
		jsonReader : {
   		 	root:'result'
		},
...
$(function(){
	...
	if(recordId == null){
        ...
        $('.title-part').text('新增');
        $('title').text('检索分类-新增');
        $('.search-category').attr('action', hostUrl + 'univ/search/searchCategory/save');
	...
	// 修改分类
        ...
        $.ajax({
            url: hostUrl + 'univ/search/searchCategory/get/'+recordId + '?random='+Date.now(),
            type:'GET',
            success:function (resultData) {
                if(resultData && resultData.success) {
					...
					if (searchCategory.status == true) {
						formItems.filter(':not(:radio)[name!="name"][name!="hostUrl"]').prop('disabled','disabled');
						formItems.filter(':radio').prop('disabled','disabled');
					}
                    
                    $('.search-category').attr('action',baseUrl + 'univ/search/searchCategory/update/'+searchCategory['id']);
