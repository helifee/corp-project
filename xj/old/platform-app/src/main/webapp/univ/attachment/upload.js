...
Attachment.prototype.init = function() {
	...
	$('#fileupload').fileupload({
		...
		if ($('.totalstatus').hasClass('hidden')) {
    		$('.totalstatus').removeClass('hidden');
    	}
	    ...
	    data.context.find('.up_delete').on('click', function(e){
	    	...
	    	if (nodeParent.children('li').size()==0) {
        		$('.up_info .totalpercent').text(0);
   		    	$('.up_info .totalloaded').text(0);
   		    	$('.up_info .totalsize').text(0);
   		    	$('.up_total .progress-bar').css('width', '0%');
        		$('.totalstatus').addClass('hidden');
   			}
		...
		var progress = parseInt(data.loaded / data.total * 100, 10);
    	$('.up_info .totalpercent').text(progress);
    	$('.up_info .totalloaded').text(self.formatFileSize(data.loaded));
    	$('.up_info .totalsize').text(self.formatFileSize(data.total));
    	$('.up_total .progress-bar').css('width', progress + '%');
		...
		if (data.result.success) {
			...
			self.files.push(data.result.result[0]);
...
Attachment.prototype.submit = function() {
	$.ajax({
		url:attachment.serverAddr + 'attachment/saveBatch',
		type:'POST',
		contentType: "application/json",
		data:JSON.stringify(attachment.files),
		dataType:"JSON",
		success:function(dt) {
