...
function loadDataFromDb(){
        var id = urlParams['id'];
        var uAll = hostUrl + "sys/middeleware_topic/mqTopic/get/"+id;
        $.ajax({
                type:'get',
                url:uAll,
...
function saveTopic(){
        ...
        if("edit" == type){
                var id = urlParams['id'];
                url = hostUrl+'sys/middeleware_topic/mqTopic/update/'+id;
                baseFormJson.id = id;
                methodType = "put";
          }else if("add" == type){
                url = hostUrl+'sys/middeleware_topic/mqTopic/save'
          }
