...
$(function () {
    ...
    function getOperationAuthorition() {
        $.ajax({
            type: 'GET',
            url: hostUrl + 'sys/authentication/getUserAuthenticationOperation?_t='+new Date().getTime()+'&appCode=OA&menuCode=' + urlParams.btnMenuCode,
            dataType: 'json',
    ...
    function bindButton() {
        ...
        $('#checkApprovalBtn').on('click',function () {
            ...
            if (rowId && rowId != '') {
                var rowData = $('#contentRowGrid').jqGrid('getRowData', rowId);
                if(rowData.processInstanceId&&rowData.processInstanceId!=''){
                    window.open(hostUrl+'flow/runtime/approve/flow.html?flCode='+rowData.flCode+'&businessId='+rowId+'&appId=OA&userId='+rowData.createPersonId);
                }else{
    ...
    function checkDataAuthEdit(id){
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: hostUrl + "oa/content/contentRowType/check/dataAuthEdit/"+id+"?_t="+new Date().getTime(),
            dataType: "json",
    ...
    function updateContentRowTypeState(state, param,rowData) {
        $.ajax({
            type: "put",
            contentType: "application/json",
            url: hostUrl + "oa/content/contentRowType/updateState/" + rowData.id + "/" + $.xljUtils.htmlDecode(param)+ "/" + state ,
            dataType: "json",
            success: function (result) {
                if(result&&result.success){
                    var msg;
