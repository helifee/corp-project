...
$(function () {
    ...
    function initContentChildGrid(parentId) {
        ...
        var url = hostUrl + 'oa/content/contentChild/queryList';
        $.xljUtils.initJqGrid({
    ...
    $('#createBtn').on('click',function () {
       ...
       /* window.open(hostUrl + 'content/contentChild/contentChild_edit.html?act=create&parentId='
    ...
    $('#updateBtn').on('click',function () {
        ...
        if(idVal&&idVal!=''){
            window.open(hostUrl + 'content/contentChild/contentChild_edit.html?act=update&id='+idVal,'_blank');
        }else{
    ...
    $('#deleteBtn').on('click',function () {
        ...
        $.xljUtils.confirm('blue','确定要删除这'+ids.length+'条数据吗？',function () {
            ...
            if (idsStr && idsStr != '') {
                $.ajax({
                    url: hostUrl + "oa/content/contentChild/deleteBatch/" + idsStr,
                    type: 'DELETE',
                    dataType: 'JSON',
