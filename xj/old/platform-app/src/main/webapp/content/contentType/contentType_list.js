...
$(function () {
    ...
    function initContentTypeGrid() {
        jQuery("#contentTypeGrid").jqGrid({
                url: baseUrl + "oa/content/contentType/page",
            ...
            $.xljUtils.confirm("blue", tipText, function () {
                if (ids && ids != '') {
                    ...
                    if(delBusinessObject) {
                        $.ajax({
                            url: baseUrl + "oa/content/contentType/deletePseudoAllObjectByIds/" + ids,
                            type: 'DELETE',
                            dataType: 'JSON',
    ...
    function deleteBusinessObject(ids) {
        ...
        $.ajax({
            url: hostUrl + "flow/businessObject/deletePseudoBatch/" + ids,
            type: 'DELETE',
            dataType: 'JSON',
            async: false,
            success: function (resultData) {
    ...
    function clickOperationBtn(operationType) {
        var defaultUrl = hostUrl + "content/contentType/contentType_edit.html";
        ...
        switch (operationType) {
            ...
            case 'displayDataItem':
                if (id) {
                    window.open(hostUrl + "content/contentAttribute/contentAttribute_list.html?contentTypeId=" + id);
                } else {
    ...
    $('#flowModelBtn').on('click',function () {
        ...
        $.ajax({
            type: 'GET',
            url: baseUrl + "flow/businessObject/get/"+rowData.id + '?_t=' + new Date().getTime(),
            dataType: "JSON",
            async:false,
            success: function (resultData) {
                if (resultData&&resultData.success) {
        ...
        if(businessObject) {
            ...
            $.ajax({
                type: 'GET',
                url: baseUrl + "flow/fl/getDefaultFl/"+rowData.code + '?_t=' + new Date().getTime(),
                dataType: "JSON",
                async:false,
                success: function (resultData) {
                    if (resultData&&resultData.success) {
            ...
            if(!flTemplate){
                window.open(hostUrl+'flow/builder/fl_template.html?appId=9d6cba61c4b24a5699c339a49471a0e7&businessObjectId='+businessObject.id+'&businessObjectName='+encodeURIComponent(businessObject.name)+'&act=update&isDefault=true');
            }else{
                window.open(hostUrl+'flow/builder/fl_template.html?flId='+flTemplate.id+'&businessObjectId='+businessObject.id+'&act=update');
        ...
        if(businessObject) {
            ...
            $.ajax({
                ...
                url: baseUrl + "oa/content/contentType/update/" + rowData.id,
            ...
            //查询对应的默认流程模板
            if(!flTemplate){
                window.open(hostUrl+'flow/editor/fl_new.html?appId=9d6cba61c4b24a5699c339a49471a0e7&businessObjectId='+businessObject.id+'&businessObjectName='+encodeURIComponent(businessObject.name));
            }
            return;
        }*/
    });
    ...
    function saveContentTypeBusiObject(rowData) {
        ...
        $.ajax({
            type: 'POST',
            contentType: "application/json",
            url: hostUrl + "flow/businessObject/saveObjectAndVariableData",
    ...
    $('#funcAuthBtn').on('click',function () {
        ...
        window.open(hostUrl+'sysManager/authority/authFun_list.html?menuId='+id);
    });
