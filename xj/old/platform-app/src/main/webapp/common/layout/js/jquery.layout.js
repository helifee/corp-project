...
(function ($) {
    ...
    function setSize(container) {
        ...
        if (opts.fit == true) {
            ...
            if (!p.hasClass('.tab-pane')) {
                var outerContainer = $(cc.parents('.layout-body')[0]);
                var tabH = outerContainer.find('.tab_h');
                $(outerContainer[0]).css({overflow:'hidden'});
                p.width($(outerContainer[0]).panel('options').width - 2).outerHeight($(outerContainer[0]).panel('options').height - tabH.outerHeight() -5);

            }
            //2017/03/07 end

            cc.width(p.width()).outerHeight(p.height());
        }
     ...
     * 改变嵌套layout的大小
     */
    function resizeNestedLayout(panels) {
        //2017/03/08 start
        var northNestedLayout = panels.north.find('.easyui-layout');
        if (northNestedLayout.length > 0) {
            northNestedLayout.layout('resize');
        }

        var eastNestedLayout = panels.east.find('.easyui-layout');
        if (eastNestedLayout.length > 0) {
            eastNestedLayout.layout('resize');
        }

        var westNestedLayout = panels.west.find('.easyui-layout');
        if (westNestedLayout.length > 0) {
            westNestedLayout.layout('resize');
        }

        var centerNestedLayout = panels.center.find('.easyui-layout');
        if (centerNestedLayout.length > 0) {
            centerNestedLayout.layout('resize');
        }

        var southNestedLayout = panels.south.find('.easyui-layout');
        if (southNestedLayout.length > 0) {
            southNestedLayout.layout('resize');
        }
    ...
    function resizeNestedGrid() {
        ...
        $.each(fitGridTables,function (i,fitGridTable) {
            ...
            var gridContainerPanel = $(fitGridTable).parents('div[region]')[0];
            if(gridContainerPanel){
                $(gridContainerPanel).css({
                    overflow:'hidden'
                });
                var gridContainerPanelWidth = $(gridContainerPanel).panel("options").width;
                var gridContainerPanelHeight = $(gridContainerPanel).panel("options").height;
                gridContainerPanelHeight = gridContainerPanelHeight-gridHeadrHeight-gridFooterHeight;
                gridContainerPanelHeight = gridContainerPanelHeight-$($(gridContainerPanel).siblings('.panel-header')[0]).outerHeight()-2;

                $(fitGridTable).jqGrid().setGridWidth(gridContainerPanelWidth-4,true);
                $(fitGridTable).jqGrid().setGridHeight(gridContainerPanelHeight);
            }
        });
    }
