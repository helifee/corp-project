      ...
      /*      orgGridFooter.attr('id','_relationFlowGridFooter'+new Date().getTime());*/
        ...
        _createModalHeaderContainer: function () {
            var modalHeaderContainer = $('<div class="modal-header">' +
                ...
                '   <h5 class="modal-title modal-jz" id="orgGridModalLabel">select relation flow/选择相关流程</h5>' +
                '   <button type="button" class="btn btn-sm blue pull-right mr20 modal-cancel">cancel/取消</button>' +
                '   <button type="button" class="btn btn-sm blue pull-right mr20 modal-save">determine/确定</button>' +
                '</div>');
        ...
        _createGridHeader:function () {
            var orgGridHeader = $('<div class="modal_title">' +
                ...
                '   <span>relation flow list/相关流程列表</span>' +
                '   <input type="text" class="form-control" placeholder="name/full path 名称/全路径">'+
                '   <button type="button" title="query" class="btn btn-sm btnMsearch rm-pad">'+
        ...
        _initGrid:function (gridObj,orgGridFooter) {
            var that = this;
            gridObj.jqGrid({
                url: hostUrl + "flow/instanceOperateLog/queryRelatedInstanceListByKeyword?_t="+new Date().getTime(),
                ...
                postData:{keyword: ''},
                colNames : ['ID', 'name', 'code', 'flow templet code', 'business ID', 'initiator','launch time','status'],//jqGrid的列显示名字
                colModel : [
                    {name: 'id', label: 'id', width: 75, align: "center", hidden: true},
                    {name: 'name', label: 'name', width: 75, align: "center"},
                    {name: 'code', label: 'code', align: "center", width: 55},
                    {name: 'flCode', label: 'flow templet code', align: "center", width: 55},
                    {name: 'businessId', label: 'business ID', align: "center", width: 55},
                    {name: 'startUserName', label: 'initiator', align: "center", width: 55},
                    {name: 'startDate', label: 'launch time', align: "center", width: 55},
                    {name: 'status', label: 'status', align: "center", width: 55}
                ],
                viewrecords: true,
                rownumbers: true,
                multiboxonly: true,
