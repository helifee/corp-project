	...
	$.extend($.xljUtils, {
		...
		xljDownloadFromFileInfo: function (filePath, fileName) {
            ...
            if (userAgent.indexOf("Firefox") > -1) {
                window.open($.xljUtils.serverAddr + "univ/attachment/attachment/filesExportHelper?filePath="+filePath+"&fileName="+fileName);
            }else{
                window.open(encodeURI(encodeURI($.xljUtils.serverAddr + "univ/attachment/attachment/filesExportHelper?filePath="+filePath+"&fileName="+fileName)));
            }
		},
	...
	XljAttachment.prototype = {
		...
		_createAttachmentModal:function(){
			...
			if (this.options.mode == $.xljUtils.modeEnum.table) {
				attHtmls.push('<table class="att-table">');
				attHtmls.push('	<tr>');
			...
			// 非table显示情况下
				...
				// 编辑模式下添加附件控件可用
					...
					attHtmls.push('        <br><span class="attachment-url-icon"></span>');
				...
				success:function(dt) {
					...
					if (that.options.mode != $.xljUtils.modeEnum.table) {
						...
						$.each(dt.result, function (index, file) {
							...
							if (that.options.mode != $.xljUtils.modeEnum.view) {
								fileLi.append($('<a href="javascript:void(0);" class="up_delete">删除</a>'));
							}
					...
					// 表格显示
						...
						$.each(dt.result, function (index, file) {
							...
							if(fileType.indexOf(file.fullName.substring(file.fullName.lastIndexOf(".")+1).toLowerCase())>-1){
		...
		_createFileInfoEle: function(file) {
			var that = this;
			return $('<li/>').append(
					$('<img class="fileType" style="width: 10%;"/>').attr('src', function() {
				...
				$('<div class="fileName" style="width: 68%;"/>').append($('<p/>').text(file.name).attr('title',file.name))
			...
			).append($('<a href="javascript:void(0);" style="width: 12%" class="up_delete">删除</a>'));
		},
		// 添加附件操作处理
		...
		target.on('click',function() {
			...
			obj.GROUP = file.path;
			obj.NAME = file.name;
			var extensionName = obj.FILENAME.substring(obj.FILENAME.lastIndexOf(".")+1).toLowerCase();
			if(officeType.indexOf(extensionName)>-1){
				$.ajax({
					...
					success:function (resultData ) {
						if(resultData) {
							...
							if(successFlag) {
								...
								debugger;
								if("html"==exName){
									// window.open($.xljUtils.serverAddr+resultData.msg);
									window.open(encodeURI(encodeURI($.xljUtils.serverAddr+"/mobile/approve/approve_view.html?path="+resultData.msg.replace(/\\/g,"/")+"&fileName="+file.name)));
								}else{
									// window.open("../../../pdf.html?path="+resultData.msg.replace(/\\/g,"/"));
									window.open(encodeURI(encodeURI($.xljUtils.serverAddr+"/pdf.html?path="+resultData.msg.replace(/\\/g,"/")+"&fileName="+file.name)));
								}
							}else {
			...
			}else if(pngType.indexOf(extensionName) > -1){
				$.post($.xljUtils.serverAddr + "univ/attachment/attachment/getStorageIP",
					{ filePath: file.path },
					function (ip) {
						if (ip) {
							// window.location.href=location.protocol + '//' +ip+":"+$.xljUtils.fdfsStoragePort+"/"+file.path;
							window.open(encodeURI(encodeURI($.xljUtils.serverAddr+"/mobile/approve/approve_view.html?path="+location.protocol + '//' +ip+":"+$.xljUtils.fdfsStoragePort+"/"+file.path+"&fileName="+file.name)));
						}
					}
				);
				// window.open(location.protocol + '//' +file.url+":"+$.xljUtils.fdfsStoragePort+"/"+file.path);
			}else{
				$.xljUtils.tip("red","该文件不支持预览！")
				return;
			}
		});
	},
...
//            }
			...
			var $this = $('.fileupload', this.$element).fileupload({
			    ...
			    dataType: 'json',
			    formData:{appId:this.options.appId, businessId:this.options.businessId,categoryId:this.options.categoryId, type:'file'},
			    autoUpload: false,
