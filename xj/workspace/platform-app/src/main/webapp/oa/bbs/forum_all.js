...
function bindButton() {
	...
	$('#gotoEssenceBtn').click(function () {
		...
		$("#list2").jqGrid('setGridParam', {postData:postData,url:baseUrl+"oa/bbs/topic/page",page:1}).trigger('reloadGrid');
	});
	//收藏按钮
...
function getOperationAuthorition() {
	...
	$.ajax({
		type: 'GET',
		url: hostUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=sytz',
		dataType: 'json',
		//contentType: 'application/json',
		async: false,
