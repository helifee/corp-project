...
$(function(){
	...
	$('#thruDate').siblings('.input-group-addon-menu').on('click',function () {
		WdatePicker({
			el: 'thruDate',
			dateFmt: "yyyy-MM-dd",
			errDealMode:-1,
    ...
    function initCategoryList() {
        var categoryGrid = jQuery("#serveyCreateList").jqGrid({
            url: hostUrl + 'oa/servey/serveyCreate/pageByUser',
  ...
  //修改调查结束时间
    ...
    $("#seeFlowBtn").unbind('click').on('click',function () {
        ...
        if(rowData.status == "草稿"){
        	...
        	var pcUrl = "/platform-app/oa/servey/serveyCreate/serveyCreate_flow.html";
	 ...
	 if(idsVal&&idsVal!="") {
		 ...
		 if(flag == "false"){
		   ...
		   pop_text_open("blue", "确认要删除这【" +idsVal.length + "】条数据吗？",function(){
			   ...
			   $.ajax({
		        	 url:hostUrl+"/oa/servey/serveyCreate/deleteBatch/"+idsVal,
		        	 type:'DELETE',
		        	 dataType:'JSON',
		        	 contentType:'application/json',
		        	 data:JSON.stringify({}),
...
function preview(){
    ...
    var rowData = $('#serveyCreateList').jqGrid("getRowData",idVal);
	window.open(hostUrl+"oa/servey/serveyQuestionnaire/serveyQuestionnaire_edit.html?serveyId=" + idVal + "&oper=add&serveyName=" + encodeURI(rowData.name, "UTF-8") + "&type=1");
}
...
function investigationOver(id){
	...
	$.ajax({
		contentType: "application/json",
	    type: 'POST',
	    url:hostUrl+"oa/servey/serveyCreate/investigationOver?time="+Math.random(),
	    dataType: "json",
...
function remindersNotInvestigators(id){
	$.ajax({
		contentType: "application/json",
	    type: 'POST',
	    url:hostUrl+"oa/servey/serveyCreate/remindersNotInvestigators?time="+Math.random(),
	    dataType: "json",
	   	data:JSON.stringify({"serveyId":id}),
	   	success:function (xhr,textStatus ) {
	   		 if (xhr){
...
function updateDate(){
	...
	$.ajax({
		contentType: "application/json",
	    type: 'POST',
	    url:hostUrl+"oa/servey/serveyCreate/updateThruDate?time="+Math.random(),
	    dataType: "json",
