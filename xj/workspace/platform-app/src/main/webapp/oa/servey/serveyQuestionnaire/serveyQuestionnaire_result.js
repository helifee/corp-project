...
;(function ($,window,document,undefined) {
    ...
    $(document).ready(function () {
        try {
            ...
            if(oper == 'DY'){
            	...
            	$.ajax({ //发送更新的ajax请求
                    type: "post",
                    url: hostUrl+"flow/sysNoticeMsg/deleteOpTypeDataByParamMap?time="+Math.random(),
                    dataType: "json",
                    async: false,
    ...
    function createContentRowTypeForm() {
        $.ajax({
            ...
            url: baseUrl + "oa/servey/serveyQuestion/getCountResponseAnswerByQuestionId?time=" + Math.random(),
...
//            initData.createDate = $.xljUtils.formatDate('yyyy-MM-dd hh:mm:ss',new Date());
            ...
            $.ajax({
            	contentType: "application/json",
                type: 'POST',
                url: baseUrl + "oa/servey/serveyResponse/queryListByParameter?time="+Math.random(),
                dataType: "json",
                async:false,
...
//            }
    ...
    function enumPropertiesResult(questionId) {
        ...
        $.ajax({
            ...
            type: 'POST',
            async: false,
            url: baseUrl + 'oa/servey/serveyQuestionOption/queryList',
            dataType: "json",
    ...
    function judgeExitServeyResponseByServeyId(serveyId) {
        $.ajax({
            ...
            type: 'POST',
            async: false,
            url: baseUrl + 'oa/servey/serveyResponse/queryList',
            dataType: "json",
            data: JSON.stringify({serveyId: serveyId}),
            success: function (data) {
                console.log(data);
    ...
    function submitForm(saveType) {
		...
		if(saveType == 'update') {
	    	...
	    	url = baseUrl + "oa/servey/serveyResponse/update/"+id;
	    	 method = 'PUT';
		}else {
			ServeyResponseDto.delflag=false;
			//暂存操作
			if(saveType == 'temporary'){
				url = baseUrl + "oa/servey/serveyResponse/temporary/save";
			}
		}
