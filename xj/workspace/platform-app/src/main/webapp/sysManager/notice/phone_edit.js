...
(function(window,document){
    var _dataEdit = {
    	...
    	save_form:function(){//保存规则编号类型
    		var my = this;
    		//映射后台保存方法
			var url=hostUrl+"sys/sysNoticePhoneServer/save";
			//请求方式
			var type = 'POST';
			...
			if(this.dataPar.upId){
				//映射后台修改方法
				url=hostUrl+"sys/sysNoticePhoneServer/update/"+my.dataPar.upId;
				var type = 'PUT';
			}
    	    $.ajax({
    	       url:url,
    	...
    	initFrom:function(){
    		...
    		if(this.dataPar.upId){
    			$.ajax({
        	        type:'get',
        	        url:hostUrl+'sys/sysNoticePhoneServer/get/'+this.dataPar.upId+'?time='+Math.random(),
        	        success: function(data) {
