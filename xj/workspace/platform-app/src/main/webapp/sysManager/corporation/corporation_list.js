...
if(ids&&ids!='') {
	  pop_text_open("blue",'确认要删除这条数据吗？',function(){
			$.ajax({
				url:hostUrl+"sys/base/baseCorporation/delete/"+ids,
				type:'DELETE',
				dataType:'JSON',
...
function getCorporationData(){
    jQuery("#list2").jqGrid(
            {
                url: hostUrl+'sys/base/baseCorporation/page',
...
function getAccontData(){
    jQuery("#list3").jqGrid({
		url: hostUrl+'sys/base/baseCorporationAccont/getAccontByCorporationId',
        ajaxGridOptions: { contentType: 'application/json' },
        mtype : "POST",  
        contentType : "application/json",  
        datatype : "json", 
...
function  updateStatus(e){
	...
	if(e==dataStatus){
		...
		$.ajax({
			url:hostUrl+"sys/base/baseCorporation/updateStatus/"+ids,
			type:'PUT',
			dataType:'JSON',
			success:function (resultData ) {
				if (resultData&&resultData.success) {
