...
function selectPayParent(id){
	  $.ajax({
          type:'POST',
          url:hostUrl+'base/payType/payTypeParanetList',
          dataType:'json',
...
function getPayTypeData(id){
	$.ajax({
        type:'get',
        url:hostUrl+"base/payType/get/"+id+'?time='+Math.random(),
        success: function(data) {
        	if(data.success){
	...
	var payTypeDto={};
		...
		if(type=="add"){
			...
			var ids=payTypeDto.id;
			   $.ajax({
			       url:hostUrl+"base/payType/update/"+ids,
			       data:JSON.stringify(payTypeDto),
			       type:'put',
			       contentType:'application/json',
			       dataType:'JSON',
