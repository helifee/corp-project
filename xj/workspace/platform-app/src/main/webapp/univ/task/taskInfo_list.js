...
function deleteClick() {
    ...
    if (selIds.length == 0) {
    	...
    	if ($.xljUtils.confirm('blue', '是否要删除选中的'+selIds.length + '行数据', function(){
			$.xljUtils.xljAjax({
				type:'DELETE',
		        url:hostUrl + 'univ/task/taskInfo/deleteBatch/' + selIds.join(',')
			}, function(data){
...
function pauseClick() {
    ...
    if (selIds.length == 0) {
    	...
    	if ($.xljUtils.confirm('blue', '是否要暂停选中的'+selIds.length + '条任务', function(){
			$.xljUtils.xljAjax({
				type:'PUT',
		        url:hostUrl + 'univ/task/taskInfo/updateTaskState/' + selIds.join(','),
...
function executeClick() {
    ...
    if (selIds.length == 0) {
    	...
    	if ($.xljUtils.confirm('blue', '是否要立即执行选中的'+selIds.length + '条任务', function(){
			$.xljUtils.xljAjax({
				type:'GET',
		        url:hostUrl + 'univ/task/taskInfo/executeNow/' + selIds.join(',')
			}, function(data){
        		$.xljUtils.reLoadJqGridData('#list');
...
function loadGridData(){
	...
	$.xljUtils.initJqGrid({
		gridSelecter:'#list',
		url : hostUrl + 'univ/task/taskInfo/page?random='+Date.now(),
		postData : {},
		...
		editUrl:hostUrl + 'univ/task/taskInfo_edit.html'
	});
};
