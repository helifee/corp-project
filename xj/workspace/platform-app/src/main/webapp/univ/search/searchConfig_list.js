...
function initPropsGrid(){
	...
	$.xljUtils.initJqGrid({
		...
		multiselect:true,
		sortname : "updateDate",
		editUrl:hostUrl + 'univ/search/searchProperty_edit.html'
	});
}
...
function initCategoryGrid(){
	...
	$.xljUtils.initJqGrid({
		...
		url:hostUrl + 'univ/search/searchCategory/page',
		...
		multiselect:true,
		sortname : "updateDate",
		editUrl:hostUrl + 'univ/search/searchCategory_edit.html'
	});
}
...
$(function() {
    ...
    $('.btn-enable-props').click(function(){
		...
		$.xljUtils.xljAjax({
			type:'PUT',
			url: baseUrl + 'univ/search/searchProperty/update/'+selId,
	...
	$('.btn-enable-category').click(function(){
		...
		$.xljUtils.xljAjax({
			type:'POST',
			url: hostUrl + 'univ/search/searchCategory/updateStatus',
    ...
    $('.btn-del-props').click(function(){
        ...
        if (selIds.length > 0) {
        	...
        	if ($.xljUtils.confirm('blue', '是否要删除选中的'+selIds.length + '行数据', function(){
				...
				$.xljUtils.xljAjax({
					type:'DELETE',
    		        url:hostUrl + 'univ/search/searchProperty/deleteBatch/' + selIds.join(',')
				},function (data) {
					$.xljUtils.reLoadJqGridData('#searchPropsGrid');
    ...
    $('.btn-del-category').click(function(){
    	...
    	if (selIds.length > 0) {
    		...
    		if ($.xljUtils.confirm('blue', '是否要删除选中的'+selIds.length + '行数据', function(){
				...
				$.xljUtils.xljAjax({
					type:'DELETE',
    				url:hostUrl + 'univ/search/searchCategory/deleteBatch/' + selIds.join(',')
				},function (data) {
					$.xljUtils.reLoadJqGridData('#categoryGrid');
    				$.xljUtils.tip('green',data.msg);
				});
