...
(function(window,document){
    var _tableList = {
    	...
    	get_dataList:function(){
    		var my = this;
    		jQuery('#tree').jqGrid({
				url:hostUrl+"oa/office/officeHouse/queryTreeGridList"+'?time='+Math.random(),
    	...
    	updateStatus:function(data){
    		$.ajax({
    			url:hostUrl+"oa/office/officeHouse/updateStatus"+'?t_='+new Date().getTime(),
    			type:'POST',
    			dataType:'JSON',
    	...
    	to_deleteEntry:function(){
    		...
    		pop_text_open("blue",str,function(){
    			$.ajax({
    				url:hostUrl+"oa/office/officeHouse/deletePseudoBatch/"+delIds+'?t_='+new Date().getTime(),
    				type:'DELETE',
    				dataType:'JSON',
    	...
    	upRulerSort:function (type){
    		...
    		$.ajax({
    			url:hostUrl+"oa/office/officeHouse/updateSort/"+ids+'?t_='+new Date().getTime(),
    			type:'put',
    			dataType:'JSON',
    			contentType:"application/json",
    			data:'{"sortType":'+type+'}',
		...
		getOperationAuthorition:function () {
			...
			$.ajax({
				type: 'GET',
				url: hostUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=YPFL',
				dataType: 'json',
				//contentType: 'application/json',
				async: false,
