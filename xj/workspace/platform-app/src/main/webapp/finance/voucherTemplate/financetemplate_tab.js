...
(function(window, document) {
	var _tabList = {
		...
		getCompany:function() {
			...
			$.ajax({
				url : hostUrl + "finance/accountSet/queryAccounSetList",
				data : JSON.stringify(data),
				type : 'POST',
	...
	bindExcelEvent:function(){
			...
			//submit url
			$("#excelModalFrom").attr("action",hostUrl + "finance/excel/excelHelper");
		},
		//复选框改变事件
		...
		excelConfirm:function(){
			...
			if(data.type==2){
				location.href=hostUrl + "finance/excel/excelExportHelper?excelCompanyId="+selectCompany;
			}else{
...
//				}
				...
				$.ajax({
					url : hostUrl + "finance/excel/excelImportHelper",
