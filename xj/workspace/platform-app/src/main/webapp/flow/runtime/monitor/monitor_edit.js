...
function queryMonitorObject(){
	$.ajax({
	       url: hostUrl+"flow/monitorSetting/get/"+monitorId+"?t="+ new Date().getTime(),
	       type:'GET',
...
function queryAndShowMonitorData(setttingId){
	var urlText = hostUrl+"flow/monitorPerson/queryList";
...
function queryAndShowFlowData(setttingId){//   
	var urlText = hostUrl+"flow/monitorFl/queryMonitorFlowList";
	var dataList = commonQueryAndReturnData(setttingId, urlText);
...
function queryAndShowMonitoredData(setttingId){
	var urlText = hostUrl+"flow/monitoredPerson/queryList";
...
function saveFormData(monitorObjDto){
	$.ajax({
	       url: hostUrl+"flow/monitorSetting/saveAllSettingData",
...
function initChooseGrid(){
	...
	if(chooseType == "monitor" || chooseType == "monitored"){
		...
		$("#choosePostDiv").show();
		$("#chooseFlowDiv").hide();
		paramUrl = hostUrl+"flow/approveType/queryList";
		...
		$("#choosePostDiv").hide();
		$("#chooseFlowDiv").show();
		paramUrl = hostUrl+"flow/fl/queryFlowBusiObjectList";
		colModel = [{name:'id',label:'ID',align:"center",hidden:true},
		            {name:'name',label:'名称',align:"center"},
