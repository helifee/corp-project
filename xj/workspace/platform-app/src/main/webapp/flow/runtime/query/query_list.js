...
function initTwoQueryGridList(){
	...
	var urlText1 = hostUrl+"flow/instance/queryInstanceBy"; //;
	...
	var urlText2 = hostUrl+"flow/agent/queryAgentInstanceList";//
...
function adminInstance(instanceId, status, businessObjectId, businessId){
	...
	var url = hostUrl+"flow/runtime/approve/flow.html?instanceId=" + instanceId
...
function batchModifyReader(){
	...
	if(selectedIds && selectedIds.length>0){
    	var url = encodeURI(hostUrl+"flow/runtime/query/batch_modify_reader.html");
    	openWin(url);
    }else{
