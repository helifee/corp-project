...
function querySystemAppInfo(systemId){
	$.ajax({
	       url: hostUrl+"sys/res/appSystem/get/"+systemId,
	       type:'GET',
	       contentType:'application/json',
	       dataType:'JSON',
	       success:function (resultData) {
...
function queryBusiVariableList(){
    jQuery("#jqgridList").jqGrid(//创建jqGrid组件
        {
            url : hostUrl+"flow/businessObjectVariable/queryListByCondition",//获取数据的地址
...
function updateSort(e){
	...
	if(ids){
		...
		$.ajax({
			url:hostUrl+"flow/businessObjectVariable/updateSort/"+ids,
			type:'put',
			dataType:'JSON',
			contentType:"application/json",
...
function deleteItem() {
	...
	$.ajax({ //发送更新的ajax请求
        type: "post",
        url: hostUrl+"flow/businessObjectVariable/queryRelatedCountByPrefixMap",
        dataType: "json",
        async: false,
	...
	pop_text_open("blue", popText, function() {
		$.ajax({
		       url: hostUrl+"flow/businessObjectVariable/deletePseudo/"+lastSel_dataId,
		       type:'DELETE',
		       contentType:'application/json',
		       dataType:'JSON',
		       success:function (resultData) {
...
function checkDeleteVariableAction(varCode){ 
	var checkResult = "";
	var urlText = hostUrl+"/flow/step/bizVarBeUsedInFlow?businessObjectId="+businessObjectId+"&varCode="+varCode;
	$.ajax({
	       url: urlText,
	       type: 'GET',
	       async: false,
