...
function getCommonToolsData(id){
	   $.ajax({
	       url:hostUrl+"sys/sysCommonTools/get/"+id+'?time='+Math.random(),
	       type:'get',
	       success: function(data) {
	    	   if(data.success){
...
/*	   $.ajax({
		   url:hostUrl+"sys/sysCommonTools/showPic/"+id+'?time='+Math.random(),
	       type:'get',
	       success: function(data) {
	    	   if(data.success){
...
function saveForm(btType){
	var  url = hostUrl+"sys/sysCommonTools/save";
	if(type=="add"){
	}else{
		url = hostUrl+"sys/sysCommonTools/update";
	}
	var ajax_option={
