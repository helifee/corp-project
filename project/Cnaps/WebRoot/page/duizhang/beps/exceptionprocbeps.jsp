	      		...
	      		<table width="100%" border="0" cellspacing="0" cellpadding="0">
	        		<tr>
	          			...
	          			<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" >
                				<tr>
                					<td colspan="6">
                						<div  class="text_title"><span class="text_blue2">小额异常查询列表</span></div>
                					</td>
                				</tr>
                			</table>
					             	...
					             	<c:forEach items="${entity.duizhangList}" var="po">
					             		<tr class="text_list" onmouseover="this.bgColor='#CE4C56'" onmouseout="this.bgColor='E6E6E6'" style="cursor: hand;" bgcolor="E6E6E6">
							                  <td  ><div class="gridCell_standard">
							                  	<c:if test="${po.pmtTp eq 'beps.130.001.01'}">CIS通用回执</c:if>
							                  	<c:if test="${po.pmtTp eq 'beps.121.001.01'}">客户发起普通贷记业务报文    </c:if>
												<c:if test="${po.pmtTp eq 'beps.122.001.01'}">金融机构发起普通贷记业务报文</c:if>  
												<c:if test="${po.pmtTp eq 'beps.123.001.01'}">实时贷记业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.125.001.01'}">定期贷记业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.127.001.01'}">普通借记业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.131.001.01'}">实时借记业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.133.001.01'}">定期借记业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.380.001.01'}">批量代收业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.382.001.01'}">批量代付业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.384.001.01'}">实时代收业务报文            </c:if>  
												<c:if test="${po.pmtTp eq 'beps.385.001.01'}">实时代付业务报文            </c:if>      
												<c:if test="${po.pmtTp eq 'ccms.314.001.01'}">信息类报文                  </c:if>
												<c:if test="${po.pmtTp eq 'ccms.315.001.01'}">信息类报文</c:if>
							                  </div></td>
