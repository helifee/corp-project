...
function  PubQueryAccount(paymentGroupNum){
			if(isNull(trim(paymentGroupNum))){
				...
				var pop = createPopWin("popid",'系统正处理...',"<%=request.getContextPath()%>/page/transfer/shouquanCons.jsp");
					...
					return;
								...
								document.getElementById("proposerCstmrId").value=obj.cstmrid==null?"": obj.cstmrid;//客户号
						      	document.getElementById("receAccountType").value=obj.accttp==null?"": obj.accttp;//账户类型
						      	document.getElementById("receAccountType1").value=obj.accttp==null?"": obj.accttp;//账户类型
			}
			});
	   	}
   ...
   // alert("data！");
		...
		userMap.accttp =document.getElementById("payAccountType").value; //账号类型
		  PubService.saveOthersBankAccountMsg(userMap, saveFun); 
		}
		//查询他行户信息 个人转账跨行通兑 实时借记业务 付款人信息
	function queryBypaymentGroupNum(paymentGroupNum){
        ...
        PubService.queryOthersBankAccountMsg(paymentGroupNum,function(obj){
		  ...
		  if(obj==null||obj.accountnumber==null){
		      ...
		      document.getElementById("payName").value=obj.accountname==null?"": obj.accountname  ;//人名称
			  document.getElementById("payAddress").value=obj.addr==null?"": obj.addr  ; //人地址
			   document.getElementById("payBankNum").value= obj.issuer==null?"": obj.issuer ;   //行号
			  document.getElementById("payBankName").value=obj.issuernm==null?"":obj.issuernm ; //行名
			  document.getElementById("payAccountType").value=obj.accttp==null?"": obj.accttp;//账户类型	
			   document.getElementById("payOpenBankNum").value= obj.issuer==null?"": obj.issuer ;   //开户行行号
			  document.getElementById("payOpenBankName").value=obj.issuernm==null?"":obj.issuernm ; //开户行行名
			...
			function jisuan(){
			   ...
			   var msg = "@";
                   ...
                   var payAddress = document.getElementById("payAddress");
                   ...
                   var moneyNum = document.getElementById("moneyNum");
                     ...
                     var payAccountType = document.getElementById("payAccountType");
                     var receAccountType1  = document.getElementById("receAccountType1");
                     var fkqshhh  = document.getElementById("fkqshhh");
				...
				if(isNull(trim(cdtrMmbId.value))){
					...
					if(isNull(trim(payAccountType.value))){
					msg += payAccountType.title+"不能为空！@";
				    }

					if(isNull(trim(receAccountType1.value))){
					msg += receAccountType1.title+"不能为空！@";
				    }
				...
				document.getElementById("moneyNum").value=rmoney(document.getElementById("moneyNum").value);//汇款金额
				  document.getElementById("handingCharge").value=rmoney(document.getElementById("handingCharge").value );
				    document.getElementById("postCharge").value=rmoney( document.getElementById("postCharge").value);
				     ...
				     document.getElementById("dbtramtacctid").value=rmoney( document.getElementById("payerAcount").value);
				     document.getElementById("dbamtnm").value=rmoney(document.getElementById("payName").value);
					open2();
				  	}
		 }
...
　　return false;
 ...
 <script>
      function countLen(){
      		this.document.getElementById('chckCdLen').value=this.document.getElementById('password').value.length;
      }
      </script>
	</head>
	<body onload="sendDate()">
		<form name="form1" method="post"
			action="<%=path%>/transfer/PersonCrashSpanBankExchangeAction.do?method=sendCredit">
			<input id="signval" type="hidden" value="sign0">
			<input type="hidden" name="token" value="${token}" />
			<!-- 防止重复提交 -->
			<input id="cardcrash" name="cardcrash" type="hidden"
				value="zhuanzhang">
			...
			<input id="yxj" name="yxj" type="hidden" maxlength="19" value="NORM" />


			<!-- 签发模式 -->
			<input  id="signMd" name="po.signMd" type="hidden" maxlength="19" value="03" />
											...
											<tr>
												<td>
													<div class="text_title">
														<span class="text_blue2">个人转账跨行通兑</span>
													</div>
												</td>
											</tr>
											<tr>
														...
														<div class="table_content">
															<table>
																...
																<tr>
																	...
																	<td>
																		<input name="po.paymentGroupNum" id="paymentGroupNum"
																	...
																	<td>
																		<input name="endtoendid" id="dddbsh" type="hidden"
																			value="1234" maxlength="19" title="端到端标识号" />
																...
																<tr>
																	...
																	<td>
																		<select name="po.businessSizeCode"
																			...
																			title="业务类型编码">
																			<option value="D102" selected="selected">
																				个人储蓄通兑业务
																			</option>
																	...
																	<td>
																		<select name="po.businessClassCode"
																			id="businessClassCode" style="width: 180px;"
																			title="业务种类编码">
																...
																<tr>
																	...
																	<td>
																		<select name="po.checkModeCode" id="checkModeCode"
																			style="width: 180px;" title="密码验证算法">
																			<option value="CD00" selected="selected">
																				支付密码单密码
																			</option>
																			<option value="CD01">
																				支付密码器密码
																	...
																	<td>
																		<input style="width: 180px;" name="po.password" onblur="countLen();"
																			id="password" type="password" size="19"
																			maxlength="19" title="密码" />
																		<span class="STYLE1">*</span>
																...
																<td >
																	<input style="width:180px;" name="po.chckCdLen" id="chckCdLen" type="text" readonly="readonly" 
																		maxlength="2" title="密码验证码长度" /><span  class="STYLE1">*</span>
														...
														<div class="table_content">
															<table>
																...
																<tr>
																	...
																	<td>
																		<input name="po.payerAcount" id="payerAcount"
																			type="text"
																...
																<tr>
																	...
																	<td colspan="3">
																		<input name="po.payAddress" id="payAddress"
																			class="text_tablehead_b_addr"
																			onKeyPress="actkeyPress()"
																			onkeyup="limitLength(value,70,'提示：','payAddress')" />
																...
																<tr>
																	...
																	<td>
																		<input name="po.payOpenBankNum" id="payOpenBankNum"
																			type="hidden" class="text_tablehead_b_c"
																	...
																	<td>
																		...
																		<input name="po.payOpenBankName" id="payOpenBankName"
																			type="hidden" title="付款人开户行名称" maxlength="60"
																...
																<tr>
																	...
																	<td>
																		<select name="po.payAccountType" title="付款人账户类型"
																			id="payAccountType">
																			<option value="">
																				请选择
																			</option>
																	...
																	<td>
																			<input name="po.dbtrmmbid" id="fkqshhh" type="text"  
																		 title="付款清算行行号" maxlength="12" readonly="readonly"
																		onKeyPress="actkeyPress()" /> 
																...
																<tr>
																	...
																	<td>
																		<input name="po.payBankNum" id="payBankNum"
																			type="text" class="text_tablehead_b_c" title="付款行行号"
																	...
																	<td>
																		<input name="po.payBankName" id="payBankName"
																			type="text" title="付款行名称" maxlength="60"
																			readonly="readonly" onKeyPress="actkeyPress()" />
																		<!--  <span  class="STYLE1">*</span>-->
																	</td>
																</tr>
														...
														<div class="table_content">
															<table>
																...
																<tr>
																	...
																	<td>
																		<input name="po.receAcount" id="receAcount"
																			type="text"
																	...
																	<td>
																		<input name="po.name" id="name" type="text"
																			readonly="readonly" title="收款人名称" maxlength="60"
																...
																<tr>
																	...
																	<td>
																		<input name="po.cdtrBrnchId" id="cdtrBrnchId"
																			type="hidden" class="text_tablehead_b_c"
																	...
																	<td>
																		<input name="po.cdtrBrnchNm" id="cdtrBrnchNm"
																			type="hidden" value="${bankInfo.participantname}"
																...
																<tr>
																	...
																	<td>
																		<input name="po.certnum" id="certnum" type="text"
																			size="19" onblur="checkIdcard(this.value,'certnum')"
																			readonly="readonly" maxlength="32" title="收款人证件号" />
																			<span  class="STYLE1">*</span>
																	</td>
																</tr>
																<tr>
																	...
																	<td>
																		<input name="po.phone" id="phone" type="text"
																			readonly="readonly" maxlength="20" title="收款人联系电话" />

																	</td>
																	<td class="text_tablehead_b">
																		收款人账户类型
																	</td>
																	<td><input  name="po.receAccountType" id="receAccountType" type="hidden" readonly="readonly" title="收款人账户类型" />
																		<select name="po.receAccountType1" title="收款人账户类型"
																			disabled="disabled" id="receAccountType1">
																			<option value="">
																				请选择
																			</option>
																			<option value="AT01">
																...
																<tr>
																	...
																	<td>
																		<input type="text" maxlength="30"
																			name="po.proposerCstmrId" title="收款人客户号"
																			readonly="readonly" id="proposerCstmrId">
																		<input type="hidden" readonly="readonly"
																			name="po.proposerAcctCcy" readonly="readonly"
																			title="收款人账户币种" id="proposerAcctCcy" value="CNY">
																...
																<tr>
																	...
																	<td colspan="3">
																		<input type="text" class="text_tablehead_b_addr"
																			name="po.cdtrAddr" id="address" readonly="readonly"
																			onKeyPress="actkeyPress()"
																			onkeyup="limitLength(value,70,'提示：','address')" />
																...
																<tr>
																	...
																	<td>
																		<input name="po.cdtrIssuer" id="receOpenBankNum"
																			type="hidden" class="text_tablehead_b_c"
																			value="${bankInfo.bankcode}" title="收款行开户行行号"
																			maxlength="12" readonly="readonly"
																			onKeyPress="actkeyPress()" />
																	...
																	<td>
																		<input name="po.cdtrIssuerNm" id="receOpenBankName"
																			type="hidden" readonly="readonly"
																			value="${bankInfo.participantname}" title="收款人开户行名称"
																			maxlength="60" onKeyPress="actkeyPress()" />
																	</td>
																</tr>
																...
																<tr>
																	...
																	<td>
																		<input name="po.cdtrMmbId" id="cdtrMmbId"
																			type="hidden" readonly="readonly" title="收款行清算行行号"
																			maxlength="12" value="${bankInfo.directbankcode }"
																			onKeyPress="actkeyPress()" />
																		<!--	<span  class="STYLE1">*</span>-->
																	</td>
					...
					<td >					
						...
						<input type="radio" class="text_tablehead_b_rad" name="po.waive" id="waiven" value="N"  onclick="clrAmt0()">不收取
																...
																<tr>
																	...
																	<td>
																		<input name="po.handingCharge" id="handingCharge"
																			type="text" readonly="readonly" title="手续费"
																			maxlength="12" />
																...
																<tr>
																	...
																	<td>
																		<input name="po.postCharge" id="postCharge"
																			type="text" readonly="readonly" title="邮电费"
																			maxlength="12" />
																	...
																	<td colspan="4">
																		<input name="po.otherPlaceCharge"
																			id="otherPlaceCharge" type="text" readonly="readonly"
																...
																<tr>
																	...
																	<td colspan="3">
																		<textarea name="po.postscript" id="postscript"
																			rows="2" cols="70" onKeyPress="charPress()"
																			onkeyup="limitLength(value,135,'提示：','postscript')"></textarea>
																	</td>
