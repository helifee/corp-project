    ...
    if(id!=""&&id!=null){
        ...
        $.ajax({
            ...
            success: function(data) {
                $("#sysCodeItemFrom").find("input[name='id']").val(data.result.sid);
                ...
                if(data.result.parent_id!=undefined){
                    var parentName = $.hrUtils.getHRCodeNameById(data.result.parent_id);
                    $("#sysCodeItemFrom").find("input[name='parent_name']").val(parentName);
...
window.saveInfo = function(){
    ...
    var uAlla = hostUrl + "sys/sysSerialNumber/getSysCodeItemValueByType/"+numType+"?time=" + new Date().getTime();
    $.ajax({
        type:'get',
        url:uAlla,
        success: function(data) {
            ...
            $.ajax({
                ...
                success:function (xhr) {
                    if (xhr){
                        if(xhr.success) {
                            window.opener.sysCodeItemCallBack(xhr.result.sid);
                            closeWindow();
                        }else{
                            if(xhr.code=="50000"){
...
window.updateInfo = function(){
    ...
    $.ajax({
        url:hostUrl+"sys/sysCodeItem/update/"+sysCodeItemID,
        type:'put',
        dataType:'JSON',
        contentType:'application/json',
        data:JSON.stringify(sysCodeItemDto),
