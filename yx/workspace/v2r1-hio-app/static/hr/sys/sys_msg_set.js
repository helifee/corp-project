...
(function ($, window, document, undefined) {
    ...
    function getitems(id) {
        ...
        $.ajax({
            type: "POST",
            url: hostUrl + "sys/sysMsgSet/queryList",
    ...
    window.save = function () {
        ...
        if (id == null || id == "") { // 页面默认加载一个项目，这个项目的id存在隐藏域中，保存时，如果有id  是更新，没有的话 是新增  点击新增按钮，隐藏域的ID会清除，点击左侧树。就从新赋值
            ...
            $.ajax({
                type: "POST",
                url: hostUrl + "sys/sysMsgSet/save",
    ...
    function updateMethod() {
        ...
        $.ajax({
            type: "PUT",
            url: hostUrl + "sys/sysMsgSet/update/" + postID,
            data: JSON.stringify(postDto),
            dataType: "JSON",
            contentType: "application/json",
            success: function (data) {
    ...
    function delMsgInfo() {
        ...
        if (id && id != '') {
            $.xljUtils.confirm("blue", "预警设置删除后系统将不会发送改预警的消息，确认要删除这【" + name + "】条数据吗？", function () {
                $.ajax({
                    type: "DELETE",
                    url: hostUrl + "sys/sysMsgSet/deletePseudoBatch/" + id,
                    data: JSON.stringify({}),
                    dataType: "JSON",
                    contentType: "application/json",
                    success: function (data) {
    ...
    function initOrgList() {
        ...
        jqGridOrg = jQuery("#orgList").jqGrid(
            {
                url: baseUrl + '/sys/sysMsgScopeOrg/queryListByCondition',//创建完成之后请求数据的url
    ...
    function saveOrgInfo(orgId, orgName) {
        ...
        if (id != null && id != "") {
            ...
            $.ajax({
                type: "POST",
                url: hostUrl + "sys/sysMsgScopeOrg/save",
                data: JSON.stringify(postDto),
                dataType: "JSON",
                contentType: "application/json",
                success: function (data) {
    ...
    function delOrg() {
        ...
        if (idsVal && idsVal != "") {
            $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                $.ajax({
                    url: baseUrl + "/sys/sysMsgScopeOrg/deleteBatch/" + idsVal,
                    type: 'DELETE',
                    dataType: 'JSON',
                    contentType: 'application/json',
                    data: JSON.stringify({}),
    ...
    function initPostList() {
        ...
        jqGridPost = jQuery("#postList").jqGrid(
            {
                url: baseUrl + '/sys/sysMsgScopePost/queryListByCondition',//创建完成之后请求数据的url
                datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
                mtype: "post",//向后台请求数据的ajax的类型。可选post,get
                ajaxGridOptions: {contentType: 'application/json'},
                contentType: "application/json",
                postData: {"msgSetId": id},
                autowidth: true,
                colNames: ['id', '消息设置id', '岗位名称','岗位编码','岗位类型','状态','目录'],
                colModel: [
                    {name: 'id', index: 'id', editable: true, sortable: false, hidden: true},
                    {name: 'msgSetId', index: 'msgSetId', editable: true, sortable: false, hidden: true},
                    ...
                    {name: 'catalogId', index: 'catalogId', editable: true, sortable: false, align: 'center'}
                ],
                multiselect: true,
                multiboxonly: true,
    ...
    function savePostInfo(postId, postName) {
        ...
        if (id != null && id != "") {
            ...
            $.ajax({
                type: "POST",
                url: hostUrl + "sys/sysMsgScopePost/save",
                data: JSON.stringify(postDto),
                dataType: "JSON",
                contentType: "application/json",
                success: function (data) {
    ...
    function delPost() {
        ...
        if (idsVal && idsVal != "") {
            $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                $.ajax({
                    url: baseUrl + "/sys/sysMsgScopePost/deleteBatch/" + idsVal,
                    type: 'DELETE',
                    dataType: 'JSON',
                    contentType: 'application/json',
                    data: JSON.stringify({}),
    ...
    function initPersonList() {
        ...
        jqGridPerson = jQuery("#personList").jqGrid(
            {
                url: baseUrl + '/sys/sysMsgScopePerson/queryListByCondition',//创建完成之后请求数据的url
                datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
                mtype: "post",//向后台请求数据的ajax的类型。可选post,get
                ajaxGridOptions: {contentType: 'application/json'},
                contentType: "application/json",
    ...
    function savePersonInfo(personId, personName) {
        ...
        if (id != null && id != "") {
            ...
            $.ajax({
                type: "POST",
                url: hostUrl + "sys/sysMsgScopePerson/save",
                data: JSON.stringify(postDto),
                dataType: "JSON",
                contentType: "application/json",
                success: function (data) {
    ...
    function delPerson() {
        ...
        if (idsVal && idsVal != "") {
            $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                $.ajax({
                    url: baseUrl + "/sys/sysMsgScopePerson/deleteBatch/" + idsVal,
                    type: 'DELETE',
                    dataType: 'JSON',
                    contentType: 'application/json',
                    data: JSON.stringify({}),
