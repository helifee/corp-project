;(function($, window, document, undefined){
     ...
     */
     window.submitAddForm = function(){
        ...
        hrEmpLanguageAbilityDto.delflag=0;//有效标志位
        hrEmpLanguageAbilityDto.personId=id;//关联人员Id
        hrEmpLanguageAbilityDto.id=uuid;//主键
        $.ajax({
            type: 'POST',
            url: hostUrl+'emp/hrEmpLanguageAbility/save?time=' + Math.random(),
            ...
            success: function (xhr) {
                if (xhr) {
                    if (xhr.success) {
                        ...
                        window.parent.$('#hrEmpLanguageAbilityForm').jqGrid().trigger("reloadGrid");
                    } else {
                        //异常处理
                        switch (xhr.code) {
                            case "50000":
     ...
     */
     window.submitEditForm = function(){
        ...
        $.ajax({
            type: 'PUT',
            url: hostUrl+'emp/hrEmpLanguageAbility/update/'+id,
            data: JSON.stringify(hrEmpLanguageAbilityDto),
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (xhr) {
    ...
    function editLanaguageAbility(id){
        url = hostUrl+'emp/hrEmpLanguageAbility/get/'+id;
        $.ajax({
            type:'GET',
