;(function($, window, document, undefined){
    ...
    //todo 初始化页面数据
    function initData(){
        url = hostUrl+'emp/hrEmpChange/queryLastInfoList';//查询最新的记录，如果
        $.ajax({
            type:'POST',
            url:url,
                ...
                if(xhr){
                    if(xhr.success){
                        if(xhr.result!=null && xhr.result.length > 0){
                            postId = xhr.result[0].sid;//平台position主键
                            ...
                            //var postBefore =(xhr.result[0].postAfter=='' || xhr.result[0].postAfter == undefined)?'':$.hrUtils.getPtPostNameById(xhr.result[0].postAfter);
                            //$("#postBeforeName").val(postBefore);
                            $("#postAfter").val(xhr.result[0].postAfter);//调动后岗位
                            //var postAfter =(xhr.result[0].postAfter=='' || xhr.result[0].postAfter == undefined)?'':$.hrUtils.getPtPostNameById(xhr.result[0].postAfter);
    ...
    window.submitAddForm = function(){
        ...
        hrEmpChangeDto.postId = postId;//position主键ID
        $.ajax({
            type: 'POST',
            url: hostUrl+'emp/hrEmpChange/saveEmpChange?time=' + Math.random(),
            ...
            success: function (xhr) {
                if (xhr) {
                    if (xhr.success) {
                        ...
                        window.parent.$('#hrEmpChangeForm').jqGrid().trigger("reloadGrid");
                    } else {
                        //异常处理
                        switch (xhr.status) {
                            case 1000:
     ...
     */
     window.submitEditForm = function(){
        ...
        $.ajax({
            type: 'PUT',
            url: hostUrl+'emp/hrEmpChange/updateEmpChange/'+id,
            data: JSON.stringify(hrEmpChangeDto),
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (xhr) {
    ...
    function editHrEmpChangeInfo(id){
        url = hostUrl+'emp/hrEmpChange/get/'+id;
        $.ajax({
            type:'GET',
            ...
            success: function(xhr) {
                ...
                if(xhr){
                    if(xhr.success){
                        ...
                        $("#postBeforeName").attr("value",$.hrUtils.getPtPostNameById(xhr.result.postBefore));
                        ...
                        $("#postAfterName").attr("value",$.hrUtils.getPtPostNameById(xhr.result.postAfter));
