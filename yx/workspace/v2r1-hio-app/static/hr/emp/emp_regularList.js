...
(function ($, window, document, undefined) {
    ...
    $(function () {
        ...
        window.queryByConditon = function () {
            ...
            var postData = $("#empRegularForm").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.delflag = 0;
            queryCondition.type = '1068100111';
            queryCondition.startNew=0;
            var limitNew = $('#empRegularForm').getGridParam('rowNum');
            queryCondition.limitNew = parseInt(limitNew);
            $("#empRegularForm").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#empRegularForm").jqGrid("getGridParam", "postData");
        ...
        $("#exportBtn").click(function(){
            ...
            var sortname = $("#empRegularForm").jqGrid().getGridParam("sortname");
            var sortorder = $("#empRegularForm").jqGrid().getGridParam("sortorder");
            var applyOrgId = $("#applyOrgId").val();//所属公司和部门
            var applyStatus = $("#applyStatus").val();//审批状态
            var dateOfApply = $("#dateOfApply").val();//申请时间
            ...
            applyTypeParam.attr('value','1068100111');//转正类型
            ...
            exportNameParam.attr('value','转正申请');
        ...
        $("#addBtn").click(function(){
            var winObjEI = window.location.href="emp_regular.html?oper=add";
            // var isClose = 1;
            // //关闭open页面时刷新父页面列表
            // var loop = setInterval(function () {
            //     if (winObjEI.closed && isClose == 1) {
            //         isClose--;
            //         $("#empRegularForm").jqGrid().trigger('reloadGrid');
            //     }
            // }, 1000);
        });
        /**
        ...
        $("#editBtn").click(function(){
            var idsVal = $('#empRegularForm').jqGrid('getGridParam', 'selarrrow');
            if (idsVal && idsVal != "") {
                if (idsVal.length > 1) {
                    $.xljUtils.tip("blue", "只能选择一行数据进行编辑！");
                    return;
                } else {
                    var rowId = $('#empRegularForm').jqGrid("getGridParam", "selrow");
                    rowData = $('#empRegularForm').jqGrid('getRowData', rowId);
                    if(rowData.status == '草稿'){
                        var winObjEI = window.location.href="emp_regular.html?oper=edit&id="+idsVal;
                        // var isClose = 1;
                        // //关闭open页面时刷新父页面列表
                        // var loop = setInterval(function () {
                        //     if (winObjEI.closed && isClose == 1) {
                        ...
                        var winObjEI = window.location.href="emp_regular_read.html?oper=edit&id="+idsVal;
                    }
                }
            } else {
        ...
        window.parentReloadById = function (callBackId) {
            $('#empRegularForm').jqGrid("setGridParam", {
                gridComplete:function(){
                    if(callBackId != undefined && callBackId != ""){
                        $("#empRegularForm").setSelection(callBackId);
                    }
                }
            }).trigger("reloadGrid");
        };
    ...
    function initSysInfoSetList() {
        //创建jqGrid组件
        var jqGridSysInfoSet = jQuery("#empRegularForm").jqGrid(
            {
                ...
                multiselect: true,
                multiboxonly:true,
                postData:{"delflag":0,type:'1068100111'},
                jsonReader: {
                    repeatitems: false
                },
                ...
                ondblClickRow:function () {
                    ...
                    rowData = $('#empRegularForm').jqGrid('getRowData', rowId);
                    if(rowData.status == '草稿'){
                        var winObjEI = window.location.href="emp_regular.html?oper=edit&id="+rowId;
                        // var isClose = 1;
                        // //关闭open页面时刷新父页面列表
                        // var loop = setInterval(function () {
                        //     if (winObjEI.closed && isClose == 1) {
                        ...
                        // $.xljUtils.tip("blue", "非草稿状态数据不能修改！");
                        var winObjEI = window.location.href="emp_regular_read.html?oper=edit&id="+rowId;
                    }
                },
                gridComplete: function () {
    ...
    function delOption() {
        ...
        if (idsVal && idsVal != "") {
            for(var i in idsVal){
                var rowData = $("#empRegularForm").getRowData(idsVal[i]);
                if (rowData.status != "草稿") {
                    flag = false;
                    break;
                }
                            ...
                            if (xhr) {
                                if (xhr.success) {
                                    ...
                                    if (focusId == null) {
                                        ...
                                        queryData.page =  $('#empRegularForm').getGridParam('page')-1;
                                        queryData.rowNum = $('#empRegularForm').getGridParam('rowNum');
                                    }
                                    queryData.gridComplete = function (){
                                        if(focusId != null && focusId != ""){
                                            $("#empRegularForm").setSelection(focusId);
                                        }else if (focusId == null || focusId == ""){
                                            $("#empRegularForm tr").last().find(":input[role='checkbox']").trigger("click");
                                        }
                                        focusId="";
                                    }
                                    $('#empRegularForm').jqGrid("setGridParam",queryData).trigger("reloadGrid");
                                } else {
            ...
            if (idsVal.length > 1) {
                ...
                if (rowData.status != "草稿") {
                    if (rowData.isGoApp=='true') {//经过审批流提交的流程
                        toFlowView(rowData.id,FLCODE_RYZZ);
                    }else{
