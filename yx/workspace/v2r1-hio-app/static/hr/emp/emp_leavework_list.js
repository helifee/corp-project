...
(function ($, window, document, undefined) {
    ...
    $(function () {
        ...
        window.queryByConditon = function () {
            ...
            var postData = $("#leaveWorkForm").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.delflag = 0;
            queryCondition.type = '1068100114';
            queryCondition.startNew=0;
            var limitNew = $('#leaveWorkForm').getGridParam('rowNum');
            queryCondition.limitNew=parseInt(limitNew);
            $("#leaveWorkForm").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#leaveWorkForm").jqGrid("getGridParam", "postData");
        ...
        $("#exportBtn").click(function(){
            ...
            var sortname = $("#leaveWorkForm").jqGrid().getGridParam("sortname");
            var sortorder = $("#leaveWorkForm").jqGrid().getGridParam("sortorder");
            var applyOrgId = $("#applyOrgId").val();//所属公司和部门
            var applyStatus = $("#applyStatus").val();//审批状态
            var dateOfApply = $("#dateOfApply").val();//申请时间
            ...
            applyTypeParam.attr('value','1068100114');//入职类型
            ...
            exportNameParam.attr('value','离职申请');
        ...
        $("#addNewBtn").click(function(){
            var idsVal = $('#leaveWorkForm').jqGrid('getGridParam', 'selarrrow');
            var winObjEI = window.location.href="emp_leavework.html?oper=add";
            // var isClose = 1;
            // //关闭open页面时刷新父页面列表
            // var loop = setInterval(function () {
            //     if (winObjEI.closed && isClose == 1) {
            //         isClose--;
            //         $("#leaveWorkForm").jqGrid().trigger('reloadGrid');
            //     }
            // }, 1000);
        });
        /**
        ...
        $("#editBtn").click(function(){
            ...
            if (idsVal && idsVal != "") {
                if (idsVal.length > 1) {
                    ...
                    var rowId = $('#leaveWorkForm').jqGrid("getGridParam", "selrow");
                    var rowData = $('#leaveWorkForm').jqGrid('getRowData', rowId);
                    if(rowData.status == '草稿') {
                        var idsVal = $('#leaveWorkForm').jqGrid('getGridParam', 'selarrrow');
                        var winObjEI = window.location.href="emp_leavework.html?oper=edit&id=" + idsVal;
                        // var isClose = 1;
                        // //关闭open页面时刷新父页面列表
                        // var loop = setInterval(function () {
                        //     if (winObjEI.closed && isClose == 1) {
                        ...
                        var winObjEI = window.location.href="emp_leavework_read.html?oper=edit&id=" + idsVal;
                    }
                }
            } else {
        ...
        window.parentReloadById = function (callBackId) {
            $('#leaveWorkForm').jqGrid("setGridParam", {
                gridComplete:function(){
                    if(callBackId != undefined && callBackId != ""){
                        $("#leaveWorkForm").setSelection(callBackId);
                    }
                }
            }).trigger("reloadGrid");
        };
        ...
        $("#changeBtn").click(function () {
            ...
            if (idsVal && idsVal != "") {
                if (idsVal.length > 1) {
                    ...
                    var rowId = $('#leaveWorkForm').jqGrid("getGridParam", "selrow");
                    rowData = $('#leaveWorkForm').jqGrid('getRowData', rowId);
    ...
    function initSysInfoSetList() {
        //创建jqGrid组件
        var jqGridSysInfoSet = jQuery("#leaveWorkForm").jqGrid(
            {
                ...
                multiselect: true,
                multiboxonly:true,
                postData:{"delflag":0,"type":'1068100114'},
                jsonReader: {
                    repeatitems: false
                },
                ...
                ondblClickRow:function () {
                    ...
                    if(rowData.status == '草稿') {
                        var idsVal = $('#leaveWorkForm').jqGrid('getGridParam', 'selarrrow');
                        var winObjEI = window.location.href="emp_leavework.html?oper=edit&id=" + rowId;
                        // var isClose = 1;
                        // //关闭open页面时刷新父页面列表
                        // var loop = setInterval(function () {
                        //     if (winObjEI.closed && isClose == 1) {
                        ...
                        //$.xljUtils.tip("blue", "非草稿状态数据不能修改！");
                        var winObjEI = window.location.href="emp_leavework_read.html?oper=edit&id=" + rowId;
                    }
                },
                rowNum: 20,
    ...
    function delOption() {
        ...
        if (idsVal && idsVal != "") {
            for(var i in idsVal){
                var rowData = $("#leaveWorkForm").getRowData(idsVal[i]);
                if (rowData.status != "草稿") {
                    flag = false;
                    break;
                }
                            ...
                            if (xhr) {
                                if (xhr.success) {
                                    ...
                                    if (focusId == null) {
                                        ...
                                        queryData.page =  $('#leaveWorkForm').getGridParam('page')-1;
                                        queryData.rowNum = $('#leaveWorkForm').getGridParam('rowNum');
                                    }
                                    queryData.gridComplete = function (){
                                        if(focusId != null && focusId != ""){
                                            $("#leaveWorkForm").setSelection(focusId);
                                        }else if (focusId == null || focusId == ""){
                                            $("#leaveWorkForm tr").last().find(":input[role='checkbox']").trigger("click");
                                        }
                                        focusId="";
                                    }
                                    $('#leaveWorkForm').jqGrid("setGridParam",queryData).trigger("reloadGrid");
                                } else {
            ...
            if (idsVal.length > 1) {
                ...
                if (rowData.status != "草稿") {
                    if (rowData.isGoApp=='true') {//经过审批流提交的流程
                        toFlowView(rowData.id,FLCODE_RYLZ);
                    }else{
