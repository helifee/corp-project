...
(function ($, window, document, undefined) {
    ...
    //导出其他子集模板
    $("#exportModel1").unbind('click').on('click', function () {
        exportModel1();
    });
    ...
    function saveInfo() {
        ...
        if (file != undefined) {
            $.ajax({
                url: hostUrl + "emp/empPersonInfo/uploadFile",
                type: 'post',
                dataType: 'json',
                async: false,
                data: formdata,
    ...
    function saveInfo1() {
        ...
        var path = "";
        if ("work" == type) {
            path = hostUrl + "emp/empWorkHistory/uploadFile";
        } else if ("edu" == type) {
            path = hostUrl + "emp/hrEmpEducation/uploadFile";
        } else if ("family" == type) {
            path = hostUrl + "emp/hrEmpFamily/uploadFile";
        } else if ("language" == type) {
            path = hostUrl + "emp/hrEmpLanguageAbility/uploadFile";
        } else if ("rewardPunish" == type) {
            path = hostUrl + "emp/hrEmpRewardPunish/uploadFile";
        } else if ("change" == type) {
            path = hostUrl + "emp/hrEmpChange/uploadFile";
        }
     ...
     * 导出excel文件
     */
    function exportModel1() {
        var type = $('#type').val();
        var path = "";
        if ("work" == type) {
            window.open("../modelDir/工作信息导入模板.xlsx","exportTarget");
        } else if ("edu" == type) {
            window.open("../modelDir/教育信息导入模板.xlsx","exportTarget");
        } else if ("family" == type) {
            window.open("../modelDir/家庭信息导入模板.xlsx","exportTarget");
        } else if ("language" == type) {
            window.open("../modelDir/语言能力导入模板.xlsx","exportTarget");
        } else if ("rewardPunish" == type) {
            window.open("../modelDir/奖惩信息导入模板.xlsx","exportTarget");
        } else if ("change" == type) {
            window.open("../modelDir/变动信息导入模板.xlsx","exportTarget");
        }
        pop_tip_open("", "导出成功");
    }
