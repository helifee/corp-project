...
(function ($, window, document, undefined) {
    ...
    function fillSub(){
        ...
        if(orgAfter!='' && subsidyRankAfter!=''&& subsidyRankAfter!='其它' && workPlaceRankAfter!='') {
            ...
            //todo 根据代码项和机构Id获取薪资
            $.ajax({
                url: baseUrl + "wage/wageStandardSubsidy/querySubsidyStandardDetails",
                type: 'POST',
                dataType: 'JSON',
                contentType: 'application/json',
                data: JSON.stringify({
    ...
    function getEmpInfoSetById(personId) {
        ...
        $.ajax({
            type: 'post',
            url: hostUrl+"emp/hrWageChange/queryList",
        ...
        $.ajax({
            type: 'post',
            url: hostUrl+"emp/hrWageSubchange/queryList",
    ...
    window.submitAddForm = function(){
        ...
        hrUpRankInfoTmpDto.delflag=0;//有效标志位
