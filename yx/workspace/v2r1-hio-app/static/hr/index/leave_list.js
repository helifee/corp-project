...
$(function () {
    ...
    window.queryByConditon = function () {
        if(dateFlag){
            ...
            var postData = $("#hrEmpLeaveList").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.startNew=0;
            var limitNew = $('#hrEmpLeaveList').getGridParam('rowNum');
            queryCondition.limitNew = parseInt(limitNew);
            $("#hrEmpLeaveList").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#hrEmpLeaveList").jqGrid("getGridParam", "postData");
...
//todo 导出报表
    $("#exportBtn").click(function () {
        ...
        form.attr('action', hostUrl + "emp/hrEmpIndex/leaveExcel?"+window.parent.JZY.s.getAccessTokenByAuthorization());
        ...
        exportNameParam.attr('value', '本月离职人员');//入职类型
        form.append(exportNameParam);
        //todo 导出类型
        var exportNameParam = $('<input>');
        exportNameParam.attr('type', 'hidden');
        exportNameParam.attr('name', "type");
        exportNameParam.attr('value', 'leave');//标志本月在职
        form.append(exportNameParam);
...
function initPersonList() {
    //创建jqGrid组件
    var jqGridHrEmpEntry = jQuery("#hrEmpLeaveList").jqGrid(
        {
            url: hostUrl + 'emp/hrEmpIndex/queryLeaveList',//创建完成之后请求数据的url
            datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
            mtype: "post",//向后台请求数据的ajax的类型。可选post,get
            ajaxGridOptions: {contentType: 'application/json'},
            contentType: "application/json",
            colNames: ['id','姓名', '手机号', '所属机构', '人员类别', '入职时间', '离职时间'],//列名
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                {name: 'id', label: 'id', editable: true, sortable: false, hidden: true, align: "center"},
                {name: 'realName', label: 'name',width: divWidth * 0.20, align: "center"},
                ...
                {name: 'leaveTime', index: 'leaveTime',  width: divWidth * 0.16, align: "center",formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}},
            ],
            shrinkToFit: true,
            autoScroll: true,
            sortable: false,
          ...
          /*  multiselect: true,
            ...
            onSelectRow: function () {
                var rowId = $('#hrEmpLeaveList').jqGrid("getGridParam", "selrow");
                rowData = $('#hrEmpLeaveList').jqGrid('getRowData', rowId);
            },
