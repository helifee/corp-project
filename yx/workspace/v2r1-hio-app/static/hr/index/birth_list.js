...
function initPersonList() {
    //创建jqGrid组件
    var jqGridHrEmpWorking = jQuery("#hrEmpBirthList").jqGrid(
        {
            url: hostUrl + 'emp/hrEmpIndex/queryBirthList',//创建完成之后请求数据的url
            datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
            mtype: "post",//向后台请求数据的ajax的类型。可选post,get
            async: false,
            ajaxGridOptions: {contentType: 'application/json'},
            contentType: "application/json",
            colNames: ['id','姓名', '手机号', '所属机构','性别', '出生日期', '年龄'],//列名
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                ...
                {name: 'isMale', index: 'isMale',  width:divWidth * 0.16, align: "center"},
                {name: 'birthday',index: 'birthday', width: divWidth * 0.16,align: "center",formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}},
                {name: 'age', index: 'age', width: divWidth * 0.16, align: "center"},
            ],
            autoScroll: true,
            ...
            onSelectRow: function () {
                var rowId = $('#hrEmpBirthList').jqGrid("getGridParam", "selrow");
                rowData = $('#hrEmpBirthList').jqGrid('getRowData', rowId);
            },
...
$(function () {
    ...
    window.queryByConditon = function () {
         ...
         if(queryDay !=null && queryDay != undefined){
            ...
            var postData = $("#hrEmpBirthList").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.startNew=0;
            var limitNew = $('#hrEmpBirthList').getGridParam('rowNum');
            queryCondition.limitNew = parseInt(limitNew);
            $("#hrEmpBirthList").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#hrEmpBirthList").jqGrid("getGridParam", "postData");
...
//todo 导出报表
    $("#exportBtn").click(function () {
        ...
        var sortname = $("#hrEmpBirthList").jqGrid().getGridParam("sortname");
        var sortorder = $("#hrEmpBirthList").jqGrid().getGridParam("sortorder");

        form.attr('style', 'display:none');   //在form表单中添加查询参数
        form.attr('target', 'exportTarget');
        form.attr('method', 'post');
        form.attr('action', hostUrl + "emp/hrEmpIndex/birthExcel?"+window.parent.JZY.s.getAccessTokenByAuthorization());
        ...
        exportNameParam.attr('value', '本月生日人员');//入职类型
        form.append(exportNameParam);
        //todo 导出类型
        var exportNameParam = $('<input>');
        exportNameParam.attr('type', 'hidden');
        exportNameParam.attr('name', "type");
        exportNameParam.attr('value', 'birth');//标志本月在职
        form.append(exportNameParam);
