...
function initPersonList() {
    //创建jqGrid组件
    var jqGridHrEmpWorking = jQuery("#hrEmpRegularList").jqGrid(
        {
            url: hostUrl + 'emp/hrEmpIndex/queryRegularList',//创建完成之后请求数据的url
            datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
            mtype: "post",//向后台请求数据的ajax的类型。可选post,get
            ajaxGridOptions: {contentType: 'application/json'},
            contentType: "application/json",
            colNames: ['id','姓名', '手机号', '所属机构', '入职时间', '人员类别', '试用期(月)','转正日期', '是否已转正'],//列名
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                {name: 'id', label: 'id', editable: true, sortable: false, hidden: true, align: "center"},
                {name: 'realName', label: 'name',   width: divWidth * 0.125, align: "center"},
                {name: 'mobile', index: 'phone', width:  divWidth * 0.125, align: "center"},
                {name: 'prefixName', index: 'prefixName',  width:  divWidth * 0.125, align: "center"},
                {name: 'entryTime',index: 'entryTime',width:  divWidth * 0.125,align: "center",formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}},
                {name: 'personType', index: 'personType', width: divWidth * 0.125, align: "center"},
                {name: 'probationPeriod', index: 'probationPeriod', width:  divWidth * 0.125, align: "center"},
                {name: 'regularTime', index: 'regularTime',  width: divWidth * 0.125, align: "center",formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}},
                {name: 'personType', index: 'personType', width: divWidth * 0.125, align: "center",formatter: typeFmatter},
            ],
            shrinkToFit: true,
            autoScroll: true,
            sortable: false,
            ...
            onSelectRow: function () {
                var rowId = $('#hrEmpRegularList').jqGrid("getGridParam", "selrow");
                rowData = $('#hrEmpRegularList').jqGrid('getRowData', rowId);
            },
...
$(function () {
    ...
    window.queryByConditon = function () {
        if(dateFlag){
            ...
            var postData = $("#hrEmpRegularList").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.startNew=0;
            var limitNew = $('#hrEmpRegularList').getGridParam('rowNum');
            queryCondition.limitNew = parseInt(limitNew);
            $("#hrEmpRegularList").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#hrEmpRegularList").jqGrid("getGridParam", "postData");
    ...
    $("#exportBtn").click(function () {
        ...
        var sortname = $("#hrEmpRegularList").jqGrid().getGridParam("sortname");
        var sortorder = $("#hrEmpRegularList").jqGrid().getGridParam("sortorder");
        form.attr('style', 'display:none');   //在form表单中添加查询参数
        form.attr('target', 'exportTarget');
        form.attr('method', 'post');
        form.attr('action', hostUrl + "emp/hrEmpIndex/regularExcel?"+window.parent.JZY.s.getAccessTokenByAuthorization());
        //查询条件
        ...
        exportNameParam.attr('value', '本月转正人员');
        form.append(exportNameParam);
        //todo 导出类型
        var exportNameParam = $('<input>');
        exportNameParam.attr('type', 'hidden');
        exportNameParam.attr('name', "type");
        exportNameParam.attr('value', 'regular');//标志转正
        form.append(exportNameParam);
