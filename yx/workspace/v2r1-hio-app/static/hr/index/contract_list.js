...
function initPersonList() {
    //创建jqGrid组件
    var jqGridHrEmpWorking = jQuery("#hrEmpContractList").jqGrid(
        {
            url: hostUrl + 'emp/hrEmpIndex/queryContractList',//创建完成之后请求数据的url
            datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
            mtype: "post",//向后台请求数据的ajax的类型。可选post,get
            async: false,
            ajaxGridOptions: {contentType: 'application/json'},
            contentType: "application/json",
            colNames: ['id','姓名', '手机号', '所属机构', '入职时间', '现合同开始时间', '现合同结束时间'],//列名
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                {name: 'id', label: 'id', hidden: true, align: "center"},
                {name: 'realName', label: 'name', width: divWidth * 0.18, align: "center"},
                ...
                {name: 'nowBeginTime', index: 'nowBeginTime', width: divWidth * 0.16, align: "center", formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}},
                {name: 'nowEndTime',index: 'nowEndTime',width: divWidth * 0.16,align: "center",formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}}
            ],
            shrinkToFit: true,
            autoScroll: true,
            ...
            viewrecords: true,
            sortable: false,
            sortname: 'now_begin_time',//初始化的时候排序的字段
            sortorder: "desc",
            postData: {},
            jsonReader: {
                repeatitems: false
...
$(function () {
    ...
    window.queryByConditon = function () {
        if(dateFlag){
            ...
            var postData = $("#hrEmpContractList").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.startNew=0;
            var limitNew = $('#hrEmpContractList').getGridParam('rowNum');
            queryCondition.limitNew = parseInt(limitNew);
            $("#hrEmpContractList").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#hrEmpContractList").jqGrid("getGridParam", "postData");
...
//todo 导出报表
    $("#exportBtn").click(function () {
        ...
        form.attr('action', hostUrl + "emp/hrEmpIndex/contractExcel?"+window.parent.JZY.s.getAccessTokenByAuthorization());
        ...
        exportNameParam.attr('value', '本月合同到期人员');//入职类型
        form.append(exportNameParam);
        //todo 导出类型
        var exportNameParam = $('<input>');
        exportNameParam.attr('type', 'hidden');
        exportNameParam.attr('name', "type");
        exportNameParam.attr('value', 'contract');//标志本月在职
        form.append(exportNameParam);
