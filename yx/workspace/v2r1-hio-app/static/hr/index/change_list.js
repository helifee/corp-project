...
$(function () {
    ...
    window.queryByConditon = function () {
        if(dateFlag){
            ...
            var postData = $("#hrEmpChangeList").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.startNew=0;
            var limitNew = $('#hrEmpChangeList').getGridParam('rowNum');
            queryCondition.limitNew = parseInt(limitNew);
            $("#hrEmpChangeList").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#hrEmpChangeList").jqGrid("getGridParam", "postData");
    ...
    $("#exportBtn").click(function () {
        ...
        var sortname = $("#hrEmpContractList").jqGrid().getGridParam("sortname");
        var sortorder = $("#hrEmpContractList").jqGrid().getGridParam("sortorder");

        form.attr('style', 'display:none');   //在form表单中添加查询参数
        form.attr('target', 'exportTarget');
        form.attr('method', 'post');
        form.attr('action', hostUrl + "emp/hrEmpIndex/changeExcel?"+window.parent.JZY.s.getAccessTokenByAuthorization());
        ...
        exportNameParam.attr('value', '本月调动人员');//入职类型
        form.append(exportNameParam);
        //todo 导出类型
        var exportNameParam = $('<input>');
        exportNameParam.attr('type', 'hidden');
        exportNameParam.attr('name', "type");
        exportNameParam.attr('value', 'change');//标志本月在职
        form.append(exportNameParam);
...
function initPersonList() {
    //创建jqGrid组件
    var jqGridHrEmpEntry = jQuery("#hrEmpChangeList").jqGrid(
        {
            url: hostUrl + 'emp/hrEmpIndex/queryChangeList',//创建完成之后请求数据的url
            datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
            mtype: "post",//向后台请求数据的ajax的类型。可选post,get
            ajaxGridOptions: {contentType: 'application/json'},
            contentType: "application/json",
            colNames: ['id','姓名', '手机号', '所属机构', '人员类别','入职时间', '调动时间','原机构','原岗位','现机构','现岗位'],//列名
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                {name: 'id', label: 'id',  hidden: true, align: "center"},
                {name: 'realName', label: 'name', width:  divWidth * 0.10, align: "center"},
                {name: 'mobile', index: 'phone',  width: divWidth * 0.10, align: "center"},
                {name: 'prefixName', index: 'prefixName', width: divWidth * 0.10, align: "center"},
                ...
                {name: 'effectTime',index: 'effectTime',width: divWidth * 0.10,align: "center",formatter: 'date',formatoptions: {srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d'}},
                {name: 'orgBefore',index: 'orgBefore', width: divWidth * 0.10,align: "center"},
                {name: 'postBefore',index: 'postBefore', width: divWidth * 0.10,align: "center"},
                {name: 'orgAfter',index: 'orgAfter', width: divWidth * 0.10,align: "center"},
                {name: 'postAfter', index: 'postAfter',  width: divWidth * 0.10, align: "center"},
            ],
            shrinkToFit: true,
            autoScroll: true,
            sortable: false,
            ...
            onSelectRow: function () {
                var rowId = $('#hrEmpChangeList').jqGrid("getGridParam", "selrow");
                rowData = $('#hrEmpChangeList').jqGrid('getRowData', rowId);
            },
