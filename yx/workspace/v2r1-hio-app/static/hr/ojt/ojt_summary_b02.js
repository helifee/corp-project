...
;(function($, window, document, undefined){
    ...
    function exportInfo() {
        //表格数据
        rowData = $('#listPlannedSort').jqGrid('getRowData');
        var urlBody = "ojt/hrOjtExamPapers/exportSummaryB02";
    ...
    $("#queryDat").on('click',function () {
        ...
        $("#listPlannedSort").jqGrid('setGridParam',{datatype:'json',postData:{"obj":obj}}).trigger('reloadGrid');
    });
    /*培训类别分析表页面的展示*/
    /*初始化进行页面的展示*/
    function listPaperList(){
        var ubody = "/ojt/hrOjtExamPapers/queryClassifyListByCondition";
        var uall = hostUrl+ubody;
        //创建jqGrid组件
        jqGridRule_social = jQuery("#listPlannedSort").jqGrid(
            {
                url: uall,
                ...
                colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                    ...
                    {name : 'common/common_hr_mobile.jsName',label : '课程类别',width : 55,align : "center"},
                    {name : 'totalTrainingNum',label : '合计培训人数',width : 55,align : "center"},
                    {name : 'totalTime',label : '合计培训课时(分)',width : 55,align : "center",
        ...
        $("#gview_listPlannedSort .ui-jqgrid-bdiv").height((w_h-145)+"px");
    }

    function timeFmatter(cellvalue,options,rowObject){
        var time = cellvalue==null?0:(cellvalue/60);
    ...
    function initYearSelect(){
        ...
        for(var i = 2016; i <= year; i++) {
            $("#annualTraining").append("<option value='" + i + "'>" + i + "</option>")
        }
    }
