...
;(function ($,window,document,undefined) {
                    ...
                    $.each(list,function(index,value){
                        for(var key in value){
                            if(key=="code"&&value[key]=="2"){//编辑权限
                                $("#updateSaveByFormulaBtn").show();//保存
    ...
    window.saveFormulaFirst = function(){
        ...
        var num = $("#minuteEarly3").val();
        var money;
        if(rules=="everyMonth"){
            money = $("#minuteEarly1").val();
            if(money==""||money==null) {
                pop_tip_open("blue", "每月固定值不能为空！");
                return false;
            }else if(num==""||num==null){
                pop_tip_open("blue", "最少出勤天数不能为空！");
                return;
            }

        }else if(rules=="everyAttend"){
            money = $("#minuteEarly2").val();
            if(money==""||money==null) {
                pop_tip_open("blue", "每出勤工作日值不能为空！");
                return false;
            }
        }else{
            pop_tip_open("blue", "请选择使用计算规则！");
            return false;
        }

        data = rules + "#" + money + "#" + num + ";" + fType + ";" + itemId + ";" + itemCode + ";" + correspond + ";" + leaveCode;
        if(sign=="1"){
    ...
    //点击单选框的时候，进行之后文本框的控制
    window.typeChange = function(str0,str1){
        var temp0 = "minuteEarly"+str0;
        var temp1 = "minuteEarly"+str1;
        if(str0=='2'&&str1=='1'){
            $("#minuteEarly3").attr("disabled","disabled");
            $("#minuteEarly3").val("");
        }else{
            $("#minuteEarly3").removeAttr("disabled","disabled");
        }
        $("#"+temp1).attr("disabled","disabled");
        $("#"+temp1).val("");
        $("#"+temp0).removeAttr("disabled");
    };
                                                    ...
                                                    if (resultData) {
                                                        ...
                                                        if (successFlag) {
                                                            ...
                                                            for(num in resultVal){
                                                                ...
                                                                allowanceType = obj.allowanceType;
                                                                allowanceMoney = obj.allowanceMoney;
                                                                allowanceNum = obj.allowanceNum;
                                                                if(allowanceType=="everyMonth"){
                                                                    $("#rules1").click();
                                                                    $("#minuteEarly1").val(allowanceMoney);
                                                                }else if(allowanceType=="everyAttend"){
                                                                    $("#rules2").click();
                                                                    $("#minuteEarly2").val(allowanceMoney);
                                                                }
                                                                $("#minuteEarly3").val(allowanceNum);
                                                            }
                                                        } else {
    ...
    window.goBack = function () {
        localStorage.setItem('gotoTab',JSON.stringify("allowanceItemList"));//跳转至薪资项目设置页签
    ...
    //输入天数校验
    window.importCheck = function (type) {
        
        if(type==1||type==3){         //每月固定值1、应出勤工作日 minuteEarly3
            $("input[name='rules'][value='everyMonth']").prop("checked",true);
            typeChange(1,2);
        }else{       //每出勤工作日 minuteEarly2
            $("input[name='rules'][value='everyAttend']").prop("checked",true);
            typeChange(2,1);
        }
    };
