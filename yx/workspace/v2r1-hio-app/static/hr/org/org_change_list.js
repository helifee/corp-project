...
(function ($, window, document, undefined) {
    ...
    function resizeGrid() {
        //右边一个列表
        $(".ui-jqgrid-bdiv table").jqGrid().setGridHeight($('.mytable1').height() - 0);
    ...
    function pageInit() {
        ...
        jqGridPost = jQuery("#orgChangeList").jqGrid(
            {
                url: hostUrl + '/org/orgChange/queryListByUnion',//创建完成之后请求数据的url
    ...
    function fieldFmatter(cellvalue, options, rowObject) {
        ...
        var value=$.hrUtils.getHRInfoNameById("oa_sys_organization", cellvalue);
        if(value!=null&&value!=''){
            return value;
        }else{
            return cellvalue;
    ...
    function fieldValueFmatter(cellvalue, options, rowObject) {
        if(rowObject.changeField!=null){
            if(rowObject.changeField=='leader_id'){//负责人
    ...
    function del() {
        ...
        if (idsVal && idsVal != "") {
            $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                $.ajax({
                    url: hostUrl + "/org/orgChange/deleteBatch/" + idsVal,
                    type: 'DELETE',
                    dataType: 'JSON',
                    contentType: 'application/json',
                    data: JSON.stringify({}),
