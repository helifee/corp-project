...
$(function () {
    ...
    //新增回显
    var orgnazationDto = localStorage.getItem("orgnazationDto");
    if (!orgnazationDto && orgnazationDto != null) {
        refreshDictionaryTree(orgnazationDto);
    }
...
//机构变更信息
function changeOrg() {
    window.location.href = "org_change_list.html?edit_orgId=" + edit_orgId;
}
...
var show = false;
$("#searchTreeBtn").unbind('click').on('click', function () {
    show = !show;
    if(show){
    ...
    var height = window.innerHeight;//$(window).height()
    $('.dictionary-tree').height(height - $('.tit-box').outerHeight() - $('.searchBox:visible').outerHeight() - 28);
});
...
function checkOrgPostRepeat(post_id, org_id) {
    ...
    $.ajax({
            type: 'post',
            url: hostUrl + "/org/orgPostRelation/queryListByCondition/",
            dataType: 'JSON',
...
function doDelOrg(edit_orgId, parentNode) {
    $.xljUtils.confirm("blue", "确认要删除当前机构吗？", function () {
        $.ajax({
            //url: baseUrl + "/org/org/delete/" + edit_orgId,
            url: hostUrl + "/org/org/deletePseudo/" + edit_orgId,
            type: 'DELETE',
            dataType: 'JSON',
            contentType: 'application/json',
            data: JSON.stringify({}),
...
function updateCheXiaoBtn1() {
    ...
    if (checkRemark(remark)) {
        $.ajax({
            url: hostUrl + "/org/org/updateOrgStatus",
            type: 'PUT',
            dataType: 'JSON',
            contentType: 'application/json',
            data: JSON.stringify({
...
function checkRemark(remark) {
    if (remark == null || remark == "") {
        $.xljUtils.tip("red", "请填写理由说明！");
        return false;
    }
    if (remark.length > 500) {
        $.xljUtils.tip("red", "理由说明长度不能大于500！");
        return false;
    }
    return true;
}
...
function editOrgInfo(sign) {
    if (sign == 1) {//修改
        ...
        if (nodes.length < 1) {
            ...
            var allNodes = zTreeObj.getNodes();
            var rootNode = allNodes[0];
            if (rootNode.id == nodes[0].id) {//根节点
                pop_tip_open("blue", "根目录不允许修改");
            } else {
                edit_orgId = nodes[0].id;
                openPa("org_edit.html?editType=" + editType + "&edit_orgId=" + edit_orgId);
                // window.location.href = "org_edit.html?editType=" + editType + "&edit_orgId=" + edit_orgId;
                // localStorage.setItem('orgRootNode',JSON.stringify(orgRootNode));
                // localStorage.setItem('orgNode',JSON.stringify(orgNode));
            }
        }
    } else {//新增
        var allNodes = zTreeObj.getNodes();
...
function showOrgTu() {
    var nodes = zTreeObj.getSelectedNodes();
    if (nodes.length < 1) {//未选择节点
        window.location.href = "../orgChart/org_chart.html";
    } else {
        var allNodes = zTreeObj.getNodes();
        var rootNode = allNodes[0];
        if (rootNode.id == nodes[0].id) {//根
            window.location.href = "../orgChart/org_chart.html";
        } else {
            edit_orgId = nodes[0].id;
            window.location.href = "../orgChart/org_chart.html?orgId=" + edit_orgId;
        }
    }
}
//返回上一级
...
window.inviteUser = function () {
    var ids = '';
    var rowIds = $('#hrEmpSetList').jqGrid("getGridParam", "selarrrow");
    $(rowIds).each(function (i,k) {
        var rowData = $('#hrEmpSetList').jqGrid('getRowData', k);
        ids+=',' + rowData.id;
    })
    ids = ids.substring(1);
    var condition = {};
    condition.ids = ids;
    $.ajax({
        url:hostUrl+"//emp/empPersonInfo/inviteUser",
