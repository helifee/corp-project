...
;(function ($, window, document, undefined) {
    ...
    $(function () {
        ...
        var myKqFlag = $.xljUtils.getUrlParam('myKqFlag');
        var teamKqDate = $.xljUtils.getUrlParam('teamKqDate');

        //从我的考勤页面返回的时候，重新查询那个月的考勤结果
        if (myKqFlag !== null && myKqFlag !== undefined && myKqFlag === 'true' &&
            teamKqDate !== null && teamKqDate !== undefined && teamKqDate !== '') {
            $("#nowKqMonth").val(teamKqDate);
            $("#month2").val(teamKqDate);
        }
        pageInit();
...
//grid 自适应宽度
    ...
    /*
        window.resultByDayInit = function (postData) {
            ...
            var jqdata = [
                ...
                {name: 'month', label: "月份", width: 150, align: "center", formatter: dateFormate},
    ...
    */
    ...
    window.onchangeKqSummaryByMonth = function () {
        ...
        var date_year = "";
        var date_month = "";
        if (month !== undefined && month !== null && month.length >= 7) {
            date_year = month.substring(0, 4);
            date_month = month.substring(5);
            var nowKqMonth = date_year + "." + date_month;
            var nowKqStartDate = month + ".01";
            var nowKqEndDate = month + "." + $.kqUtils.getDaysInMonth(date_year, date_month);
    ...
    window.resultByMonthInit = function (postData) {
        // var date = $("#nowKqMonth").val();
        var ubody = "self/hrSelfTeamkq/queryKqResultListByMonth";
        var uall = hostUrl + ubody;
        //创建jqGrid组件
        jqGridByMonth = jQuery("#listResultByMonth").jqGrid(
            {
                ...
                colModel: [
                    ...
                    {name: 'orgName', label: "所属机构", width: 150, align: "center", frozen: true},
                    {name: 'postName', label: "职务", width: 150, align: "center", frozen: true},
                    {name: 'date', label: "月份", width: 100, align: "center", frozen: true, formatter: dateFormate},
                    // {name: 'personCode', label: "人员编号", width: 100, align: "center"},
                    {name: 'planId', label: "方案名称", width: 150, hidden: true, align: "center"},
                    {name: 'planName', label: "方案名称", width: 150, align: "center"},
    ...
    window.daysInitByMonth = function () {
        ...
        var nowKqStartDate;
        var nowKqEndDate;
        if (month2 === undefined || month2 == null || month2 === "") {
            var date = new Date();
            ...
            nowKqStartDate = month2 + ".01";
            nowKqEndDate = month2 + "." + $.kqUtils.getDaysInMonth(date.getFullYear(), month);
            $("#nowKqMonth").val(month2);
            $("#nowKqStartDate").val(nowKqStartDate);
            $("#nowKqEndDate").val(nowKqEndDate);
        } else {
            var date = new Date(month2.replace(/-/,"/"))
            nowKqStartDate = month2 + ".01";
            nowKqEndDate = month2 + "." + $.kqUtils.getDaysInMonth(date.getFullYear(), month);
            $("#nowKqStartDate").val(nowKqStartDate);
            $("#nowKqEndDate").val(nowKqEndDate);
 ...
 */
    ...
    //出差
    function dateFormate(cellValue, options, rowObject) {
        var date = rowObject.date;
        var personId = rowObject.personId;
        var result = '<a href="javascript:void(0)"  style="color: #0000FF" onclick="getSelfKq(\'' + date + '\',\'' + personId + '\')">' + cellValue + '</a>';
        return result;
    }

    window.getSelfKq = function (date, personId) {
        var url = '../kq/kq_myKq.html?personId=' + personId + "&date=" + date + "&backflag=team&teamKqFlag=true";
        location.href = url;
        // alert(url);
    };
