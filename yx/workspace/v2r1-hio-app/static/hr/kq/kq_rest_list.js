...
// var restApplyTypeLists;
    window.addRestApplyInfo = function (sign) {
        if (sign == 0) {
            // window.open("kq_rest_add_local_local.html?type=add");
            window.location.href = "kq_rest_add_local.html?type=add";
        } else if (sign == 3) {//我的考勤跳转
            // window.open("kq_rest_add_local_local.html?type=add&myKqFlag=true");
            window.location.href = "kq_rest_add_local_local.html?type=add&myKqFlag=true&myPersonId=" + myPersonId;
        }
    };
    ...
    window.editRestApplyInfo = function (sign) {
        ...
        if (ids == "" || ids.length == 0) {
            ...
            restApplyId = ids[0];
            // window.open("kq_rest_add_local_local.html?type=update");
            window.location.href = "kq_rest_add_local_local.html?type=update&restApplyId=" + restApplyId;
        } else if (sign == 4) {//我的考勤跳转
            // restApplyTypeLists = restApplyTypeList;
            restApplyId = ids[0];
            // window.open("kq_rest_add_local_local.html?type=update&myKqFlag=true");
            window.location.href = "kq_rest_add_local_local.html?type=update&myKqFlag=true&restApplyId=" + restApplyId + "&myPersonId=" + myPersonId;
        }
    };
    ...
    window.restApplyListQuery = function () {
        ...
        //
        // var statusList = [];
        // if (SPStatus !== undefined && SPStatus != null && SPStatus !== "") {
        //     statusList = SPStatus.split(",");
        // }
        //
        // var typeList = [];
        // if (restType !== undefined && restType != null && restType !== "") {
        //     typeList = restType.split(",");
        // }
    ...
    window.getKqRestInfoById = function (ids) {
        $.ajax({
            url: hostUrl + "kq/hrKqRest/querySysApplyIds/" + ids,
            type: 'POST',
            dataType: 'JSON',
            async: false,
            contentType: 'application/json',
    ...
    window.showResLeaveMode = function (sign) {
        ...
        if (ids == "" || ids.length == 0) {
            ...
            if (count == 0) {
                var rowData = $("#listRest").getRowData(ids[0]);
                getRestInfoByIdForRes(rowData.applyId);
            } else {
    ...
    window.applyApproval = function () {
        ...
        if (ids == "" || ids.length == 0) {
            ...
            var applyEndDate = rowData.applyEndDate;
            $.ajax({
                url: hostUrl + "kq/hrKqRest/kqRestApproval",
                type: 'POST',
                dataType: 'JSON',
                async: false,
                contentType: 'application/json',
    ...
    window.approvalSucess = function () {
        ...
        if (ids == "" || ids.length == 0) {
            ...
            $.ajax({
                url: hostUrl + "kq/hrKqRest/kqRestApprovalSuccess",
                type: 'POST',
                dataType: 'JSON',
                async: false,
                contentType: 'application/json',
    ...
    window.approvalBack = function () {
        ...
        if (ids == "" || ids.length == 0) {
            ...
            var applyEndDate = rowData.applyEndDate;
            $.ajax({
                url: hostUrl + "kq/hrKqRest/kqRestApprovalBack",
                type: 'POST',
                dataType: 'JSON',
                async: false,
                contentType: 'application/json',
...
//初始化日期控件
    ...
    window.getHolidayNameById = function (id) {
        $.ajax({
            url: hostUrl + "kq/hrKqRest/getHolidayNameById",
            type: 'POST',
            dataType: 'JSON',
            async: false,
            contentType: 'application/json',
    ...
    function dateFormatter(cellValue, options, rowObjec) {
        if (cellValue == null || cellValue == "") {
            ...
            var date = cellValue.substring(0, 10);
            var time = cellValue.substring(11, 16);
            if (time !== undefined && time != null && (time === '09:00' || time === '12:00')) {
                time = '上午';
            }
            if (time !== undefined && time != null && (time === '13:30' || time === '18:00')) {
                time = '下午';
            }
            return date + " " + time;
        }
    ...
    function updateRestApplyStatus(id) {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            contentType: 'application/json',
            data: JSON.stringify({"status": $.hrUtils.APPLY_DRAFT}),
            success: function (data) {
                if (data.success == true) {
