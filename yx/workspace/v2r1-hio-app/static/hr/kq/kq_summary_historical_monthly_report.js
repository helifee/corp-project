...
;(function ($, window, document, undefined) {
    ...
    $(function () {
        //var nowKqMonth = $.xljUtils.getUrlParam("nowKqMonth");
        var nowKqStartDate = $.xljUtils.getUrlParam("nowKqStartDate");
        var nowKqEndDate = $.xljUtils.getUrlParam("nowKqEndDate");
        $("#kqStartDate").val(nowKqStartDate);//title调用

        $("#kqEndDate").val(nowKqEndDate);
        //年度选项初始化
                ...
                $.each(list, function (index, value) {
                    for (var key in value) {
                        if (key == "code" && value[key] == "2") {//编辑权限
                            $("#exportBtnMonth").show();//月结果导出
                            $("#exportBtnDay").show();//日结果导出
                        }
    ...
    //考勤历史月报年度涉及年度查询
    function initYear(str) {
        //保证temp_11在年切换时不丢失
        var x = $('#temp').eq(0);
        var p = $('#temp_11').eq(0);
        x.after(p);
        var mytable = $("#temp_11").eq(0);
        mytable.hide();
        ...
        $.ajax({    //查询所有的历史计薪期间
            type: "POST",
            url: hostUrl + "kq/hrKqSummaryFileMonth/queryHistoryHead",
            dataType: "JSON",
            async: false,
            data: JSON.stringify(queryPersonData),
            contentType: "application/json",
            success: function (resultData) {
                if (resultData.success) {
                    if (str == null || str == "") {
                        ...
                        if (data != null) {
                            for (var k in data) {
                                $("#yearSelect").append("<option value=" + data[k].year + ">" + data[k].year + "</option>");
                                nowYear = data[k].year;
                            }
                        }
                    }
                    ...
                    var allSumListDate = resultData.result.allSumList;
                    var noSignList = resultData.result.noSignList;
                    var allDutySumList = resultData.result.allDutySumList;
                    var monthDiv = "";
                    if (data1 != null) {
                        for (var k in data1) {
                            var tempData1 = data1[k].month.substring(0, 4);
                            var month = data1[k].month.substr(5);
                            /*if (tempData1 == nowYear) {*/
                            var month = data1[k].month;
                            ...
                            monthDiv = monthDiv + "<div  class=\"user-list clearfix\" style=\"height: 80px;line-height: 73px\">"
                                + "<span class='l'><span class=\"expend-col\" onclick=\"showExpendDiv('" + data1[k].month + "')\"><i></i></span>"
                                + "<span class=\"tit-big\"><span class=\"tit-big-title\">" + month + "月</span>考勤报表</span></br>"
                                + "<span class=\"tit-big-time\">" + data1[k].updateDate + "</span>"
                                + "</span>"
                                + "<div class=\"my-btn-group\">"
                                + "<span class=\"tit-big\"> 总人数</br><span class='tb_f'>" + allSumListDate[k] + "人</span></span>"
                                + "<span class=\"tit-big\"> 全勤人数</br><span class='tb_f'>" + allDutySumList[k] + "人</span></span>"
                                + "<span class=\"tit-big\"> 不考勤人数</br><span class='tb_f'>" + noSignList[k] + "人</span></span>"
                                + "</div></div><div id=\"" + data1[k].month + "\"></div>";
                            /* }*/
                            $("#monthDivHeader").html(monthDiv);
    ...
    window.showExpendDiv = function (str) {
        ...
        if (mytable.is(":hidden") == true) {
            var kqStartDate = $("#kqStartDate").val();
            var kqEndDate = $("#kqEndDate").val();
            //alert(kqStartDate);
            var nowKqStartDays = kqStartDate.substring(8, 10);
            var nowKqStartDate = "";
            var nowKqEndDate = "";
            var nowKqMonth = str.replace("-", ".");
            if (nowKqStartDays == '01') {
                nowKqEndDays = getDaysInMonth(nowKqMonth.substring(0, 4), nowKqMonth.substring(5, 7));
                nowKqStartDate = nowKqMonth + "." + nowKqStartDays.substring(1, 2);
                nowKqEndDate = nowKqMonth + "." + nowKqEndDays;
            } else if (nowKqStartDays > 1) {
                nowKqEndDays = nowKqStartDays - 1;
                nowKqStartDate = nowKqMonth + "." + nowKqStartDays;
                var nextKqMonth = getNextMonth(nowKqMonth, ".");
                nowKqEndDate = nextKqMonth + "." + nowKqEndDays;
            }

            $("#month1").val(str);
            $("#month2").val(str);
            $("#nowKqStartDate").val(nowKqStartDate.replace("-", "."));
            $("#nowKqEndDate").val(nowKqEndDate.replace("-", "."));
            var queryData = {
                "date": str,
                "type": "01"
            };
        ...
        jqGridByDay = jQuery("#listResultByDay").jqGrid(
            {
                ...
                gridComplete: function () {
                    ...
                    //$.xljUtils.addGridScroll("tableDay");
                    //$.xljUtils.removeGridScroll("tableDay");
                    //$.xljUtils.addGridScroll("tableDay");
                    $.xljUtils.addGridScroll();
                    $.xljUtils.gridResizeFn();
    ...
    //获取下一个月 格式为yyyy-mm的日期，如：2017-12
    function getNextMonth(date, split) {
