...
//grid 自适应宽度
    ...
    function queryBussTypeList() {
        $.ajax({
            ...
            dataType: 'JSON',
            contentType: 'application/json',
            data: JSON.stringify({"status": "1092100169", "bussType": "02"}),//只查询启用状态的公出类型
            success: function (data) {
                if (data.success) {
    ...
    function saveApplyForm(sign, applyFlag) {
        ...
        if (applyFlag !== undefined && applyFlag !== null && applyFlag === true) {
            ...
            setTimeout(function () {
                window.location.href = "kq_local_city_trip.html";
            }, 300);
        }
    }
    ...
    function addSaveDocumentsForm() {
        ...
        sysApplyDto.status = $("#approvalStatus").val();
