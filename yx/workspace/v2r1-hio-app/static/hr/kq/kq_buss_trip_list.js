...
$(function () {
    ...
    if (wagePeriodDto === undefined || wagePeriodDto == null) {//计薪期间没有设置记录
        ...
        if (startDate === undefined || startDate === null || startDate === "") {//计薪期间有设置，但是开始日期为空
            ...
            //配置菜单 /hr-app/hr/kq/kq_buss_trip_list_myKq.html?myFlag=true 然后挂载到
...
function flowView() {
    ...
    if (idsVal && idsVal != "") {
        if (idsVal.length > 1) {
            ...
            var rowData = $('#listBussTrip').jqGrid('getRowData', rowId);
...
function pageInit() {
    ...
    if (myFlag == true || myFlag == "true") {
        bussTripInit({"personId": myPersonId});
    } else {
        bussTripInit({"personId": ""});
    }
    // bussTripInit({"personId": ""});
...
function addBussApplyInfo(sign) {
    // window.open("kq_buss_trip_add_local.html?type=add");
    // window.location.href = "kq_buss_trip_add_local.html?type=add";
    if (sign == 0) {
        window.location.href = "kq_buss_trip_add_local.html?type=add";
    } else if (sign == 3) {//我的考勤跳转
        window.location.href = "kq_buss_trip_add_local.html?type=add&myKqFlag=true&myPersonId=" + myPersonId;
    }
}
...
function editBussApplyInfo(sign) {
    ...
    if (ids == "" || ids.length == 0) {
        ...
        bussApplyId = ids[0];
        // window.open("kq_buss_trip_add_local.html?type=update");
        window.location.href = "kq_buss_trip_add_local.html?type=update&bussApplyId=" + bussApplyId;
    } else if (sign == 4) {//我的考勤跳转
        bussApplyId = ids[0];
        // window.open("kq_buss_trip_add_local.html?type=update&myKqFlag=true");
        window.location.href = "kq_buss_trip_add_local.html?type=update&myKqFlag=true&bussApplyId=" + bussApplyId + "&myPersonId=" + myPersonId;
        ;
    }
}
    ...
    jqGridKqBuss = jQuery("#listBussTrip").jqGrid(
        {
            ...
            colModel: [
                ...
                {name: 'tripTypeValue', label: "出差类型", width: 100, align: "center"},
...
function getKqBussInfoById(ids) {
    $.ajax({
        url: hostUrl + "kq/hrKqBussTrip/querySysApplyIds/" + ids,
        type: 'POST',
        dataType: 'JSON',
        async: false,
        contentType: 'application/json',
