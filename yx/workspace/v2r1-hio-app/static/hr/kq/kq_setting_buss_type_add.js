...
$(function () {
    ...
    if (type === 'add') {
        ...
        bussTypeEditId = $.xljUtils.getUrlParam("bussTypeEditId");//window.opener.bussTypeEditId;
        systemItemFlag = $.xljUtils.getUrlParam("systemItemFlag");//window.opener.systemItemFlag;
        if (systemItemFlag === "true") {
            systemItemFlag = true;
        }
        pageInit();
        getHolidayTypeInfoById(bussTypeEditId);
    }
        ...
        success: function (data) {
            if (data.success) {
                ...
                for (i in holidayUnitTypeList) {
                    var typeId = holidayUnitTypeList[i].sid;
        ...
        success: function (data) {
            if (data.success) {
                ...
                for (i in calculateRuleTypeList) {
                    var typeId = calculateRuleTypeList[i].sid;
        ...
        success: function (data) {
            if (data.success) {
                ...
                for (i in holidayTypeList) {
                    var typeId = holidayTypeList[i].sid;
        ...
        success: function (data) {
            if (data.success) {
                ...
                for (i in holidayStatusList) {
                    var typeId = holidayStatusList[i].sid;
        ...
        success: function (data) {
            if (data.success) {
                ...
                for (i in holidayPropertyList) {
                    var typeId = holidayPropertyList[i].sid;
                    var typeName = holidayPropertyList[i].name;
                    if (systemItemFlag == true) {
                        selTypeObj.append("<option value=" + typeId + ">" + typeName + "</option>");
                        // $("#property  option[value='" + holidayPropertyList[i].sid + "']").attr("selected", true);
...
function addSaveForm(sign) {
    ...
    $.ajax({
        url: hostUrl + "kq/hrKqBusstypeSetting/save",
        type: 'POST',
        dataType: 'JSON',
        contentType: 'application/json',
...
function checkIfRename() {
    var name = $("#name").val();
    $.ajax({
        url: hostUrl + "kq/hrKqBusstypeSetting/queryIfRename",
        type: 'POST',
        dataType: 'JSON',
        contentType: 'application/json',
        async: false,
...
function editSaveForm() {
    ...
    $.ajax({
        url: hostUrl + "kq/hrKqBusstypeSetting/update/" + hrKqHolidaytypeSettingDto.id,
        type: 'PUT',
        dataType: 'JSON',
        contentType: 'application/json',
        data: JSON.stringify(hrKqHolidaytypeSettingDto),
