...
//********本地测试使用数据****开始
            ...
            $("#sysApplyFrom").find("input[name='approvalStatusValue']").val(approvalStatusValue);
//********本地测试使用数据****结束
            initUuidRestApply();
            ...
            restApplyId = $.xljUtils.getUrlParam("restApplyId");
...
//grid 自适应宽度
    ...
    window.initUuidDocuments = function () {
        ...
        $("#approvalStatus").val($.hrUtils.APPLY_DRAFT);
    ...
    function saveApplyForm(sign, applyFlag) {
        ...
        if (applyFlag !== undefined && applyFlag !== null && applyFlag === true) {
            setTimeout(function () {
                // window.history.go(-1);
                window.location.href = "kq_rest_list_myKq.html?myFlag=true&myPersonId=" + myPersonId;
            }, 300);
        } else {
            setTimeout(function () {
                window.location.href = "kq_rest_list.html";
            }, 300);
        }
    }
    ...
    function addSaveApplyForm() {
        ...
        //     }
        // }
        hrKqRestDto.sid = $("#restApplyFormId").val();
        ...
        hrKqRestDto.topicName = $("#topicName").val();
        hrKqRestDto.approvalStatus = $("#approvalStatus").val();
        hrKqRestDto.businessId = $("#businessId").val();
        $.ajax({
            url: hostUrl + "kq/hrKqRest/save",
            type: 'POST',
            dataType: 'JSON',
            async: false,
            contentType: 'application/json',
 ...
 */
     ...
     * 更新请假信息
     */
    function updateApplyForm(restApplyId) {
    ...
    window.userCallback = function (data, success) {
        ...
        var queryRestListByPerson = {
            "personId": personId,
            "year": year,
            "approvalStatus": $.hrUtils.APPLY_PASS
        };
