...
//********本地测试使用数据****结束
        ...
        noPunchApplyId = $.xljUtils.getUrlParam("noPunchApplyId");
...
function saveApplyForm(sign, applyFlag) {
    ...
    if (applyFlag !== undefined && applyFlag !== null && applyFlag === true) {
        setTimeout(function () {
            // window.history.go(-1);
            window.location.href = "kq_noPunchCard_list_myKq.html?myFlag=true&myPersonId=" + myPersonId;
        }, 300);
    } else {
        setTimeout(function () {
            window.location.href = "kq_noPunchCard_list.html";
        }, 300);
    }
...
function addSaveApplyForm() {
    ...
    hrKqNotPunchDto.sid = $("#notPunchId").val();
    ...
    hrKqNotPunchDto.delflag = 0;
    hrKqNotPunchDto.topicName = $("#topicName").val();
    hrKqNotPunchDto.approvalStatus = $("#approvalStatus").val();
    hrKqNotPunchDto.businessId = $("#businessId").val();
    ...
    $.ajax({
        url: hostUrl + "kq/hrKqNotPunch/save",
        type: 'POST',
        dataType: 'JSON',
        async: false,
        contentType: 'application/json',
...
function updateApplyForm(noPunchApplyId) {
    ...
    hrKqNotPunchDto.sid = noPunchApplyId;
    hrKqNotPunchDto.topicName = $("#topicName").val();
    hrKqNotPunchDto.approvalStatus = $("#approvalStatus").val();
    hrKqNotPunchDto.businessId = $("#businessId").val();
    $.ajax({
        url: hostUrl + "kq/hrKqNotPunch/update/" + noPunchApplyId,
        type: 'PUT',
        dataType: 'JSON',
        async: false,
        contentType: 'application/json',
