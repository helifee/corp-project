...
;;(function ($,window,document,undefined) {
    var HrSingleSelector = function (ele, opts) {
        this.$element = ele;
        this.defaults = {
            ...
            code_set_id:null,//选择的数据的Name存储input域
        ...
        _init:function () {
			...
			modalContainer.on('show.bs.modal',function () {
                ...
                case 'region':
                        ...
                        that._initTree(tree,url,that.options.treeParam,modalContainer);
                        break;
                case 'org-selectDeptOnly'://机构只选择部门
                    var url = hostUrl + 'org/orgRoot/getTree';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    ...
                    break;
                case 'person':
                    var url = hostUrl+'/org/orgRoot/getUserTree';
					...
					if(typeof that.options.treeParam.userStatus == 'undefined'){
						that.options.treeParam.userStatus = '1';//在职
					}else{
					...
					that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
				case 'post-standard'://选择标准岗位
                    //var url = hostUrl+'sys/org/post/getPostTree';
                    var url = hostUrl+'org/post/getPostTree';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.rootDelFlag = 0;
                    ...
                    that.options.treeParam.postStatus = 1;//有效
                    that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
                case 'post'://选择机构下岗位
                    var url = hostUrl+'org/post/getPostTreeMul';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.rootDelFlag = 0;
                    that.options.treeParam.orgDelFlag = 0;
                    that.options.treeParam.postDelFlag = 0;
                    // that.options.treeParam.orgId = "";
                    // alert(that.options.treeParam.orgId);
                    that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
                case 'role':
                ...
                case 'menu':
                    ...
                    that.options.treeParam.appDelFlag = 0;
                    that.options.treeParam.menuDelFlag = 0;
                    that.options.treeParam.delflag = 0;
                    that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
                case 'hrSysCode':
                    var url = hostUrl+'sys/sysCodeItem/getCodeTree';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.delflag = 0;
                    that.options.treeParam.status = true;//只查询有效的代码项进行选择
                    that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
                case 'hrSysCodeSet':
                    var url = hostUrl+'sys/sysCodeSet/getCodeSetTree';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.delflag = 0;
                    ...
                    break;
                //已下是选择平台的
                case 'person-platform'://人员-平台
                    var url = hostUrl+'sys/org/user/getUserTree';
					//var url = hostUrl + 'sys/org/roleUser/selectUserPostTree';
					if(that.options.treeUrl){
						url = that.options.treeUrl;
					}
					that.options.treeParam.rootDelFlag = 0;
                ...
                case 'org-platform':
					...
					break;
                case 'company':
                    var url = hostUrl + 'org/orgRoot/getCompanyTree';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.rootDelFlag = 0;
                    ...
                    that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
                case 'allCompany':   //无需权限查询所有公司
                    var url = hostUrl + 'org/orgRoot/getAllCompanyTree';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.rootDelFlag = 0;
                    ...
                    that._initTree(tree,url,that.options.treeParam,modalContainer);
                    break;
                case 'hrEvaDimensionalityTree':
                    var url = hostUrl+'eva/hrEvaDimensionalityTree/queryEvaDimensionalityTreeList';
                    if(that.options.treeUrl){
                        url = that.options.treeUrl;
                    }
                    that.options.treeParam.delflag = 0;
...
//                    	this.options.treeParam = JSON.stringify(this.options.treeParam)=='{}'?null:this.options.treeParam
            ...
            modalHeaderContainer.find('button.save-modal-data').on('click',function () {
                if(that.zTreeObj){
                    ...
                    if(undefined!=that.options.treeSettings.check){
                        ...
                        if($selectorType=='region'&&treeNode.id=='1'){ //不能选择根节点
                            $.xljUtils.tip('blue','省市县顶级目录不能选择！');
                            return;
                        }
                        /*
                        if($selectorType=='region'&&treeNode.type=='cata'){ //没有不能选择的根节点
                            if(that.options.treeParam.rootSelect&&that.options.treeParam.rootSelect=='1'){
                                treeNode.id= ''; //portal新闻切换全集团
                            }else{
                                $.xljUtils.tip('blue','不能选择根节点！');
                        ...
                        */
                        if($selectorType=='person-platform'&&treeNode.type!='user'){
                            $.xljUtils.tip('blue','只能选择人员！');
                            return;
                        }
                        if($selectorType=='org-selectDeptOnly'&&(treeNode.type!='dept'&&treeNode.type!='virtual')){
                            $.xljUtils.tip('blue','只能选择部门或虚拟机构！');
                            return;
                        }
                        if($selectorType=='post-standard'&&treeNode.type!='post'){
                        ...
                        if($selectorType=='hrEvaDimensionalityTree'&&treeNode.type!='0'){
                            $.xljUtils.tip('blue','只能选择子维度！');
                            return;
                        }
						//定义可选节点
                    ...
                    var settings = $.extend(true, {
                        ...
                        callback: {
                            onDblClick:function (event, treeId, treeNode) {
                                ...
                                if($selectorType=='region'&&treeNode.id=='1'){ //根节点不能选择
                                    $.xljUtils.tip('blue','省市县顶级目录不能选择！');
                                    return;
                                }
                                /*
                                if($selectorType=='region'&&treeNode.type=='cata'){ //现在没有主根节点
                                    return;
                                }
                                */
                                if($selectorType=='org-selectDeptOnly'&&(treeNode.type!='dept'&&treeNode.type!='virtual')){
                                ...
                                if($selectorType=='hrSysCode'&&treeNode.type!='code'){
                                    return;								
                                }
                                // if($selectorType=='hrSysCodeSet'){
                                //     return;
                                // }
                                if($selectorType=='person-platform'&&treeNode.type!='user'){
                                    return;																
                                }
                                if($selectorType=='company'&&treeNode.type=='cata'){
                                    return;
                                }
                                if($selectorType=='hrEvaDimensionalityTree'&&treeNode.type!='0'){
                                    $.xljUtils.tip('blue','只能选择子维度！');
    ...
    $.fn.extend({
        hrxljSingleSelector:function (options) {
            if(!options){
                return HrSingleSelector;
            }
            var opts = options;
            return this.each(function (index,ele) {
				if(opts.isTransParam){
            		 $(ele).unbind();
                     var hrSingleSelector = new HrSingleSelector(this,opts);
            	}else{					  
				...
				var hrSingleSelector = $(ele).data('_singleSelector');
                //var hrSingleSelector = $(document.body).data(eleId);
                if(!hrSingleSelector){
                    hrSingleSelector = new HrSingleSelector(this,opts);
                    //$(document.body).data(eleId,hrSingleSelector);
					
					hrSingleSelector._init();
                        if(!opts.immediatelyShow){
                            $(ele).on('click',function () {
                                hrSingleSelector.modalContainer.modal({show:true,backdrop:'static'});
                            });
                        }
                        $(ele).data('_singleSelector',hrSingleSelector)
            		}else{
                        hrSingleSelector.modalContainer.modal({show:true,backdrop:'static'});
                    }
            	}
        ...
        hrxljSingleSelectorUtil:function () {
            var eles = $(this).find('.hr-single-selector');
            $.each(eles,function () {
                ...
                var code_set_id = $(this).attr('data-codeSetId');
                if(code_set_id){
                    opts.code_set_id = code_set_id;
                }
         ...
         * @param options
         */
        hrxljSingleSelectorReset:function (options) {
            var opt = options;
            this.each(function(index,ele){
                var hrSingleSelector = $(ele).data('_singleSelector');
                if(hrSingleSelector){
                    hrSingleSelector.modalContainer.remove();
                    hrSingleSelector.zTreeObj = null;
                    if(opt){
                        hrSingleSelector.options = opt;
                    }
                    hrSingleSelector._init();
                }else{
                    $(this).hrxljSingleSelector(opt);
                }
