...
;(function($, window, document, undefined) {
	...
	$.extend($.xljUtils, {
		...
		queryAttachmentUrlList:function(appId, businessId, categoryId, callback,isAsync) {
			...
			$.ajax({
				...
				url:$.xljUtils.serverAddr + 'disk/attachment/queryList',
				type:'POST',
				contentType: "application/x-www-form-urlencoded",
                data:{"appId": appId, "businessId": businessId, "categoryId":categoryId},
				async:isAsync,
				success:function(dt) {
					callback(true, dt);
				},
		...
		/**
		 * 根据文件路径和文件名下载文件
		 * @param {String} sid 文件sid
		 */
		xljDownloadFromFileInfo: function (fileInfo) {
            window.open($.xljUtils.serverAddr + "disk/attachment/downLoadByFileId?fileId="+fileInfo.sid+"&"+window.parent.JZY.s.getAccessTokenByAuthorization());
		},
	...
	var XljAttachment = function(ele, opts) {
        ...
        this.options.serverAddr = this.options.serverAddr.replace('platform-app/attachment/', 'platform-app/');
	...
	XljAttachment.prototype = {
		...
		_createAttachmentModal:function(){
			...
			// 非table显示情况下
				...
				// 编辑模式下添加附件控件可用
					...
					attHtmls.push('        	<input id="fileupload" class="fileupload" type="file" name="resource" multiple></a>');
		...
		_queryList:function() {
			...
			$.ajax({
				...
				url:this.options.serverAddr + 'disk/attachment/queryList',
				type:'POST',
				data:queryParam,
				dataType:"JSON",
				success:function(dt) {
						...
						if (that.options.mode != $.xljUtils.modeEnum.table) {
							...
							$.each(dt.result, function (index, file) {
								...
								fileLi.append(
									$('<img class="fileType"/>').attr('src', function () {
										if (file.fullName && /.docx?$/g.test(file.fullName)) {
											return that.options.serverAddr + 'univ/images/doc02.png'+"&"+window.parent.JZY.s.getAccessTokenByAuthorization();
										} else {
											return "";//that.options.serverAddr + 'univ/images/doc03.png?'+window.parent.JZY.s.getAccessTokenByAuthorization();
										}
									})
								).append(
								...
								if ("url" == file.type || fileType.indexOf(file.fullName.substring(file.fullName.lastIndexOf(".") + 1).toLowerCase()) > -1) {
									fileLi.append($('<a href="javascript:void(0);" class="up_preview">预览</a>')); //TODO 平台附件暂时不支持预览功能，打算二期再实现
								}
								...
								fileLi.find('a.up_delete').on('click', function (e) {
									...
									$.ajax({
										...
										url: that.options.serverAddr + 'disk/attachment/deletefile',
										type: 'POST',
										dataType: 'JSON',
		...
		_submit: function(callback) {
			...
			$.ajax({
				...
				url:this.options.serverAddr + 'disk/attachment/saveBatch',
				type:'POST',
...
//				console.log('--- uploadedCount -- :' + that.options.fileList.uploadedCount);
		...
		_bindPreviewEvent:function(target, file) {
			...
			target.on('click',function() {
				...
				if("url"==file.type){
					...
					var extensionName = file.extendName;
					if (officeType.indexOf(extensionName.toLowerCase()) > -1) {
						$.ajax({
							//url: $.xljUtils.serverAddr + "/univ/attachment/attachment/docConverter" + "?time=" + Math.random(),
							url: $.xljUtils.serverAddr + "disk/attachment/previewByFileId/"+ file.sid + "?time=" + Math.random(),
							...
							success: function (resultData) {
								if (resultData.status == 200) {
									var successFlag = (resultData.status == 200?true:false);
									if (successFlag) {
										var exName =  resultData.result.substring(resultData.result.lastIndexOf(".")+1);
										if("html"==exName){
											window.open(baseUrl+"/common/html/viewer.html?path="+resultData.result.replace(/\\/g,"/")+"&fileName="+encodeURIComponent(file.name));
										}else{
											window.open(baseUrl+"common/pdf/web/viewer.html?path="+resultData.result.replace(/\\/g,"/")+"&fileName="+encodeURIComponent(file.name));
										}
									} else {
						...
						// 	function (ip) {
						// 		if (ip) {
						// 			//window.open($.xljUtils.serverAddr+"/mobile/approve/approve_view.html?path="+location.protocol + '//' +ip.result+":"+$.xljUtils.fdfsStoragePort+"/"+file.path+"&fileName="+encodeURIComponent(file.name));
						// 			window.open(location.protocol + '//' +ip.result+"/"+file.path+"?fileName="+encodeURIComponent(file.name));
						// 		}
						// 	}
						// );
						$.ajax({
		...
		_submitUrlAttachment: function(oData, backCall) {
			...
			if (that.options.autoSubmit === true) {
				...
				sSubmitUrl = this.options.serverAddr + 'disk/attachment/saveBatch';
			} else {
			...
			$.ajax({
				...
				success:function(dt) {
					...
					var fileLi = $('<li/>').append(
						$('<img class="fileType"/>').attr('src', "")//that.options.serverAddr + 'univ/images/doc03.png?'+window.parent.JZY.s.getAccessTokenByAuthorization())
					).append(
						$('<div class="fileName"/>').append(
					...
					if (dt.success) {
						...
						// 预览
						//$('.up_files ul').find('.up_preview').css("display","block");//TODO 平台附件暂时不支持预览功能，打算二期再实现
						...
						fileLi.find('.up_delete').on('click', function(e) {
							$.ajax({
								...
								url:that.options.serverAddr + 'disk/attachment/deletefile'+fileInfo.id,
								type:'DELETE',
								dataType:'JSON',
								success:function(rs) {
									if (rs.success) {
		...
		_init:function() {
			...
			if (this.options.mode === $.xljUtils.modeEnum.view || this.options.mode === $.xljUtils.modeEnum.table) {
				...
				$('.file-choose-upload-button', that.$element).click(function(){
					...
					// TODO 暂时没有打包下载接口
					//var downloadForm = $('<form action="' + that.options.serverAddr +'univ/attachment/attachment/downloadAll" method="post"/>')
					var downloadForm = $('<form action="' + that.options.serverAddr +'disk/attachment/downloadAll" method="post"/>')
			...
			if (that.options.autoSubmit === true) {
				...
				uploadSubmitUrl = this.options.serverAddr + 'disk/attachment/upload';
			} else {
				//uploadSubmitUrl = this.options.serverAddr + 'univ/attachment/attachmentTemp/upload?'+window.parent.JZY.s.getAccessTokenByAuthorization();
				uploadSubmitUrl = this.options.serverAddr + 'disk/attachmentTemp/upload?'+window.parent.JZY.s.getAccessTokenByAuthorization();
			}
			// 判断IE版本
//			var _dataType = 'json';
...
//                _dataType = 'text';
//            }
			var ffName = new Object();
			var $this = $('.fileupload', this.$element).fileupload({
				url: uploadSubmitUrl,
				//forceIframeTransport:true,
				dataType: 'text',
				formData:$.extend({},{appId:this.options.appId, businessId:this.options.businessId,categoryId:this.options.categoryId, type:'file', chunkSize:5 * 1000 * 1000,fileName:ffName.ff}, this.options.formData),
				autoUpload: false,
				...
				if(data.files[0].name.length>44){
					$.xljUtils.tip('red','上传附件名称长度超出范围(最大40个字)!');
					return false;
				}
				// 添加文件
				...
				var node = $(data.context);
				ffName.ff = data.files[0].name;
				//submitFormData = $.extend({},{appId:this.options.appId, businessId:this.options.businessId,categoryId:this.options.categoryId, type:'file', chunkSize:5 * 1000 * 1000,fileName:fileName}, this.options.formData);
				...
				var node = $(data.context);
				var resultData = eval("("+data.result+")");
				var success = (resultData.status == 200?true:false);
				...
				//that._showTotalSize(data.files[0].size)
					.append($('<span class="up_status up_ok">'+(success?'上传完成':'上传失败')+'</span><span>' + that._showTotalSize(data.files[0].size) + '</span>')));
				...
				var isSingleUpload = that.options['singleUpload'] === false;
				if (success) {
					var fileInfo = $.extend({}, resultData.result.file);
					//delete fileInfo.fileBytes;
					that.options.fileList.newList.push(fileInfo);
					that.options.fileList.totalSize += fileInfo.fileSize;
...
//					console.log('--- uploadedCount -- :' + that.options.fileList.uploadedCount);
					...
					// 预览
					// node.find('.up_preview').css("display","block");  //TODO 平台附件暂时不支持预览功能，打算二期再实现
					that._bindPreviewEvent(node.find('.up_preview'), resultData.result.file)
				}
				// 上传失败
				else {
					node.find('.up_delete').on('click', function(e) {
