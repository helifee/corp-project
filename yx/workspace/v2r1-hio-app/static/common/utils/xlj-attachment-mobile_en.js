...
;(function($, window, document, undefined) {
    ...
    $.extend($.xljUtils, {
        ...
        serverAddr:location.protocol + '//' + location.host + '/platform-app/',
        /** fdfs存储服务端口 */
        ...
        xljDownloadFromFileInfo: function (fileInfo) {
            ...
            window.open($.xljUtils.serverAddr + "disk/attachment/downLoadByFileId?fileId=" + fileInfo.sid + "&access_token=" + access_token);
        },
    ...
    XljAttachment.prototype = {
        ...
        _createAttachmentModal:function(){
            ...
            if (this.options.isTable == $.xljUtils.modeEnum.isTable) {
                var _aClass = this.options.mode == $.xljUtils.modeEnum.view ?"mui-navigate-right":"";
                var _vClass = this.options.mode == $.xljUtils.modeEnum.view ?"attachNum":"attachAddFiels";
                attHtmls.push('<a class="'+_aClass+'" href="#">attachment/附件<div id="attachmentSum" class="'+_vClass+'"></div></a>');
        ...
        _queryList:function() {
            ...
            if(this.options.isTable != $.xljUtils.modeEnum.isTable){
                // 查询时可以不传appid
                var queryParam = {businessId: this.options.businessId, categoryId:this.options.categoryId,sidx:"name",sord:"asc"};
                if (!$.xljUtils.isEmpty(this.options.appId)) {
                    queryParam.appId = this.options.appId;
                }
                $.ajax({
                    // url:this.options.serverAddr + 'univ/attachment/attachment/queryList',
                                                    ...
                                                    if (successFlag) {
                                                       var exName =  resultData.result.substring(resultData.result.lastIndexOf(".")+1);
                                                        if("html"==exName){
                                                            ...
                                                            window.location.href=encodeURI(encodeURI(baseUrl+"/common/html/viewer.html?path="+resultData.result.replace(/\\/g,"/")+"&fileName="+file.name));
                                                        }else{
                                                            // window.location.href=encodeURI(encodeURI($.xljUtils.serverAddr+"/pdf.html?path="+resultData.msg.replace(/\\/g,"/")+"&fileName="+file.name));
                                                            window.location.href=encodeURI(encodeURI(baseUrl+"common/pdf/web/viewer.html?path="+resultData.result.replace(/\\/g,"/")+"&fileName="+file.name));
                                                        }
                                                    } else {
            ...
            }else if (that.options.mode !== $.xljUtils.modeEnum.add) {
                // 查询时可以不传appid
                ...
                $("#attachmentSum",that.$element).empty().append('<a href="javascript:void(0);" class="fileinput-button"><span>add attachment/添加附件</span><input id="fileupload" class="fileupload" type="file" accept="image/jpg,image/jpeg,image/png,image/gif" name="files[]" multiple></a>');
            }
        },
        // 附件信息提交
        ...
        _init:function() {
            ...
            var that = this;
            // 预览模式不进行上传功能初始化
            if (this.options.mode === $.xljUtils.modeEnum.add && this.options.isTable === $.xljUtils.modeEnum.isTable) {
                var uploadSubmitUrl;
                if (that.options.autoSubmit === true) {
                    ...
                    var access_token = $.xljUtils.getUrlParam("accessToken");
                    access_token = access_token == null ? "" : access_token.replace('Bearer ','');
                    uploadSubmitUrl = this.options.serverAddr + 'disk/attachmentTemp/upload?access_token=' + access_token;
                }
