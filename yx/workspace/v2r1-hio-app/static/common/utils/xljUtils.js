...
(function ($) {
    ...
    $.extend($.xljUtils, {
         ...
         * </div>
         */
        resizeNestedGrid: function (marginSize,shrinkToFit) {
            ...
            $.each(fitGridTables, function (i, fitGridTable) {
                ...
                if($('.tip-box-alert:visible')[0]){
                    gridHeight += $($('.tip-box-alert:visible')[0]).outerHeight();
                }
                $(fitGridTable).jqGrid('setGridHeight',gridHeight);
                shrinkToFit = (typeof shrinkToFit == 'undefined')?true:shrinkToFit;
                $(fitGridTable).jqGrid('setGridWidth',$(gridContainer).width(), shrinkToFit);
        ...
        createValidateRule: function(validateForm){
            ...
            $.each(formElements,function (i, formElement) {
                ...
                //特殊字符
                var specialVal = $(formElement).attr("data-special");
                if(specialVal&&specialVal=="true") {
                    formElementRulesJson["special"]=true;
                    formElementMessagesJson["special"]= (formElementPlacehold?formElementPlacehold:formElementName)+"格式不正确";
                }
...
//                                    var params = dataCallback.substring(dataCallback.indexOf('(')+1);
        ...
        addCustomValidateMethod:function () {
            ...
            jQuery.validator.addMethod("special", function(value, element) {
                ...
                var regEn = /[`~!@#$%^&*()_+<>?:"{},.\/;'[\]]/im,
                    regCn = /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im;
                return this.optional(element) || (!regCn.test(value) && !regEn.test(value));
            }, "请不要录入特殊符号");
...
//                return this.optional(element) || (!(englishWord.test(value)) && !(englishWordSymbol.test(value)));
         ...
         * 设置网格冻结列后不同浏览器样式差异调整
         */
        setFrozenColumnStyle:function(_height){
        	_height = (_height == undefined || _height == null) ? 40 : _height;
        	// 冻结列样式begin
            ...
            $('.frozen-div').attr("style", "position:absolute; left:0px; top:0px; height:"+_height+"px");
            $('.frozen-bdiv').attr("style", "position:absolute; left:0px; top:"+_height+"px; overflow-y:hidden; height:" + bdiv_height + "px !important;");
            // 冻结列样式end
        }
    });
    ...
    $(function () {
        ...
        $(window).on('resize', function () {
            if(typeof window.gridForceFit=='undefined'){
                window.gridForceFit = true;
            }
            //setTimeout(function () {
               $.xljUtils.resizeNestedGrid(null,window.gridForceFit);
            //},500);
        });
