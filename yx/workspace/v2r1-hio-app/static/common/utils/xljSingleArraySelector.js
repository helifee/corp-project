...
;;(function ($,window,document,undefined) {
    var SingleArraySelector = function (ele, opts) {
        this.$element = ele;
        this.defaults = {
            selectorTypeArray:['org','person','onlyPerson','post','role', 'commonRole','busiObject','busiVariable'],//参数数组 默认【组织机构、人员、岗位、角色、菜单】
        ...
        _init:function () {
            ...
            if(this.options.selectorTypeArray&&this.options.selectorTypeArray!=null){
                for(var i in this.options.selectorTypeArray){
                    ...
                    if(this.options.selectorTypeArray[i]=='onlyPerson'){
                        option +="<option value='onlyPerson'>人员</option>";
                    }
            ...
            switch (this.options.selectorType){
                ...
                case 'onlyPerson':
                    // var url = hostUrl+'sys/org/user/getUserTree';
                    var url = hostUrl+'sys/org/roleUser/selectUserOrgTree';
                    if(this.options.treeUrl){
                        url = this.options.treeUrl;
                    }
                    this.options.treeParam.rootDelFlag = 0;
                ...
                case 'post':
                    ...
                    this.options.treeParam.rootDelFlag = 0;
                    this.options.treeParam.orgDelFlag = 0;
                    this.options.treeParam.postDelFlag = 0;
                    var commonStatus = that.options.treeParam.commonStatus;
                ...
                case 'menu':
                    ...
                    this.options.treeParam.appDelFlag = 0;
                    this.options.treeParam.menuDelFlag = 0;
                    this.options.treeParam.delflag = 0;
                    this._initTree(tree,url,this.options.treeParam,modalContainer);
                    break;
                case 'busiObject':
        ...
        _saveSelectData:function (treeNode,modalContainer) {
            ...
            if($.isArray(treeNode)){
                for(var i in treeNode){
                    ...
                    if(treeNode1.type=="user"&&this.options.selectorType=='onlyPerson'){
                        resultIds.push(treeNode1.id);
                        resultNames.push(treeNode1.name);
                        resultNodes.push(treeNode1);
                    }
    ...
    $.fn.extend({
        ...
        xljSingleArrayDraw:function (data,ele) {
            ...
            switch (selectArray[0]){
                ...
                case 'onlyPerson':
                    // var url = hostUrl+'sys/org/user/getUserTree'+"?time="+Math.random();
                    var url = hostUrl+'sys/org/roleUser/selectUserOrgTree?_t='+new Date().getTime();
                    if(that.options.treeUrl){
                        url = this.options.treeUrl;
                    }
                    that.options.treeParam.rootDelFlag = 0;
                    ...
                    console.log("---- 重新画第二个树  onlyPerson 001 url="+url);
                    console.log("---- 重新画第二个树  onlyPerson 002 that.options.treeParam="+JSON.stringify(that.options.treeParam));
                    that._initTree(tree,url,that.options.treeParam,that.modalContainer);
                    break;
                case 'post':
                    var url = hostUrl+'sys/org/post/getPostTree';
