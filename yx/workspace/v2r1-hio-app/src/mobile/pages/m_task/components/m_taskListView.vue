<template>
    <div class="m_list_view_wrap">
        <group title="">
            <cell v-for="(item,index) in data" :key="index" :title="item.taskName" :link="'/m_task/m_details_task/' + item.taskId +'?fromPage='+moduleName" is-link>
                <!-- <router-link class="task_detail_panel" :to=" "> -->
                    ...
                    <div class="list_right" solt="value">
                        ...
                        <span class="concerned" @click.stop="setTaskFollow(index, item)" v-if="moduleName!=='set'">
                            <!-- {{typeof item.isfollow}} -->
                            <x-icon class="orange" type="ios-star" size="22" v-if="item.isfollow && item.isfollow !=='0' "></x-icon>
