<template>
  <div class="approve edit free_flow">
    <!--表单项-->
    <el-row :gutter="0">
      <el-col :span="24">
        <div class="detail_content">
            <!-- <h1 class="title">{{freeFlowForm.templateName}}</h1> -->
            <el-form ref="freeFlowInfo" label-width="85px" class="free_flow_info">
              <el-form-item label="申请流程：">
                <span v-text="freeFlowForm.templateName"></span>
              </el-form-item>
              <el-form-item label="流程说明：">
                <span v-text="freeFlowForm.description"></span>
              </el-form-item>
              <el-row>
                <el-col :span="8">
                  <el-form-item label="申请人：">
                    <span v-text="freeFlowForm.createPersonName"></span>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="部门：">
                    <span v-text="freeFlowForm.dept"></span>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="日期：">
                    <span v-text="freeFlowForm.createDate"></span>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
            ...
            <flow-form
                ...
                ref="instance"
                :formMode="operateType == 1 ? 'reInstance' : 'instance'"
            ...
            <el-form ref="freeFlowForm" :model="freeFlowForm" :rules="freeFlowRules" label-width="100px" class="free_flow_content">
                ...
                <el-form-item label="配置流程：" v-if="data.freeOrTemplate === 'F'">
                    ...
                    <span v-for="(subItem,index) in freeFlowForm.nodeList" :key = "index">
                      ...
                      <em v-if=" freeFlowForm.nodeList.length -1 > index "> ==> </em>
                ...
                <el-form-item label="附件：">
                  <attach-upload
                    ref="attachUpload"
                    v-if="freeFlowForm.attachmentId"
                    :required = "false"
                    :multiple = "false"
                    :appId="freeFlowForm.appId"
                    :businessId="freeFlowForm.attachmentId"
                    :categoryId="freeFlowForm.attachmentCategoryId"
                ...
                <el-form-item label="关联项目：">
                  <el-select popper-class="select_project" v-model="freeFlowForm.projectId" :title="freeFlowForm.projectName" @change = "setProjectId(freeFlowForm.projectId)" placeholder="请选择">
                    <el-option
...
<script>
  ...
  import {addFreeFlow ,draftSave, getFlowStartView } from '@Main/approve/getData'
  //项目接口：数据接口文件
  ...
  export default {
    ...
    data() {
      return {
        ...
        //     sid:'tianjin'
        // }],
        operateType:'2',//表单操作类型operateType为空：字符串0：不处理，1新增，2修改，3删除！
        isReject:false,//是否为驳回、退回等模式：废弃
        ...
        //混合树组件2.0-抄送人-复制
        selectedDataToTreeForSharedUserCopy:{//已选树节点
          userList:[],
          userOutsideList:[]
        },
    ...
    methods: {
        ...
        initData( dataObj = {} ){
            ...
            this.freeFlowForm = JZY.u.deepExtend( {} , this.freeFlowForm , dataObj )
            //从vuxe里获取当前登陆人的sid
            this.getlistProjectListById( this.$store.state.session.sid )
            //如下两个列表需单独处理

            //自由流审批人列表，暂不回写配置的自由流，下面代码作废
            // this.freeFlowForm.nodeList = [ ...this.freeFlowForm.nodeList ]
            
            //根据流程的state状态，计算出是新增还是修改
            if (this.freeFlowForm.state == 3 || this.freeFlowForm.state == 6 
          || this.freeFlowForm.state == 7 || this.freeFlowForm.state == 8  ) {//3驳回，6退回，7驳回已处理，8退回已处理
              this.operateType = '1'  //计算为新增，1
              this.getFlowStartViewByCode( this.freeFlowForm.templateCode )  //获取新增流程的详细信息
            }
            //抄送人
            // this.sharedUsers = []
            // this.freeFlowForm.copyUserInfoList && this.freeFlowForm.copyUserInfoList.forEach((item)=>{
            ...
            this.selectedDataToTreeForSharedUser = {
              userList:[],
              userOutsideList:[]
            }
            this.selectedDataToTreeForSharedUserCopy = {
              userList:[],
              userOutsideList:[]
            }
            //目前后端返回的数据无法区分人员的内外部属性，只能都放到内部列表里0704，和后端沟通无果
            //0704下午，张朝阳找周亲林沟通，添加了一个返回的字段
            this.freeFlowForm.copyUserInfoList && this.freeFlowForm.copyUserInfoList.forEach((item)=>{
                if (item.userType == 1) {
                    this.selectedDataToTreeForSharedUser.userList.push({
                        sid:item.id,
                        name:item.name
                    })
                }else if(item.userType == 2){
                    this.selectedDataToTreeForSharedUser.userOutsideList.push({
                        sid:item.id,
                        name:item.name
                    })
                }
            })
            // this.selectedDataToTreeForSharedUser.userList

            console.info("edit_freeFlowForm  this.freeFlowForm ++",this.freeFlowForm)
        },
        //获取新增流程的详情，附件用
        async getFlowStartViewByCode( code = '' ) {
          let res = await getFlowStartView( code );
          console.info(res[0])
          this.freeFlowForm.appId = res[0].appId
          this.freeFlowForm.attachmentId = res[0].businessId
          this.freeFlowForm.attachmentCategoryId = res[0].categoryId
// debugger
        },
        //删除抄送人tag的事件-作废
        deleteUser( sid ) {
          ...
          this.freeFlowForm.copyUserIds.splice(this.freeFlowForm.copyUserIds.indexOf(sid), 1)
        ...
        saveInfoData:function( dataInfo = {} ){
            console.info("dataInfo=======",dataInfo)
            // this.freeFlowForm.nodeList = [ ...dataInfo ]
            ...
            if (this.data.saveMode == 'draftSave') {
                ...
                if (valid) {
                  let params = {
                    ...
                    // customFormId:this.$store.state.customForm.extInfo.sid,

                    isRemove:this.freeFlowForm.isRemove,
                    instanceId:this.freeFlowForm.instanceId,
                    // businessId:this.freeFlowForm.businessId,
                    terminalType:this.freeFlowForm.terminalType,
                    copyUserIds:this.freeFlowForm.copyUserIds,
                    attachmentId:this.freeFlowForm.attachmentId,
                    projectId:this.freeFlowForm.projectId,
                    freeOrTemplate:this.freeFlowForm.freeOrTemplate,
                    nodeList:this.freeFlowForm.nodeList,
                  ...
                  try{
                      // console.log(params)
                      // console.log(this.$store.state.customForm.valueData)
    ...
    　　　　handler(newValue, oldValue) {
                ...
                this.freeFlowForm.businessData = [ ...this.$store.state.customForm.businessData ]
