<template>
    <div class="dynamic-content">
        ...
        <dialog-follow-recordform-cu-opp :contactId="contact" :dialogVisible="recordformDialogVisible" :customer='customer'  v-if="recordformDialogVisible" @closeCreateModal="showRecordformDialog"></dialog-follow-recordform-cu-opp>
    </div>
</template>
...
import {customerFollowRecord,folUpdate,modifyCustomer} from '@Main/crm/getData'
    ...
    export default {
        ...
        methods: {
            async recordCli(){
                let res = await modifyCustomer(this.customerId).then(()=>{
            ...
            showRecordformDialog (id){
                console.log('customerID----客户id',this.followHis.customerId);
                console.log('oppID----商机id',this.followHis.opportunityId);
                this.$emit("cli")
                this.recordformDialogVisible=false;
                let customerId=this.$route.query.customerId || this.followHis.customerId;
                let contactId=this.$route.query.contactId;
                let opportunityId=this.$route.query.opportunityId || this.followHis.opportunityId;
                let planId='';
            ...
            setScroll(){
                ...
                // 　　    my.pageNum ++;
                       if(my.pageCount>my.dataTotal){
                        ...
                        let pageNum=my.pageNum;
                        let pageCount=my.pageCount;
                        let customerId=my.$route.query.customerId || my.followHis.customerId;
                        let opportunityId=my.opportunityId || my.followHis.opportunityId;
                        let contactId=my.$route.query.contactId;
                        let planId=my.planId;
                        my.customerFollowRecordData({
                            'pageNum':pageNum,
        ...
        mounted(){
            console.log('customerID----客户id',this.followHis.customerId);
            console.log('oppID----商机id',this.followHis.opportunityId);
            let customerId=this.customerId || this.followHis.customerId;
            let contactId=this.$route.query.contactId;
            let opportunityId=this.$route.query.opportunityId || this.followHis.opportunityId || this.$route.params.id;
            let planId='';
            let pageNum=this.pageNum;
            let pageCount=this.pageCount;
            // let pageNum=this.pageNum;
