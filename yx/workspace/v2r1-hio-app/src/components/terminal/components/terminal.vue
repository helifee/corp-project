<template>
    <div style="width:85%;margin:0 auto;min-width:600px;" class="terminal">
            ...
            <div style="position:absolute;top:0;left:0;right:0;overflow:auto;z-index:1;margin-top:30px;max-height:500px"
                ...
                <div class="terminal-window" id="terminalWindow">
                    ...
                    <p v-for="(item, index) in messageList" :key="index">
                        ...
                        <span class="cmd" v-if="!item.message.list && (!item.message.table)">{{item.message}}</span>
                        <span v-else>
                            <span>{{item.message.text}}</span>
                            <span class="cmd">

                              <ul v-if="item.message.list">
                                <li v-for="(li,index) in item.message.list" :key="index">
                                  <span v-if="li.label" :class="li.type">{{index+1}}. &nbsp;&nbsp;{{li.label}}:</span>
                                  <pre>{{li.message}}</pre>
                                </li>
                              </ul>


                                <el-table style="margin-top:10px;" v-if="item.message.table" :data="item.message.table">
                                <el-table-column label="任务列表">
                                <template slot-scope="scope">
                                <span>{{scope.$index+1}}. &nbsp;&nbsp;{{scope.row.label}}:</span>
                                </template>
                                </el-table-column>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            supportingCommandList(){
                return  Object.keys(commandList).concat(Object.keys(this.taskList))
            },
            lastLineClass() {
        ...
        created() {
            // this.supportingCommandList = Object.keys(commandList).concat(Object.keys(this.taskList))
        ...
        methods: {
            ...
            handleCommand(e) {
                ...
                if (commandArr[0] === 'help') {
                    ...
                    this.pushToList({level: 'System', message: '不支持的命令。请输入 help 获取所有受支持的命令'})
                    // this.pushToList({ level: 'System', message: 'type "help" to get a supporting command list.' })
                }
                this.inputCommand = ''
                this.autoScroll()
            ...
            printHelp(input) {
                if (!input) {
                    ...
                    this.supportingCommandList.map(command => {
                        if (commandList[command]) {
                            this.pushToList({
                                type: 'success',
                                label: command,
                                message: '---> ' + commandList[command].description
                            })
                        } else {
                            this.pushToList({
                                type: 'success',
                    ...
                    const command = commandList[input] || this.taskList[input]
