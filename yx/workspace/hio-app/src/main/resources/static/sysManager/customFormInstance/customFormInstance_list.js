...
$(function () {
	...
	if (id) {
		...
		$.ajax({
			type: 'get',
			url: serviceUrl + '/sys/base/customForm/get/' + id + '?time=' + Math.random(),
...
function pageInit(cols, id) {
	...
	jQuery("#list2").jqGrid({
		url: serviceUrl + '/sys/base/customFormInstance/page',
		ajaxGridOptions: {
	...
	if (idsArray && idsArray.length > 0) {
		if (flag) {
			pop_text_open("blue", "确认删除" + idsArray.length + "条数据吗？", function () {
				$.ajax({
					url: serviceUrl + "/sys/base/customFormInstance/deletePseudoBatch/" + ids,
					type: 'DELETE',
					dataType: 'JSON',
...
function test() {
	...
	$.ajax({
		type: 'get',
		url: serviceUrl + '/sys/base/customFormInstance/getVariable/' + id + '?time=' + Math.random(),
		success: function (data) {
			var customFormDto = data.result;
			alert(1);
		}
...
function test1() {
	$.ajax({
		type: 'get',
		url: serviceUrl + '/sys/base/customFormInstance/getInstance/b78dff747b8d45f3aea9c5a60d50e9a4/4454752b33d340fa9b5c5e95a92c1eac?time=' + Math.random(),
		success: function (data) {
			var customFormDto = data.result;
			alert(1);
		}
