...
$(function () {
	...
	$.ajax({
        type:'get',
        url:serviceUrl+'/sys/base/customArchives/get/'+id+'?time='+Math.random(),
...
function initJqGrid2(){
    jqGrid2 = jQuery("#list2").jqGrid(
        {
        	url: serviceUrl+"/sys/base/customArchivesItem/page",
...
function saveForm(formName){
	...
	if(customArchivesItemDto.id==null || customArchivesItemDto.id==""){
		//add
		$.ajax({
	       url:serviceUrl+"/sys/base/customArchivesItem/save",
	       data:JSON.stringify(customArchivesItemDto),
	       type:'POST',
	       contentType:'application/json',
	       dataType:'JSON',
		...
		//update
		$.ajax({
	       	url:serviceUrl+"/sys/base/customArchivesItem/update/"+customArchivesItemDto.id,
			data:JSON.stringify(customArchivesItemDto),
			type:'PUT',
			contentType:'application/json',
			dataType:'JSON',
...
function treeDel(){
	...
	if(ids!=null && ids!="" && ids.length>0){
		...
		pop_text_open("blue","确认删除【"+ids.length+"】条数据吗？",function(){
			$.ajax({
				url:serviceUrl+"/sys/base/customArchivesItem/deletePseudoBatch/"+idsStr,
				type:'DELETE',
				dataType:'JSON',
				success:function (resultData ) {
...
function enableOrNot(st){
	...
	if(ids!=null && ids!="" && ids.length==1){
		...
		if(status!=st){
			$.ajax({
		       url:serviceUrl+"/sys/base/customArchivesItem/enableOrNot",
