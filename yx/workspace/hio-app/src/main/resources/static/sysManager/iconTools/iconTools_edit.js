...
function pageInit(){
	...
	//添加图标组件
	/*$('#parentName,#selectCaptionName').xljIconRelationSelector({
	...
	//获取上传图标地址
	/*$.ajax({
		url:serviceUrl+'sys/iconTools/getIcon/2?time='+Math.random(),
		type:'get',
		success: function(data) {
			if(data.success){
				var iconTools=data.result;
				$("input[name='id']").val(iconTools.id);
				$("input[name='name']").val(iconTools.name);
				$("input[name='code']").val(iconTools.code);
			}else{
...
function getCommonToolsData(id){
	$.ajax({
		url:serviceUrl+"sys/iconTools/get/"+id+'?time='+Math.random(),
		type:'get',
		success: function(data) {
			if(data.success){
...
function saveForm(btType){
	var  url = serviceUrl+"sys/iconTools/save";
	if(type=="add"){
	}else{
		url = serviceUrl+"sys/iconTools/update";
	}
	var ajax_option={
