...
function pageInit(){
	jQuery("#list2").jqGrid({
		url: serviceUrl + 'sys/base/customFormInstance/fundPage',
		ajaxGridOptions: {
        ...
        colModel : [ 
            ...
            {name : 'vbusinesscode',label : '单据编号',editable:true,width : 80,sortable:false,align:'center'},
            {name : 'vtheme',label : '主题',editable:true,width : 80,sortable:false,align:'center'},
            {name : 'vbusinesstype',label : '业务类型',editable:true,width : 60,sortable:false,align:'center',formatter: vbusinesstypeFormatter},
            {name : 'napplymny',label : '申请金额',editable:true,width : 50,sortable:false,align:'center'},
			{name : 'npaymny',label : '付款金额',editable:true,width : 50,sortable:false,align:'center'},
...
function importFundPayment(){
	...
	if(flag){
		$.ajax({
		   url: serviceUrl+"sys/base/customFormInstance/importFundPayment/"+ids,
...
function synFund(){
	$.ajax({
		   url: serviceUrl+"sys/base/customFormInstance/synFund",
	       data:JSON.stringify({}),
	       type:'POST',
	       contentType:'application/json',
	       dataType:'JSON',
