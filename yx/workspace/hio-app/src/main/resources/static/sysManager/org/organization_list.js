...
function getOrgTree() {
	//获取组织机构树地址
    var urlBody = "http://127.0.0.1:9999/platform-app/sys/org/root/getOrgTreeByType";
    var urlAll =   urlBody;
    var jsonData={
    		rootDelFlag:0,
    		orgDelFlag:0,
...
function syncDataOrg(){
	...
	if(nodes.length < 1 ){
		...
		var rootNode = allNodes[0];
		if(rootNode.id == nodes[0].id){
			pop_tip_open("blue","根目录不同步");
		}else{
			var uBody = "sys/sync/syncData/syncDataOrgOne/"+nodes[0].id;
		    var uAll = serviceUrl + uBody;
			$.ajax({
				type:'POST',
		        url:uAll,
