...
(function(window,document){
    var _billEdit = {
    	...
    	save_form:function(){
			var my = this;
			//映射后台保存方法
			var url=serviceUrl+"sys/num/bill/saveBillAndRuler";
			//请求方式
			var type = 'POST';
			//获取form表单元素并遍历赋值
    	...
    	initBillFrom:function(){
    		if(this.dataPar.upBillId){
    			$.ajax({
        	        type:'get',
        	        url:serviceUrl+'sys/num/bill/get/'+this.dataPar.upBillId+'?time='+Math.random(),
    	...
    	loadRulerData:function(){
    		...
    		if(billId){
    			jQuery("#list3").jqGrid({
    				url: serviceUrl+'sys/num/ruler/getRuleListByBillId'+'?time='+Math.random(),
