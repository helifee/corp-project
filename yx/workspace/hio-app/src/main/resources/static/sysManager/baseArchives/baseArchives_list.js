...
function pageInit(){
    jQuery("#list2").jqGrid(
    {
        url: serviceUrl+'/sys/base/customArchives/queryList',
...
function toSave(){
	...
	$.ajax({
	   url: serviceUrl+"/sys/base/customArchives/saveList",
...
function toupdate(){
	...
	if(editId){
		...
		$.ajax({
	        type:'get',
	        url:serviceUrl+'/sys/base/customArchives/queryIsHaveChild/'+editId+'?time='+Math.random(),
	        success: function(resultData) {
	        	if(resultData) {
	                var successFlag = resultData.success;
	                var result = resultData.result;
...
function toDelete(){
	...
	if(id) {
		pop_text_open("blue","确认删除数据吗？",function(){
			$.ajax({
				url:serviceUrl+"/sys/base/customArchives/deletePseudo/"+id,
				type:'DELETE',
				dataType:'JSON',
				success:function (resultData) {
...
function enableOrNot(st){
	...
	if(id!=null && id!=""){
		...
		if(status!=st){
			$.ajax({
		       url:serviceUrl+"/sys/base/customArchives/enableOrNot",
