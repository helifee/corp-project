...
function getSettlementTradesData(){
    jQuery("#list2").jqGrid(
            {
                url: serviceUrl+'sys/base/settlementTrades/getSettlementTradesPage',
	...
	var ids=$('#list2').jqGrid('getGridParam','selrow');
		...
		var prev=	$("#"+ids).prev().attr("id");
			  pop_text_open("blue",'确认要删除这条数据吗？',function(){
					$.ajax({
						url:serviceUrl+"sys/base/settlementTrades/deletePseudo/"+ids,
						type:'DELETE',
						dataType:'JSON',
...
function  updateStatus(e){
	...
	$.ajax({
		url:serviceUrl+"sys/base/settlementTrades/updateStatus/"+ids,
		type:'PUT',
		dataType:'JSON',
		success:function (resultData ) {
			if (resultData&&resultData.success) {
