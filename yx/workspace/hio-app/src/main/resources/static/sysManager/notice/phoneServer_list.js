    ...
    var _phoneList = {
    	...
    	loadPhonePageData:function(){
    		...
    		jQuery("#phonelist").jqGrid({
    			url: serviceUrl+'sys/sysNoticePhoneServer/vaguePage',
    	...
    	updatePhoneStatus:function(){
    		...
    		if(rowData.isDefault=="是"){
	    		...
	    		$.ajax({
	    			url:serviceUrl+"sys/sysNoticePhoneServer/setDefault/"+ids,
	    			type:'PUT',
	    			dataType:'JSON',
    	...
    	deletePhone:function(){
    		...
    		pop_text_open("blue",'确认要删除这条数据吗？',function(){
	    		$.ajax({
	    			url:serviceUrl+"sys/sysNoticePhoneServer/deletePseudoBatch/"+ids,
	    			type:'DELETE',
	    			dataType:'JSON',
	    			success:function (resultData ) {
	    				if (resultData&&resultData.success) {
