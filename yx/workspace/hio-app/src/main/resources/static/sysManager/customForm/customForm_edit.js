...
function initButton(){
	...
	//分类替换绑定方法
  	...
  	$('.resource-selecter').xljSingleSelector({
		...
		selectorType:'eeee',//选择器类型，默认是组织机构选择器:org表示组织机构选择器； person表示人员选择器；post表示岗位选择器； role表示角色选择器；menu表示菜单选择器
		treeUrl:serviceUrl + '/sys/res/resource/getResourceTreeAll'+'?time='+Math.random(),// 生成zTree树的请求url,不指定使用默认对应类型的url
...
function getDataItemSelect(appId){
	$.ajax({
        type:'POST',
        url:serviceUrl + "sys/res/dataItem/queryDataItemAndPointList",
        dataType:'json',
...
function initFormData(appId) {
    ...
    if("add"==type){
        ...
        $("#custForm").attr('action',serviceUrl+'/sys/base/customForm/save');
        $("#custForm").attr('method','POST');
    }else if("edit"==type){
    	$(".custform_title").html("自定义表单-修改");
    	$('#dataAuthControl').css('display', '');
        $.ajax({
            url:serviceUrl+'/sys/base/customForm/getForUpdate/'+id+"?time="+Math.random(),
            type:'GET',
            success:function (resultData) {
                if(resultData&&resultData.success) {
                    ...
                    for(var item in result) {
                        if($("#custForm :input[name='"+item+"']").length>0){
                        	if(item!="isInner" && item!="status" && item!="dataItemControl" && item!="isEx"){
                    ...
                    $("#custForm").attr('action',serviceUrl+'/sys/base/customForm/update/'+result['id']);
        ...
        $.ajax({
            ...
            success:function (resultData) {
                if(resultData&&resultData.success) {
                    ...
                    $("#custForm").attr('action',serviceUrl+'/sys/base/customForm/saveTemplate');
                    $("#custForm").attr('method','POST');
                }
            }
        });
