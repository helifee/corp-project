...
function getProjectTypeData(){
	...
	// jQuery("#list2").jqGrid('clearGridData');
	   jQuery("#list2").jqGrid({
		    url: serviceUrl+'sys/base/baseProjectType/getTypetree',
...
var prev=$("#"+ids).prev().attr("id");
	if(ids&&ids!='') {
		  pop_text_open("blue",'确认要删除该产品类型及该类型下面的所有类型吗？',function(){
				$.ajax({
					url:serviceUrl+"sys/base/baseProjectType/deleteBatch/"+ids,
					type:'DELETE',
					dataType:'JSON',
	...
	jQuery("#list2").jqGrid('saveRow',rowid);
	  ...
	  $('#list2').jqGrid().trigger("reloadGrid");
		return;
   			...
   			$.ajax({
   				url:serviceUrl+"sys/base/baseProjectType/updateName/"+rowid,
   				type:'PUT',
   				dataType:'JSON',
   				contentType:"application/json",	
	...
	if(e==dataStatus){
		...
		if(e==1&&rowData.isLeaf=="false"){//启用
			$.xljUtils.confirm("blue", "是否同时启用所有下级节点", function () {
				...
				$.ajax({
					url:serviceUrl+"sys/base/baseProjectType/updateStatus/"+rowid,
					type:'PUT',
					dataType:'JSON',
...
function updateSort(e){
	...
	if(ids){
		...
		$.ajax({
			url:serviceUrl+"sys/base/baseProjectType/updateSort/"+ids,
			type:'put',
			dataType:'JSON',
			contentType:"application/json",
