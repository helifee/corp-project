...
e("selectUserButtonId").addEventListener('tap',function(){
	...
	var urlText = "/mobile/approve/selectUser_en.html"+tempParamText;
					...
					if(nextList && nextList.length>0){
						for (var i=0; i<nextList.length; i++) {
							...
							if ("|BH|HF|TG|GTFQR|ZB|WYY|JS|BJS|GTFQR|XB|TY|DH|".indexOf("|"+nextList[i].operationCode+"|")>-1) {
								str = "<div class=\"mui-input-row mui-radio mui-left\"> <label>"+nextList[i].eName+"</lable>";
								var checkedText = "";
								//if(nextList[i].operationCode == opCode){
								if(i==0){
									checkedText = "checked=\"checked\" ";
									$("#userNote").val(nextList[i].eName);
								}
								str += "<input type=\"radio\" name=\"cz\""+checkedText+" opname=\""+ nextList[i].eName
									+"\"  code=\""+ nextList[i].operationCode + "\" value=\""+ nextList[i].operationCode
									+ "_"+ nextList[i].eName +"\" onclick=\"czClick(this)\"/> </div>";
								$("#clfs").append(str);
							}
						}
					}
					...
					if (resultData) {
						...
						if (successFlag) {
							...
							if ("html" == exName) {
								...
								window.location.href = encodeURI(encodeURI($.xljUtils.serverAddr + "/mobile/approve/approve_view.html?path=" + resultData.msg.replace(/\\/g, "/") + "&fileName=" + name));
							} else {
								// window.location.href=$.xljUtils.serverAddr+"/pdf.html?path="+resultData.msg.replace(/\\/g,"/");
								window.location.href = encodeURI(encodeURI($.xljUtils.serverAddr + "/pdf.html?path=" + resultData.msg.replace(/\\/g, "/") + "&fileName=" + name));
							}
						} else {
							alert('获取静态页面失败！');
						}
			...
			$.post($.xljUtils.serverAddr + "univ/attachment/attachment/getStorageIP",
				...
				function (ip) {
					if (ip) {
						...
						window.location.href = encodeURI(encodeURI($.xljUtils.serverAddr + "/mobile/approve/approve_view.html?path=" + location.protocol + '//' + ip + ":" + $.xljUtils.fdfsStoragePort + "/" + path + "&fileName=" + name));
					}
				}
			);
		} else {
