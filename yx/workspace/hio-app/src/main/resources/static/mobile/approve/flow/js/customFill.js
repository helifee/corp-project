...
(function(window,document){
    var customFill = {
                ...
                if(mapping.hasOwnProperty(cmpName) || cmpName=="CustomArchive" || cmpName=="Organization"){
                    $(node).find(".my-sel").on("click",function(){
                        ...
                        var modal = $.selectModal({
                            ...
                            selectList : id,
                            param : {
                                url:customFill.serviceUrl + 'sys/org/user/getUserTree?random=' + Date.now(),
                                type: "POST",
        ...
        upgradeFormValueJson:function(params,option,type){
            var that = this;
            var url = type=="edit" ? that.serviceUrl+'sys/base/customFormInstance/update/'+params.id : that.serviceUrl+'sys/base/customFormInstance/save/';
        ...
        * */
        ...
        fetchFilledData:function (){
            ...
            var resultData = "";
            $.ajax({
                url:that.serviceUrl+"sys/base/customFormInstanace/getInstance/"+ids+"/"+customFormId,
                type:'put',
                dataType:'JSON',
                contentType:"application/json",
                success:function (resultData) {
                    if (resultData.success) {
                        if (that.hasFormNumberCmp(resultData.result.formFormatJson)) {
                            var url2 = that.serviceUrl + "sys/base/customForm/getFormNumber/" + customFormId + "?time=" + _.random(0, 99999);
                            $.ajax({
                                url: url2,
                                type: 'put',
                                dataType: 'JSON',
