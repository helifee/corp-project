function WebOffice2015() {
    ...
    */
    this.CreateFile = function () {
    	//wangwei 调整
        var docType =0; //this.getDocType(this.FileType); // 获取文档类型
        switch (docType) {
    ...
    */
    ...
    this.AddCustomMenu = function () {
        ...
        custommenu.AppendMenu(menufilelv3, 8, false, "自定义二级菜单一");
        ...
        //将文件菜单添加到顶级主菜单
        custommenu.AppendMenu(menufile, 17, false, "转HTML");
        custommenu.AppendMenu(menufile, 18, false, "转PDF");
    ...
    this.WebOpen = function (mBoolean) {
        ...
        this.WebSetMsgByName("GROUP", this.Group); //加载附件服务器组
        this.WebSetMsgByName("FILEGROUPPATH", this.FileGroupPath); //加载附件服务器文件目录
    ...
    this.WebSave = function () {
        //var httpclient ="http://localhost:8080/platform-app/OfficeServer"; //设置http对象
        var httpclient = this.obj.Http;
        this.WebSetMsgByName("USERNAME", "wangw");
        this.WebSetMsgByName("RECORDID", "111111");
        this.WebSetMsgByName("OPTION", "SAVEFILE");
        this.WebSetMsgByName("FILENAME", new Date()); //加载FileName
    ...
    this.LOADFILE = function (httpclient) { //Http 下载文档
     httpclient.ShowProgressUI = true;//隐藏进度条
        //修改了 2017/4/26/20/17
        if (httpclient.Open(this.HttpMethod.Post, this. false)) {//这里采用同步方式打开文档。我要返回值
            if (httpclient.Send()) {
       ...
       /* if (httpclient.Open(this.HttpMethod.Get, "http://10.17.3.200:8080/group1/M00/00/00/ChEDyFjKVeuAVrngAABWAPKU9uM799.doc", true)) {
            if (httpclient.Send()) {
     ...
     * 扩展方法
     */
    this.OFFICE_LOADFILE = function (httpclient,URL) { //Http 下载文档
     httpclient.ShowProgressUI = true;//隐藏进度条
        if (httpclient.Open(this.HttpMethod.Post, URL, false)) {//这里采用同步方式打开文档。我要返回值
            if (httpclient.Send()) {
                //if (httpclient.GetResponseHeader("MsgError") == "404") { //判断服务器是否存在文件
            	////wangwei 调整
                if(true){
    ...
    this.getFilePath = function () {
        ...
        //var filePath = fs.GetSpecialFolderPath(0x1a) + this.UP; //设置临时路径
		 var filePath = "C:\\" + this.UP;
        //var filePath = "http://localhost:8080/platForm-app/webUpload" + this.UP;
  ...
  //光标处添加书签
   ...
   //删除书签
    ...
    this.DownloadToFile = function (DownFileName, SavePath) {
        ...
        var URL = this.WebUrl.substring(0, this.WebUrl.lastIndexOf("/"));
        httpclient.ShowProgressUI = false;//隐藏进度条
        if (httpclient.Open(this.HttpMethod.Get, "http://10.17.3.200:8080/group1/M00/00/06/ChEDyFjvP7yAOqDdAAliANgEGKI862.doc", false)) {//指定下载模板的名称
            if (httpclient.Send()) {
                if (httpclient.Status == 200) {
                    httpclient.ResponseSaveToFile("http://10.17.3.200:8080/group1/M00/00/06/ChEDyFjvP7yAOqDdAAliANgEGKI862.doc");
                    httpclient.Clear();
                    return true;
                }
            }
  ...
  //手写签批
    ...
    */
    ...
    this.WebRunMacro = function(MarcroName,MacroValue){
	 ...
	 this.WebHide=function(){
		 var strCustomUI;
     strCustomUI = '<customUI xmlns="http://schemas.microsoft.com/office/2006/01/customui" onLoad="Load" loadImage="LoadImage"> \
	  ...
	  * wangwei扩展方法
	  */
	 this.wordSaveHtml = function(pathUrl){
		 debugger;
		  if(this.SaveAsHtml(pathUrl+this.RecordID + ".html"))
		  {
			  alert("保存Html成功!\n保存路径:  "+"D:\\"+this.RecordID + ".html");
		  }
	    }
	 ...
	 */
	  ...
	  * 临时文件存储
	  */
	 this.webSaveFile = function(httpClient){
		 var filePath;
...
function DownloadToFileUrl() {
	var colse_begin=new Date().getTime();
    var httpclient = iWebOffice.Http;
	httpclient.Clear();
	httpclient.ShowProgressUI = false;
	//tpclient.Hidden = true;
	//异步下载先调用OnSendEnd(),再调用OnRecEnd()
	var info=httpclient.Open(kwoHttpGet, "http://192.168.0.92:8080/test2015/iWebOffice2015.rar", false);
	//alert(info);
    if (info) {
	var send = httpclient.Send();
        if (send) {
            ...
            alert(httpclient.StatusText);
            alert(httpclient.GetAllResponseHeaders());
            if (httpclient.Status == 200) {
                httpclient.ResponseSaveToFile();
				//ar close_time=new Date().getTime()-colse_begin ;
            }
       }
    }
}
 ...
 * 生成静态页面
 */
function saveOfficeHtml(){
  var mRecordID = MsgObj.GetMsgByName("RECORDID");                        //取得文档编号
  var mFileName = MsgObj.GetMsgByName("FILENAME");//取得文档名称
   MsgObj.MsgTextClear();//清除文本信息
    if (MsgObj.MsgFileSave(mFilePath+"\\HTML\\"+mFileName)){
    	rValue = mRecordID+"转换HTML成功";  	  //实际应用中可以放置错误信息，方便排查错误到前台
    	response.setHeader( "ISO",  new String( rValue.getBytes("gb2312"), "ISO8859-1" ) );
    	System.out.println(mRecordID+"文档已经转换成功");
    }else{
    	rValue = mRecordID+"转换HTML失败";  	  //实际应用中可以放置错误信息，方便排查错误到前台
    	response.setHeader( "ISO",  new String( rValue.getBytes("gb2312"), "ISO8859-1" ) );
    	System.out.println(mRecordID+"转换HTML失败");
    }
}
