...
function WebOffice2015() {
    ...
    this.Group = null;//附件服务器文件组
    this.FileGroupPath=null;//附件服务器文件目录
    //设置手写签批用户
    ...
    this.WebSave = function () {
        ...
        this.WebSetMsgByName("FILENAME", this.FileName); //加载FileName
  //      this.SaveAsMht(this.getFilePath()+this.RecordID + ".mht");//保存mht文件   暂时注释 20170706 -10：55
    ...
    this.SAVEFILE = function (httpclient, FileName) {//时发生交互OPTION值为：SAVEFILE
        ...
        httpclient.AddFile("FileData", FileName);
        httpclient.AddFile("FileData1", FileName.substring(0,FileName.lastIndexOf("."))+ ".mht"); //需要上传的文件
     ...
     */
    //预览
    this.OfficePreview = function(fileId){
    	var httpclient = this.obj.Http; //设置http对象
        httpclient.Clear();
        this.WebSetMsgByName("RECORDID",fileId);
        this.WebSetMsgByName("OPTION","PREVIEW");
        this.WebSetMsgByName("FILENAME",fileId + ".mht");//加载FileName
        this.WebSetMsgByName("ISMINVERSION",this.IEVersion());//ie是否为低版本

    	this.SaveAsMht(this.getFilePath()+fileId + ".mht");
        httpclient.AddForm("FormData", this.WebSendMessage());
        httpclient.AddFile("FileData", this.getFilePath()+fileId + ".mht");    //需要上传的文件
        this.WebClearMessage();
        httpclient.ShowProgressUI = false;           //隐藏进度条
        if (httpclient.Open(this.HttpMethod.Post, "http://"+window.location.host+"/platform-app/iwebOffice/docPreview?time="+Math.random(), false)) {//这里采用同步方式打开文档。我要返回值
            if (!httpclient.Send()) {
                if (httpclient.Status == 0) {
                    if (!httpclient.Send()) {
                        return false;
  ...
  //保存为mht
    this.SaveAsMht = function(FilePath){
        if ((this.FileType  == ".doc") || (this.FileType  == ".docx") || (this.FileType  == ".wps"))
        {
            try
            {
            	var ret = this.obj.ActiveDocument.SaveAs(FilePath,9,false,"",false,"",false,false,false,false,false,0);
