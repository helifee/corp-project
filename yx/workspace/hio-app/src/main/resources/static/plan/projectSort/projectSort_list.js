...
function initappGridObj(){
    ...
    appGridObj = jQuery("#company_datagrid").jqGrid(
        {
        	url: baseUrl+'pl/projectOption/queryList',
            ...
            colModel : [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
		                     {name : 'qiId',label : 'company_id',width : 75, align:"center",hidden : true},
		                     {name : 'id',label : 'id',width : 75, align:"center",hidden : true},
		                     {name : 'projectId',label : 'id',width : 75, align:"center",hidden : true},
		                     {name : 'companyId',label : 'id',width : 75, align:"center",hidden : true},
		                     {name : 'sort',label : '序号',width : 75, align:"center",hidden : true,editable : true},
	                         {name : 'companyName',label : '公司名称', align:"center",width : 55,editable : true},
	                         {name : 'projectName',label : '项目名称',width : 90,align:"center",editable : true},
	                         {name : 'isDisplay',label : '显示/隐藏',width : 100,align:"center",editable : true,edittype : "select",editoptions : {value : "0:显示;1:隐藏"},
...
function upOrDownApp(status){
	...
	if(ids == "" || ids == null){
		...
		var uBody = "pl/projectOption/upOrDown";
...
function updatestatus(status){
	...
	if(ids == "" || ids == null){
		...
		var uBody = "pl/projectOption/update/"+ids;
	    var uAll = serviceUrl + uBody;
		$.ajax({
			type:'PUT',
	        url:uAll,
...
function syncompanys(){
		...
		var uBody = "pl/projectOption/getProjects";
	    var uAll = serviceUrl + uBody;
		$.ajax({
			type:'PUT',
	        url:uAll,
