...
function expandedSearch1() {
    var s_Area = $('.expand-search');
    var s_btn = $('.btn-adv > i');
    if(s_Area.height() == 36) {
        s_Area.css({"height":"72px"});
...
function publishTemplate(){
	...
	if(id) {
		pop_text_open("blue","确认发布么？",function(){
			$.ajax({
			   url: serviceUrl+"/pl/plTemplate/update/"+id,
...
function enableOrNot(st){
	...
	if(id!=null && id!=""){
		...
		if(status!=st){
			$.ajax({
		       url:serviceUrl+"/pl/plTemplate/enableOrNot",
		...
		if(row.status!='发布'){
			if(id) {
				pop_text_open("blue","引入预置模板将会删除当前暂存版所有业务事项，是否继续引入？",function(){
					$.ajax({
					   url: serviceUrl+"/pl/plTemplate/includeDefaultTemplate",
...
function saveControl() {
    ...
    $.ajax({
		   url: serviceUrl+"/pl/plSysConfig/update/49f8c0c4831148bc93492b62365c08b3",
...
function  getScale(){
	  $.ajax({
        type:'POST',
        url:serviceUrl+'pl/plProjectScale/queryList',
        dataType:'json',
...
function  getCategroy(){
	  $.ajax({
        type:'POST',
        url:serviceUrl+'pl/plProjectCategory/queryList',
        dataType:'json',
