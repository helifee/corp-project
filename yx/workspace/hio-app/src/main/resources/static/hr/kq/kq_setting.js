...
function editAnnualFormula() {
    ...
    if (typeStatus == "1092100170") {
        ...
        window.location.href = "kq_setting_basic_annual_formula_set.html";
    }
}
...
$(".right-content .con-tit button").on("click", function (e) {
    ...
    if ($(this).attr('class').indexOf('byrest') > 0) {
        ...
        $("#restDiv2").css("display", "block");//节假日年度为block（显示）
        $("#typeDiv").css("display", "none");//假期类别设置为block（隐藏）
        $("#othDiv").css("display", "none");//其他设置为block（隐藏）
        $(".show1").css("display", "inline");
    } else if ($(this).attr('class').indexOf('bytype') > 0) {
        $("#durDiv").css("display", "none");//考勤期间设置设置为none（隐藏）
        $("#restDiv").css("display", "none");//节假日设置设置为block（隐藏）
        $("#restDiv2").css("display", "none");//节假日年度为block（显示）
        $("#typeDiv").css("display", "block");//假期类别设置设置为block（显示）
        $("#othDiv").css("display", "none");//其他设置为block（隐藏）
        $(".show1").css("display", "none");
    } else if ($(this).attr('class').indexOf('byoth') > 0) {
        $("#durDiv").css("display", "none");//考勤期间设置设置为block（隐藏）
        $("#restDiv").css("display", "none");//节假日设置设置为none（隐藏）
        $("#restDiv2").css("display", "none");//节假日年度为block（显示）
        $("#typeDiv").css("display", "none");//假期类别设置设置为none（隐藏）
        $("#othDiv").css("display", "block");//其他设置为block（显示）
        $(".show1").css("display", "none");
    } else {
 ...
 */
function kqPlanInit() {
    var ubody = "kq/hrKqPlan/queryListOrder";
...
function kqSetForRest1Init() {
    ...
    var ubody = "kq/hrKqBusstypeSetting/queryList2";
    var uall = serviceUrl + ubody;
    ...
    jqGridHoliday = jQuery("#listKqSetForRest1").jqGrid(
        {
            ...
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                ...
                {name: 'calculateRule', label: '计算规则', width: 90, align: "center"},
                {name: 'type', label: '假期类型', width: 90, align: "center"},
                {name: 'status', label: '状态', width: 90, align: "center"},
                {name: 'property', label: '出差属性', width: 90, align: "center"},
    ...
    $("#userDiv2 .tableStyle").css("height", ($('.mytable').height()) + 'px');
}
    ...
    if (id == null || id == "") {
        ...
        if (ids == "" || ids.length == 0) {
           ...
           // window.open("kq_setting_trip_type_add.html?type=update");
            window.location.href = "kq_setting_buss_type_add.html?type=update&holidayEditId="+holidayEditId;
        }
    } else {
        holidayEditId = id;
        //window.open("kq_setting_trip_type_add.html?type=update");
    ...
    jqGridExchange = jQuery("#listKqSetForRest2").jqGrid(
        {
            ...
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                ...
                // {name: 'publicDate', label: '公休日期', width: 90, align: "center",formatter:"date",formatoptions: {newformat:'Y-m-d'}},
                {name: 'remark', label: '说明', width: 90, align: "center"}
            ],
            rowNum: -1,//一页显示多少条 -1全部
            sortname: 'id',//初始化的时候排序的字段
...
function editExchange(id) {
    if (id == null || id == "") {
        ...
        if (ids == "" || ids.length == 0) {
            ...
            window.open("kq_setting_exchange_add.html?type=update");
        }
    } else {
        exchangeEditId = id;
        window.open("kq_setting_exchange_add.html?type=update");
    ...
    jqGridHolidayType = jQuery("#listKqSetForType").jqGrid(
        {
            ...
            colModel: [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                ...
                {name: 'type', label: '假期类型', width: 90, align: "center"},
                {name: 'status', label: '状态', width: 90, align: "center"},
                {name: 'property', label: '假期属性', width: 90, align: "center"},
                {name: 'orgIds', label: '适用公司范围', hidden: true, width: 90, align: "center"},
                {name: 'orgNames', label: '适用公司范围', width: 90, align: "center"},
                {name: 'remark', label: '说明', width: 90, align: "center"}
            ],
...
function editHolidayType(id) {
    ...
    window.location.href="kq_setting_holiday_type_add.html?type=update&holidayTypeEditId="+editId+"&systemItemFlag="+systemItemFlag;
...
function kqSetForRegulationInit() {
    ...
    regulationSet = jQuery("#regulationSet").jqGrid(
        {
            url: uall,
            // url : baseUrl+'/hr/hrDemo/queryList',//创建完成之后请求数据的url
            datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
            mtype: "post",//向后台请求数据的ajax的类型。可选post,get
            ajaxGridOptions: {contentType: 'application/json'},
            contentType: "application/json",
...
function editHolidayInfo(sign) {
    if (sign == 0) {
        ...
        //window.open("kq_setting_buss_type_add.html?type=add");
        window.location.href = "kq_setting_buss_type_add.html?type=add";
    }
}
...
function editExchangeInfo(sign) {
    if (sign == 0) {
        // exchangeAddType = 0;
        window.open("kq_setting_exchange_add.html?type=add");
    }
}
...
function editHolidayTypeInfo(sign) {
    if (sign == 0) {
       ...
       window.location.href = "kq_setting_holiday_type_add.html?type=add";
        //window.open("kq_setting_holiday_type_add.html?type=add");
    }
}
...
function saveRegulationSetForm(sign) {
    ...
    hrKqRegulationDto.delflag = false;
    // var id = $("#id").val;
    // hrKqRegulationDto["id"] = id;

    $.ajax({
        // url: serviceUrl + "kq/hrKqRegulation/saveOrUpdate",
        url: serviceUrl + "kq/hrKqRegulation/update/2c06c731a8ec494dadacfcc652e6017e",
        type: 'PUT',
        dataType: 'JSON',
        contentType: 'application/json',
...
function queryRegulation() {
    $.ajax({
        url: serviceUrl + "kq/hrKqRegulation/queryOne",
        type: "post",
        data: "{}",
        dataType: 'JSON',
        contentType: 'application/json',
...
function editStatus(sign) {
    ...
    if (ids == "" || ids.length == 0) {
        ...
        $.ajax({
            type: "put",
            url: serviceUrl + "kq/hrKqHolidaytypeSetting/updateBatch/" + ids,
...
//     $.ajax({
//         url: serviceUrl + "kq/hrKqHolidaySetting/queryListByYear",
//         type: "post",
//         // data: "{}",
