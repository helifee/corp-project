...
(function ($, window, document, undefined) {
    ...
    $(function () {
        ...
        window.queryByConditon = function(){
            ...
            $("#empEntityListForm").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#empEntityListForm").jqGrid("getGridParam", "postData");
        ...
        $("#addNewBtn").click(function(){
            var winObjEI = window.open("emp_entry.html?oper=add");
            // var isClose = 1;
            // //关闭open页面时刷新父页面列表
            // var loop = setInterval(function () {
            //     if (winObjEI.closed && isClose == 1) {
            //         isClose--;
            //         $("#empEntityListForm").jqGrid().trigger('reloadGrid');
            //     }
            // }, 1000);
        });
        /**
            ...
            if (idsVal && idsVal != "") {
                if (idsVal.length > 1) {
                    ...
                    if(rowData.status == '草稿'){
                        var winObjEI = window.open("emp_entry.html?oper=edit&id="+rowData.id);
                    }else{
                        //如果是非草稿状态只能查看
                        var winObjEI = window.open("emp_entry_read.html?oper=edit&id="+rowData.id);
                    }
                }
            } else {
