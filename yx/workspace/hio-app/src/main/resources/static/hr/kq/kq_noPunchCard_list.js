...
function addNotPunchApplyInfo(sign) {
    window.open("kq_noPunchCard_add.html?type=add");
    /*    if (sign == 0) {
     window.open("kq_noPunchCard_add.html?type=add");
     } else if (sign == 3) {//我的考勤跳转
     window.open("kq_noPunchCard_add.html?type=add&myKqFlag=true");
     }*/
}
...
function editNotPunchApplyInfo(sign) {
    ...
    if (ids == "" || ids.length == 0) {
        ...
        window.open("kq_noPunchCard_add.html?type=update");
    } else if (sign == 4) {//我的考勤跳转
        noPunchApplyId = ids[0];
        // window.open("kq_noPunchCard_add.html?type=update&myKqFlag=true");
        window.open("kq_noPunchCard_add.html?type=update");
    }
}
    ...
    jqGridKqNoPunch = jQuery("#listNoPunchCard").jqGrid(
        {
            ...
            colModel: [
                ...
                {name: 'name', label: "主题", width: 100, align: "center"},
...
function getSysApplyIdsById(ids) {
    $.ajax({
        url: serviceUrl + "kq/hrKqNotPunch/querySysApplyIds/" + ids,
        type: 'POST',
        dataType: 'JSON',
        contentType: 'application/json',
        data: "{}",
...
function updateApplyInfo() {
    ...
    $.ajax({
        url: serviceUrl + "kq/hrKqNotPunch/update/" + bussApplyId,
        type: 'PUT',
        dataType: 'JSON',
        async: false,
        contentType: 'application/json',
...
function bussApproval(sign) {
    ...
    $.ajax({
        url: serviceUrl + "kq/hrKqNotPunch/update/" + restApplyId,
        type: 'PUT',
        dataType: 'JSON',
        async: false,
        contentType: 'application/json',
...
function flowView() {
    ...
    if (idsVal && idsVal != "") {
        if (idsVal.length > 1) {
            ...
            rowData = $('#listNoPunchCard').jqGrid('getRowData', rowId);
