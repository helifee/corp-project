;(function ($, window, document, undefined) {
    ...
    function initSysInfoItemList() {
        ...
        var jqGridSysInfoItem = jQuery("#sysInfoItemList").jqGrid(
            {
                url: serviceUrl + '/sys/sysInfoItem/queryListBySetId',//创建完成之后请求数据的url
    ...
    $("#addBtn").click(function () {
        window.open("sys_info_item_edit.html?type=add&setId="+setId);
    });
    //编辑
    ...
    function toUpdate() {
        ...
        if (idsVal && idsVal != "") {
            if (idsVal.length > 1) {
                ...
                var rowId = $('#sysInfoItemList').jqGrid("getGridParam", "selrow");
                rowData = $('#sysInfoItemList').jqGrid('getRowData', rowId);
                window.open("sys_info_item_edit.html?type=update&itemId=" + rowData.id);
            }
        } else {
        ...
        if(!isDel){
            ...
            if (idsVal && idsVal != "") {
                $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                    $.ajax({
                        url: serviceUrl + "sys/sysInfoItem/deleteBatch/" + idsVal,
                        type: 'DELETE',
                        dataType: 'JSON',
                        contentType: 'application/json',
                        data: JSON.stringify({}),
