...
(function ($, window, document, undefined) {
    ...
    $(function () {
        ...
        window.queryByConditon = function () {
            ...
            var postData = $("#empChangeForm").jqGrid("getGridParam", "postData");
            $.each(postData, function (k, v) {
                delete postData[k];
            });
            queryCondition.delflag = 0;
            queryCondition.type = '1068100113';
            queryCondition.startNew=0;
            var limitNew = $('#empChangeForm').getGridParam('rowNum');
            queryCondition.limitNew=parseInt(limitNew);
            $("#empChangeForm").jqGrid('setGridParam',{datatype:'json',postData:queryCondition}).trigger('reloadGrid');//动态赋值
            var postData = $("#empChangeForm").jqGrid("getGridParam", "postData");
        ...
        $("#addNewBtn").click(function(){
            var winObjEI = window.open("emp_personChange.html?oper=add");
            // var isClose1 = 1;
            // //关闭open页面时刷新父页面列表
            // var loop = setInterval(function () {
            //     if (winObjEI.closed && isClose1 == 1) {
            //         isClose1--;
            //         $('#empChangeForm').jqGrid().trigger("reloadGrid");
            //     }
            // }, 1000);
        });
        /**
        ...
        $("#editBtn").click(function(){
            var idsVal = $('#empChangeForm').jqGrid('getGridParam', 'selarrrow');
            if (idsVal && idsVal != "") {
                if (idsVal.length > 1) {
                    $.xljUtils.tip("blue", "只能选择一行数据进行编辑！");
                    return;
                } else {
                    var rowId = $('#empChangeForm').jqGrid("getGridParam", "selrow");
                    var rowData = $('#empChangeForm').jqGrid('getRowData', rowId);
                    if(rowData.status == '草稿'){
                        var winObjEI = window.open("emp_personChange.html?oper=edit&id="+idsVal);
                        // var isClose = 1;
                        // //关闭open页面时刷新父页面列表
                        // var loop = setInterval(function () {
                        ...
                        var winObjEI = window.open("emp_personChange_read.html?oper=edit&id="+idsVal);
                    }
                }
            } else {
        ...
        window.parentReloadById = function (callBackId) {
            $('#empChangeForm').jqGrid("setGridParam", {
                gridComplete:function(){
                    if(callBackId != undefined && callBackId != ""){
                        $("#empChangeForm").setSelection(callBackId);
                    }
                }
            }).trigger("reloadGrid");
        };
        ...
        $("#exportBtn").click(function(){
            ...
            var sortname = $("#empChangeForm").jqGrid().getGridParam("sortname");
            var sortorder = $("#empChangeForm").jqGrid().getGridParam("sortorder");
            var applyOrgId = $("#applyOrgId").val();//所属公司和部门
            var applyStatus = $("#applyStatus").val();//审批状态
            var dateOfApply = $("#dateOfApply").val();//申请时间
            ...
            applyTypeParam.attr('value','1068100113');//入职类型
            ...
            exportNameParam.attr('value','调动申请');
        ...
        // $("#exportBtn").click(function(){
        //     //表格数据
        //     var rowData = $('#empChangeForm').jqGrid('getRowData');
        ...
        //     var urlBody = "emp/hrUpRankInfoTmp/exportRegularInfo";
        ...
        //                     form.attr('target','exportTarget');
        //                     form.attr('method','post');
        //                     form.attr('action',serviceUrl + "/emp/hrUpRankInfoTmp/exportRegularInfoClient");
        //                     //添加后台导出参数
        //                     var input1 = $('<input>');
    ...
    function initSysInfoSetList() {
        //创建jqGrid组件
        var jqGridSysInfoSet = jQuery("#empChangeForm").jqGrid(
            {
                ...
                onSelectRow: function () {
                    var rowId = $('#empChangeForm').jqGrid("getGridParam", "selrow");
                    rowData = $('#empChangeForm').jqGrid('getRowData', rowId);
                },
                gridComplete: function () {
                ...
                ondblClickRow:function () {
                    ...
                    var rowData = $('#empChangeForm').jqGrid('getRowData', rowId);
                    if(rowData.status == '草稿'){
                        var winObjEI = window.open("emp_personChange.html?oper=edit&id="+rowId);
                        // var isClose = 1;
                        // //关闭open页面时刷新父页面列表
                        // var loop = setInterval(function () {
                        //     if (winObjEI.closed && isClose == 1) {
                        ...
                        //$.xljUtils.tip("blue", "非草稿状态数据不能修改！");
                        var winObjEI = window.open("emp_personChange_read.html?oper=edit&id="+rowId);
                    }
                },
                rowNum: 20,
    ...
    function delOption() {
        ...
        if (idsVal && idsVal != "") {
            for(var i in idsVal){
                var rowData = $("#empChangeForm").getRowData(idsVal[i]);
                if (rowData.status != "草稿") {
                    flag = false;
                    break;
                }
                            ...
                            if (xhr) {
                                if (xhr.success) {
                                    ...
                                    if (focusId == null) {
                                        ...
                                        queryData.page =  $('#empChangeForm').getGridParam('page')-1;
                                        queryData.rowNum = $('#empChangeForm').getGridParam('rowNum');
                                    }
                                    queryData.gridComplete = function (){
                                        if(focusId != null && focusId != ""){
                                            $("#empChangeForm").setSelection(focusId);
                                        }else if (focusId == null || focusId == ""){
                                            $("#empChangeForm tr").last().find(":input[role='checkbox']").trigger("click");
                                        }
                                        focusId="";
                                    }
                                    $('#empChangeForm').jqGrid("setGridParam",queryData).trigger("reloadGrid");
                                } else {
    ...
    function checkApply(){
        ...
        if (idsVal && idsVal != "") {
            if (idsVal.length > 1) {
                ...
                rowData = $('#empChangeForm').jqGrid('getRowData', rowId);
                if (rowData.status != "草稿") {
                    toFlowView(rowData.id,FLCODE_RYDD);
                } else {
