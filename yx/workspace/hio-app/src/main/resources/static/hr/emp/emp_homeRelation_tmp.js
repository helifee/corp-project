;(function($, window, document, undefined){
    ...
    window.submitAddForm = function(){
        ...
        var hrEmpFamilyTmpDto = {};
        for(var i in formElements){
            if(formElements[i].name == 'birth'){
                if(formElements[i].value != ''){
                    hrEmpFamilyTmpDto[formElements[i].name]=formElements[i].value+' 00:00:00';
                }
                continue;
            }
            ...
            hrEmpFamilyTmpDto[formElements[i].name]=formElements[i].value;
        }
        hrEmpFamilyTmpDto.delflag=false;//有效标志位
        hrEmpFamilyTmpDto.applyId=applyId;//审批单ID
        hrEmpFamilyTmpDto.id = uuid;//主键
        $.ajax({
            type: 'POST',
            url: baseUrl+'emp/hrEmpFamilyTmp/save?time=' + Math.random(),
            data: JSON.stringify(hrEmpFamilyTmpDto),
            dataType: "json",
    ...
    window.submitEditForm = function(){
        ...
        for(var i in formElements){
            if(formElements[i].name=="birth"){
                if(formElements[i].value != '') {
                    ...
                    hrEmpFamilyTmpDto[formElements[i].name] = null;
                }
                continue;
            }
            if(formElements[i].name.endWith("_name")){
        ...
        $.ajax({
            type: 'PUT',
            url: baseUrl+'emp/hrEmpFamilyTmp/update/'+id,
            data: JSON.stringify(hrEmpFamilyTmpDto),
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (xhr) {
    ...
    function editHomeRelation(id){
        url = baseUrl+'emp/hrEmpFamilyTmp/get/'+id;
        $.ajax({
            type:'GET',
            ...
            success: function(xhr) {
                ...
                if(xhr){
                    if(xhr.success){
                        ...
                        var birth = (xhr.result.birth == null || xhr.result.birth == '')?'':changeTimeStyle(xhr.result.birth).Format("yyyy-MM-dd");
