;(function ($, window, document, undefined) {
    ...
    function initSysInfoSetList() {
        ...
        jqGridSysInfoSet = jQuery("#sysInfoSetList").jqGrid(
            {
                url: serviceUrl + '/sys/sysInfoSet/queryList',//创建完成之后请求数据的url
                datatype: "JSON",//请求数据返回的类型。可选json,xml,txt
                mtype: "post",//向后台请求数据的ajax的类型。可选post,get
                ajaxGridOptions: {contentType: 'application/json'},
                contentType: "application/json",
                postData:{'module':module},
                colNames: ['id','module', '指标集名称', '编码', '记录类型', '显示顺序', '指标集属性', '新增联动方法', '修改联动方法', '删除联动方法', '状态'],//列名
                ...
                ondblClickRow:function(rowId){
                    //双击跳转查看代码项页面
                    rowData = $('#sysInfoSetList').jqGrid('getRowData',rowId);
                    window.location.href="sys_info_item.html?setId=" + rowData.id+"&module="+$('#module').val()+"&setName="+encodeURI(rowData.name,"UTF-8");
                },
    ...
    $("#addBtn").click(function () {
        module=$('#module').val();
        if(""!=module){
            var winObjEI= window.open("sys_info_set_edit.html?type=add&module="+module);
            var isClose = 1;
            //关闭open页面时刷新父页面列表
            var loop = setInterval(function () {
                if (winObjEI.closed && isClose == 1) {
                    isClose--;
                    //这里写刷新代码
                    $('#sysInfoSetList').jqGrid('setGridParam',{datatype:'json',postData:{'module':module}}).trigger("reloadGrid");
                }
            }, 1000);
        }else{
    ...
    function toUpdate() {
        ...
        if (idsVal && idsVal != "") {
            if (idsVal.length > 1) {
                ...
                var rowId = $('#sysInfoSetList').jqGrid("getGridParam", "selrow");
                rowData = $('#sysInfoSetList').jqGrid('getRowData', rowId);
                var winObjEI=window.open("sys_info_set_edit.html?type=update&module="+module+"&setId=" + rowData.id);
                var isClose = 1;
                //关闭open页面时刷新父页面列表
                var loop = setInterval(function () {
                    if (winObjEI.closed && isClose == 1) {
    ...
    function toSysItem() {
        ...
        if (idsVal && idsVal != "") {
            if (idsVal.length > 1) {
                ...
                var rowId = $('#sysInfoSetList').jqGrid("getGridParam", "selrow");
                rowData = $('#sysInfoSetList').jqGrid('getRowData', rowId);
                window.location.href="sys_info_item.html?setId=" + rowData.id+"&setName="+encodeURI(rowData.name,"UTF-8");
            }
        } else {
        ...
        if(!isDel){
            ...
            if (idsVal && idsVal != "") {
                $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                    $.ajax({
                        url: serviceUrl + "/sys/sysInfoSet/deleteBatch/" + idsVal,
                        type: 'DELETE',
                        dataType: 'JSON',
                        contentType: 'application/json',
                        data: JSON.stringify({}),
