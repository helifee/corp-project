;(function($, window, document, undefined){
    ...
    window.submitAddForm = function(){
        ...
        empWorkHistoryDto.delflag=false;//有效标志位
        empWorkHistoryDto.personId=id;//关联人员Id
        empWorkHistoryDto.id=uuid;//主键
        $.ajax({
            type: 'POST',
            url: baseUrl+'emp/hrEmpFamily/save?time=' + Math.random(),
    ...
    window.submitEditForm = function(){
        ...
        $.ajax({
            type: 'PUT',
            url: baseUrl+'emp/hrEmpFamily/update/'+id,
            data: JSON.stringify(empWorkHistoryDto),
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (xhr) {
    ...
    function editHomeRelation(id){
        url = baseUrl+'emp/hrEmpFamily/get/'+id;
        $.ajax({
            type:'GET',
