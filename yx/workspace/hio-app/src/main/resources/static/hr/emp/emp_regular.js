;(function($, window, document, undefined){
    ...
    window.personCallBack = function(data){
        if(data.id != ''){
            ...
            $.ajax({
                type:'POST',
                url:baseUrl+'sys/sysApply/queryList',
                dataType:'json',
                contentType:'application/json',
                data:JSON.stringify({"personId":data.id,"type":"1068100111"}),
                success: function (xhr, textStatus) {
                    console.log(xhr);
                    if (xhr) {
                        if (xhr.success) {
                            if(xhr.result.length>0){
                                $.xljUtils.tip("green", "人员已经转正,请勿重复操作！");
                            }else{
                                $("#name").val(data.id);
    ...
    function getEmpInfoSetById(personId){
        ...
        $.ajax({
            ...
            success: function(data, textStatus) {
                ...
                $("#orgName").val($.hrUtils.getHRPrefixOrgNameById(data.result.orgId));
    ...
    function initSysApply() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //todo  获取单据号
                var applyCode = $.hrUtils.getApplyCodeByType('EMPZZSQ');
    ...
    window.submitAddForm = function(){
        ...
        empPersonInfoTmpDto.deptId = deptId;//部门ID
        if($("#entryTime").val() != ''){
            empPersonInfoTmpDto.entryTime = $("#entryTime").val()+' 00:00:00';//入职时间
        }
        if($("#entryOrgTime").val() != ''){
            empPersonInfoTmpDto.entryOrgTime = +' 00:00:00';//加入本公司时间
        }
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
                if(data.result.length > 0){
                    ...
                    deptId = data.result[0].deptId;//部门ID
                }
            },
            error: function (xhr, textStatus, errorThrown) {
                console.log(xhr);
