...
function addBussApplyInfo(sign) {
    window.open("kq_buss_trip_add.html?type=add");
    /*    if (sign == 0) {
            // restApplyTypeLists = restApplyTypeList;
        } else if (sign == 3) {//我的考勤跳转
            window.open("kq_buss_trip_add.html?type=add&myKqFlag=true");
        }*/
}
...
function editBussApplyInfo(sign) {
    ...
    if (ids == "" || ids.length == 0) {
        ...
        bussApplyId = ids[0];
        window.open("kq_buss_trip_add.html?type=update");
    } else if (sign == 4) {//我的考勤跳转
        bussApplyId = ids[0];
        // window.open("kq_buss_trip_add.html?type=update&myKqFlag=true");
        window.open("kq_buss_trip_add.html?type=update");
    }
}
