;(function($, window, document, undefined){
    ...
    function resizeHeight() {
        ...
        //右侧table
        $(".xj-main-grid").height($(window).height()-400 + "px");
        $(".ui-jqgrid-bdiv table").jqGrid().setGridHeight($('.xj-main-grid').height());
    ...
    $(function(){
        ...
        $("#addNewBtn").click(function(){
            var winObjEI = window.open("emp_addUprank.html?oper=add&applyId="+applicationUUID);//新增晋升晋级绑定审批单ID
            var isClose = 1;
            //关闭open页面时刷新父页面列表
            var loop = setInterval(function () {
                if (winObjEI.closed && isClose == 1) {
                    isClose--;
                    $('#empUprankApplyForm').jqGrid().trigger("reloadGrid");
                }
            }, 1000);
        });
        ...
        $("#applyBtn").click(function(){
            ...
            if(statusValue == '草稿'){
                var empUprankNum = $("#empUprankApplyForm").getGridParam("reccount");
                if(empUprankNum==0){
                    $.xljUtils.tip("red", "无晋升人员，不能发起审批！");
                }else{
                    applyFlag = true;
                    saveOrUpdate();
                }
    ...
    function saveOrUpdate(){
        if(saveFlag){
            $("#empUprankForm").attr("data-validate-success", " window.submitAddForm()");//保存
            $("#empUprankForm").submit();
        }else{
            var statusValue = $("#statusValue").val();
            if(statusValue == "草稿"){
                $("#empUprankForm").attr("data-validate-success", " window.submitEditForm()");//更新
                $("#empUprankForm").submit();
            }else{
    ...
    function getInfoByAppId(applyId){
        $.ajax({
            ...
            success: function(data, textStatus) {
                ...
                $("#empUprankForm").find("input[name='topic']").val(data.result.name);//主题
                //todo  获取单据号
                $("#empUprankForm").find("input[name='code']").val(data.result.code);//单据号
                //用户的信息
                //制单人
                $("#empUprankForm").find("input[name='creater']").val(data.result.creater);
                $("#empUprankForm").find("input[name='createrName']").val(data.result.createrName);
                createrId = data.result.personId;
                //经办人
                $("#empUprankForm").find("input[name='applicant']").val(data.result.applicant);
                $("#empUprankForm").find("input[name='applicantName']").val(data.result.applicantName);

                $("#empUprankForm").find("input[name='companyId']").val(data.result.companyId);
                $("#empUprankForm").find("input[name='companyName']").val(data.result.companyName);

                $("#empUprankForm").find("input[name='deptmentId']").val(data.result.deptId);//机构ID
                $("#empUprankForm").find("input[name='deptName']").val(data.result.deptName);
                var dept = $("#deptmentId");
                //先清空
                dept.empty();
                //机构信息
                ...
                $("#empUprankForm").find("input[name='applyDate']").val(applyDate);//申请日期
                $("#empUprankForm").find("input[name='type']").val(APPLY_TYPE_JGBZ);//机构编制申请
                ...
                $("#empUprankForm").find("input[name='status']").val(status);
                var statusValue = $.hrUtils.getHRCodeNameById(status);
                $("#empUprankForm").find("input[name='statusValue']").val(statusValue);
                $("#empUprankForm").find("input[name='approvalDate']").val(data.result.approvalDate);
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                $.xljUtils.tip("red","服务异常,请联系管理员！");
            }
    ...
    function initSysApply() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#empUprankForm").find("input[name='topic']").val(data.result.name);//主题
                //todo  获取单据号
                var applyCode = $.hrUtils.getApplyCodeByType('EMPJSSQ');
                $("#empUprankForm").find("input[name='code']").val(applyCode);
                //用户的信息
                //制单人
                $("#empUprankForm").find("input[name='creater']").val(data.result.creater);
                $("#empUprankForm").find("input[name='createrName']").val(data.result.createrName);
     ...
     * 批量删除晋升晋级人员
     */
    function deleteEmpUprankApply(){
        var idsVal = $('#empUprankApplyForm').jqGrid('getGridParam', 'selarrrow');
        if (idsVal && idsVal != "") {
            $.xljUtils.confirm("blue", "确认要删除这【" + idsVal.length + "】条数据吗？", function () {
                $.ajax({
                    url: serviceUrl + "emp/hrUpRankInfoTmp/deleteBatch/" + idsVal,
                    type: 'DELETE',
                    dataType: 'JSON',
                    contentType: 'application/json',
                    data: JSON.stringify({}),
    ...
    window.submitAddForm = function(){
        ...
        sysApplyDto.type="1068100110";//审批类型
                    ...
                    if (xhr.success) {
                        ...
                        if(applyFlag){
                            ...
                            if(applicationFlag == 1){
                                var obj = $("#empUprankApplyForm").jqGrid("getRowData");
                                if(obj.length>0){
                                    var checkFlag = true;
                                    ...
                                    if(checkFlag){
                                        toApplyByFlCode(FLCODE_JSJJ,applicationUUID);
                                    }else{
                                        $.xljUtils.tip("blue", "晋升人员需具有相同机构、部门和岗位！");
                                    }
                                }else{
                                    //todo 校验列表，防止晋升列表为空，审批无法获取业务变量
                                    $.xljUtils.tip("blue", "晋升人员列表不能为空！");
                                }
                            }else{
                                //todo 不走审批，直接同步正式表
                                $.ajax({
                                    type: 'POST',
                                    url: baseUrl+'emp/empPersonInfoTmp/applyPass/'+applicationUUID+"/jsjj",
                                    dataType: "json",
