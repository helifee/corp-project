...
function toSendMessagePage(){
	...
	if(!$('#registerForm').valid()){
		...
		if(username==null||username==undefined||username.trim()==""){
			$('#register #name').parent().find('.errmsgtip').text('您的名字不能为空！');
			return;
		}
		var phone=$('#register #phone').val();
		if(phone==null||phone==undefined||phone.trim()==""){
			$('#register #phone').parent().find('.errmsgtip').text('手机号不能为空!')
			return ;
		}
		...
		if(code==null||code==undefined||code.trim()==""){
			$('#register #uPicCode').parent().find('.errmsgtip').text('请输入验证码！');
			return;
		}
...
function  checkblur(argument) {
	...
	$('#register #uPicCode').on('blur',function() {
		...
		if(code==null||code==undefined||code.trim()==""){
			...
			$(this).parent().find('.errmsgtip').text('验证码长度不能大于6！');
		}else{
			checkPicCode(code,0);
		}
	})
