...
function pageInit(){
 $.xljUtils.initJqGrid({
  gridSelecter:"#list",
  url : serviceUrl+'flow/sysNoticeMsgUserConfig/queryList',//创建完成之后请求数据的url
...
function changeState(n){
 ...
 if(idsVal&&idsVal!="") {
  ...
  $.ajax({
   url:serviceUrl+"flow/sysNoticeMsgUserConfig/update/"+idVal,
   type:'PUT',
   dataType:'JSON',
   contentType:'application/json',
...
function del(){
 ...
 if(idsVal&&idsVal!="") {
  pop_text_open("blue", "确认要删除这【" +idsVal.length + "】条数据吗？",function(){
   $.ajax({
    url:serviceUrl+"flow/sysNoticeMsgUserConfig/deleteBatch/"+idsVal,
    type:'DELETE',
    dataType:'JSON',
    contentType:'application/json',
    data:JSON.stringify({}),
