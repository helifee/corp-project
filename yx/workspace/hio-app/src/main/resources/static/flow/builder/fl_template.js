...
define(['xljUtils', 'utils','actions'], function (xljUtils, utils,action) {
    ...
    var validateFlowChart = function () {
        ...
        if(allCells){
            ...
            return {validateFlag:hasStartCell&&!hasMoreStartCell&&!hasMoreEndCell&&hasEndCell&&cellConnectorIsCorrect,errorMsg:errorMsg};
        }else{
            errorMsg = '没有流程图';
    ...
    var getFlDataForSave = function (flBaseData) {
        ...
        if(flBaseData&&flBaseData.flData){
            ...
            if (!flData.code || $.trim(flData.code) == '') {
                ...
                window.flDataDef = new $.Deferred().resolve(flBaseData).promise();
    ...
    var initFlDatas = function () {
        ...
        if (act == 'create') {//新建
            ...
            $.ajax({
                type: "get",
                url: "http://127.0.0.1:9999/platform-app/flow/fl/getAll/" + flId + "?entryType=true&time=" + new Date().getTime(),
                success: function (data) {
    ...
    var referenceFl = function (referenceId) {
        ...
        $.ajax({
            type: "get",
            url:   "http://127.0.0.1:9999/platform-app/flow/fl/getAll/" + referenceId + "?entryType=true&time=" + new Date().getTime(),
            success: function (data) {
                var flData = data.result;
                delete flData['code'];
                delete flData['name'];
