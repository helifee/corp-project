...
var queryResDataList = function (paramData){
    ...
    $.ajax({ //发送更新的ajax请求
        type: "POST",
        url: serviceUrl+"sys/org/orgnazation/queryResListByIds?_t="+new Date().getTime(),
        dataType:"json",
...
var addStandardPostRow = function (table) {
    ...
    var trObj = $('<tr id="tr_' + index + '"></tr>');
    //类型
    var typeTd = $('<td style="width:15%;"></td>');
    var typeInputObj = $('<select class="form-control addInputWidth" id="type_' + index + '" name="type" >' +
    ...
    var scopeInputObj = $('<select class="form-control" style="width:100%;" id="participantScope_' + index + '" name="participantScope">' +
    ...
    var participantInputObj = $(
        ...
        '    <input type="text" class="form-control" id="participantName_' + index + '" name="participantName" placeholder="请选择..." readonly="true">' +
        '    <input type="hidden" id="participantId_' + index + '" name="participantId">' +
        '    <input type="hidden" id="status_' + index + '" name="status" value="">' +
    ...
    participantInputObj.find('.input-group-addon').xljMultipleSelector({
        title: '选择标准岗位',
        selectorType: 'role',
        targetId: 'participantId_' + index,
        targetName: 'participantName_' + index,
        treeParam: {type: '1'},
