...
function queryVariableList(busiObjectId){
	...
	$.ajax({ //发送更新的ajax请求
	    type : "post",  
	    url : serviceUrl+"flow/businessObjectVariable/queryBusiVariableListByTemlateId",    
	    dataType : "json",  
...
function addRow(blockIdx,itIdx){
	...
	//新增的对象默认: 角色-本公司
	newObj.sort = 100;
	newObj.participantType = "1";//1-所有角色 2-指定较色
	newObj.participantTypeName = "所有角色";
	newObj.participantIdName = "所有角色";
	newObj.participantId = "all_role";
	...
	showResultTextToLabel(blockIdx);
}
...
function redrawTBodyOfTable(blockIdx, dataObjList){
	...
	$.each(dataObjList,function(index,item){//遍历mapList的数组数据
		...
		var itIdx = item.sort;
		//1-绘制TR标签--绘制第一列的数据
		var row = $("<tr class=\"td-average\" id='"+blockIdx+"_"+itIdx+"' onclick='singleSelectTR(this)'></tr>");
		...
		var td1Middle = "";
		for(var i=1; i<typeItemArray.length; i++){
			if( type == i){
		...
		td2Middle = getDiv2HtmlOfRoleType(blockIdx, itIdx, item.participantIdName, item.participantId);
 ...
 * @param obj
 */
function singleSelectTR(obj){
	if(new RegExp('one').test(obj.id)){
		selectOneTRID = obj.id;
	}else if(new RegExp('two').test(obj.id)){
		selectTwoTRID = obj.id;
	}
	var classText = "ui-row-ltr ui-state-highlight";
...
function getTableAllTRTDData(blockIdx){
	...
	$("#tableTBody_"+blockIdx).find("tr").each(function(){
		...
		if(userType==1 || userType==2){//选择的是角色,读取第2行的角色数据
...
function typeSelectChange(obj){
	...
	div3Obj.html("");//将第3列的DIV清空
	if(1==selVal){//选定的是所有角色
		div2Obj.html( getDiv2HtmlOfRoleType(blockType, sort, "所有角色", "all_role") );
		//class="form-inline"  class="radio-style"
		div3Obj.addClass("form-inline");
		var div3ObjHtml = div3Obj.html();
		div3Obj.html( getDiv3HtmlOfRoleType(blockType, sort, 312, '',''));
	}else if(2==selVal){//选定的是指定角色
 ...
 * @param trSort
 */
function getDiv2HtmlOfRoleType(blockType, trSort, valueText, valueId){
	var valueHtml = "";
	...
	var endText = "";
	if( "all_role" != valueId){
		endText = "<span class=\"input-group-addon w28 multiple-selector\" id='participantId_"+blockType+"_"+trSort+"_31' data-selectorType='role' data-savecallback=\"multiSelectorCallback\" >" +
	    "<a href=\"javascript:void(0);\" class=\"fa fa-ellipsis-h\" aria-hidden=\"true\"></a></span></div>";
	}
		
	return "<div class=\"input-group\" style=\"width:100%;\">"+
    "<input type=\"text\" class=\"form-control\" id='participantIdName_"+blockType+"_"+trSort+"'" +
    " name='participantIdName_"+blockType+"_"+trSort+"' " +
    " placeholder='请选择...' "+valueHtml+" readonly='true'>"+
    "<input type='hidden' id='participantId_"+blockType+"_"+trSort+"'" +
    " name='participantId_"+blockType+"_"+trSort+"' "+valueIdHtml+"/>"+endText;
...
function chooseOuterData(blockType,trSort,busiType){
	...
	if("11"==busiType){
		title = "指定人员的选择"; 
		busiUrl = serviceUrl+"sys/org/user/getUserTree";
...
*/
		...
		title = "指定岗位的选择";
		busiUrl = serviceUrl+"sys/org/post/getPostTree";
		paramData.rootStatus = 1;
		paramData.rootDelFlag = 0;
		paramData.orgStatus = 1;
		paramData.orgDelFlag = 0;
...
*/
		...
		title = "标准角色的选择";
		busiUrl = serviceUrl+"sys/org/roleCatalog/getRoleTree";
...
*/
		...
		title = "指定组织的选择";
		busiUrl = serviceUrl+"sys/org/root/getTree";
		paramData.rootStatus = 1;
		paramData.rootDelFlag = 0;
		paramData.orgStatus = 1;
		paramData.orgDelFlag = 0;
 ...
 * @returns{String} 
 */ 
function getDiv3HtmlOfChooseTypeTextValue(blockType, trSort, typeValue, paramValueName, paramValue){
	var valueNameHtml = "";
	...
	if("11" == typeValue || "21" == typeValue){
        ...
        "<span class=\"input-group-addon w28 multiple-selector\" id='participantIdDiv_"+blockType+"_"+trSort+"_"+typeValue+"' "+roleText+"  data-savecallback=\"multiSelectorCallback\" >" +
        "<a href=\"javascript:void(0);\" class=\"fa fa-ellipsis-h\" aria-hidden=\"true\"></a></span></div>";
	}else{
...
function getDiv3HtmlOfChooseTypeFormValue(blockType, trSort, typeValue, selectedValue){
	...
	$.each(variableList,function(index,item){//遍历mapList的数组数据
		if(selectedValue == item.id){
			middleContent += "<option value="+item.id+" selected>"+item.name+"</option>";
		}else{
			middleContent += "<option value="+item.id+">"+item.name+"</option>";
		}
...
function queryFormValueList(busiObjectId){
	...
	$.ajax({ //发送更新的ajax请求
	    type: "POST",  
	    url:  "http://127.0.0.1:9999/platform-app/flow/businessObjectVariable/queryList",
	    dataType:"json",  
	    async: false,
...
function queryResDataList(paramData){
	...
	$.ajax({ //发送更新的ajax请求
	    type: "POST",  
	    url: serviceUrl+"sys/org/orgnazation/queryResListByIds",    
	    dataType:"json",  
	    async: false,
