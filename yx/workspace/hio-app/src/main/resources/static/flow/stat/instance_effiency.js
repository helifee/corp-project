...
$(function() {
	...
	$('.singleArray-first').xljSingleArraySelector({
	    ...
	    saveCallback: function(selectData,selectArray,ele){
	    	...
	    	/*$("#flowId").empty().append("<option value='-1'>请选择</option>");
...
function compareTypeChange(){
	...
	//console.info("compareTypeChange >>> type="+type);
	if("between"== type){
		$("#endSumSpan").show();
	}else{
		$("#endSumSpan").hide();
...
function exportData(){  
	...
	var url = serviceUrl+"flow/instanceStat/exportInstanceEfficiency"+paramText; 
...
function exportData_01(){  
	...
	$.ajax({  
        type: 'GET',  
        url: serviceUrl+"flow/instanceStat/exportInstanceEfficiency",  
...
function avgFormatter(cellvalue, options, rowObject) {
	...
	+' onclick="gotoDetail(\'AVG\', \''+statWay+'\', \''+statWayId+'\');">'+cellvalue+'</a>';        
}

//最大时长的链接
function maxFormatter(cellvalue, options, rowObject) {
	var statWay = rowObject.statWay;
	var statWayId = rowObject.statWayId;
	if(cellvalue==0){
		return "0";
	...
	return '<a id=\'' +options.rowId+ '_a\' href=\'javaScript:void(0);\' style=\'font-weight: bold;color:#3c8dbc;\''
	+' onclick="gotoDetail(\'MAX\', \''+statWay+'\', \''+statWayId+'\');">'+cellvalue+'</a>';        
}

//最小时长的链接
function minFormatter(cellvalue, options, rowObject) {
	var statWay = rowObject.statWay;
	var statWayId = rowObject.statWayId;
	if(cellvalue==0){
		return "0";
	...
	return '<a id=\'' +options.rowId+ '_a\' href=\'javaScript:void(0);\' style=\'font-weight: bold;color:#3c8dbc;\''
	+' onclick="gotoDetail(\'MIN\', \''+statWay+'\', \''+statWayId+'\');">'+cellvalue+'</a>';        
}
...
function gotoDetail(statType, statWay, statWayId){
	...
	//console.info("gotoDetail >>> statType="+statType+"; statWay="+statWay+"; statWayId="+statWayId);
	var url = encodeURI(serviceUrl+"flow/stat/instance_effiency_detail.html?statWayId="+statWayId+"&statType="+statType+"&statWay="+statWay
...
function initStatGridList(){
	...
	var urlText = serviceUrl+"/flow/instanceStat/statInstanceEfficiency";
	var colModel = [ 
        ...
        {name:'avgSum',  label:'平均时长（小时）',  width:"100", align:"center", sortable:false, formatter: avgFormatter},
...
function personalInstance(instanceId){
	...
	var url = encodeURI(serviceUrl+"flow/runtime/approve/approve.html?requestSource=DB&instanceId="+instanceId+"&time="+Math.random());
	openWin(url);
}
...
function queryAppSystemList() {
    ...
    $.ajax({ //发送更新的ajax请求
        type: "post",
        url: serviceUrl+"sys/res/appSystem/queryListForSelect",
