...
function initInstanceGridList(){
	...
	var urlText1 = serviceUrl+"flow/instance/queryListByApprover"; //;
	var colModel1 = [
					{name:'id',  hidden:true}, 
...
function initSingleGrid(itemGrid, postParam, urlText, colModel,pagerIdText){
	itemGrid.jqGrid( {//创建jqGrid组件
	    ...
	    ondblClickRow:function(	rowId){
	    	...
	    	var url = serviceUrl+"flow/runtime/approve/flow.html?instanceId=" + rowData.instanceId
...
function flowSearchAction(){
	...
	$.ajax({ //发送更新的ajax请求
	    type : "post",  
	    url : serviceUrl+"flow/fl/queryListByApprover",    
	    dataType : "json",  
	    data : JSON.stringify(paramData),//将对象序列化成JSON字符串  ,
	    contentType : 'application/json;charset=utf-8', //设置请求头信息  
	    success : function(data){
	    	flowDataList = data.result;	
	    	flowParamList = new Array();
	    	for(var idx=0; idx<flowDataList.length; idx++){
	    		var item = flowDataList[idx];
	    	...
	    	//开始去准备显示数据
	    	flowDataList = showFlowGridDataAfterPostDataProcess(flowParamList, flowDataList);
	    	flowGrid.jqGrid('clearGridData',false);//先清除旧数据
	    	for(var i=0; i<=flowDataList.length; i++){
	    		flowGrid.jqGrid('addRowData', i + 1, flowDataList[i]);
	    	}
			$.xljUtils.addGridScroll();
			$.xljUtils.gridResizeFn();
	    },  
...
function instanceReplaceAction(){
	...
	$.ajax({  
	    type: "POST",  
	    url: serviceUrl+"flow/instanceGroup/replaceInstanceGroup",  
...
function flowReplaceAction(){
	...
	$.ajax({  
	    type: "POST",  
	    url: serviceUrl+"flow/participant/replaceFlowParticipant?time=" + new Date().getTime(),  
...
$('.singleArray-first').xljSingleArraySelector({
    selectorTypeArray:['onlyPerson','post','role','commonRole'],
...
function queryAndShowUserPostList(userId, postSelectId){
    ...
    $.ajax({ //发送更新的ajax请求
        type: "post",
        url: serviceUrl+"sys/org/orgnazation/userRPOM",
        dataType: "json",
        async: true,
