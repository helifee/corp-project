...
function validateInstanceDataAuth(instanceId) {
	...
	$.ajax({
		type: 'GET',
		url: serviceUrl + 'flow/instance/validateInstanceDataAuth/'+ instanceId,
		dataType: 'JSON',
		async:false,
		success: function(data) {
			if(data.success) {
...
//	    	}
	    	...
	    	//加载业务表单
				...
				setTimeout(function(){
					...
					//加载流程实例流转图
					//$('#instanceChartIframe').attr('src',serviceUrl + 'flow/runtime/approve/flow_chart.html?instanceId='+ $.xljUtils.getUrlParams().instanceId);
				}, 1000);
...
function commonModifyInstance(type){
	var url = encodeURI("http://127.0.0.1:8081/platform-app/flow/runtime/query/modifyInstance.html?type=" + type + "&instanceId=" + getCurrentInstanceId());
	openWin(url);
}
...
function changeInstanceReaderStatus(instanceId){
	var paramData = {instanceId: instanceId};
    var fullUrl =   "http://127.0.0.1:9999/platform-app/flow/instanceOperateLog/changeToReadIntoHaveRead";
...
function gridReload() {
	$.ajax({
	    url:  "http://127.0.0.1:9999/platform-app/flow/instance/flowView",
	    data: JSON.stringify(postData),
	    type: 'POST',
	    contentType: 'application/json',
	    dataType: 'JSON',
