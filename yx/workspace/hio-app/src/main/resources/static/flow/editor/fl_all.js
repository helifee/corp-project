...
function init(variableListDef) {
	...
	if ($.getUrlParam("flStatus") == 0) {
		...
		$.ajax({
			type : 'get',
			async : false,
			url :  "http://127.0.0.1:9999/platform-app/flow/fl/getAll/" + $.getUrlParam('flId') + "?time=" + new Date().getTime(),
...
//	window.localStorage.clear();
	...
	$.ajax({
		type : "get",
		async : false,
		url :   "http://127.0.0.1:9999/platform-app/flow/fl/getAll/" + referredFlId,
		success : function(data) {
