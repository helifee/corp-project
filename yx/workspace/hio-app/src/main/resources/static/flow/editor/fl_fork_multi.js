...
function getBizObjectVal() {
	...
	$.ajax({
		type: 'GET',
		url: serviceUrl + 'flow/businessObjectVariable/queryVariableUsedInExpression?businessObjectId=' + $.getUrlParam('businessObjectId'),
...
//			forkLines[index].target = lineCache.targetId;	//值已从画布传过来
		...
		$.ajax({
			type:'get',
			url:  'http://127.0.0.1:9999/platform-app/sys/uuid/generator/getGuuids?count=' + forkLines.length,
			success: function(data) {
