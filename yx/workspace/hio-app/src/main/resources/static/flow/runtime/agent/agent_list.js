...
function getBusinessObjectPage() {
    ...
    jQuery("#jqgridList").jqGrid(
        {
            url: serviceUrl+"flow/agent/page",//获取数据的地址
            postData: {delflag: 0},
            ...
            ondblClickRow:function(rowid){
           	    /*var urlText = serviceUrl + "flow/runtime/businessObject/businessObject_edit.html?appId=" + selectedAppId + "&id=" + rowid;
                openWin(urlText);
                var urlText = serviceUrl + "flow/runtime/businessObject/businessObject_edit.html?appId=" + selectedAppId + "&id=" + lastSel_dataId;
                openWin(urlText);*/
                var urlText = serviceUrl + "flow/runtime/agent/agent_edit.html?agentId=" + rowid;
                openWin(urlText);
            },
            //加载失败事件
 ...
 */
function newItem() {
    openWin("http://127.0.0.1:8081/platform-app/flow/runtime/agent/agent_edit.html?agentId=-1");
}
...
function modifyItem() {
    ...
    var urlText =   "http://127.0.0.1:8081/platform-app/flow/runtime/agent/agent_edit.html?agentId=" + rowIds[0];
    openWin(urlText);
}
...
function deleteItem() {
	...
	pop_text_open("blue", popText, function() {
		$.ajax({
			type : "delete",
			url : serviceUrl + "flow/agent/deletePseudoBatch/" + idText,
			dataType : "json",
