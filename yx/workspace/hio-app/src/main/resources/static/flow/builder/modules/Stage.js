...
define(['sidebar', 'toolbar', 'actions', 'utils'], function (sidebar, toolbar, actions, utils) {
    ...
    var setContextMenu = function (container, graphStage) {
        ...
        function createPopupMenu(graph, menu, cell, evt) {
            if (cell != null) {
                menu.addItem('剪切', serviceUrl + 'flow/builder/images/editor/toolbar/cut.png', function () {
                    actions.getAction(graph, 'cut');
                });
                menu.addItem('复制',serviceUrl + 'flow/builder/images/editor/toolbar/copy.png', function () {
                    actions.getAction(graph, 'copy');
                });
                menu.addItem('粘贴', serviceUrl + 'flow/builder/images/editor/toolbar/paste.png', function () {
                    actions.getAction(graph, 'paste');
                });
                menu.addItem('删除', serviceUrl + 'flow/builder/images/editor/toolbar/delete.png', function () {
                ...
                if(cell.isEdge()){
                    ...
                    if(cell.nodeType=='task'){
                        ...
                        if(cell.extra&&cell.extra!=''){
                            ...
                            if(extraJson.isAddLabel){
                                tip += '环节审批人设置方式：在模板环节中设置;\n';
                            }else if(!extraJson.isAddLabel){
    ...
    var setDefaultCellStyle = function (graph) {
        ...
        style[mxConstants.STYLE_ALIGN] = mxConstants.ALIGN_CENTER;
        style[mxConstants.STYLE_IMAGE] = serviceUrl + 'flow/builder/images/editor/sidebar/condition.png';
        graph.getStylesheet().putCellStyle('fork', style);

        style = mxUtils.clone(style);
        style[mxConstants.STYLE_IMAGE] = serviceUrl + 'flow/builder/images/editor/sidebar/dispatch.png';
        graph.getStylesheet().putCellStyle('join', style);
        
        //抄送样式
        style = mxUtils.clone(style);
        style[mxConstants.STYLE_IMAGE] = serviceUrl + 'flow/builder/images/editor/sidebar/message.png';
