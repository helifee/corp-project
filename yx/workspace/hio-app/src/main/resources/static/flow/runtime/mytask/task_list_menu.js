	...
	$('#deleteFqTaskBtn').on('click',function () {
		...
		$.xljUtils.confirm('blue', tipText, function () {
			...
			if (selRowIds && selRowIds != '') {
				$.ajax({
					url: serviceUrl + "flow/sysNoticeMsg/deletePseudoBatch/" + selRowIds,
					type: 'DELETE',
					dataType: 'JSON',
    ...
    jQuery("#jqgridList").jqGrid(//创建jqGrid组件
        {
            ...
            colModel : [
				...
				{name:'appCode',   label:'来源', width:50, align:"center" , formatter: appCodeFormatter},
	...
	if(selRowIds && selRowIds.length >0){
		...
		if (selRowIds && selRowIds != '') {
			$.ajax({
				...
				url: serviceUrl + "flow/sysNoticeMsg/updateStatusOfNoticeMsgBatch?_t="+new Date().getTime() ,
				data:JSON.stringify({'id':selRowIds}),
				dataType: "json",
...
function refreshJqGridData(){
	...
	postData = {dataType: queryType, keyword:keyword,receiverId:receiverId, timeType: timeType,appCode:resourceType};
