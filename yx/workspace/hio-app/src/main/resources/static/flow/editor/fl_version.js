...
function queryFlowInfo(flCode){
	...
	$.ajax({
	       url: serviceUrl+"flow/fl/queryList",
	       type: 'post',
...
function queryFlVersionList(){
    versionGrid = jQuery("#jqgridList").jqGrid(//创建jqGrid组件
        {
            url : serviceUrl+"flow/fl/queryList",//获取数据的地址
            ...
            ondblClickRow:function(rowid){
        		...
        		var urlText = serviceUrl + "flow/builder/fl_template.html?act=update&flId="+rowid+"&businessObjectId="+selItem.businessObjectId;
                var status = selItem.status;
                console.log("--->>>status="+status);
                if(status!="草稿"){
                	urlText = serviceUrl + "flow/builder/fl_template.html?act=view&flId="+rowid+"&businessObjectId="+selItem.businessObjectId;
                }
        		openWin(urlText);
            },
...
function deleteItem() {
	...
	if(status!=0 && status !="草稿"){
		...
		$.ajax({
		       url: serviceUrl+"flow/fl/deletePseudo/"+lastSel_dataId,
		       type:'DELETE',
		       contentType:'application/json',
		       dataType:'JSON',
		       success:function (resultData) {
...
function useFlow() {
    ...
    if(!id){
        ...
        $.ajax({
            type: "get",
            url:  "http://127.0.0.1:9999/platform-app/flow/fl/getAll/" + id + "?entryType=true&time=" + new Date().getTime(),
            success: function (data) {
                if(data.success && data.result){
                    ...
                    flData.status = '1';
                    $.ajax({
                        url : serviceUrl + "flow/fl/saveAll",
