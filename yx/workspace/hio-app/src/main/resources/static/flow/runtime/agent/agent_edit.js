...
function queryAgentObject(){
	$.ajax({
	       url: serviceUrl+"flow/agent/get/"+agentId+"?t="+ new Date().getTime(),
	       type:'GET',
...
function updateFormData(agentObjDto){
	$.ajax({
	       url: serviceUrl+"flow/agent/update/"+agentObjDto.id,
...
function saveFormData(agentObjDto){
	$.ajax({
	       url: serviceUrl+"flow/agent/save",
	       data: JSON.stringify(agentObjDto),
	       type: 'POST',
	       contentType: 'application/json',
	       dataType: 'JSON',
...
function queryAndShowFlowData(agentId){//   
	var urlText = serviceUrl+"flow/agentFl/queryAgentFlowList";
	...
	for(var i=0; i<dataList.length; i++){
		...
		flowGrid.jqGrid('addRowData', i + 1, itemData);
	}
}
...
function queryAndShowPostData(agentId){//   
	var urlText = serviceUrl+"flow/agentPost/queryAgentPostList";
//	var urlText = serviceUrl+"flow/agentPost/queryList";
	var dataList = commonQueryAndReturnData(agentId, urlText);
	...
	for(var i=0; i<dataList.length; i++){
		...
		postGrid.jqGrid('addRowData', i + 1, itemData);
	}
}
...
function submitAndCloseModelWindow(){
	...
	for(var i=0; i<=chooseArray.length; i++){
		...
		if(dataItem){
			...
			if(existFlag == "NO"){
				dataGrid.jqGrid('addRowData', i + 1, dataItem);
			}
		}
	}
...
function queryUserFlows(postdata){
    $.ajax({ //发送更新的ajax请求
        type: "post",
        url: serviceUrl+"flow/fl/queryFlowBusiObjectList",
        dataType: "json",
        async: false,
        data: JSON.stringify(postdata),
        contentType: 'application/json;charset=utf-8', //设置请求头信息
        success: function (data) {
    		 ...
    		 for(var i=0; i<dataList.length; i++){
                 flowChooseGrid.jqGrid('addRowData', i + 1, dataList[i]);
    		 }
			$.xljUtils.addGridScroll();
			$.xljUtils.gridResizeFn();
        },
