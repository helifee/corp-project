	...
	$('#addRelateFlow').xljFlowRelationSelector({
		saveCallback: function(selectedData ,ele) {
			$.each(selectedData, function(index, item){
				var url = serviceUrl + 'flow/runtime/approve/flow.html?flCode='
	...
	$("#_approveHistory").jqGrid(
		{
			...
			jsonReader : {
				root : function (data) {
					...
					var approvalLists = result!=null?result.approvalLists:null;
					var postArr = [];
			...
			colModel : [
					...
					{name: 'approverName',label : 'incumbent person/责任人',width: 70,editable:false,sortable:false,cellattr:addCellAttr,formatter:approverFormatter},
					{name: 'taskStatus',label : '责任人点亮状态',hidden: true,sortable:false,editable:false},
					{name: 'approvalType',label : 'operate/操作',width: 70,editable:false,sortable:false,cellattr:addCellAttr},
					{name: 'taskComments',label : 'approval suggestion/处理意见',width: 250,editable:false,sortable:false,cellattr:addCellAttr},
					{name: 'taskEndTime',label : 'dispose time/处理时间',editable:false,sortable:false,cellattr:addCellAttr},
			...
			gridComplete : function() {                         //当表格所有数据都加载完成而且其他的处理也都完成时触发此事件，排序，翻页同样也会触发此事件
				...
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(0)").attr("title", "order/序号");
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(1)").attr("title", "link name/环节名称");
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(2)").attr("title", "post name/岗位");
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(3)").attr("title", "incumbent person/责任人");
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(4)").attr("title", "operate/操作");
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(5)").attr("title", "approval suggestion/处理意见");
				$("#_approveHistoryDiv tr.ui-jqgrid-labels th:visible:eq(6)").attr("title", "dispose time/处理时间");
			...
			afterInsertRow: function(rowId, rowData) {
				if(rowData.setApproverWhenStart == true) {
				 var selectPersonScript = $("<input type='button' class='personSelectInput' value='select'  onclick='addMultipleGird(this)' />");
			...
			loadComplete : function(xhr) {                       //当从服务器返回响应时执行
				if(xhr.success){
					...
					$(".settitle input").val($.xljUtils.htmlDecode(resultData.flowTitle));
					$(".settitle").show();
	...
	if(backData.length > 0) {
		...
		$.each(backData, function(index, item) {
			...
			var selectPersonScript = $("<label>" + approverNames + "</label><br/>"
					...
					+ "<input type='button' value='select' langua='en' onclick='addMultipleGird(this)'/>");
...
function setNextAc(acDtoList){
	var nexAc = '';
	for(var i = 1; i < acDtoList.length; i++){
		var acDto = acDtoList[i];
		if(acDto.acType != '4' && acDto.acType != '5'){
			if(acDto.acType == '3'){
				nexAc = acDto.name;
			}else{
	...
	if(validate()){
		...
		$('#_currentAttachment').xljAttachmentSubmit(function(success, result) {
			if(success == false) {
				...
				$.ajax({
					url : serviceUrl + "flow/instance/saveAllInstanceData",
					...
					success : function(data) {
						if (data) {
							...
							if (successFlag) {
								...
								//打开
								var oUrl= serviceUrl + "flow/runtime/approve/flow.html"
...
function loadBusinessForm() {
	...
	//针对旧的业务系统参数
	var agentUrl = serviceUrl + 'agent.html';
	url = url 
