...
function saveAndAdd() {
	submitForm(function(data){
		...
		$form.attr('action',serviceUrl + 'univ/search/searchProperty/save');
...
$(function(){
	...
	if(recordId == null){
        ...
        $('title').text('检索属性-新增');
        $(window).data('edit-mode','add');
        $(".search-property").attr('action',serviceUrl + 'univ/search/searchProperty/save');
	...
	// 修改属性
        ...
        $.xljUtils.xljAjax({
        	 url:serviceUrl + 'univ/search/searchProperty/get/'+recordId + '?random=' + Date.now(),
             type:'GET'
        }, function(data){
			...
			if (data.status == true || data.category == 0/*系统预置*/) {
				...
				$(".search-property").attr('action',serviceUrl + 'univ/search/searchProperty/update/'+data['id']);
