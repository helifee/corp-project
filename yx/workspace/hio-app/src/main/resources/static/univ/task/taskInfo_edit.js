...
function afterSaveAndAdd(resultJson) {
	...
	$form.attr('action',serviceUrl + 'univ/task/taskInfo/save');
...
$(function(){
	...
	if(recordId == null){
        ...
        $('title').text('任务信息-新增');
        $(window).data('edit-mode','add');
        $(".taskInfo-form").attr('action',serviceUrl + 'univ/task/taskInfo/save');
	...
	// 修改属性
        ...
        $.xljUtils.xljAjax({
        	url:serviceUrl + 'univ/task/taskInfo/get/'+recordId + '?random=' + Date.now(),
            type:'GET'
        }, function(data){
            ...
            $(".taskInfo-form").attr('action',serviceUrl + 'univ/task/taskInfo/update/'+data['id']);
