...
//	}
	...
	function submitForm(successCallback) {
		...
		var mtype = "POST";
		var urlBody = serviceUrl + 'univ/attachment/attachmentCategory';
	...
	$(function () {
		...
		if ("add" === window.opener.editMode) {
			...
			$.xljUtils.xljAjax({
				type:'GET',
		        url:serviceUrl + 'univ/attachment/attachmentCategory/get/'+window.opener.curTreeNode.id + '?random=' + Date.now()
			}, function(data) {
				var category = data;
		...
		$('.category-selecter').xljSingleSelector({
			...
			treeUrl:serviceUrl + 'univ/attachment/attachmentCategory/queryList?random=' + Date.now(),// 生成zTree树的请求url,不指定使用默认对应类型的url
