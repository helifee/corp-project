...
//登陆人
    ...
    */
    ...
    function loadAssociatedList(id) {
       $('#associatedList').empty();
        try{
            $.ajax({
                type: "post",
                url: serviceUrl+'oa/contentRowTypeAssociated/queryList?_t'+new Date().getTime(),
                contentType:"application/json",
                dataType: "json",
                            ...
                            for(var i in result){
                                ...
                                if(associated.type=='0'){
                                    ...
                                    if(associated.contentType=='NEWS'){
                                        jumpUrl = serviceUrl+'content/contentRowType/contentRowType_staticPage.html?from=associated&id='+associated.referenceId;
                                        ...
                                        jumpUrl = serviceUrl+'content/contentRowType/contentRowType_doc_view.html?from=associated&id='+associated.referenceId;
                                        $('#associatedList').append('<li><a target="_blank" href="'+jumpUrl+'" title='+associated.title+'>《'+associated.title+'》</a></li>');
                                    }
                                }
                            }
    ...
    function submitForm(saveType) {
        ...
        if (oper == 'add') {
            url = serviceUrl + 'oa/content/contentRowType/save';
            method = 'POST';
