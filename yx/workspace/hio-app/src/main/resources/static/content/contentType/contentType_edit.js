...
/*    $("#code").blur(function(){
    ...
    function getContentTypeForUpdate(contentTypeId) {
        $.ajax({
            url:serviceUrl + 'oa/content/contentType/get/'+contentTypeId + '?_t='+new Date().getTime(),
            type:'GET',
            dataType:'JSON',
...
/*function initSelector() {
    ...
    $.ajax({
        type: 'POST',
        contentType: "application/json",
        url: serviceUrl + "/sys/res/dataItem/queryList",
...
function saveContentTypeBusiObject(rowData) {
    ...
    var businessObject ;
    var url = serviceUrl + "flow/businessObject/saveObjectAndVariableData";
    var type = 'POST';
    /*if($.xljUtils.getUrlParams().act=='update'){
        url = serviceUrl+"flow/businessObject/update/"+processBusinessObject.id;
        type = 'PUT';
    }*/
    //保存业务对象
    $.ajax({
