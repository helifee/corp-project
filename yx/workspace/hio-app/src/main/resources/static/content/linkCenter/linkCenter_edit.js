...
function getLinkCenterData(id){
	   $.ajax({
	       url:serviceUrl+"oa/sys/sysLinkCenter/get/"+id+'?time='+Math.random(),
	       type:'get',
	       async:false,
	       success: function(data) {
	    	   if(data.success){
...
function getParentName(parentId){
	$.ajax({
        type:'post',
        url:serviceUrl+'oa/sys/sysLinkCenter/queryList',
        dataType:'json',
...
function saveForm(btType){
	var  url = serviceUrl+"oa/sys/sysLinkCenter/save";
	if(type=="add"){
	}else{
		url = serviceUrl+"oa/sys/sysLinkCenter/update";
	}
	var ajax_option={
