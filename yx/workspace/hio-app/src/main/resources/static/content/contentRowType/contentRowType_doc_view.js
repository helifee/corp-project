...
// 是否发起流程
  ...
  function addHitNum() {
	   ...
	   from = $.xljUtils.getUrlParam('from');
		if(from&&from=='portal'){
			$.ajax({
				...
				url: serviceUrl + "oa/content/contentRowType/hit/" + contentRowTypeId+"?time="+Math.random(),
				dataType: "json",
	...
	function bindButton() {
		...
		$("#newBtn").click(function () {
			...
			versionConfirm("blue",function () {
				...
				$.ajax({
					type: 'POST',
					url: serviceUrl + 'oa/content/contentRowType/attachment/save',
					contentType: "application/json",
					dataType:'json',
	...
	function getCurrentVersion() {
		...
		$.ajax({
			type: "get",
			url: serviceUrl + "oa/content/contentRowType/get/current/version/" + contentRowTypeId+"?time="+Math.random(),
			dataType: "json",
			async:false,
	...
	function validateDataAuth(contentRowTypeId) {
		$.ajax({
			type: "get",
			url: serviceUrl + "oa/content/contentRowType/validateDataAuth/" + contentRowTypeId+"?time="+Math.random(),
			dataType: "json",
			async:false,
			success: function (xhr) {
				if(xhr.success){
	...
	function getSameList(){
		$.ajax({
			contentType: "application/json",
			type: "post",
			url: serviceUrl + "oa/content/contentRowType/querySameList",
			dataType: "json",
	...
	function loadPage(){
		...
		versionGrid.jqGrid(
			{
				url : serviceUrl+"oa/content/contentRowType/versionList",
   ...
   window.gotoList = function () {
	   window.open(serviceUrl+"content/contentRowType/contentRowType_list.html?contentType=2&newOpenWin=true&contentTypeId="+contentTypeId+"&contentChildId="+contentChildId);
   }
//============================
	//初始化收藏夹树
	function initFavoriteTree() {
		$.ajax({
			type: "POST",
			url: serviceUrl + "oa/favorite/queryList",
			data: JSON.stringify({}),
			dataType: "json",
			contentType: 'application/json',
