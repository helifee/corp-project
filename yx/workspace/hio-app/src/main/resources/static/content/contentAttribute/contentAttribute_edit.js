$(function () {
    ...
    function initFormDataForUpdate() {
        ...
        $.ajax({
            type: "get",
            url: serviceUrl + "oa/content/contentRowAttribute/get/" + attributeId + '?time=' + Math.random(),
            dataType: "json",
    ...
    function changeFieldNameHtml(code) {
        $.ajax({
            ...
            type: 'POST',
            url: serviceUrl + 'oa/dictionary/contentDictionary/queryList',
            dataType: "json",
...
function saveFormData(saveType) {
    var subType = 'POST';
    var url = serviceUrl + "oa/content/contentRowAttribute/save";
    var act = $('body').data('act');
    switch (act){
        case 'create':
            break;
        case 'update':
            var id = $('#id').val();
            url = serviceUrl + "oa/content/contentRowAttribute/update/" + id;
            subType = 'PUT';
            break;
    }
        ...
        success: function (result) {
            if(result&&result.success){
                if (saveType == 'saveAndCreate') {
                    ...
                    window.location.href = serviceUrl + 'content/contentAttribute/contentAttribute_edit.html?act=create&contentTypeId='+$('#contentTypeId').val();
                } else {
                    //window.opener.userOnId = id;
                    if(window.opener&&$.isFunction(window.opener.reloadContentTypeAttrGrid)){
                        //window.opener.userOnId = id;
