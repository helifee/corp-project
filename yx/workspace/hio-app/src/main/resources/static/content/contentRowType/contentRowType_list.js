;(function ($, window, document, easyDialog) {
    ...
    function getUserBeanRelationInfo() {
        $.ajax({
            type: "GET",
            url: serviceUrl + "http://127.0.0.1:9000/platform-app/oa/content/contentRowType/getUserBeanRelationInfo?time=" + Math.random(),
            dataType: 'json',
    ...
    function initSearchAttr() {
        ...
        $.ajax({
            type: "POST",
            url: serviceUrl + "oa/content/contentRowAttribute/queryContentRowQueryAttributeList/" + contentTypeId + "?time=" + Math.random(),
            dataType: 'json',
            contentType: 'application/json',
...
/**
    ...
    function listTree() {
        ...
        $.ajax({
            type: "POST",
            //url: serviceUrl+"oa/content/contentChild/getContentChildTreeById/"+contentTypeId+"?time="+Math.random(),
      ...
      //查看审批
        $('#checkApprovalBtn').on('click',function () {
            ...
            if (rowId && rowId != '') {
                var rowData = $('#contentRowGrid').jqGrid('getRowData', rowId);
                if(rowData.processInstanceId&&rowData.processInstanceId!=''){
                    window.open(serviceUrl+'flow/runtime/approve/flow_view.html?flCode='+rowData.flCode+'&businessId='+rowId+'&appId=OA&userId='+rowData.createPersonId);
                }else{
    ...
    function checkDataAuthEdit(id){
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: serviceUrl + "oa/content/contentRowType//check/dataAuthEdit/"+id,
            dataType: "json",
    ...
    function updateContentRowTypeState(state, param) {
        $.ajax({
            type: "put",
            contentType: "application/json",
            url: serviceUrl + "oa/content/contentRowType/updateState/" + rowData.id + "/" + state + "/" + $.xljUtils.htmlDecode(param),
            dataType: "json",
            success: function (result) {
