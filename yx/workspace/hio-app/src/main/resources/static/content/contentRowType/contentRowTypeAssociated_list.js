;;(function ($,window,document,undefined) {
    ...
    function listTree(){
        ...
        $.ajax({
            type: "POST",
            //url: serviceUrl+"oa/content/contentChild/getContentChildTreeById/null?time="+Math.random(),
    ...
    function delAssociatedDoc() {
        ...
        if (ids.length != 0){
            $.xljUtils.confirm("blue","确认要删除这【"+ids.length+"】条数据吗？" ,function(){
                $.ajax({
                    url:serviceUrl+'oa/contentRowTypeAssociated/deleteBatch/'+ids,
                    type:'delete',
                    contentType:'application/json',
                    dataType:'JSON',
                    data:JSON.stringify(),
    ...
    function saveStaticPage() {
        ...
        // if($.trim(title)!=''&&$.trim(url)!=''){
            $.ajax({
                url:serviceUrl+'oa/contentRowTypeAssociated/save',
                type:'POST',
                contentType:'application/json',
                dataType:'JSON',
    ...
    */
    function pageInit(){
        $.xljUtils.initJqGrid(
            {
                gridSelecter:'#list',
                url : serviceUrl+'oa/contentRowTypeAssociated/queryList',//创建完成之后请求数据的url
    ...
    function ContentRowTypePageInit(){
        $("#contentRowTypeGrid").jqGrid(
            {
                url : serviceUrl+'oa/content/contentRowType/queryList',//创建完成之后请求数据的url
                datatype : "JSON",//请求数据返回的类型。可选json,xml,txt
                mtype : "post",//向后台请求数据的ajax的类型。可选post,get
                ajaxGridOptions: { contentType: 'application/json' },
