...
$(function () {
    ...
    function getFlTemplateList() {
        $.ajax({
            url : serviceUrl + "flow/fl/queryFlList",
            type:'POST',
        ...
        $('#flowTempateList').parents('.fullWidth').find('.iconMore_bd').parent('a').attr('href',serviceUrl + 'flow/editor/fl_list.html?_t='+new Date().getTime());
    ...
    function getUserList() {
        $.ajax({
            //url: serviceUrl + "sys/org/user/page",
            url: serviceUrl + "sys/org/user/queryUserListByOrgIdForPortal",
            type: 'POST',
        ...
        $('#userList').parents('.fullWidth').find('.iconMore_bd').parent('a').attr('href',serviceUrl + 'sysManager/org/organization_list.html?_t='+new Date().getTime());
...
function switchTaskTab(tabType) {
    ...
    $.ajax({ //发送更新的ajax请求
        type: "POST",
        url: "http://127.0.0.1:9999/platform-app/flow/sysNoticeMsg/querySysNoticeMsgByPage?_time="+new Date().getTime(),
        dataType: "JSON",
        ...
        success: function (data) {
            if (data.success) {
                ...
                if (tabType == 'toApprove') {
                    ...
                    $('#moreMsgListBtn').attr('href',  'http://127.0.0.1:8081/platform-app/flow/runtime/mytask/task_list.html?dataType=DB');
                    $('#toApproveDataList').empty();
                    $('#toApproveDataList').append(ulObj.html());

                } else if (tabType == 'toRead') {
                    $('#toRead_A').text('待阅(' + total + ')');
                    $('#moreMsgListBtn').attr('href',  'http://127.0.0.1:8081/platform-app/flow/runtime/mytask/task_list.html?dataType=DY');
                    $('#toReadDataList').empty();
                    $('#toReadDataList').append(ulObj.html());
                } else if (tabType == 'haveDone') {
                    $('#moreMsgListBtn').attr('href',   'http://127.0.0.1:8081/platform-app/flow/runtime/mytask/task_list.html?dataType=HAVE_DONE');
                    $('#haveDoneDataList').empty();
                    $('#haveDoneDataList').append(ulObj.html());
                } else if (tabType == 'myStart') {
                    $('#moreMsgListBtn').attr('href', 'http://127.0.0.1:8081/platform-app/flow/runtime/mytask/task_list.html?dataType=FQ');
...
function checkLogin(url){
    ...
    $.ajax({
        type: 'GET',
        url: serviceUrl + 'sys/thirdPartyAuthentication/checkLogin?_time='+new Date().getTime(),
        dataType:'JSON',
        async:false,
        success: function (resultData) {
...
function getEmailInfo() {
    $.ajax({
        type: 'GET',
        url: 'http://127.0.0.1:9999/platform-app/flow/sysNoticeMsg/getUserEmail?_time=' + new Date().getTime(),
        dataType: 'JSON',
        success: function (resultData) {
            if (resultData.success) {
                ...
                var outlook = serviceUrl + 'outlook_mail.html';
                var ulObj = $('<ul></ul>');
