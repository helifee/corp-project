                    ...
                    if(result) {
                        ...
                        for (var item in categoryMap){
                            ...
                            for(var j in componentList){
                                ...
                                var mydraggable = $('<div class="mydraggable" style="display:inline-block;padding:3px;"></div>');
                                mydraggable.append('<span style="display:inline-block;width: 50px;cursor:move">' +
                                    '<span class="glyphicon glyphicon-th" style="font-size:30px;display:block;width:100%;text-align:center;">' +
                                    '</span><span style="display:block;width:100%;text-align:center;margin-top:3px;">' + component.title +
                                    '</span>' +
                                    '</span>');
    ...
    function containmentResizableObjs(resizableObj) {
        ...
        resizableObj.resizable({
            ...
            stop:function (event,ui) {
                ...
                for (var i=startCol;i<tds.length;i++){
                	tdCnt++;
//                    targetUiWidth += $(tds[i]).width();	//会后四舍五入的误差
                    if(i==(startCol+x-1)){
                        break;
                    }
                }
                targetUiWidth = tdCnt * tdWidth1 - 2;

                var targetUiHeight = 0;
                var trs = $("#backgroundTable tr");
                var trCnt = 0;
                for (var i=startRow;i<trs.length;i++){
//                    targetUiHeight += $(trs[i]).outerHeight();	//会后四舍五入的误差
                	trCnt++;
                    if(i==(startRow+y-1)){
                        break;
                    }
                }
                targetUiHeight = trCnt * tdHeight - 2;
...
function saveComponentAttr() {
    ...
    if(displayTitle=='N'){
        $('div[id="'+componentId+'"]').find('.news_title').hide();
    }else{
        $('div[id="'+componentId+'"]').find('.news_title').show();
    }
