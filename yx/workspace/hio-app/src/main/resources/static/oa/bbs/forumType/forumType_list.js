...
function pageInit(n){
    $("#list").jqGrid({
        url: serviceUrl+'oa/bbs/forumType/getTree',
...
function addForumType(){
	...
	$.ajax({
        type:'get',
        url:serviceUrl+'oa/bbs/forumType/getCount/'+rowId+'?time='+Math.random(),
...
function toLocation(oper){
    ...
    if(rowData){
        ...
        $.ajax({
            url:serviceUrl+'/oa/sys/sysNaviMenu/move/'+oper,
            type:'PUT',
            dataType:'JSON',
...
function del(){
    ...
    if(idsVal&&idsVal!="") {
    	$.ajax({
            type:'get',
            url:serviceUrl+'oa/bbs/forumType/isExistForum/'+idsVal+'?time='+Math.random(),
            success: function(data) {
            	var count=data.result;
            	if(count>0){
            		...
            		pop_text_open("blue", "确认要删除这条数据吗？",function(){
    		            $.ajax({
    		                url:serviceUrl+"oa/bbs/forumType/deletePseudo/"+idsVal,
    		                type:'DELETE',
    		                dataType:'JSON',
    		                contentType:'application/json',
    		                data:JSON.stringify({}),
