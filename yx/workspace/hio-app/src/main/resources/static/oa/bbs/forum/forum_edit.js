...
//		targetName:'parentName',//选择的数据的Name存储input域
    ...
    if (oper == "add") {
        ...
        $('#forumForm').attr('action', serviceUrl + 'oa/bbs/forum/save');
        //初始化UUID
        $.ajax({
            type: "GET",
...
function editForum(id) {
    ...
    $.ajax({
        type: 'GET',
        url: serviceUrl + "oa/bbs/forum/get/" + id + '?time=' + Math.random(),
        //data: JSON.stringify({}),
        dataType: "json",
    ...
    $('#forumForm').attr('action', serviceUrl + 'oa/bbs/forum/update/' + id);
...
function getLoginMsg(){
	$.ajax({
        type: "post",
        url: serviceUrl+"oa/bbs/forum/queryLoginUser",
        dataType: "json",
