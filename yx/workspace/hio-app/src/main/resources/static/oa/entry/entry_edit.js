...
(function(window,document){
    var _entryEdit = {
    	...
    	save_form:function(){//保存规则编号类型
    		var my = this;
    		//映射后台保存方法
			var url=serviceUrl+"sys/quick/entry/saveEntryAndAuthor";
			$.ajax({
		       url:url,
    	...
    	loadSelect:function(e_val){
    		...
    		$.ajax({
  	          type:'POST',
  	          url:serviceUrl+'sys/quick/entry/queryList'+'?time='+Math.random(),
  	          dataType:'json',
...
//    			$('#iconUpload').xljAttachment({appId:'1',businessId:uuId,categoryId:'1',mode:'add',singleUpload:true});
    	...
    	initFrom:function(){
    		...
    		if(this.dataPar.upId){
    			$.ajax({
        	        type:'get',
        	        url:serviceUrl+'sys/quick/entry/get/'+this.dataPar.upId+'?time='+Math.random(),
        	        success: function(data) {
