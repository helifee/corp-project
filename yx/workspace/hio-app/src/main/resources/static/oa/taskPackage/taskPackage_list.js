...
function getTaskPackageData(){
    jQuery("#list2").jqGrid(
             {
                url: serviceUrl+'oa/taskPackage/getTaskPackagePage',
	...
	if(status=="已发布"){
		...
		return;
		  ...
		  pop_text_open("blue",'确认要删除这条数据吗？',function(){
				$.ajax({
					url:serviceUrl+"oa/taskPackage/deletePseudo/"+ids,
					type:'DELETE',
					dataType:'JSON',
...
/*	var delrowData = $('#list2').jqGrid('getRowData',ids);
	...
	$.ajax({
		url:serviceUrl+"oa/taskPackage/publishMessage/"+ids,
		type:'PUT',
		dataType:'JSON',
		success:function (resultData ) {
			if (resultData&&resultData.success) {
...
function updateStatus(options){
	...
	$.ajax({
		url:serviceUrl+"oa/taskPackage/updateStatus",
		type:'PUT',
	    contentType : "application/json", 
		dataType:'JSON',
...
function reminder(){
	...
	$.ajax({
		url:serviceUrl+"oa/taskPackage/reminder/"+ids,
		type:'post',
		dataType:'JSON',
		success:function (resultData ) {
			if (resultData&&resultData.success) {
...
function getOperationAuthorition() {
	...
	$.ajax({
		type: 'GET',
		url: serviceUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=rwbgl',
		dataType: 'json',
		//contentType: 'application/json',
		async: false,
