;(function ($,window,document,UE) {
    ...
    function echoQuote(from) {
        $.ajax({
            type: 'POST',
            url: serviceUrl + "oa/bbs/reply/quote/" + id + '?_t=' + new Date().getTime(),
            dataType: "json",
    ...
    function editData() {
        $.ajax({
            type: 'GET',
            url: serviceUrl + "oa/bbs/topic/get/" + id + '?_t=' + new Date().getTime(),
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (xhr, textStatus) {
                if (xhr) {
    ...
    function submitForm(saveType) {
        ...
        if(oper=='add'){
            url = serviceUrl + "oa/bbs/topic/save";
            method = 'POST';
            postData.id = $('#id').val();

        }else
        if(oper=='edit'){
            url = serviceUrl + "oa/bbs/topic/update/"+id;
            method = 'PUT';
            postData.id = id;
        }
