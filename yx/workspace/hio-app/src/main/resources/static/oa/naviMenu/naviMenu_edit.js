...
$(function () {
    function initPortalSelector() {
        ...
        $('#portalSelectorEle').xljMultipleSelector({
            ...
            treeUrl:serviceUrl + 'oa/portal/portalPage/queryList?_t='+new Date().getTime(),
...
function initPage() {
    ...
    if (oper == "add") {
      ...
      //  $('#naviMenuForm').attr('action', serviceUrl + '/oa/sys/sysNaviMenu/save');
...
function editNaviMenu(id) {
    ...
    $.ajax({
        type: 'GET',
        url: serviceUrl + "/oa/sys/sysNaviMenu/get/" + id + '?time=' + Math.random(),
        //data: JSON.stringify({}),
        dataType: "json",
 ...
 */
function submitForm() {
    var  url = serviceUrl+"oa/sys/sysNaviMenu/save";
    if(oper=="add"){
    }else{
        url = serviceUrl+"oa/sys/sysNaviMenu/update";
    }
    var ajax_option={
