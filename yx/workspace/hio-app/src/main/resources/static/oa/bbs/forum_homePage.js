...
function initHomePageData(){
	$.ajax({
		url:serviceUrl+"oa/bbs/forum/queryHomepage",
		type:'post',
		dataType:'json',
		data:JSON.stringify({}),
        contentType:'application/json',
...
function getOperationAuthorition() {
	...
	$.ajax({
		type: 'GET',
		url: serviceUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=ltsy',
		dataType: 'json',
		//contentType: 'application/json',
		async: false,
