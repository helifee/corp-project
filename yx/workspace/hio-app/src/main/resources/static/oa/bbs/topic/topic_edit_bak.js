...
//	$('#topicForm').height($(window).height() - $('.xj-form-header').height());
    ...
    if (oper == "add") {
        ...
        $('#topicForm').attr('action', serviceUrl + 'oa/bbs/topic/save');
        //初始化UUID
        $.ajax({
            type: "GET",
            url: serviceUrl+"oa/content/contentChild/getGuuid",
            dataType: "json",
...
function editForum(id,oper) {
    ...
    $.ajax({
        type: 'GET',
        url: serviceUrl + "oa/bbs/topic/get/" + id + '?time=' + Math.random(),
        //data: JSON.stringify({}),
        dataType: "json",
    ...
    $('#topicForm').attr('action', serviceUrl + 'oa/bbs/topic/update/' + id);
