...
function initJqGrid2(){
    jqGrid2 = jQuery("#list2").jqGrid(
        {
        	url: serviceUrl+"/oa/bbs/forum/page",
...
function formManage(){
	...
	if(ids && ids.length==1){
		$.ajax({
	        type:'get',
			url : serviceUrl + '/oa/bbs/forumPermission/getPermissionFormRole/'+ids+'?time='+Math.random(),
...
//	        		}
	        	    ...
	        	    $("#bbsForumPermissionForm").attr('action',serviceUrl+'/oa/bbs/forumPermission/save');
...
function formDel(){
	...
	if(ids&&ids!='') {
		$.ajax({
	        type:'get',
			url : serviceUrl + 'oa/bbs/forum/isExistTopic/'+ids+'?time='+Math.random(),
	        success: function(data) {
	        	var resultData=data.result;
	        	if(resultData){
	        		if(resultData.hasInstance){
	        			...
	        			pop_text_open("blue","确认删除【"+delCount+"】个版块吗？",function(){
							$.ajax({
								url:serviceUrl+"/oa/bbs/forum/deletePseudo/"+ids,
								type:'DELETE',
								dataType:'JSON',
