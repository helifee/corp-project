...
$(function () {
    function initShortcutMenuList() {
        $.ajax({
            type: 'POST',
            url: serviceUrl + 'oa/shortcutMenu/queryList?_t='+new Date().getTime(),
            dataType: 'json',
                    ...
                    if(result){
                        ...
                        for (var i = 0; i < result.length; i++) {
                            ...
                            if(obj.innerLink){
                                ...
                                if(!reg.test(obj.linkAddr)){
                                    obj.linkAddr = serviceUrl + obj.linkAddr;
                                }
                        ...
                        if(result.length<12){
                            ...
                            liObj.on('click',function () {
                                ...
                                window.open(serviceUrl+'oa/shortcutMenu/shortcutMenu_edit.html');
                            });
    ...
    window.removeShortcutMenu = function (id) {
        $.ajax({
            url: serviceUrl + "oa/shortcutMenu/delete/" + id,
            type: 'DELETE',
            dataType: 'JSON',
            success: function (resultData) {
                if (resultData && resultData.success) {
