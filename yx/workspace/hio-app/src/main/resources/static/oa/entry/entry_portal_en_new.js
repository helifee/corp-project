...
(function(window, document) {
    var _entryPlugins = {
        ...
        loadPlugins: function() {
            ...
            $.ajax({
                url: "http://127.0.0.1:9999/platform-app/sys/quick/entry/queryPortalList"+'?time='+Math.random(),
                type: 'POST',
                dataType: 'json',
                contentType: 'application/json',
                data: JSON.stringify(d/ataP),
        ...
        openUrl: function(aitem,my) {
            ...
            if (url) {
				if(url.indexOf("http:")>=0&&isInner!=0){
					url = url;
				}else if(url.indexOf(serviceUrl)==-1&&isInner!=0){
            		url= serviceUrl+url;
            	}
            	if(isInner!=1&&url.indexOf('#[userName]')==-1){
					...
					var uAll = "http://127.0.0.1:9999/platform-app/sys/quick/entry/getUserAndSessionId/"+resourceId;
					$.ajax({
						type:'get',
						url:uAll,
