$(function () {
    ...
    function initPortalPage() {
        $.ajax({
            type: 'GET',
            url: serviceUrl+'oa/portal/portalPage/get/'+urlParam.portalPageId+'?_time='+new Date().getTime(),
            success:function (resultData) {
                if(resultData.success) {
                    var result = resultData.result;
                    if (result.portalPageOwner!='_NA_'){
                        $('#returnDefualt').show();
                        $('#originPortalPageId').val(result.originPortalPageId);
                    }else{
                        $('#returnDefualt').hide();
                    }
                }
    ...
    $('#saveBtn').on('click', function () {
        ...
        if (portalPageComponents) {
            ...
            $.ajax({
                type: "POST",
                url: serviceUrl + "oa/portal/portalComponentPosition/saveBatchForPersonal",
                ...
                success: function (resultData) {
                    if (resultData) {
                        ...
                        if (successFlag) {
                            ...
                            if(window.opener&&$.isFunction(window.opener.reloadPage)){
                                window.opener.reloadPage(result[0].portalPageId);
                            }
                            window.location.href = serviceUrl + 'oa/portal/personal_theme_copy.html?portalPageId='+result[0].portalPageId+'&_time='+new Date().getTime();
                        } else {
    ...
    //恢复默认
    $('#returnDefualt').on('click', function () {
        var originPortalPageId = $('#originPortalPageId').val();
        if(originPortalPageId!=''){
            var portalPageId = urlParam.portalPageId;
            $.xljUtils.confirm('blue', '确定要恢复系统默认首页吗？', function () {
                if (portalPageId && portalPageId != '') {
                    $.ajax({
                        url: serviceUrl + "oa/portal/portalPage/delete/" + portalPageId,
                        type: 'DELETE',
                        dataType: 'JSON',
                        success: function (resultData) {
                            if (resultData && resultData.success) {
                                $.xljUtils.tip('blue', "门户重置成功！");
                                if(window.opener&&$.isFunction(window.opener.reloadPage)){
                                    window.opener.reloadPage(originPortalPageId);
                                }
                                window.location.href = serviceUrl + 'oa/portal/personal_theme_copy.html?portalPageId='+originPortalPageId+'&_time='+new Date().getTime();
                            }
                        }
                    });
                }
    ...
    function scaleXHelpLine() {
        ...
        for (var i = 0; i <= 64; i++) {
            ...
            scaleLineObj.css({
                ...
                left: containerOffset.left + spaceSize * i,
                height: ($('#componentContainer').height()) + 'px',
    ...
    function scaleYHelpLine() {
        ...
        for (var i = 0; i <= Math.floor(h / 25); i++) {
            ...
            scaleLineObj.css({
                position: 'absolute',
                top: containerOffset.top + spaceSize * i,
                left: containerOffset.left,
                width: ($('#componentContainer').width()) + 'px',
    ...
    function assembleComponentList(component) {
        ...
        groupNewsContainerObj.attr('data-moreUrl', component.moreUrl);
        groupNewsContainerObj.attr('id', 'pre_' + component.componentId);
    ...
    $("#componentContainer").droppable({
        ...
        drop: function (event, ui) {
            ...
            var startPosition = $($('.scale-help-line-x')[0]).offset().left;
            var endPosition = $($('.scale-help-line-x')[8]).offset().left;
            ...
            positionObj.startColNum = 0;//开始列
            positionObj.startRowNum = 0;//开始行(像素)
        ...
        dropObj.resizable({
            ...
            resize: function (event, ui) {
                ...
                if ($element[0].style.top && $element[0].style.top != '') {
                    eleOffset = {
                        top: parseInt($element[0].style.top.replace('px', '')),
                        left: parseInt($element[0].style.left.replace('px', ''))
                    };
                } else {
                    var componentParentContainerOffset = $('#componentParentContainer').offset();
                    eleOffset = {
                        top: parseInt($element.offset().top - componentParentContainerOffset.top),
                        left: parseInt($element.offset().left - componentParentContainerOffset.left - 17 + 64)
                    };
                }
                ...
                bottomLineObj.css({
                    position: 'absolute',
                    top: eleOffset.top + $helper.height(),
                    width: ($('#componentContainer').width()) + 'px',
                    'border-top': '1px dotted gray',
                    height: '1px',
                    'z-index': '333'
                ...
                leftLineObj.css({
                    ...
                    top: '0px',
                    left: eleOffset.left,
                    height: ($('#componentContainer').height() + 15) + 'px',
                    'border-left': '1px dotted gray',
                    width: '1px',
                    'z-index': '333'
                });
                ...
                rightLineObj.css({
                    position: 'absolute',
                    top: '0px',
                    left: eleOffset.left + $helper.width(),
                    height: ($('#componentContainer').height() + 15) + 'px',
                    'border-left': '1px dotted gray',
                    width: '1px',
                    'z-index': '333'
                ...
                offsetTipDivObj.css({
                    position: 'absolute',
                    top: (eleOffset.top + $size.height - 150) + 'px',
                    left: eleOffset.left + $size.width - 200,
                    height: '150px',
            ...
            stop: function (event, ui) {
                ...
                /*var $element = ui.element;//resize对象
                 ...
                 var componentParentContainerOffset = $('#componentParentContainer').offset();
                 eleOffset = {top:parseInt($element.offset().top-componentParentContainerOffset.top),left:parseInt($element.offset().left-componentParentContainerOffset.left-17+32)};
                 }

                 var containerWidth = $('#componentContainer').width();
                 var startCol = Math.round((eleOffset.left-32)/(containerWidth/32));
                 var endCol = (Math.round((eleOffset.left-32+$helper.width())/(containerWidth/32)-1));
                 ...
                 //初始化组件位置信息
                 positionObj1.componentHeight = $helper.height();//组件高度，以px计算，初始300px
                 positionObj1.componentWidth = endCol-startCol+1;//组件宽度，以列计算，初始8列
                 positionObj1.startColNum = startCol;//开始列
                 positionObj1.startRowNum = startY;//开始行(像素)
                 $('body').data('portalPageComponents',componentPositionObjs1);*/
    ...
    function definedDragObj(dragObj) {
        dragObj.draggable({
            ...
            start: function (event, ui) {
                ...
                $.ajax({
                    type: "GET",
                    url: serviceUrl + "sys/uuid/generator/getGuuid?_t=" + new Date().getTime(),
                    dataType: "json",
                    async: false,
    ...
    function assemblePortalPageComponents(positionObj) {
        ...
        if (isDel == 'N') {
            componentObj.find('.glyphicon-remove').hide();
        }

        if (displayMoreMenu == 'N') {
            $(componentObj.find('.glyphicon-th-list')[0]).hide();
        }
...
function removePortlet(btn, title) {
    ...
    $.xljUtils.confirm('blue', '确定删除组件【' + title + '】吗？', function () {
        ...
        if (positionId) {
            $.ajax({
                type: "DELETE",
                url: serviceUrl + "/oa/portal/portalComponentPosition/delete/" + positionId,
                data: JSON.stringify({portalPageId: $("body").data('portalPageId')}),
                dataType: "json",
                success: function (resultData) {
                    if (resultData && resultData.success) {
