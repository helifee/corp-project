   ...
   /* $('#btnContainer').append("<button type='button' class='btn btn-sm btn-adv' id='closeBtn'>关闭</button>");
    ...
    function HitNum() {
        try{
            $.ajax({
                type: 'PUT',
                url: serviceUrl+"oa/bbs/topic/hit/"+id,
                dataType:"json",
       ...
       /* $('#transBtn').click(function() {*/
        ...
        $('#delBtn').click(function(ele) {
                ...
                $.xljUtils.confirm("blue","确认要删除本贴吗？" ,function(){
                    $.ajax({
                        type: "delete",
                        url: serviceUrl+"oa/bbs/topic/deletePseudo/"+id,
                        dataType:"json",
        ...
        $('#favoriteModal').on('shown.bs.modal',function () {
            var resourceLink = serviceUrl+"oa/bbs/topic/topic_detail.html?id="+id;
        ...
        $('#reply-btn').click(function () {
            ...
            $('#loadFile').xljAttachmentSubmit(function (isSuccess, obj) {
                if (isSuccess) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: serviceUrl+"oa/bbs/reply/save",
                        dataType:"json",
                        contentType:'application/json',
                        data:JSON.stringify(postData),
                        success: function(xhr){
    ...
    function updateStatus(oper,param) {
        $.ajax({
            type: "put",
            contentType: "application/json",
            url: serviceUrl + "oa/bbs/topic/updateStatus/"+id,
    ...
    function screen_ban_fn(isFloorHost,oper,userId,varId,param,banDay) {
        ...
        if(oper=='ban'){
            ...
            url = serviceUrl +"oa/bbs/forumUser/updateStatus/"+userId;
        }else if(oper=='screen'){
                 postData = JSON.stringify({'id':varId,'oper':oper,'param':param,'banDay':banDay});
                 if(isFloorHost=='true'){
                     url = serviceUrl +"oa/bbs/topic/updateStatus/"+varId;
                 }else{
                     url = serviceUrl +"oa/bbs/reply/updateStatus/"+varId;
                 }
    ...
    function contentDelFn(isFloorHost,varId) {
        var url;
        if(isFloorHost=='true'){
            url = serviceUrl +"oa/bbs/topic/deletePseudo/"+varId;
        }else {
            url = serviceUrl +"oa/bbs/reply/deletePseudo/"+varId;
        }
        $.ajax({
            type: "DELETE",
            contentType: "application/json",
    ...
    var ajaxUtil = new AjaxUtil({
        url: serviceUrl + "oa/bbs/topic/detail/page",
        data:
    ...
    function echoQuote(from,referId) {
        $.ajax({
            type: 'POST',
            url: serviceUrl + "oa/bbs/reply/quote/" + referId + '?_t=' + new Date().getTime(),
            dataType: "json",
