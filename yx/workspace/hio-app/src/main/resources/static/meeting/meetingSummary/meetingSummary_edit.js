...
$(function() {
	...
	$('.meetingTitle-selecter').xljSingleSelector({
		...
		treeUrl:serviceUrl + 'oa/meeting/meeting/getAllMeetingByRecordUser?random=' + Date.now(),// 生成zTree树的请求url,不指定使用默认对应类型的url
	...
	var startDate, endDate;
	 ...
	 if(id != null && update == 'false'){	//查看页面
		...
		//回显附件
		 ...
		 $.ajax({
	  	        type:'get',
	  	        url:serviceUrl + 'oa/meeting/meeting/getMeetingInfo/'+meetingId+"?time="+Math.random(),
	  	        success: function(data) {
	  	        	var meeting=data.result;
...
function submitForm(type, flag) {
	...
	if(type == 'update') {
    	...
    	url = serviceUrl + "oa/meeting/meetingSummary/update/"+id+"?time="+Math.random();;
	}else {
		meetingSummaryDto.delflag=false;
		...
		//暂存操作
		if(type == 'tempSave'){
			url = serviceUrl + "oa/meeting/meetingSummary/temporary/save";
		}
	}
