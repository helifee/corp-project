...
function getReployMeetingUsrInfo(meetingId){
	...
	$.ajax({
		...
		url: serviceUrl+"oa/meeting/meetingPartner/getUserIdentifiyAndIsJoinAndIsSummMeetingId"+"?time="+Math.random(),
...
function getUserIdentifiyByMeetingId(meetingId){
	...
	$.ajax({
		type: "POST",
		contentType: "application/json",
		url: serviceUrl+"oa/meeting/meetingPartner/getUserIdentifiyByMeetingId"+"?time="+Math.random(),
		data:JSON.stringify(meetingPartnerDto),
		async : false,
		dataType:"JSON",
		success: function (data) {
...
function isExitMeetingSummary(meetingId){
    ...
    paramData = JSON.stringify({'meetingId':meetingId});
		$.ajax({
			type:'post',
			url:serviceUrl+'oa/meeting/meetingSummary/queryList'+"?time="+Math.random(),
			dataType:'json',
			contentType:'application/json',
			data:paramData,
			async: false,
...
function startTimeIsBigThanTotay(id){
      ...
      paramData = JSON.stringify({'meetingId':id});
		$.ajax({
			type:'post',
			url:serviceUrl+'oa/meeting/meetingReply/queryMeetingReployByNew'+"?time="+Math.random(),
			dataType:'json',
			contentType:'application/json',
			data:paramData,
			async: false,
...
function replyInfo() {
    ...
    $.ajax({
    	type: "POST",
    	contentType: "application/json",
        url:serviceUrl + 'oa/meeting/meetingReply/getUserInfoByMeetingId/'+'?time='+Math.random(),
        data:JSON.stringify(meetingReloyDto),
		dataType:"JSON",
        success: function(data) {
        	var meeting=data.result;
...
function inputMeetSummary(){
	...
	window.open(serviceUrl+"meeting/meetingSummary/meetingSummary_edit.html?meetingId="+meetingId+"&createSummary="+true+"&act="+"create");
}
...
function queryMeetSummary(){
	...
	window.open(serviceUrl+"meeting/meetingSummary/meetingSummary_view.html?id="+meetingSummaryId+"&update="+false+"&act="+"view");
}
...
function queryCollectionId(){
    ...
    paramData = JSON.stringify({'code':"MEETING"});
		$.ajax({
			type:'post',
			url:serviceUrl+'oa/favorite/queryList'+"?time="+Math.random(),
			dataType:'json',
			contentType:'application/json',
			data:paramData,
			success: function(data) {
				if(data.success){
					if(data.result){
						...
						if(favorite.length > 0){
							...
							var urlTem = serviceUrl+"sys/uuid/generator/getGuuid"+"?time="+Math.random();
							$.ajax({
							      type:'get',
							      url:urlTem,
							      success: function(data) {
	    	...
	    	$.ajax({
	    		...
	    		success: function (result) {
	    			if(result.success) {	//新增
	    				...
	    				var urlTemTem = serviceUrl+"sys/uuid/generator/getGuuid"+"?time="+Math.random();
	    				$.ajax({
	    			      type:'get',
	    			      url:urlTemTem,
	    			      success: function(data) {
