	...
	$(".deleteMeeting").click(function() {
        ...
        if (gr == "" || gr == null) {
            ...
            $.xljUtils.confirm("blue", tipText, function () {
                if (gr && gr != '') {
                    $.ajax({
                        url: serviceUrl + "oa/meeting/meeting/deleteBatch/" + gr.join(','),
                        type: 'DELETE',
                        dataType: 'JSON',
...
function deleteFavoriteMeeting(gr){
	 if (gr && gr != '') {
         $.ajax({
             url: serviceUrl + "oa/favorite/deleteBatch/" + gr.join(','),
             type: 'DELETE',
             dataType: 'JSON',
             success: function (resultData) {
                 if (resultData && resultData.success) {
...
function  initMeetingGridList(){
	jQuery('#allListRowgrid').jqGrid({
		url: serviceUrl + "oa/meeting/meeting/page"+"?time="+Math.random(),
		ajaxGridOptions: {
			contentType: 'application/json'
		},
		mtype : "POST",
...
function trackFlow(){
	...
	if(id){
		...
		if(row.status=='草稿'){
			...
			var pcUrl = "/platform-app/meeting/meeting/meeting_flow.html?businessId="+id;
