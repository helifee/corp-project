...
(function(window,document){
    var _formEdit = {
    	...
    	save_form:function(){//保存规则编号类型
    		var my = this;
    		//映射后台保存方法
			var url=serviceUrl+"oa/office/officeHouse/save";
			//请求方式
			var type = 'POST';
			...
			if(this.dataPar.upId){
				//映射后台修改方法
				url=serviceUrl+"oa/office/officeHouse/updateEntity/"+my.dataPar.upId;
				var type = 'PUT';
			}
    	    $.ajax({
    	       url:url,
    	...
    	loadSelect:function(e_val){
    		...
    		$.ajax({
  	          type:'POST',
  	          url:serviceUrl+'oa/office/officeHouse/queryList'+'?time='+Math.random(),
  	          dataType:'json',
  	          contentType:'application/json',
  	          async:false,
    	...
    	initFrom:function(){
    		...
    		if(this.dataPar.upId){
    			$.ajax({
        	        type:'get',
        	        url:serviceUrl+'oa/office/officeHouse/get/'+this.dataPar.upId+'?time='+Math.random(),
        	        success: function(data) {
...
//    			my.save_form();
		...
		modalSelEvent: function () {
			...
			$('#parentNodeName,#selectParentNode').on('click', function () {
				var urlAll = serviceUrl+'oa/office/officeHouse/queryTreeList'+'?time='+Math.random();
				var dataPost = {
					delflag: 0,
					sidx: "sortNumber",
					state: "1"
