		...
		.ready(
				function() {
					...
					if (act == 'import') { // 快速录入页面
						...
						editable = true;
						// url = serviceUrl + "oa/office/officeRecord/page";
					} else { // 库存查询页面
...
function toSave() {
	...
	if (allData.length > 0) {
		...
		$.ajax({
			url : serviceUrl + "/oa/office/officeRecord/saveBatch",
		 ...
		 * serviceUrl+"/oa/office/officeRecord/update/" + gr,
...
function getOfficeHouse() {
	...
	$.ajax({
		type : 'POST',
		url : serviceUrl + 'oa/sys/quick/entry/queryList' + '?time=' + Math.random(),
		dataType : 'json',
		contentType : 'application/json',
		async : false,
...
function addRowData() {
	...
	$.ajax({
		url : serviceUrl + "oa/office/officeRecord/save",
		type : 'post',
		dataType : 'JSON',
		contentType : "application/json",
	...
	if(!rowid) {
		...
		if(rowid&&rowid!='') {
			  pop_text_open("blue",'确认要删除这条数据吗？',function(){
					$.ajax({
						url:serviceUrl+"oa/office/officeRecord/delete/"+rowid,
						type:'DELETE',
						dataType:'JSON',
						success:function (resultData ) {
