$(function() {
	...
	$('.deleteStore').click(function() {
		if(window.confirm("您真的要删除所选记录吗？")) {
			$.ajax({
	        	   type: "delete",
	        	   url: serviceUrl + "oa/office/officeRecord/delete/" + id,
	        	   dataType:"json",
		...
		//选择父节点
		 $('.fatherNodeSelector').click(function() {
			 $(this).xljSingleSelector({
		            ...
		            treeUrl: serviceUrl + "oa/office/officeHouse/getOfficeHouseTreeById/null?t=" + (+new Date()),
	...
	$('.saveResult').click(function() {
		...
		$.ajax({
			url: serviceUrl + "oa/office/officeRecord/update/" + id,
			type: "PUT",
			dataType:"JSON",
...
function getOfficeRecordInfo(id) {
	$.ajax({
		url: serviceUrl + "oa/office/officeRecord/get/" + id,
		type: "GET",
		success: function(data) {
...
function officeManager(id, status) {
	var colModelArr, url;
	if(status == 2) {	//入库记录
		url = serviceUrl + 'oa/office/officeGrantRecord/queryList';
		colModelArr = [
