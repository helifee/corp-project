...
//			appCode:""
    	...
    	save_form:function(){
    		var my = this;
			//映射后台保存方法
			var url=serviceUrl+"finance/voucherFormSetting/save";
			//请求方式
			var type = 'POST';
			if(my.dataPar.upDataId){
    			//映射后台修改方法
				url=serviceUrl+"finance/voucherFormSetting/update/"+$('#formSettingForm').find("input[name='id']").val();
				type = 'PUT';
    		}
...
//    		this.dataPar.appCode=$.xljUtils.getUrlParam('app_code');
    	...
    	initObjFrom:function(){
    		if(this.dataPar.upDataId){
    			$.ajax({
        	        type:'get',
        	        url:serviceUrl+'finance/voucherFormSetting/get/'+this.dataPar.upDataId+'?time='+Math.random(),
