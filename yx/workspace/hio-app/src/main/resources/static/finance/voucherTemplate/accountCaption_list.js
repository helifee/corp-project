...
/*	$.xljUtils.resizeNestedGrid();*/
	...
	$('#assNames,#selectAssNames').on('click', function() {
		...
		$.ajax({
			type: "post",
			url: serviceUrl + "finance/assType/queryAssType",
			dataType: "json",
...
function saveAccountCaption() {
	...
	if (!accountCaptionDto.id) {
		...
		$.ajax({
			url : serviceUrl + "finance/accountCaption/save",
		...
		$.ajax({
			url : serviceUrl + "finance/accountCaption/update/"
...
function delAccountCaption() {
	...
	pop_text_open("blue", '确认要删除这条数据吗？', function() {
		$.ajax({
			url : serviceUrl + "finance/accountCaption/deletePseudo/" + id,
			type : 'DELETE',
			dataType : 'JSON',
			success : function(resultData) {
...
function editAccountCaption() {
	...
	$.ajax({
		type : 'get',
		url : serviceUrl + 'finance/accountCaption/get/' + id + '?time='
				+ Math.random(),
		success : function(data) {
			if (data.success) {
