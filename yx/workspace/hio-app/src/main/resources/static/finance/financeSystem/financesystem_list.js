...
function getFinanceSystemList() {
	 jQuery("#systemGrid").jqGrid(
	            {
	                url: serviceUrl+'finance/sysRegister/getSystemRegisterpage',
...
function delFinanceSystem() {
	...
	pop_text_open("blue", '确认要删除这条数据吗？', function() {
		$.ajax({
			url : serviceUrl + "finance/sysRegister/deletePseudo/" + id,
			type : 'DELETE',
			dataType : 'JSON',
...
function updateStatus(e) {
	...
	if (e == dataStatus) {
		...
		$.ajax({
			url : serviceUrl + "finance/sysRegister/updateStatus/" + id,
			type : 'PUT',
			dataType : 'JSON',
			success : function(resultData) {
				if (resultData && resultData.success) {
