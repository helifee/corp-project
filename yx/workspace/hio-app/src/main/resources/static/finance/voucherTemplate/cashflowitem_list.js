...
(function(window,document){
    var _gridList = {
    	...
    	get_dataList:function(){
    		var my = this;
    		jQuery('#tree').jqGrid({
				url:serviceUrl+"finance/cashFlowItem/queryTreeList"+'?time='+Math.random(),
    	...
    	to_updateData:function(ids){
    		...
    		$.ajax({
    	        type:'get',
    	        url:serviceUrl+'finance/cashFlowItem/get/'+ids+'?time='+Math.random(),
    	        success: function(data) {
    	...
    	to_deleteData:function(){
    		...
    		pop_text_open("blue",str,function(){
    			$.ajax({
    				url:serviceUrl+"finance/cashFlowItem/deletePseudoBatch/"+delIds,
    				type:'DELETE',
    				dataType:'JSON',
    	...
    	save_form:function(){
			var my = this;
			//映射后台保存方法
			var url=serviceUrl+"finance/cashFlowItem/save";
			//请求方式
			var type = 'POST';
			if(my.dataPar.isup){
    			//映射后台修改方法
				url=serviceUrl+"finance/cashFlowItem/update/"+$('#cashFlowModalFrom').find("input[name='id']").val();
				type = 'PUT';
    		}
			//获取form表单元素并遍历赋值
