...
function getBrachData(id){
	 $("#projectBranchId").empty();
		var data={
		  ...
		  $.ajax({
	          type:'POST',
	          url:serviceUrl+'sys/org/orgnazation/queryBrachByCompanyId',
	          dataType:'json',
...
function getCompanyGrid() {
	var accountSetId = $("#selectCompany").val();
	 jQuery("#companyCompare").jqGrid(
	            {
	                url: serviceUrl+'finance/accountSetCompany/getaccountSetCompanypage'+'?time='+Math.random(),
...
function saveCompanyCompare() {
	...
	if (!companyCompareDto.id) {
		...
		$.ajax({
			url : serviceUrl + "finance/accountSetCompany/save",
		...
		$.ajax({
			url : serviceUrl + "finance/accountSetCompany/update/"
...
function delCompanyCompare() {
	...
	pop_text_open("blue", '确认要删除这条数据吗？', function() {
		$.ajax({
			url : serviceUrl + "finance/accountSetCompany/deletePseudo/" + id,
			type : 'DELETE',
			dataType : 'JSON',
			success : function(resultData) {
...
function editCompanyCompare() {
	...
	$.ajax({
		type : 'get',
		url : serviceUrl + 'finance/accountSetCompany/get/' + id + '?time='
				+ Math.random(),
		success : function(data) {
			if (data.success) {
