...
(function(window,document){
    var _DataEdit = {
    	...
    	save_form:function(){
    		var my = this;
			//映射后台保存方法
			var url=serviceUrl+"finance/accountSet/save";
			//请求方式
			var type = 'POST';
			if(my.dataPar.upDataId){
    			//映射后台修改方法
				url=serviceUrl+"finance/accountSet/update/"+$('#accountSetForm').find("input[name='id']").val();
				type = 'PUT';
    		}
    	...
    	initObjFrom:function(){
    		if(this.dataPar.upDataId){
    			$.ajax({
        	        type:'get',
        	        url:serviceUrl+'finance/accountSet/get/'+this.dataPar.upDataId+'?time='+Math.random(),
