...
(function(window,document){
	...
	_tempList = {
		...
		saveTempAll: function () {
			var my = this;
			//映射后台保存方法
			var url = serviceUrl + "finance/voucherTemplateType/saveTempAll";
			//请求方式
			var type = 'POST';
			//获取form表单元素并遍历赋值
		...
		zTreeOnClick: function (event, treeId, treeNode) {
			...
			$.ajax({
				type: 'post',
				url: serviceUrl + 'finance/voucherTemplate/queryList' + '?time=' + Math.random(),
				dataType: 'json',
				contentType: 'application/json;charset=utf-8',
		...
		loadTempEntryData: function () {
			...
			jQuery("#tempEntryGrid").jqGrid({
				url: serviceUrl + 'finance/voucherTemplateEntry/queryList' + '?time=' + Math.random(),
...
//                    }
		...
		to_updateTempType: function (ids) {
			...
			$.ajax({
				type: 'get',
				url: serviceUrl + 'finance/voucherTemplateType/get/' + nodes[0].id + '?time=' + Math.random(),
				success: function (data) {
		...
		to_deleteTempType: function () {
			...
			pop_text_open("blue", "确认要删除该节点吗?", function () {
				$.ajax({
					url: serviceUrl + "finance/voucherTemplateType/deletePseudoBatch/" + nodes[0].id,
					type: 'DELETE',
					dataType: 'JSON',
		...
		saveTempType: function () {
			var my = this;
			//映射后台保存方法
			var url = serviceUrl + "finance/voucherTemplateType/save";
			//请求方式
			var type = 'POST';
			if (my.dataPar.isup) {
				//映射后台修改方法
				url = serviceUrl + "finance/voucherTemplateType/update/" + $('#cashTempTypeFrom').find("input[name='id']").val();
				type = 'PUT';
			}
			//获取form表单元素并遍历赋值
		...
		loadFieldData: function () {
			...
			if (!my.dataPar.isOne) {
				jQuery("#objFieldGrid").jqGrid({
					url: serviceUrl + 'finance/businessField/getTreeByObjId' + '?time=' + Math.random(),
					ajaxGridOptions: {contentType: 'application/json;charset=utf-8'},
					mtype: "POST",
					treeGrid: true,
		...
		saveCopyTempType: function (typeId) {
			var my = this;
			//映射后台保存方法
			var url = serviceUrl + "finance/voucherTemplateType/saveAllCopyTemp";
			//请求方式
			var type = 'POST';
			//获取form表单元素并遍历赋值
			var dataArr = $("#cashTempTypeFrom").serializeArray();
