$(function () {
    var serviceUrl = 'http://127.0.0.1:9999/platform-app/';
    /**
     * 隐藏搜索按钮
     */
//    $('.menu_search').css({display:"none"});
    ...
    $('#changePortalTheme').on('click',function () {
        ...
        if(currentPortalPageId){
            window.open( 'http://127.0.0.1:8081/platform-app/oa/portal/personal_theme_copy.html?portalPageId='+currentPortalPageId+'&_time='+new Date().getTime(),'_blank');
            return;
        }
    ...
    $('#feedback').on('click',function () {
        window.open(serviceUrl + "oa/feedback/feedback_edit.html?type=add");
    });
    /**
     * 初始化所有系统
     */
    function initAllProject() {
        ...
        var urlAll = '/platform-app/' + urlBody;
    ...
    function renderMenu(parentMenu, ulObj) {
        ...
        if (!children || children.length == 0) {
            ...
            aObj.attr('title', menuName);
            aObj.append("<span class='menuName-span'>"+menuName+"</span>");
        } else {
    ...
    function resizeMenuContainer() {
        ...
        var scrollContainerWidth = navContainerWidth - navbarRightWidth - 190;
    ...
    function openOaPortal() {
        ...
        if(currentPortalPageId){
            ...
            $('#xj-index-iframe').attr('src', serviceUrl + 'oa/portal/portal_view_copy.html?portalPageId='+currentPortalPageId + '&_t='+new Date().getTime());
            return;
        }

        $.ajax({
            url:serviceUrl + 'oa/portal/portalPage/getPortalWithPermision?_t='+new Date().getTime(),
            type:'GET',
            async:false,
            dataType:'JSON',
            success:function (resultData) {
    ...
    function initPortalHomeList() {
        ...
        $.ajax({
            url:serviceUrl + 'oa/portal/portalPage/getPortalListWithPermision?_t='+new Date().getTime(),
            type:'GET',
            dataType:'JSON',
            success:function (resultData) {
                if(resultData&&resultData.success){
...
function childParamCache(param){
    if(param){
        $('body').data('_cacheParam',param);
    }else{
        var cacheParam =  $('body').data('_cacheParam');
        delete $('body').data()['_cacheParam'];
        return cacheParam;
    }
