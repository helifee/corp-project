$(function () {
    ...
    $('#reLoginBtn').on('click', function () {
        var logoutUrl = serviceUrl + "sys/authentication/logout";
        $.ajax({
            type: 'POST',
            url: logoutUrl,
            dataType: 'json',
            ...
            success: function (json) {
                if (json.success) {
                    window.location.href = serviceUrl + 'login.html';
                } else {
    ...
    $('#updatePersonalInfo').on('click',function () {
        window.open(serviceUrl + "sysManager/org/updateMyInfo.html");
    });

    //修改密码
    $('#updatePersonalPwd').on('click',function () {
        window.open(serviceUrl + "sysManager/org/updateMyPwd.html");
    });
    ...
    $('#changePortalTheme').on('click',function () {
        ...
        if(currentPortalPageId){
            window.open( '/platform-app/oa/portal/personal_theme_copy.html?portalPageId='+currentPortalPageId+'&_time='+new Date().getTime(),'_blank');
            return;
        }
        ...
        /*$.ajax({
         ...
         window.open(serviceUrl + 'oa/portal/personal_theme_copy.html?portalPageId='+portalPageId+'&_time='+new Date().getTime(),'_blank');
         //window.location.href = serviceUrl + 'oa/portal/portal_view.html?portalPageId='+portalPageId;
         }else{
    ...
    function openOaPortal() {
        ...
        if(currentPortalPageId){
            ...
            $('#xj-index-iframe').attr('src',  'platform-app/oa/portal/portal_view_copy.html?portalPageId='+currentPortalPageId + '&_t='+new Date().getTime());
            return;
        }

        $.ajax({
            url:  'platform-app/oa/portal/portalPage/getPortalWithPermision?_t='+new Date().getTime(),
            type:'GET',
            async:false,
            dataType:'JSON',
            success:function (resultData) {
                if(resultData&&resultData.success){
                    ...
                    if(result) {
                        ...
                        $('#xj-index-iframe').attr('src', serviceUrl + 'oa/portal/portal_view_copy.html?portalPageId='+portalPageId+'&_t='+new Date().getTime());
                    }else{
                        $.xljUtils.tip('red','门户信息获取失败，请联系管理员！');
                    }
    ...
    function initPortalHomeList() {
        ...
        $.ajax({
            url:  'http://localhost:9999/platform-app/oa/portal/portalPage/getPortalListWithPermision?_t='+new Date().getTime(),
            type:'GET',
            dataType:'JSON',
            success:function (resultData) {
                if(resultData&&resultData.success){
    ...
    $('#msgBtn').on('click',function () {
        ...
        $.ajax({
            type: 'POST',
            contentType: "application/json",
            url: serviceUrl + "flow/sysNoticeMsg/queryFirstTypeStatData",
            data: JSON.stringify({}),
            dataType: "JSON",
            async:false,
            success: function (resultData) {
                if(resultData.success){
                    ...
                    for(var item in result){
                        switch (item){
                            case 'taskToDoSum':
                                $('#msgList').append('<li><a href="javascript:void(0);" class="clearfix" onclick="openMsgList(this)" data-msgtype="TASKTODO" ><span style="float:left;" title="Process reminder/流程提醒">Process reminder/流程提醒</span><span style="padding-left: 10px;float:right;">'+result[item]+'</span></a></li>');
                                break;
                            // case 'meetingSum':
                            ...
                            //     break;
                            // /*case 'directorsSum':
                            //     $('#msgList').append('<li><a href="javascript:void(0);"  class="clearfix" onclick="openMsgList(this)" data-msgtype="TASKTODO" >流程提醒<span style="padding-left: 10px;float:right;">'+taskToDoSum+'</span></a></li>');
                            //     break;*/
                            ///* case 'clockInSum':
                            //     $('#msgList').append('<li><a href="javascript:void(0);"  class="clearfix" onclick="openMsgList(this)" data-msgtype="CLOCKIN" >打卡提醒<span style="padding-left: 10px;float:right;">'+result[item]+'</span></a></li>');
                            //     break;*/
                            // case 'otherSum':
    ...
    window.openMsgList = function (ele) {
        ...
        window.open( 'http://127.0.0.1:8081/platform-app/flow/runtime/mytask/task_list.html?firstType='+firstType);
    };
    ...
    $('#saveFavoriteBtn').on('click',function () {
        $('#favoriteForm').attr('action',serviceUrl+'oa/favorite/save');
    ...
    $('#saveParentFavoriteBtn').on('click',function () {
        $('#parentFavoriteForm').attr('action',serviceUrl+'oa/favorite/save');
        var url = serviceUrl + "sys/uuid/generator/getGuuid" + "?time=" + Math.random();
        $.ajax({
            type: 'get',
            url: url,
    ...
    function initSaveFavorite() {
        ...
        $.ajax({
            type: "POST",
            url: serviceUrl + "oa/favorite/save",
    ...
    function initFavoriteTree() {
        $.ajax({
            type: "POST",
            url: serviceUrl + "oa/favorite/queryList?_t="+new Date().getTime(),
            data: JSON.stringify({}),
            dataType: "json",
            contentType: 'application/json',
                        ...
                        callback:{
                            ...
                            beforeRemove:function (treeId,treeNode) {
                                ...
                                $.xljUtils.confirm('blue','确定要删除收藏【'+treeNode.name+'】吗？',function () {
                                    $.ajax({
                                        url: serviceUrl + "oa/favorite/delete/" + id,
                                        type: 'DELETE',
                                        dataType: 'JSON',
                                        success: function (resultData) {
                                            if (resultData && resultData.success) {
                            ...
                            beforeRename:function (treeId,treeNode,newName) {
                                ...
                                var flag = false;
                                $.ajax({
                                    url:serviceUrl+'oa/favorite/update/'+treeNode.id,
    ...
    function popFloatWindow() {
        try{
            $.ajax({
                type:'GET',
                url: serviceUrl+'oa/sys/sysFloatWindow/get/portal'+'?_t'+new Date().getTime(),
                dataType:'json',
                contentType:'application/json',
                //async: true,
                success: function(json) {
...
function reloadPage(updatePortalId) {
    ...
    $('.project-item').find('a[procode="OA"]').click();
    $('#xj-index-iframe').attr('src', serviceUrl + 'oa/portal/portal_view_copy.html?portalPageId='+updatePortalId + '&_t='+new Date().getTime());
}