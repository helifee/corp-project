...
//                    var url = serviceUrl+'sys/org/user/getUserTree';
                    var url = serviceUrl+'sys/org/roleUser/selectUserPostTree?_t='+new Date().getTime();
                    if(this.options.treeUrl){
                        url = this.options.treeUrl;
                    }
                    this.options.treeParam.rootDelFlag = false;
...
//                    var url = serviceUrl+'sys/org/roleUser/selectUserPostTree?_t='+new Date().getTime();
                ...
                case 'menu':
                    ...
                    break;
                case 'busiObject':
                    var url = serviceUrl+'flow/businessObject/getTreeBySystemApp';
                    if(this.options.treeUrl){
                        url = this.options.treeUrl;
                    }
                    ...
                    break;
                case 'busiVariable':
                    var url = serviceUrl+'flow/businessObjectVariable/getVariableTreeByBusiObject';
                    if(this.options.treeUrl){
                        url = this.options.treeUrl;
                    }
        ...
        _initTree:function (treeObj,url,param,modalContainer) {
            ...
            $.ajax({
                ...
                success: function(json){
                    ...
                    if(that.options.selectorType=='onlyPerson'){
                    	treeData = {
                    			simpleData: {
                    				enable: true
                    			}
        ...
        xljSingleArrayDraw:function (data,ele) {
            ...
            if(selectArray&&selectArray!=null){
                for(var i in selectArray){
                    ...
                    if(selectArray[i]=='person'||selectArray[i]=='onlyPerson'){
                        option +="<option value='person'>人员</option>";
                    }
            ...
            switch (selectArray[0]){
                case 'org':
                    ...
                    break;
                case 'person':
//                    var url = serviceUrl+'sys/org/user/getUserTree'+"?time="+Math.random();
                    var url = serviceUrl+'sys/org/roleUser/selectUserPostTree?_t='+new Date().getTime();
                    if(that.options.treeUrl){
                        url = this.options.treeUrl;
                    }
