...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			if(data.result  && ts.p.rowNum>0) {
                //二次开发改进
                var d = new Object();
                //总页数
                d['total'] = Math.ceil(data.result.total / data.result.limit)*1;
                //当前页
                d['page'] = Math.ceil((data.result.start*1+1) / data.result.limit)*1;
//                if(d['page']==0){
//                    d['page']=1;
//                }
                //查询出的记录数
                //d['records'] = data.result.list.length*1;
              //  d['records'] = data.result.list?data.result.list.length*1:0;
                d['records'] = data.result.list?data.result.total:0;
                //包含实际数据的数组
                d['rows'] = data.result.list;
                data=d;
            }
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				var data=$.isFunction(ts.p.serializeGridData)? ts.p.serializeGridData.call(ts,ts.p.postData) : ts.p.postData;
				
				//二次开发
                var start=(data.page-1)*data.rows;
                var limit=data.rows*1;
                data['start']=start;
                data['limit']=limit*1;
			...
			if(shrink===true) {
				...
				if (hs) {
					if(nwidth-gw-(initwidth+brd*vc) !== scw){
				 		cr = nwidth-gw-(initwidth+brd*vc)-scw-3;
					}
				} else {
					cr = nwidth-gw-(initwidth+brd*vc)-3;//减去scw  18像素  miying-add
				}
		...
		if (editable==="1") {
			...
			if (o.url === 'clientArray') {
				...
				$.ajax($.extend({
					...
					//modify by lvjie 20180301  修改为get方式请求，解决用品及档案入库调整库存的bug
					type: 'GET',		
					//type: o.mtype,
					async : false, //?!?
...
$.jgrid.extend({
	...
	setTreeGrid : function() {
		return this.each(function (){
			...
			$t.p.gridview =  true;
			if($t.p.rowTotal === null ) { $t.p.rowNum = -1; }
	...
	expandRow: function (record){
		...
		//重置滚动条
		$.xljUtils.gridResizeFn();
		if($(".roleScroll").length>0) {
			$(".roleScroll").getNiceScroll().show().resize();
		}
