 ...
 * ========================================================= */
 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		//添加自定义方法，获取假期json数据
		_getJson: function() {
			var postData = { year:$("#year").val(), dayType:"2"};
			console.info(">>>> getJson() is called.... postData=");
			console.info(postData);
			$.ajax({
				type: "post",
				url: serviceUrl+"flow/calendarBasic/queryList",
				dataType: "json",
				...
				success: function (retData) {
					if(retData.success){
						...
						//步骤1: 设置左边的工作日期: 周一 周二 ....周日是否被选中
						var valueArray = data.workDay.split(",");
						$.each(valueArray,function(index,value){
						    $("input[name='workDay'][value='"+value+"']")[0].checked = true;
						});
						...
						//步骤3: 遍历节假日的数据,把相应的日期设置为选中的红色状态
						holidayList = [];//清空holidayList的数据
						var dayArray = data.detailList;
						$.each(dayArray,function(index,itemData){
						    var year = itemData.year;
							var month = itemData.month;
							var dayText = itemData.day;
							var dayType = itemData.dayType;
							var object = {year:year, month:month, day:dayText, dayType:dayType};
							holidayList.push(object);
						});
					}
				...
				while(currentDate <= lastDate)
				{
					...
					do
					{
						...
						if(currentDate < firstDate) {
							...
							//添加自定义代码						
							var now_year = this.options.startYear;
							holidayList.map(function(datum){
								if (datum.year == now_year &&
									datum.month == m+1 &&
									datum.day == currentDate.getDate() &&
									datum.dayType == 2) {
									cellContent.addClass("holidayColor");
								}
							});
		...
		_applyEvents: function () {
			...
			this.element.find('.day-content').click(function(e) {
