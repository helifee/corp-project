...
if("undefined" == typeof serviceUrl){
	serviceUrl = 'http://127.0.0.1:9999/platform-app/';
}
var sessionSign = 0;
(function ($) {
    if (!$.xljUtils) {
    ...
    $.extend($.xljUtils, {
        ...
        btnPowerOperation: function (appCode,btnMenuCode) {
            ...
            $.ajax({
                type: 'GET',
                url: serviceUrl + 'sys/authentication/getUserAuthenticationOperation?_t='+new Date().getTime()+'&appCode='+appCode+'&menuCode=' + btnMenuCode,
                dataType: 'json',
                async: false,
        ...
        addMaskLayer:function(text){
            ...
            var maskBox = '<div class="loading"><div class="load-box"> <div  class="anzhuangimg">' +
...
//                return this.optional(element) || (!(englishWord.test(value)) && !(englishWordSymbol.test(value)));
    ...
    $(function () {
        ...
        window.open = function (url,windowName,windowFeatures,replace) {
            ...
            if(replace){
                oldOpen(url,windowName,windowFeatures,replace);
                return;
            }
            if(windowFeatures){
                oldOpen(url,windowName,windowFeatures);
                return;
            }
            if(windowName){
                oldOpen(url,windowName);
                return;
            }
            oldOpen(url);
        };
        ...
        function initSysLogo() {
            var logoSrcOld = $('.xj-index-logo img').attr('src');
            $('.xj-index-logo img').attr('src',"");
            $.ajax({
                url:serviceUrl+'sys/sysLogo/getLogo?_s='+sessionSign+'&time='+Math.random(),
                type:'GET',
                async: true,
                dataType:'JSON',
                success:function (resultData) {
