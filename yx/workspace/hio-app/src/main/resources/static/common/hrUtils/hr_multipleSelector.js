            ...
            modalContainer.on('show.bs.modal', function () {
                ...
                switch (that.options.selectorType) {
                    ...
                    case 'person':
                        ...
                        //url = serviceUrl+'sys/org/user/getUserTree';
                        // url = serviceUrl + 'sys/org/roleUser/selectUserPostTree';
                        url = serviceUrl + 'org/orgRoot/getUserTree';
                        if (that.options.treeUrl) {
                            url = that.options.treeUrl;
                        }
                        ...
                        if(that.options.treeParam.userStatus==undefined){
                            that.options.treeParam.userStatus = '1143100260';
                        }
                        ...
                        //初始化右侧grid
                        //jqGrid的列显示名
                        var colNames = ['人员ID全路径', '人员ID', '姓名', '所属机构ID全路径', '所属机构', '岗位ID', '岗位', '性别', '性别'];//,'用户类型'
                        var colModel = [ //jqGrid每一列的配置信息。包括名字，索引，宽度,对齐方式.....
                    ...
                    case 'post':
                        ...
                        // url = serviceUrl+'sys/org/post/getPostTree';
                        url = serviceUrl + 'org/post/getPostTreeMul';
                        if (that.options.treeUrl) {
                            url = that.options.treeUrl;
                        }
                        that.options.treeParam.rootDelFlag = false;
                    ...
                    case 'role':
                        ...
                        url = serviceUrl + 'sys/org/roleCatalog/getRoleTree';
                        if (that.options.treeUrl) {
                            url = that.options.treeUrl;
                        }
                        that.options.treeParam.delflag = false;
...
//                	this.options.treeParam = JSON.stringify(this.options.treeParam)=='{}'?null:this.options.treeParam
        ...
        _createGridHeader: function () {
            var orgGridHeader = $('<div class="modal_title">' +
                ...
                '   <span>组织列表</span>' +
                '   <input type="text" class="form-control" placeholder="名称/全路径/序号">' +
        ...
        xljMultipleSelectorReset:function (options) {
            ...
            this.each(function(index,ele){
                ...
                if(multilSelector){
                    ...
                    $(ele).xljMultipleSelector(options);
                }
