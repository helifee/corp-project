...
//构建模板列表
function queryFlByGroupList(){
    //创建jqGrid组件
    _flListGrid = $("#_flList").jqGrid(
        {
            url : serviceUrl + "flow/fl/queryFlByGroupList",//获取数据的地址
...
function newFl(){
	...
	//openWin(url);
	window.open(serviceUrl+'flow/editor/fl_new.html?appId='+v_appId+'&businessObjectId='+v_businessObjectId+'&businessObjectName='+encodeURIComponent(v_businessObjectName));
}
/**
 * 查看流程模板
 */
...
function deleteFl(){
	...
	pop_text_open("blue", popText, function() {
		$.ajax({
			type : "delete",
			url : serviceUrl + "flow/fl/deletePseudoBatch/" + idText,
			dataType : "json",
			contentType : 'application/json;charset=utf-8', //设置请求头信息
			success : function(data) {
...
function resizeHeight(){
  $(".slide-left .ztree-box").height(($(window).height()-149)+"px");
}
//ztree 自适应宽度
