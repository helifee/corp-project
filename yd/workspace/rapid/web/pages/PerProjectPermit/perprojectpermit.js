/**
 * [PerProjectPermit] author by $YourName$
...
Ext.namespace('com.ysys.www.perprojectpermit');

/**
 * 查询表单
 * @class com.ysys.www.perprojectpermit.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.perprojectpermit.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		...
		com.ysys.www.perprojectpermit.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.perprojectpermit.querywin
 * @extends Ext.Window
 */
com.ysys.www.perprojectpermit.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.perprojectpermit.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.perprojectpermit.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.perprojectpermit.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
	    ...
	    com.ysys.www.perprojectpermit.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.perprojectpermit.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.perprojectpermit.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.perprojectpermit.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.perprojectpermit
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.perprojectpermitGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
    	this.ds = new Ext.data.Store({
	        url:'../PerProjectPermit/extlist.do',
	        reader:new Ext.data.JsonReader({
	            root:'list',
	            totalProperty:'totalSize',
	            id:'id'
		        }
		        ,['proId','permitId','operator','updatetime',]
	        ),
	        baseParams:{
	            limit:this.pageSize
	        },
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addPerProjectPermit,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editPerProjectPermit,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deletePerProjectPermit,scope:this},'-'
		...
		//调用父类构建函数
        com.ysys.www.perprojectpermitGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.perprojectpermit.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.perprojectpermit.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	            handler:this.savePerProjectPermit,
	            scope:this
	        },{
	            text:'取消',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.perprojectpermit.queryformpanel();
	    this.querywin =  new com.ysys.www.perprojectpermit.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
	    ...
	    function rgtEdit() {
				        ...
				        this.dtlwin.setTitle('修改PerProjectPermit');
				        this.dtlwin.show();
				        this.dtlformpanel.form.reset();
				        this.dtlformpanel.form.loadRecord(record);
				        this.dtlformpanel.url = '../PerProjectPermit/extupdate.do';
	    };
	    //右键删除
		function rgtDelete() {
			messageContextMenu.hide();
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../PerProjectPermit/extdelete.do?ids='+record.data.proId,
						method:'POST',
    ...
    //新建窗口
    ,addPerProjectPermit : function(){
        this.dtlwin.setTitle('新建PerProjectPermit');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../PerProjectPermit/extsave.do';
	}
	
	//编辑操作
    ,editPerProjectPermit:function(){
    ...
    //删除操作
    ,deletePerProjectPermit:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../PerProjectPermit/extdelete.do?ids='+this.getRecordArrayUtils(records, 'proId'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
 ...
 * 注册主表格的xtype
 */
Ext.reg('perprojectpermit', com.ysys.www.perprojectpermitGrid);
