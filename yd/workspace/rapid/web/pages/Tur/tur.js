...
Ext.namespace('com.ysys.www.tur');

/**
 * 查询表单
 * @class com.ysys.www.tur.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.tur.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        ...
	        items:[{
	        	...
	        	,{xtype:'textfield',fieldLabel:'role',name:'s_role',width:288}
	        	,{xtype:'textfield',fieldLabel:'user',name:'s_user',width:288}
	        	,{xtype:'textfield',fieldLabel:'orderNum',name:'s_orderNum',width:288}
	            ]
	    });
		com.ysys.www.tur.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.tur.querywin
 * @extends Ext.Window
 */
com.ysys.www.tur.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.tur.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.tur.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.tur.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
			...
			items:[{
	        		...
	        		,{xtype:'textfield',fieldLabel:'role',name:'role',width:288}
	        		,{xtype:'textfield',fieldLabel:'user',name:'user',width:288}
	        		,{xtype:'textfield',fieldLabel:'orderNum',name:'orderNum',width:288}
	        ]
	    });
	    com.ysys.www.tur.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.tur.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.tur.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.tur.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.tur
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.turGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
		...
		this.cm = new Ext.grid.ColumnModel([
		    ...
		    this.sm,
		    this.expander
	        ,{header:'role',width:100,sortable:true,dataIndex:'role'}
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addTur,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editTur,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deleteTur,scope:this},'-'
	            ...
	            store:new Ext.data.ArrayStore({
        			fields: ['name','code'],
        			data: [['role', 'role'],['user', 'user'],['orderNum', 'orderNum']]
        		}),
	            displayField:'name',
	            valueField:'code',
		...
		//调用父类构建函数
        com.ysys.www.turGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.tur.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.tur.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	            handler:this.saveTur,
	            scope:this
	        },{
	            text:'取消',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.tur.queryformpanel();
	    this.querywin =  new com.ysys.www.tur.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
		...
		function rgtDelete() {
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../Tur/extdelete.do?ids='+record.data.id,
						method:'POST',
    ...
    //新建窗口
    ,addTur : function(){
        this.dtlwin.setTitle('新建Tur');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../Tur/extsave.do';
	}
    ...
    ,deleteTur:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../Tur/extdelete.do?ids='+this.getRecordArrayUtils(records, 'id'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
    ...
    //查询操作
    ,queryOrder:function(){
    	this.getStore().baseParams['s_role'] = this.queryformpanel.form.findField('s_role').getRawValue();
    	this.getStore().baseParams['s_user'] = this.queryformpanel.form.findField('s_user').getRawValue();
 ...
 * 注册主表格的xtype
 */
Ext.reg('tur', com.ysys.www.turGrid);
