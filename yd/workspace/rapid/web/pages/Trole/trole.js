...
Ext.namespace('com.ysys.www.trole');

/**
 * 查询表单
 * @class com.ysys.www.trole.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.trole.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		...
		com.ysys.www.trole.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.trole.querywin
 * @extends Ext.Window
 */
com.ysys.www.trole.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.trole.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.trole.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.trole.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
	    ...
	    com.ysys.www.trole.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.trole.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.trole.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.trole.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.trole
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.troleGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
    	this.ds = new Ext.data.Store({
	        url:'../Trole/extlist.do',
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addTrole,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editTrole,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deleteTrole,scope:this},'-'
		...
		//调用父类构建函数
        com.ysys.www.troleGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.trole.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.trole.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	            handler:this.saveTrole,
	            scope:this
	        },{
	            text:'取消',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.trole.queryformpanel();
	    this.querywin =  new com.ysys.www.trole.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
	    ...
	    function rgtEdit() {
				        ...
				        this.dtlwin.setTitle('修改Trole');
				        this.dtlwin.show();
				        this.dtlformpanel.form.reset();
				        this.dtlformpanel.form.loadRecord(record);
				        this.dtlformpanel.url = '../Trole/extupdate.do';
	    };
	    //右键删除
		function rgtDelete() {
			messageContextMenu.hide();
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../Trole/extdelete.do?ids='+record.data.id,
						method:'POST',
    ...
    //新建窗口
    ,addTrole : function(){
        this.dtlwin.setTitle('新建Trole');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../Trole/extsave.do';
	}
    ...
    ,deleteTrole:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../Trole/extdelete.do?ids='+this.getRecordArrayUtils(records, 'id'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
 ...
 * 注册主表格的xtype
 */
Ext.reg('trole', com.ysys.www.troleGrid);
