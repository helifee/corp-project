/**
 * [Torganization] author by $YourName$
...
Ext.namespace('com.ysys.www.torganization');

/**
 * 查询表单
 * @class com.ysys.www.torganization.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.torganization.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        ...
	        items:[{
	        	...
	        	,{xtype:'textfield',fieldLabel:'pid',name:'s_pid',width:288}
	            ]
	    });
		com.ysys.www.torganization.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.torganization.querywin
 * @extends Ext.Window
 */
com.ysys.www.torganization.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.torganization.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.torganization.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.torganization.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
			...
			items:[{
	        		...
	        		,{xtype:'textfield',fieldLabel:'pid',name:'pid',width:288}
	        ]
	    });
	    com.ysys.www.torganization.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.torganization.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.torganization.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.torganization.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.torganization
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.torganizationGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
    	this.ds = new Ext.data.Store({
	        url:'../Torganization/extlist.do',
		...
		this.cm = new Ext.grid.ColumnModel([
	        ...
	        ,{header:'pid',width:100,sortable:true,dataIndex:'pid'}
		]);
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addTorganization,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editTorganization,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deleteTorganization,scope:this},'-'
	            ...
	            store:new Ext.data.ArrayStore({
        			fields: ['name','code'],
        			data: [['name', 'name'],['sn', 'sn'],['description', 'description'],['pid', 'pid']]
        		}),
	            displayField:'name',
	            valueField:'code',
		...
		//调用父类构建函数
        com.ysys.www.torganizationGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.torganization.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.torganization.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	            handler:this.saveTorganization,
	            scope:this
	        },{
	            text:'取消',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.torganization.queryformpanel();
	    this.querywin =  new com.ysys.www.torganization.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
	    ...
	    function rgtEdit() {
				        ...
				        this.dtlwin.setTitle('修改Torganization');
				        this.dtlwin.show();
				        this.dtlformpanel.form.reset();
				        this.dtlformpanel.form.loadRecord(record);
				        this.dtlformpanel.url = '../Torganization/extupdate.do';
	    };
	    //右键删除
		function rgtDelete() {
			messageContextMenu.hide();
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../Torganization/extdelete.do?ids='+record.data.id,
						method:'POST',
    ...
    //新建窗口
    ,addTorganization : function(){
        this.dtlwin.setTitle('新建Torganization');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../Torganization/extsave.do';
	}
	
	//编辑操作
    ,editTorganization:function(){
    ...
    ,deleteTorganization:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../Torganization/extdelete.do?ids='+this.getRecordArrayUtils(records, 'id'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
    ...
    ,queryOrder:function(){
    	...
    	this.getStore().baseParams['s_pid'] = this.queryformpanel.form.findField('s_pid').getRawValue();
		this.getStore().load();
		this.querywin.hide();
    }
    //工具类
 ...
 * 注册主表格的xtype
 */
Ext.reg('torganization', com.ysys.www.torganizationGrid);
