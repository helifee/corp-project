...
Ext.namespace('com.ysys.www.mcity');

/**
 * 查询表单
 * @class com.ysys.www.mcity.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.mcity.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        ...
	        items:[{
	        	...
	        	,{xtype:'textfield',fieldLabel:'城市名',name:'s_cityname',width:288}
	        	,{xtype:'textfield',fieldLabel:'可见标志',name:'s_flag',width:288}
	        	,{xtype:'textfield',fieldLabel:'更新时间',name:'s_updatetime',width:288}
	            ]
	    });
		com.ysys.www.mcity.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.mcity.querywin
 * @extends Ext.Window
 */
com.ysys.www.mcity.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.mcity.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.mcity.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.mcity.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
			...
			items:[{
	        		...
	        		,{xtype:'hidden',fieldLabel:'城市代号',name:'cityid',width:288}
	        		,{xtype:'textfield',fieldLabel:'城市名',name:'cityname',width:288}
	        		,{xtype:'textfield',fieldLabel:'可见标志',name:'flag',width:288}
	        		,{xtype:'textfield',fieldLabel:'更新时间',name:'updatetime',width:288}
	        ]
	    });
	    com.ysys.www.mcity.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.mcity.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.mcity.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.mcity.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.mcity
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.mcityGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
    	this.ds = new Ext.data.Store({
	        url:'../Mcity/extlist.do',
	        reader:new Ext.data.JsonReader({
	            root:'list',
	            totalProperty:'totalSize',
	            id:'id'
		        }
		        ,['cityid','cityname','flag','updatetime',]
	        ),
	        baseParams:{
	            limit:this.pageSize
	        },
		...
		this.cm = new Ext.grid.ColumnModel([
		    ...
		    this.sm,
		    this.expander
	        ,{header:'城市名',width:100,sortable:true,dataIndex:'cityname'}
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addMcity,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editMcity,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deleteMcity,scope:this},'-'
	            ...
	            store:new Ext.data.ArrayStore({
        			fields: ['name','code'],
        			data: [['城市名', 'cityname'],['可见标志', 'flag'],['更新时间', 'updatetime']]
        		}),
	            displayField:'name',
	            valueField:'code',
		...
		//调用父类构建函数
        com.ysys.www.mcityGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.mcity.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.mcity.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	            handler:this.saveMcity,
	            scope:this
	        },{
	            text:'取消',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.mcity.queryformpanel();
	    this.querywin =  new com.ysys.www.mcity.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
	    ...
	    function rgtEdit() {
				        ...
				        this.dtlwin.setTitle('修改Mcity');
				        this.dtlwin.show();
				        this.dtlformpanel.form.reset();
				        this.dtlformpanel.form.loadRecord(record);
				        this.dtlformpanel.url = '../Mcity/extupdate.do';
	    };
	    //右键删除
		function rgtDelete() {
			messageContextMenu.hide();
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../Mcity/extdelete.do?ids='+record.data.cityid,
						method:'POST',
    ...
    //新建窗口
    ,addMcity : function(){
        this.dtlwin.setTitle('新建Mcity');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../Mcity/extsave.do';
	}
    ...
    ,deleteMcity:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../Mcity/extdelete.do?ids='+this.getRecordArrayUtils(records, 'cityid'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
    ...
    //查询操作
    ,queryOrder:function(){
    	this.getStore().baseParams['s_cityname'] = this.queryformpanel.form.findField('s_cityname').getRawValue();
 ...
 * 注册主表格的xtype
 */
Ext.reg('mcity', com.ysys.www.mcityGrid);
