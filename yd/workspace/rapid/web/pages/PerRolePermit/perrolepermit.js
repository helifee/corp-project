/**
 * [PerRolePermit] author by $YourName$
...
Ext.namespace('com.ysys.www.perrolepermit');

/**
 * 查询表单
 * @class com.ysys.www.perrolepermit.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.perrolepermit.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		...
		com.ysys.www.perrolepermit.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.perrolepermit.querywin
 * @extends Ext.Window
 */
com.ysys.www.perrolepermit.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.perrolepermit.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.perrolepermit.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.perrolepermit.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
			...
			items:[{
	        		...
	        		,{xtype:'hidden',fieldLabel:'角色ID',name:'roleId',width:288}
	        		,{xtype:'textfield',fieldLabel:'权限ID',name:'permitId',width:288}
	        		,{xtype:'textfield',fieldLabel:'更新时间',name:'updatetime',width:288}
	        ]
	    });
	    com.ysys.www.perrolepermit.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.perrolepermit.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.perrolepermit.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.perrolepermit.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.perrolepermit
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.perrolepermitGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
    	this.ds = new Ext.data.Store({
	        url:'../PerRolePermit/extlist.do',
	        reader:new Ext.data.JsonReader({
	            root:'list',
	            totalProperty:'totalSize',
	            id:'id'
		        }
		        ,['roleId','permitId','updatetime',]
	        ),
	        baseParams:{
	            limit:this.pageSize
	        },
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addPerRolePermit,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editPerRolePermit,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deletePerRolePermit,scope:this},'-'
	            ...
	            store:new Ext.data.ArrayStore({
        			fields: ['name','code'],
        			data: [['权限ID', 'permitId'],['更新时间', 'updatetime']]
        		}),
	            displayField:'name',
	            valueField:'code',
		...
		//调用父类构建函数
        com.ysys.www.perrolepermitGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.perrolepermit.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.perrolepermit.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	            handler:this.savePerRolePermit,
	            scope:this
	        },{
	            text:'取消',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.perrolepermit.queryformpanel();
	    this.querywin =  new com.ysys.www.perrolepermit.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
	    ...
	    function rgtEdit() {
				        ...
				        this.dtlwin.setTitle('修改PerRolePermit');
				        this.dtlwin.show();
				        this.dtlformpanel.form.reset();
				        this.dtlformpanel.form.loadRecord(record);
				        this.dtlformpanel.url = '../PerRolePermit/extupdate.do';
	    };
	    //右键删除
		function rgtDelete() {
			messageContextMenu.hide();
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../PerRolePermit/extdelete.do?ids='+record.data.roleId,
						method:'POST',
    ...
    //新建窗口
    ,addPerRolePermit : function(){
        this.dtlwin.setTitle('新建PerRolePermit');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../PerRolePermit/extsave.do';
	}
	
	//编辑操作
    ,editPerRolePermit:function(){
    ...
    ,deletePerRolePermit:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../PerRolePermit/extdelete.do?ids='+this.getRecordArrayUtils(records, 'roleId'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
 ...
 * 注册主表格的xtype
 */
Ext.reg('perrolepermit', com.ysys.www.perrolepermitGrid);
