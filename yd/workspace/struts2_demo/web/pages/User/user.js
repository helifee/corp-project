...
Ext.namespace('com.ysys.www.user');

/**
 * 查询表单
 * @class com.ysys.www.user.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.user.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		...
		com.ysys.www.user.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.user.querywin
 * @extends Ext.Window
 */
com.ysys.www.user.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.user.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.user.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.user.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
	    ...
	    com.ysys.www.user.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.user.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.user.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.user.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.user
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.userGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addUser,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editUser,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deleteUser,scope:this},'-'
		...
		//调用父类构建函数
        com.ysys.www.userGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.user.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.user.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.user.queryformpanel();
	    this.querywin =  new com.ysys.www.user.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
		...
		function rgtDelete() {
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../User/extdelete.do?ids='+record.data.userId,
						method:'POST',
    ...
    //新建窗口
    ,addUser : function(){
        this.dtlwin.setTitle('新建User');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../User/extsave.do';
	}
    ...
    ,deleteUser:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../User/extdelete.do?ids='+this.getRecordArrayUtils(records, 'userId'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
 ...
 * 注册主表格的xtype
 */
Ext.reg('user', com.ysys.www.userGrid);
