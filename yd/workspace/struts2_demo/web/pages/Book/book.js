...
Ext.namespace('com.ysys.www.book');

/**
 * 查询表单
 * @class com.ysys.www.book.queryformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.book.queryformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        ...
	        items:[{
	        	...
	        	,{xtype:'textfield',fieldLabel:'isbm',name:'s_isbm',width:288}
	        	,{xtype:'textfield',fieldLabel:'author',name:'s_author',width:288}
	            ]
	    });
		com.ysys.www.book.queryformpanel.superclass.initComponent.call(this);
	}
});

/**
 * 查询窗口
 * @class com.ysys.www.book.querywin
 * @extends Ext.Window
 */
com.ysys.www.book.querywin = Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        title:'高级查询',
	        width:455,
		...
		com.ysys.www.book.querywin.superclass.initComponent.call(this);
	}
});

/**
 * 内容表单
 * @class com.ysys.www.book.dtlformpanel
 * @extends Ext.form.FormPanel
 */
com.ysys.www.book.dtlformpanel = Ext.extend(Ext.form.FormPanel,{
	initComponent:function() {
		Ext.apply(this,{
	        labelWidth:100,
	        labelAlign:'right',
...
//	        bodyStyle:'padding:10px',
			...
			items:[{
	        		...
	        		,{xtype:'textfield',fieldLabel:'isbm',name:'isbm',width:288}
	        		,{xtype:'textfield',fieldLabel:'author',name:'author',width:288}
	        ]
	    });
	    com.ysys.www.book.dtlformpanel .superclass.initComponent.call(this);
	}
...
/**
 * 表单窗口
 * @class com.ysys.www.book.dtlwin
 * @extends Ext.Window
 */		
com.ysys.www.book.dtlwin =  Ext.extend(Ext.Window,{
	initComponent:function() {
		Ext.apply(this,{
	        width:535,
	        height:400,
		...
		com.ysys.www.book.dtlwin.superclass.initComponent.call(this);
	}
});
...
/**
 * 主表格入口
 * @class com.ysys.www.book
 * @extends Ext.grid.GridPanel
 */
com.ysys.www.bookGrid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
    	this.pageSize=10;
		...
		this.cm = new Ext.grid.ColumnModel([
	        ...
	        ,{header:'isbm',width:100,sortable:true,dataIndex:'isbm'}
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',iconCls:'addicon',handler:this.addBook,scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',iconCls:'editicon',handler:this.editBook,scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',iconCls:'deleteicon',handler:this.deleteBook,scope:this},'-'
	            ...
	            store:new Ext.data.ArrayStore({
        			fields: ['name','code'],
        			data: [['name', 'name'],['isbm', 'isbm'],['author', 'author']]
        		}),
	            displayField:'name',
	            valueField:'code',
		...
		//调用父类构建函数
        com.ysys.www.bookGrid.superclass.initComponent.call(this);
        //加载数据
        this.store.load({params:{start:0}});
        
 		//扩展类的详细弹出窗口
 		this.dtlformpanel = new com.ysys.www.book.dtlformpanel();
 		this.dtlwin =  new com.ysys.www.book.dtlwin({items:this.dtlformpanel,buttons:[{
	            text:'保存',
	    ...
	    //扩展类的查询弹出窗口
	    this.queryformpanel = new com.ysys.www.book.queryformpanel();
	    this.querywin =  new com.ysys.www.book.querywin({items:this.queryformpanel,buttons:[{
	            text:'确定',
    ...
    */
    ...
    ,onMessageContextMenu : function (grid, rowIndex, e) {
		...
		function rgtDelete() {
			...
			Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
				if (btn == 'yes'){
						Ext.Ajax.request({
						url:'../Book/extdelete.do?ids='+record.data.id,
						method:'POST',
    ...
    //新建窗口
    ,addBook : function(){
        this.dtlwin.setTitle('新建Book');
        this.dtlwin.show();
        this.dtlformpanel.form.reset();
	    this.dtlformpanel.url = '../Book/extsave.do';
	}
    ...
    ,deleteBook:function(){
		...
		Ext.MessageBox.confirm('确认删除','确定要删除这些记录?',function(btn){
			if (btn == 'yes'){
				Ext.Ajax.request({
					url:'../Book/extdelete.do?ids='+this.getRecordArrayUtils(records, 'id'),
		            method:'POST',
		            success:function(response){
		                var data = Ext.util.JSON.decode(response.responseText);
		                if (data.success == true){
    ...
    ,queryOrder:function(){
    	...
    	this.getStore().baseParams['s_isbm'] = this.queryformpanel.form.findField('s_isbm').getRawValue();
    	this.getStore().baseParams['s_author'] = this.queryformpanel.form.findField('s_author').getRawValue();
		this.getStore().load();
		this.querywin.hide();
    }
    //工具类
 ...
 * 注册主表格的xtype
 */
Ext.reg('book', com.ysys.www.bookGrid);
