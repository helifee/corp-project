...
function textEmptyCheck(){
	...
	if( ((radioCheckSign.checked==true) && (document.getElementById("startDate").value=="")) 
		...
		MsgBox.message("开始日期或结束日期不允许全为空。");
		return false ;
	}
...
function conferensituation(){
	...
	if(isOrNotNull == false){		 
		 return;
	 } 
	if(isNaN(maxPageItems) || isZhengShu(maxPageItems) == false || parseInt(maxPageItems) < 0){
		MsgBox.message("每页显示的最大记录数:输入数据不正确！");
		return;
	}
	
	 var url = "conferensituationXiangxi.action?&maxPageItems=" + maxPageItems;
...
//分页时使用的ajax提交函数。
function conferensituationPageTag(pageUrl , pageNumber , conferensituationId , startDate , endDate ,  radiobutton , items1 , maxPageItems){
