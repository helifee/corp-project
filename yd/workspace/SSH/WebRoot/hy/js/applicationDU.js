...
//全局变量。用于保存所选择的记录。
var newUpdateFlag = -1 ;//当前操作状态标记。0：新建 ；1：更改。
var hysidPublic  = 0  ; 
var hysmcPublic  = "" ;
var cjrsPublic   = 0  ;
var hysdhPublic  = "" ;
...
//
//document.getElementById('myScoll').scrollTop = document.getElementById('textarea').scrollHeight;
function tableClick(){
	var rowOldCommonIndex = -1;
	var tableNameclick = 'xianshiTable';
	if(!document.getElementsByTagName || !document.createTextNode) return;
    var rows=document.getElementById('xianshiTable').getElementsByTagName('tr');
		for(var i = 0; i < rows.length; i++){
			rows[i].onclick=function(){  
				
				rowOldCommonIndex = rowCommonIndex;
				rowCommonIndex = eval(this.rowIndex);
				document.getElementById(tableNameclick).rows[rowOldCommonIndex].style.backgroundColor='#ffffd9'; 
			} 
		}	
}
//当鼠标移动到会议室一条记录上时，当前记录行改变颜色，对应的会议室标签改变颜色。
//当鼠标从一条会议室记录上离开时，当前记录和对应的会议室标签恢复为原来颜色。
function getRowAndColumn(fhrowindex){
   	
      var tableName = 'xianshiTable';
      if(fhrowindex != -1){
    	  rowOldUpdate = fhrowindex;
      }
      if(!document.getElementsByTagName || !document.createTextNode)return;
      var rows=document.getElementById('xianshiTable').getElementsByTagName('tr');
      for(i = 0; i < rows.length; i++){       	       		
	        	rows[i].onmouseover=function(){  	 
	        		if( (rowOldUpdate == eval(this.rowIndex)) ){
...
function gudingColor(rowindex){
	var tableName = 'xianshiTable';
	if(rowindex != -1){
	  document.getElementById(tableName).rows[rowindex].style.backgroundColor='#99CC33';
	}
      if(rowOldUpdate != rowindex && rowOldUpdate != -1){
    	document.getElementById(tableName).rows[rowOldUpdate].style.backgroundColor='#ffffd9'; 
      }
    rowOldUpdate = rowindex;  
}


//更改显示函数，当点击更改时候，更改记录的文本框可用，并且可以更改。
function genggaiXianshiTable(rowindex , hysid  , hysmc , cjrs , hysdh , hyssb , hysjs , hysBg , hysqx , hysqy , hyszx , hyszy , hyspx ){
    ...
    gudingColor(rowindex);
    
}
//更改显示函数，当点击更改时候，更改记录的文本框可用，并且可以更改。
function genggaiXianshiTable1(  hysid  , hysmc , cjrs , hysdh , hyssb , hysjs , hysBg , hysqx , hysqy , hyszx , hyszy , hyspx ){
...
//页面提交函数，提交到distributeDeleteAction。
function distributeDeleteActionScroll(rowindex , hysid){
	gudingColor(rowindex);
	
if(confirm("警告：删除会议室会导致该会议室的预约情况无法显示！\n\n确定删除吗?")){
 		var url = "distributeDeleteSrollAction?hysIddelete="+hysid;
 		new Ajax.Updater('div_hy_distributeUpdate', url, {
			onLoading : function() {
			},
			onSuccess : function(response) {				
...
//页面提交函数，提交到distributeDeleteAction。
function distributeDeleteActionPage(rowindex , hysid){
	gudingColor(rowindex);
if(confirm("警告：删除会议室会导致该会议室的预约情况无法显示！\n\n确定删除吗?")){
 		var url = "distributeDeletePageAction?hysIddelete="+hysid;
 		new Ajax.Updater('div_hy_distributeUpdate', url, {
			onLoading : function() {
			},
			onSuccess : function(response) {				
...
function submitTypeDistributeScroll(){
	objHysId=document.getElementById("hysId");
	objHysMc=document.getElementById("hysMc");
	objHysRs=document.getElementById("hysRs");
	objHysJs=document.getElementById("hysJs");
	...
	//如果满足条件，则提交。
	var url = "distributeNewOrUpdateSrollAction?rowOldUpdate="+rowOldUpdate+"&newUpdateFlag="+newUpdateFlag;
//	alert(newUpdateFlag);
	new Ajax.Updater('div_hy_distributeUpdate', url, {
	parameters: $('distributeUpdateForm').serialize()	,
		onLoading : function() {
...
function submitTypeDistributePage(){
	objHysId=document.getElementById("hysId");
	objHysMc=document.getElementById("hysMc");
	objHysRs=document.getElementById("hysRs");
	objHysJs=document.getElementById("hysJs");
	...
	var maxPageItems = document.getElementById("maxPageItems").value;
	if(isNaN(maxPageItems) || isZhengShu(maxPageItems) == false || parseInt(maxPageItems) < 0){
		alert("每页显示的最大记录数:输入数据不正确！");
		return;
	}
	//如果满足条件，则提交。
	var url = "distributeNewOrUpdatePageAction?rowOldUpdate="+rowOldUpdate+"&newUpdateFlag="+newUpdateFlag;
	new Ajax.Updater('div_hy_distributeUpdate', url, {
	parameters: $('distributeUpdateForm').serialize()	,
		onLoading : function() {
		},
...
//分页时使用的ajax提交函数。
function conferensituationPageTag(pageUrl , pageNumber , items1 , maxPageItems){
...
function scrollUp(rowindex){
//	alert(rowindex);
	var myScoll = document.getElementById("myScoll");
	if(rowindex != -1){
		myScoll.scrollTop=myScoll.scrollTop+21 * rowindex;
	}
