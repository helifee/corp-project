...
<script type="text/javascript">
	...
	document.observe('dom:loaded', function() {
		...
		$('userId').activate();
		if(crossDomain) Event.observe(window, 'load', oldLogout);
	});
	function doubleLogin(){
		if(crossDomain && $('oldLogin').contentWindow.$){
			$('oldLogin').contentWindow.runAjax('/login/LOGOUT.jsp?Rand=', '', false, oldLogin);
		} else {
			newLogin();
		}
	}
	function oldLogin(){
		var param = 'USE_ID=' + $F('userId') + '&USE_PWD=' + $F('password');
		$('oldLogin').contentWindow.runAjax('/login/LOGIN.jsp', param, true, newLogin);
	}
	function newLogin(){
		$('loginForm').submit();
	}
	function enterEvent(event){
		...
		if(Event.element(event).id == 'userId'){
			$('password').focus();
		}else if(Event.element(event).id == 'password'){
			doubleLogin();
		}
	}
	function oldLogout(){
		if(crossDomain && $('oldLogin').contentWindow.$){
			$('oldLogin').contentWindow.runAjax('/login/LOGOUT.jsp?Rand=', '', false, null);
		}
	}
</script>
<!-- 登录页面css -->
...
<body>
	...
	<div class="none">
		<iframe id="oldLogin" src="http://www.yds.yd/login/jump.html"></iframe>
	</div>
	</body>
</html>