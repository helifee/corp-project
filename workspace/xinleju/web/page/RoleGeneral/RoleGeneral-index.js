...
function addRole(){
	openwindow("RoleGeneral!edit.do", "通用角色编辑");
}
function editRole(){
	if($("input:checkbox:checked").length != 1){
		...
		$.each($("input:checkbox:checked") , function() {
			var roleGeneralId = $(this).val();
			openwindow('RoleGeneral!edit.do?id='+roleGeneralId, "通用角色编辑");
		});
	}
...
function deleteRole(){
	if($("input:checkbox:checked").length != 1){
		...
		if(confirm("确定要删除吗?")){
			$.each($("input:checkbox:checked") , function() {
				var roleGeneralId = $(this).val();
				$.post('RoleGeneral!deleteRole.ajax?roleGeneralId='+roleGeneralId,  function(data2) {
					doRefreshParent();
				});
			});
...
function disable(){
	...
	var selId = findSelId();
	$.post('RoleGeneral!disable.do?id='+selId, $('#frm').serialize(), function(data) {
		$('body').unmask();
...
function enable(){
	...
	var selId = findSelId();
	$.post('RoleGeneral!enable.do?id='+selId, $('#frm').serialize(), function(data) {
		$('body').unmask();
