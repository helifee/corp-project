...
<body>
<form id="frm" name="frm" action="File!listptflow.do?category=${requestScope.category}&ownerId=${requestScope.ownerId}&receive=<%=receive %>" method="post">
<table width="100%" border="0" cellspacing="0" cellpadding="0" >
  <tr>
	  ...
	  <tr>
		<td>
			<div class="divh3_title">
	            ...
	            <a href="javascript:;" data-checkItem="checkItem_Package" data-form="frm_Package" name="fileuploadPTFlow_downloadAll_Package_">打包下载</a>
	            &nbsp;&nbsp;
	             	<script type='text/javascript'>
				     $("a[name^='fileuploadPTFlow_downloadAll_Package_']").prop("name","fileuploadPTFlow_downloadAll_Package_"+temp);
				   </script> 
    ...
    <td width="30%">
         <table width="100%"  border="0" cellspacing="0" cellpadding="0" class="divh4" style="border-left:0px">
           <tr>
          
             <td>
               <span style="cursor:pointer;" title="上传附件"><input type="file" id="file_upload" /></span>
             </td>
             <td>
               <span id="url_upload" style="margin-right:10px;cursor:pointer;" title="上传URL"><img src="images/link.png" /></span> 
             </td>
           </tr>
         </table> 
    </td>
...
<div style="min-height: 100px;"  id="filediv" >

<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#C0C0C0" onmouseover="changeto()"  onmouseout="changeback()">
      <thead>
        <tr>
          <td width="5%" height="34px" background="images/bg.gif" bgcolor="#FFFFFF"><div align="center"> <input name="fileuploadPTFlow_checkAll_upload"  type="checkbox"/></div></td>
          <td width="65%" height="34px" background="images/bg.gif" bgcolor="#FFFFFF"><div align="center"><span class="STYLE1">文件名称</span></div></td>
      ...
      <tbody id="fileQueue">
	     ...
	     <s:iterator value="page.items" var="item" status="stat">
			<tr>
			     <td height="26" bgcolor="#FFFFFF"><div align="center"><input name="fileuploadPTFlow_checkItem_${item.id}" data-id="${item.id}" data-checkItem="checkItem_${item.id}" type="checkbox"/></div></td>
...
<form action="File!downloadChooseZip.do" id="frm_Package" method="post">
  	...
  	<script type="text/javascript">
			...
			$(document).on('click',"a[name^='fileuploadPTFlow_downloadAll_Package_"+temp+"']",function(){
				var arrUploadId = [];
				$("input[name^='fileuploadPTFlow_checkItem_']:checked").each(function(index,obj){
			...
			$(document).on('click',"input[name^='fileuploadPTFlow_checkAll_upload']",function(){
				var chek=$(this).prop("checked");
				if($(this).prop("checked")==true){
					$("input[name^='fileuploadPTFlow_checkItem_']").each(function(index,obj){
