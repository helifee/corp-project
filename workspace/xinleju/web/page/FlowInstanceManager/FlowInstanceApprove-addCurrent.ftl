<#if   flowInstance?? && flowInstance.lstFlowInstanceStep??>
	...
	<#list flowInstance.lstFlowInstanceStep as flowInstanceStep>
        ...
        <#if flowInstanceStep.lstFlowInstanceStepWork??   && (flowInstanceStep.status==0||flowInstanceStep.status==1)  &&(flowInstanceStep.lstFlowInstanceStepWork?size > 0)>
    		<#--遍历流程实例环节工作组-->
    		<#list flowInstanceStep.lstFlowInstanceStepWork as flowInstanceStepWork>
    			...
    			<#if  flowInstanceStepWork.lstFlowInstanceStepWorkTask??  && (flowInstanceStepWork.status==0||flowInstanceStepWork.status==1)   &&(flowInstanceStepWork.lstFlowInstanceStepWorkTask?size > 0)>
					<#--遍历流程实例环节工作组任务-->
	        		<#list flowInstanceStepWork.lstFlowInstanceStepWorkTask as flowInstanceStepWorkTask>
				     ...
				     <#if flowInstanceStepWorkTask.status == 0>   
				        <tr>
					        ...
					        <td style="text-align:center;${taskColor}"><input type="radio" name="selectPreNode" /></td>
				        </tr>
				      </#if>
	        		</#list>
			    <#else>
	        		...
	        		<tr>
				        ...
				        <td style="max-width:200px;${workColor}" data-participantsNameValidate="${isAuto}">
				        	<#if flowInstanceStepWork.status == 0>
								...
								<div class="participants-wrap">
									<label name="currentDealPerson">${flowInstanceStepWork.participantsDefineName!''}</label>
							...
							<#else>
								...
								<div class="participants-wrap">
								    <label name="currentDealPerson">${flowInstanceStepWork.participantsName!''}</label>
								    <input id="participantsId_${flowInstanceStep_index+1}_${flowInstanceStepWork_index+1}" type="hidden" />
								    <input id="stepId_${flowInstanceStep_index+1}_${flowInstanceStepWork_index+1}" type="hidden" value="${flowInstanceStep.guid}"/>
								    <label id="participantsName_${flowInstanceStep_index+1}_${flowInstanceStepWork_index+1}"></label>
								 </div>
				        ...
				        <td style="text-align:left;${workColor}"></td>
				        <td style="text-align:left;${workColor}"></td>
				        <td style="text-align:center;${workColor}"><input type="radio" name="selectPreNode" data-workId="${flowInstanceStepWork.guid}" data-taskId=""></td>
			        </tr>
			        </#if>
