...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body style="overflow: hidden;">
		<s:form id="frm" action="TodoFi!allFiList.do">
						...
						<ul>
							<s:iterator value="#request.todoTypeMap">
								<li class="${key eq todoFiDto.todoType ? 'current' :''}">
									<a href="javascript:void(0)" onclick="selectCond('任务类型：${value}',${key},'todoTypeLi','todoFiDto.todoType')" id="${key}">${value}</a>
								</li>
							</s:iterator>
						</ul>
					</div>
						...
						<ul>
							<s:iterator value="#request.companyMap">
								<li class="${key eq todoFiDto.depIds ? 'current' :''}">
									<a href="javascript:void(0)" onclick="selectCond('组织机构：${value.name}',${key},'depIdsLi','todoFiDto.depIds')" id="${key}">${value.name}</a>
								</li>
							</s:iterator>
						</ul>
					</div>
						...
						<ul>
							<s:iterator value="#request.pCtMap">
								<li class="${key eq todoFiDto.pctId ? 'current' :''}">
									<a href="javascript:void(0)" onclick="selectCond('流程类别：${value.name}',${key},'ctIdLi','todoFiDto.pctId')" id="${key}">${value.name}</a>
								</li>
							</s:iterator>
						</ul>
						<div class="clear"></div>
						<ul>
							<s:iterator value="#request.ctMap">
								<li class="${key eq todoFiDto.ctId ? 'current' :''}">
									<a href="javascript:void(0)" onclick="selectCond('流程类别：${value.name}',${key},'ctIdLi','todoFiDto.ctId')" id="${key}">${value.name}</a>
								</li>
							</s:iterator>
						</ul>
						<div class="clear"></div>
						<ul>
							<s:iterator value="#request.cCtMap">
								<li class="${key eq todoFiDto.cctId ? 'current' :''}">
									<a href="javascript:void(0)" onclick="selectCond('流程类别：${value.name}',${key},'ctIdLi','todoFiDto.cctId')" id="${key}">${value.name}</a>
								</li>
							</s:iterator>
						</ul>
					</div>
						...
						<ul>
							<s:iterator value="#request.fiStatusMap">
								<li class="${key eq todoFiDto.fiStatus ? 'current' :''}">
									<a href="javascript:void(0)" onclick="selectCond('流程状态：${value}','${key}','fiStatusLi','todoFiDto.fiStatus')" id="${key}">${value}</a>
								</li>
							</s:iterator>
						</ul>
					</div>
			...
			<table width="100%" cellpadding="0" cellspacing="1" class="wd_tablelist01">
				...
				<s:set id="statusCreate" value="@com.xinleju.erp.flow.flowutils.utils.WsConstant@STATUS_CREATED"></s:set>
				...
				<s:set id="statusOverdue" value="@com.xinleju.erp.flow.flowutils.utils.WsConstant@STATUS_OVERDUE"></s:set>
				<s:iterator value="page.items" id="item" status="sta">
					...
					<tr>
						...
						<td align="left" title="${item[3]}">
							<a href="#" onclick="openwindow('Form!dealIndex.do?fiId=${item[0]}','fi_wi',1270,0)"><app:TruncateTag size="50" src="${item[3]}"></app:TruncateTag></a>
						</td>
			...
			<div class="page">
				...
				<app:PageTag actionName="TodoFi!allFiList.do"></app:PageTag>
			</div>
		</s:form>
	</body>
</html>
