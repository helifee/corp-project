...
var selectFlowTemps = function(){
	var dt = new Date();
	var flowIds = "";
	var flowNames = "";
	if ($("input[type='checkbox']").is(":checked")) {
		var checkIdBox = $("input[name='ids']:checked").each(function() {
			flowIds += $(this).attr("value") + ",";
			flowNames += $.trim($(this).parent().next(".flowName").text()) + ",";
		});
		if (flowIds.length > 0) {
		...
		opener.document.getElementById("flowTemps").value = flowIds;
		opener.document.getElementById("selectFlowTemps").value = flowNames;
		window.close();
	} else {
		alert("请在右边列表复选框至少选择一条记录转移！");
	}
