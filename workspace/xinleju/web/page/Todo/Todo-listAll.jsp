...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<s:form id="frm" action="Todo!listAll">
			...
			<table width="100%" cellpadding="0" cellspacing="1" class="wd_tablelist01">
				...
				<s:set id="wiTypeZc" value="@com.xinleju.erp.flow.flowutils.utils.WsConstant$WiType@ZC.code"></s:set>
				<s:set id="wiTypeLg" value="@com.xinleju.erp.flow.flowutils.utils.WsConstant$WiType@LG.code"></s:set>
				<s:iterator value="page.items" id="item" status="sta">
					...
					<tr>
						...
						<td align="center">
							<s:if test="#item.status == #statusCreate">
								创建
							</s:if>
							<s:elseif test="#item.status == #statusRunning">
								<s:if test="#item.wiType == #wiTypeZc || #item.wiType == #wiTypeLg">待审</s:if>
								<s:else>待阅</s:else>
							</s:elseif>
							<s:elseif test="#item.status == #statusComplete">
								<s:if test="#item.wiType == #wiTypeZc || #item.wiType == #wiTypeLg">结束</s:if>
								<s:else>已阅</s:else>
							</s:elseif>
							<s:elseif test="#item.status == #statusOverdue">
								作废
							</s:elseif>
							<s:else>
								未知状态
			...
			<div class="page">
				...
				<app:PageTag actionName="Todo!listAll"></app:PageTag>
			</div>
		</s:form>
	</body>
</html>
