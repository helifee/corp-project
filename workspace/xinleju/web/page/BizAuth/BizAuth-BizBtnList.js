...
function save(flag) {
	if (flag == 1) {
		...
		document.frm.action = "BizAuth!save1.do";
	}
	if (flag == 2) {
...
function loadData(value) {
	$('body').mask("保存中...");
	if (hasChange && confirm("切换业务模块时，当前页面所选的数据将会丢失，确认是否需要保存?")){
		saveWhitoutAlert
	}
	var roleId = $("#roleId").val();
	window.location.href = "BizAuth!bizBtnList.do?roleId="+roleId+"&systemCode=" + value;
}