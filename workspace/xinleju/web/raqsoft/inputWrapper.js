...
*/
	...
	bar.find('#inputButSave').click(function(){
		var data = null;//'{"sheet1":{"A1":"dept.部门","B1":"经理","C1":"www","A2":["研发部","销售部","综合部","哈哈"],"B2":["2","3","1","4"],"C2":["","","",""]}}';
		data = _getInputDatas(sgid);
		$.ajax({
			type: 'POST',
			async : false,
			url: contextPath + "/InputServlet?d="+new Date().getTime(),
			data: {action:1,sgid:sgid,data:data},
	...
	bar.find('#inputButExcelOut').click(function(){
		...
		form.append("<input type='hidden' name='data' value='"+_getInputDatas(sgid)+"'>");
