...
function initBusinessObjectGrid() {
	...
	busiObjectGrid = $("#jqgridList").jqGrid(
        {
            url: hostUrl+"flow/businessObject/queryListByCondition",//获取数据的地址
...
function newItem() {
    ...
    var urlText = baseUrl + "flow/runtime/businessObject/businessObject_edit.html?appId=" + selectedAppId + "&id=-1&parentId="+parentId;
    openWin(urlText);
}
...
function updateSort(e){
	...
	if(ids){
		...
		$.ajax({
			url:hostUrl+"flow/businessObject/updateSort/"+ids,
			type:'put',
			dataType:'JSON',
			contentType:"application/json",
...
function deleteItem() {
    ...
    /*if(dataType=="分类"){
    	$.ajax({ //发送更新的ajax请求
            type: "post",
            url: hostUrl+"flow/businessObject/queryCountLikePrefixMap",
            dataType: "json",
            async: false,
	...
	$.ajax({ //发送更新的ajax请求
        type: "post",
        url: hostUrl+"flow/businessObject/queryRelatedCountByPrefixMap",
        dataType: "json",
        async: false,
	...
	pop_text_open("blue", popText, function() {
		$.ajax({
	        url: hostUrl+"flow/businessObject/deletePseudo/" + lastSel_dataId,
	        type: 'DELETE',
	        contentType: 'application/json',
	        dataType: 'JSON',
	        success: function (resultData) {
