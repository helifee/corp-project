...
function querySystemAppInfo(appId){
	$.ajax({
	       url: hostUrl+"sys/res/appSystem/get/"+appId,
	       type: 'GET',
	       async: false,
...
function queryBusinessObject(){
	...
	$.ajax({
	       url: hostUrl+"flow/businessObject/get/"+objId+"?t="+ new Date().getTime(),
	       type:'GET',
	       contentType:'application/json',
	       dataType:'JSON',
	       success:function (resultData) {
...
function queryParentBusinessObject(parentId){
	$.ajax({
	       url: hostUrl+"flow/businessObject/get/"+parentId+"?t="+ new Date().getTime(),
	       type:'GET',
	       contentType:'application/json',
	       dataType:'JSON',
	       success:function (resultData) {
...
function setFormData(dataObj){
    ...
    //是否允许移动端审批，为空默认允许
	var ifMobile = dataObj.ifMobile||dataObj.ifMobile==null?"1":"0";
    $('input:radio[name="ifMobile"][value="'+ifMobile+'"]').attr("checked","checked");
...
function doSaveFormAction(){
	...
	var ifMobile = busiObjDto.ifMobile;
	ifMobile = ifMobile =="1"?true :false;
    busiObjDto.ifMobile = ifMobile;
...
function comparePrefixIdWithParentId(self_prefixId, parentId){
	...
	if(self_prefixId && self_prefixId.length>30){
		$.ajax({
		       url: hostUrl+"flow/businessObject/get/"+parentId,
		       type: 'GET',
		       async: false,
		       contentType:'application/json',
		       dataType:'JSON',
...
function updateFormData(busiObjDto){
	$.ajax({
	       url: hostUrl+"flow/businessObject/update/"+busiObjDto.id,
...
function saveFormData(busiObjDto){
	...
	//console.info( JSON.stringify(busiObjDto) );
	var dataParentId = busiObjDto.parentId;
	$.ajax({
	       url: hostUrl+"flow/businessObject/save",
	       data: JSON.stringify(busiObjDto),
	       type: 'POST',
	       contentType: 'application/json',
	       dataType: 'JSON',
