...
$(function() {
	...
	validateInstanceDataAuth(instanceId);
	// 
...
function validateInstanceDataAuth(instanceId) {
	if(!instanceId){
		return;
	}
	$.ajax({
		type: 'GET',
		url: hostUrl + 'flow/instance/validateInstanceDataAuth/'+ instanceId,
		dataType: 'JSON',
		async:false,
		success: function(data) {
			if(data.success) {
...
//	    	}
	    	...
	    	//加载业务表单
				...
				setTimeout(function(){
					...
					//加载流程实例流转图
					//$('#instanceChartIframe').attr('src',hostUrl + 'flow/runtime/approve/flow_chart.html?instanceId='+ $.xljUtils.getUrlParams().instanceId);
				}, 1000);
