...
$(function() {
	...
	if(tendCode != null) {
		$.ajax({
			url: hostUrl + "/sys/thirdPartyAuthentication/setLoginInfo?tendCode=" + tendCode + "&userId=" + $.getUrlParam("uid"),
			type: 'GET',
			async: true,
	...
	if(buttonDataSortArr.length <= 6){
		for(var i = buttonDataSortArr.length-1; i>=0 ;i--) {
			if(buttonDataSortArr[i].name == "print"){
				buttonHtml += "<button type='button' class='btn btn-sm btn-adv' onclick='customPrint()' title='printing/打印' id='print'>printing/打印</button>";
			}else if(buttonDataSortArr[i].name == "shut"){
				$("#shut").show();
			}else{
				buttonHtml += "<button type='button' class='btn btn-sm btn-adv' onclick='"+buttonDataSortArr[i].name+"()' id='"+buttonDataSortArr[i].name+"' title='"+buttonDataSortArr[i].tip+"/"+buttonDataSortArr[i].value+"'>"+buttonDataSortArr[i].tip+"/"+buttonDataSortArr[i].value+"</button>";
			}
		}
	}else{
		for(var i = 3; i>=0 ;i--) {
			buttonHtml += "<button type='button' class='btn btn-sm btn-adv' onclick='"+buttonDataSortArr[i].name+"()' id='"+buttonDataSortArr[i].name+"' title='"+buttonDataSortArr[i].tip+"/"+buttonDataSortArr[i].value+"'>"+buttonDataSortArr[i].tip+"/"+buttonDataSortArr[i].value+"</button>";
			if(buttonDataSortArr[i].name == "print"){
				buttonHtml += "<button type='button' class='btn btn-sm btn-adv' onclick='customPrint()' id='print' title='printing/打印'>printing/打印</button>";
			}
		}
		for(var i = 4; i < buttonDataSortArr.length;i++){
			if(buttonDataSortArr[i].name == "print"){
				aHtml += "<li><a onclick='customPrint()' id='print' title='printing/打印'>printing/打印</a></li>"
			}else if(buttonDataSortArr[i].name == "shut"){
				$("#shut").show();
			}else{
				aHtml += "<li><a onclick='"+buttonDataSortArr[i].name+"();' id='"+buttonDataSortArr[i].name+"' title='"+buttonDataSortArr[i].tip+"/"+buttonDataSortArr[i].value+"'>"+buttonDataSortArr[i].tip+"/"+buttonDataSortArr[i].value+"</a></li>"
			}
		}
		$("#prMore").show();
	}
...
function buttonDataTipFN(){
	...
	buttonDataArrTip["withDrawFlow"] = "revoke process";
...
//		data: list,
		...
		colModel : [
		    ...
		    {name: 'sequence',label : 'order/序号',align:'center', width: 40,sortable:false,editable:false,cellattr:addCellAttr},
			...
			{name: 'acName',label : 'link name/环节名称',width: 70,sortable:false,editable:false,cellattr:addCellAttr},
			...
			{name: 'postName',label : 'post name/岗位',width: 180,sortable:false,editable:false,cellattr:addCellAttr, formatter: postFormatter},
			...
			{name: 'approverName',label : 'incumbent person/责任人',width: 70,editable:false,sortable:false,cellattr:addCellAttr},
			...
			{name: 'approvalType',label : 'operate/操作',width: 70,editable:false,sortable:false,cellattr:addCellAttr, formatter: typeFormatter},
			{name: 'approvalTypeId',label : '审批类型ID',hidden: true,sortable:false,editable:false},
			{name: 'taskComments',label : 'approval suggestion/处理意见',width: 250,editable:false,sortable:false,cellattr:addCellAttr,formatter: taskFormatter},
			{name: 'taskEndTime',label : 'dispose time/处理时间',width: 70,editable:false,sortable:false,cellattr:addCellAttr,formatter: dateFormatter}
...
function initOperation(operations) {
	...
	$.each(operations, function() {
		if(this.operationCode == 'XB') {
			$('<label title="' + this.eName + '/"'+this.showName+'"><input name="approvalResult" type="radio" onclick="radioChange(\'' + this.operationCode + '\', \'' + this.defaultNote + '\')" value="' + this.operationCode + '" />' + this.eName + '/'+this.showName+'</label>'
			+'<input type="hidden" class="form-control" id="XB-noteType" readonly="readonly" ><br>').appendTo('#operate');

			$('<div id="XBDIV" style="display:none" class="input-group"><input type="text" class="form-control" id="xbPersonName" readonly="readonly" placeholder="select/选择人员（多选）">'
				+'<input type="hidden" class="form-control" id="xbPersonId" readonly="readonly" title="select staff(more)/选择人员（多选）" placeholder="select/选择人员（多选）">'
				+'<span class="input-group-addon multiple-selector" langua = "en" data-selectorType="person" data-title="select staff/选择人员" data-targetname="xbPersonName" data-targetid="xbPersonId">...</span></div>').insertAfter('#operate');


			$('#XB-noteType').val(this.noteType);
		} else if(this.operationCode == 'ZB') {
			$('<label title="' + this.eName + '/"'+this.showName+'"><input name="approvalResult" type="radio" onclick="radioChange(\'' + this.operationCode + '\', \'' + this.defaultNote + '\')" value="' + this.operationCode + '" />' + this.eName + '/'+this.showName+'</label>'
			+'<input type="hidden" class="form-control" id="ZB-noteType" readonly="readonly" ><br>').appendTo('#operate');

			$('<div id="ZBDIV" style="display:none" class="input-group"><input type="text" class="form-control" id="zbPersonName" readonly="readonly" placeholder="select/选择人员">'
				+'<input type="hidden" class="form-control" id="zbPersonId" readonly="readonly" title="select staff/选择人员" placeholder="select/选择人员">'
				+'<span class="input-group-addon single-selector" data-selectorType="person" data-title="select staff/选择人员" data-targetname="zbPersonName" data-targetid="zbPersonId">...</span></div>').insertAfter('#operate');


			$('#ZB-noteType').val(this.noteType);
		} else if(this.operationCode == 'WYY' || this.operationCode == 'TY') {
			$('<label  title="' + this.eName + '/"'+this.showName+'"><input name="approvalResult" type="radio" onclick="radioChange(\'' + this.operationCode + '\', \'' + this.defaultNote + '\')" value="' + this.operationCode + '" checked/>' + this.eName + '/'+this.showName+'</label>'
			+'<input type="hidden" class="form-control" id="TY-noteType" readonly="readonly" ><br>').appendTo('#operate');
			$('#approvalText').val(this.defaultNote);
			$('#TY-noteType').val(this.noteType);
		} else if(this.operationCode == 'DH') {
			$('<label title="' + this.eName + '/"'+this.showName+'"><input name="approvalResult" type="radio" onclick="radioChange(\'' + this.operationCode + '\', \'' + this.defaultNote + '\')" value="' + this.operationCode + '" />' + this.eName + '/'+this.showName+'</label>'
			+'<input type="hidden" class="form-control" id="DH-noteType" readonly="readonly" ><br>').appendTo('#operate');
			$('#DH-noteType').val(this.noteType);
			var selector = $('<div class="input-group select-box" id="selectDHRBox" style="display:none"><span class="input-group-addon" title="rebut/打回到">rebut/打回到</span><select id="selectDHR" class="form-control"></select></div>')
			selector.appendTo('#operate');
			$('#noteType').val(this.noteType);
			$('<div id="approveRepeatDiv" style="display:none"><span style="margin-right: 5px;" title="again approval/重新审批">again approval/重新审批</span><br>'
				+ '<input name="approveRepeat" type="radio" value="1" checked title="yes/是"/>yes/是 &nbsp&nbsp&nbsp&nbsp'
				+ '<input name="approveRepeat" type="radio" value="0" title="no/否"/>no/否</div>').appendTo('#operate');
			
		} else {
			$('<label  title="' + this.eName + '/"'+this.showName+'"><input name="approvalResult" type="radio" onclick="radioChange(\'' + this.operationCode + '\', \'' + this.defaultNote + '\')" value="' + this.operationCode + '" />' + this.eName + '/'+this.showName+'</label>').appendTo('#operate');
		}
	});
...
//			approvalSubmit.msgId = $.getUrlParam('msgId');
			approvalSubmit.taskId = item.taskId;
			approvalSubmit.msgId = item.msgId;
			return false;
		}
	});
...
function withDrawTask() {
	var url = hostUrl + 'flow/instance/withDrawTask?instanceId=' + getCurrentInstanceId();
...
function readRecordInit(instanceId) {
	...
	$("#readRecordGrid").jqGrid( {
		...
		colModel : [
			...
			{name: 'userName',label : 'person name/人员名称', align:"center" },
			{name: 'postName',label : 'post name/岗位' , align:"center"},
			{name: 'readDate',label : 'time/时间' , align:"center"}
		],
		forceFit:true,                                      //当为ture时，调整列宽度不会改变表格的宽度。
		// width: '100%',
...
function instanceTransferInit(instanceId) {
	...
	$("#instanceTransferGrid").jqGrid( {
		...
		colModel : [
		    {name: 'id',label : 'ID', hidden: true},
			{name: 'transationUserName',label : 'approval person/审批人', align:"center" },
			{name: 'acName',label : 'link name/环节名称' , align:"center" },
			{name: 'actionName',label : 'operate/操作' , align:"center" },
			{name: 'transationDate',label : 'time/时间', align:"center" }
		],
		forceFit:true,                                      //当为ture时，调整列宽度不会改变表格的宽度。
		// width: '100%',
		width:$("#lcbegin").width()-10,
...
function withDrawFlow() {
	$.ajax({
		type: 'GET',
		url: hostUrl + 'flow/instance/withDrawFlow/' + getCurrentInstanceId(),
		contentType: 'application/json; charset=utf-8',
		dataType: 'json',
		success: function(data) {
			if(data.success) {
...
function modifyReader(){
	var url = encodeURI(hostUrl+"flow/runtime/query/batch_modify_reader.html?instanceId=" + getCurrentInstanceId())
	openWin(url);
}

//调整环节
function modifyAc(){
	var url = encodeURI(hostUrl+"flow/runtime/query/modifyAc.html?&instanceId=" + getCurrentInstanceId() + "&requestSource=adjust");
	openWin(url);
}

//修改审批人
function modifyApprover(){
	var url = encodeURI(hostUrl+"flow/runtime/query/modify_approver.html?instanceId=" + getCurrentInstanceId());
	openWin(url);
}

//修改审批人意见
function modifyApproverAdvice(){
	var url = encodeURI(hostUrl+"flow/runtime/query/modify_advice.html?instanceId=" + getCurrentInstanceId());
	openWin(url);
}
...
function commonModifyInstance(type){
	var url = encodeURI(hostUrl+"flow/runtime/query/modifyInstance.html?type=" + type + "&instanceId=" + getCurrentInstanceId());
	openWin(url);
}
...
function addApproveEnglishTip(){
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(0)").attr("title", "order/序号");
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(1)").attr("title", "link name/环节名称");
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(2)").attr("title", "post name/岗位");
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(3)").attr("title", "incumbent person/责任人");
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(4)").attr("title", "operate/操作");
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(5)").attr("title", "approval suggestion/处理意见");
	$("#approvalListDiv tr.ui-jqgrid-labels th:visible:eq(6)").attr("title", "dispose time/处理时间");
}
//审批日志提示
function addApplogEnglishTip(){
//列头英文悬浮提示
	$("#instanceDiv tr.ui-jqgrid-labels th:visible:eq(0)").attr("title", "approval person/审批人");
	$("#instanceDiv tr.ui-jqgrid-labels th:visible:eq(1)").attr("title", "link name/环节名称");
	$("#instanceDiv tr.ui-jqgrid-labels th:visible:eq(2)").attr("title", "operate/操作");
	$("#instanceDiv tr.ui-jqgrid-labels th:visible:eq(3)").attr("title", "time/时间");
}
//阅读日志提示
function addReadlogEnglishTip(){
	$("#readRecordDiv tr.ui-jqgrid-labels th:visible:eq(0)").attr("title", "person name/人员名称");
	$("#readRecordDiv tr.ui-jqgrid-labels th:visible:eq(1)").attr("title", "post name/岗位");
	$("#readRecordDiv tr.ui-jqgrid-labels th:visible:eq(2)").attr("title", "time/时间");
}