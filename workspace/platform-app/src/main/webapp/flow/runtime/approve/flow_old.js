...
function validateInstanceDataAuth(instanceId,sourceInstanceId) {
	...
	$.ajax({
		type: 'GET',
		url: hostUrl + 'flow/instance/validateInstanceDataAuth/'+ instanceId + urlParam,
		dataType: 'JSON',
		async:false,
		success: function(data) {
			if(data.success) {
...
//	    	}
	    	...
	    	//加载业务表单
				...
				setTimeout(function(){
					...
					//加载流程实例流转图
					$('#instanceChartIframe').attr('src',hostUrl + 'flow/runtime/approve/flow_chart.html?instanceId='+ data.result.instanceId);
				}, 1000);
