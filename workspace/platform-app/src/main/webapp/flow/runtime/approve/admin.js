...
function modifyReader(){
	var url = encodeURI(hostUrl+"flow/runtime/query/modify_reader.html?instanceId="+instanceId);
	openWin(url);
}

//调整环节
function modifyAc(){
	var url = encodeURI(hostUrl+"flow/runtime/query/modifyAc.html?&instanceId=" + $.getUrlParam('instanceId') + "&requestSource=adjust");
	openWin(url);
}
...
function commonModifyInstance(type){
	var url = encodeURI(hostUrl+"flow/runtime/query/modifyInstance.html?type="+type+"&instanceId="+instanceId);
	openWin(url);
}
