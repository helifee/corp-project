...
function submitSettingForm() {
   ...
   var saveType = 'edit';
    ...
    // })
   if(saveType == 'save'){
      url = hostUrl+'flow/overdueSetting/save';
       method = 'POST';
       OverdueSettingDto.id =  getGuuid('id');
   }else if(saveType = 'edit'){
       url = hostUrl+'flow/overdueSetting/update/'+OverdueSettingDto.id+'?_t='+new Date().getTime();
       method = 'PUT';
   }
    $.ajax({
        url:url,
...
function echoData() {
    $.ajax({
        type:'get',
        url: hostUrl+'flow/overdueSetting/get/setting'+"?time="+Math.random(),
        success: function(data) {
            if(data.success){
