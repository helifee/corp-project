...
function getlinkCenterData(){
    jQuery("#list2").jqGrid(
            {
                url: hostUrl+'sys/sysLinkCenter/page',
	...
	var prvid=$("#"+ids).prev().attr("id");
		if(ids&&ids!='') {
			  pop_text_open("blue",'确认要删除这条数据吗？',function(){
					$.ajax({
						url:hostUrl+"sys/sysLinkCenter/deletePseudo/"+ids,
						type:'DELETE',
						dataType:'JSON',
...
function  updateStatus(e){
	...
	$.ajax({
		url:hostUrl+"sys/sysLinkCenter/updateStatus/"+ids,
		type:'PUT',
		dataType:'JSON',
		 contentType : "application/json",  
		success:function (resultData ) {
...
function updateSort(e){
	...
	if(ids){
		...
		$.ajax({
			url:hostUrl+"sys/sysLinkCenter/updateSort/"+ids,
			type:'put',
			dataType:'JSON',
			contentType:"application/json",
