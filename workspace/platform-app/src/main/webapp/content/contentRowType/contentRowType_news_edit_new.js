...
$(function () {
    ...
    function createForm(resultList) {
        ...
        $('#codeType').change(function(){
            ...
            if(codeType){
                ...
                $.ajax({
                    type: "get",
                    url: baseUrl + "oa/content/contentRowType/getCodeByCodeType?code=" + encodeURIComponent(code)+"&codeType="+encodeURIComponent(codeType)+"&time="+Math.random(),
                    dataType: "json",
    ...
    function getDataForUpdate(contentRowTypeId) {
        ...
        $.ajax({
            type: "get",
            url: baseUrl + "oa/content/contentRowType/queryObjectInfoById/" + contentRowTypeId+"?time="+Math.random(),
            dataType: "json",
            success: function (contentObj) {
                var updateData = {};
                if(contentObj.success){
    ...
    function submitForm(saveType) {
        ...
        if (urlParams.oper == 'edit') {
            url = hostUrl + 'oa/content/contentRowType/update/'+ $('#id').val();
            method = 'PUT';
        }
...
//        var formFields = $('#newContentForm :input[name]');
                    ...
                    if(successFlag){
                        ...
                        if((saveType=='temporary'||saveType=='save'||saveType=='publish')){
                            ...
                            if(saveType=='temporary') {
                                window.location.href = hostUrl + "content/contentRowType/contentRowType_news_edit.html?id=" + jsonData.id + "&oper=edit&contentTypeId=" + jsonData.contentTypeId;
                                return;
                            }
                            ...
                            window.location.href = hostUrl + 'flow/runtime/approve/start.html?businessObjectCode='+urlParams.businessObjectCode+'&businessId='+jsonData.id;
    ...
    window.cancelSaveBusiForm = function () {
        var contentRowTypeId = $('#id').val();
        window.location.href = hostUrl + 'content/contentRowType/contentRowType_news_view.html?businessId='+contentRowTypeId+'&time='+new Date().getTime()+
