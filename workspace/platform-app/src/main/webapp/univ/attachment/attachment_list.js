...
;(function($, window, document, undefined) {
	...
	function initAttachmentGrid() {
		...
		$.xljUtils.initJqGrid({
			gridSelecter:'#list2',
			url : hostUrl + 'univ/attachment/attachment/pageByCategoryIds?random='+Date.now(),
			postData : {},
	...
	function download() {
		...
		// 先查询文件存储机器的IP
		$.post(hostUrl + 'univ/attachment/attachment/getStorageIP?random='+Date.now(), 
				{filePath:path},
				function(ip){ 
					if (ip)
...
//		lastZTreeNodeId = zNodes[zNodes.length - 1].id;
	...
	function getAttachmentCategoryTree() {
		$.ajax({
			type:'POST',
			url:hostUrl + 'univ/attachment/attachmentCategory/queryList?random='+Date.now(),
			dataType:'json',
...
//		}
	...
	function deleteCategory() {
		...
		if ($.xljUtils.confirm('blue', '确定要删除附件分类：'+$.xljUtils.htmlEncode(curTreeNode.name) + '？', function(){
			...
			$.xljUtils.xljAjax({
				type:'DELETE',
		        url:hostUrl + 'univ/attachment/attachmentCategory/delete/'+ curTreeNode.id
			}, function(data){
