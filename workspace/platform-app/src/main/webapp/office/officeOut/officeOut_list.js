...
function getOfficeOutData(){
    jQuery("#list2").jqGrid(
            {
                url:hostUrl+'/oa/officeOut/getOfficeOutpage',
...
function deleteOfficeOut(){
	...
	if(status!="草稿"){
	  ...
	  pop_text_open("blue",'确认要删除这条数据吗？',function(){
			$.ajax({
				url:hostUrl+"/oa/officeOut/deletePseudo/"+ids,
				type:'DELETE',
				dataType:'JSON',
		...
		if(row.status!='草稿'){
			...
			var officeOut={
			  ...
			  $.ajax({
				  type: "post",
				  contentType: "application/json",
				  url: hostUrl+"oa/officeOut/updateCountOver",
...
function getOperationAuthorition() {
	...
	$.ajax({
		type: 'GET',
		url: hostUrl + 'sys/authentication/getUserAuthenticationOperation?t_='+new Date().getTime()+'&appCode=OA&menuCode=YPLY',
		dataType: 'json',
		//contentType: 'application/json',
		async: false,
