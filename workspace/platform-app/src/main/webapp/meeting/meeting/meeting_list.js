...
$(function(){
	...
	$(".addMeeting").click(function() {
		 window.open('meeting_edit.html?act=create');
	});
	...
	$(".deleteMeeting").click(function() {
        ...
        if (gr == "" || gr == null) {
            ...
            $.xljUtils.confirm("blue", tipText, function () {
                if (gr && gr != '') {
                    $.ajax({
                        url: baseUrl + "oa/meeting/meeting/deleteBatch/" + gr.join(','),
                        type: 'DELETE',
                        dataType: 'JSON',
	...
	$(".updateMeeting").click(function() {
		...
		if(rowData.status == '审批中'){
			$.xljUtils.tip('blue',"该会议还在审批当中，暂时不能修改！");
            return;
		}
		if (ids != null) {
			window.open("meeting_edit.html?act=update&id="+ids+"&update="+true);
		}else{
			 $.xljUtils.tip("blue", "请选择要修改的会议！");
		}
	});
	...
	$(".copyInputMeeting").click(function() {
		...
		if (ids != null) {
			...
			$.ajax({
				type: 'POST',
				url: baseUrl + 'oa/meeting/meeting/attachment/save',
				contentType: "application/json",
				dataType:'json',
				data:JSON.stringify({appId: '1',businessId: ids,categoryId: '1',newBusinessId:newId}),
				success: function (data) {
					window.open("meeting_edit.html?act=copy&update=copy&id="+ids+"&newId="+newId+"&parentPage=list");
					//window.location.href="meeting_edit.html?act=copy&update=copy&id="+meetingId+"&newId="+newId;
				}
			});
			//window.open("meeting_edit.html?act=copy&update=copy&id="+ids);
		}else{
...
function deleteFavoriteMeeting(gr){
	 if (gr && gr != '') {
         $.ajax({
             url: baseUrl + "oa/favorite/deleteBatch/" + gr.join(','),
             type: 'DELETE',
             dataType: 'JSON',
             success: function (resultData) {
                 if (resultData && resultData.success) {
...
function  initMeetingGridList(){
	jQuery('#allListRowgrid').jqGrid({
		url: hostUrl + "oa/meeting/meeting/page"+"?time="+Math.random(),
		ajaxGridOptions: {
			contentType: 'application/json'
		},
		mtype : "POST",
		...
		colModel:[
			...
			{name:'code',label:'code', index:'code', align:"center", width:55,hidden:true},
			...
			{name:'title',label:'会议主题',index:'title',align:"center",  width:100},
			{name:'createPersonName',label:'创建人',index:'createPersonName', align:"center", width:100},
			{name:'meetingType',label:'会议类别',index:'createOrgId',align:"center",  width:100},
			...
			{name:'beginTime',label:'召开时间',index:'beginTime', align:"center", width:90,
		...
		ondblClickRow:function(rowid){
			...
			//如果是草稿状态，打开的是编辑页面
			if(rowData.status == '草稿'){
				window.open("meeting_edit.html?act=update&id="+rowid+"&update="+true);
			}else{
				window.open("meeting_view.html?act=view&id="+rowid+"&update="+false+"&startTime="+rowData.beginDate);
				/*if(rowData.instanceId&&rowData.instanceId!=''){
					var pcUrl = "/platform-app/meeting/meeting/meeting_flow.html?businessId="+rowid;
...
//搜索查询方法
function query() {
	var status = $('.meeting-status').val();
	//var title = $('.meeting-title').val();
	var title=$("#corname").getInputVal();
	var queryData = {
		mtTitle:title,
		title: title
	};
...
function trackFlow(){
	...
	var ids=$('#allListRowgrid').jqGrid('getGridParam','selarrrow');
	if(!ids||ids.length==0) {
		pop_tip_open("blue","请选择行！");
		return;
	}
	if(ids.length>1){
		pop_tip_open("blue","请选择一行！");
	...
	var row = $('#allListRowgrid').jqGrid('getRowData',ids);

	if(row.status=='草稿'){
		pop_tip_open("blue","草稿状态没有审批流程");
	}else{
		if(row.instanceId == ""){
			pop_tip_open("blue","当前会议没有审批流程");
			return;
		}
		var pcUrl = "/platform-app/meeting/meeting/meeting_flow.html";
