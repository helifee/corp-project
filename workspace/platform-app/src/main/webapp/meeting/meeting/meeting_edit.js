...
$(function() {
	...
	$('.category-selecter').xljSingleSelector({
		...
		treeUrl:hostUrl + 'oa/meeting/meetingType/getTypetree?random=' + Date.now(),// 生成zTree树的请求url,不指定使用默认对应类型的url
...
function submitForm(type, flag) {
	...
	if(type == 'update') {
    	...
    	meetingDto.id=id;
    	url = baseUrl + "oa/meeting/meeting/update/"+id;
	}else {
		meetingDto.delflag=false;
		url = baseUrl+"oa/meeting/meeting/save";
		//暂存操作
		if(type == 'tempSave'){
			url = baseUrl + "oa/meeting/meeting/temporary/save";
		}
	}
		...
		success: function (result) {
			if(result.success) {	//新增
				...
				if(flag === true) {	//保存并新增
					...
					window.location.href = baseUrl + '/meeting/meeting/meeting_edit.html?act=create';
...
function deleteFavoriteMeeting(meetingOldId){
	 if (meetingOldId && meetingOldId != '') {
         $.ajax({
             url: baseUrl + "oa/favorite/delete/" + meetingOldId,
             type: 'DELETE',
             dataType: 'JSON',
...
/*function getUserInfo() {
    ...
    $.ajax({
        url:hostUrl + 'oa/meeting/meeting/getUserInfo?_t='+new Date().getTime(),
        type:'GET',
        //async:false,
        dataType:'JSON',
        success:function (resultData) {
