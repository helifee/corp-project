	...
	pageInit();
	  $('.form_datetime').datetimepicker({
		...
		$('#appName,#selectAppName').on('click', function() {
			var urlBody = "party/integrateApp/getAppTree";
...
function getUserConfigData(id){
	   $.ajax({
	       url:hostUrl+"party/userConfig/getUserConfig/"+id+'?time='+Math.random(),
	       type:'get',
	       success: function(data) {
	    	   if(data.success){
	...
	var userConfigDto={};
		...
		if(type=="add"){
			...
			for(var p in userList){
			   ...
			   $.ajax({
			       url:hostUrl+"party/userConfig/saveBatch",
			...
			var userConfigId=$("#userConfigId").val();
			   $.ajax({
			       url:hostUrl+"party/userConfig/update/"+userConfigId,
			   ...
			   $.ajax({
			       ...
			       url:hostUrl+"party/userConfig/updateBatch/",
			       data:JSON.stringify(userConfigDto),
			       type:'post',
			       contentType:'application/json',
			       dataType:'JSON',
