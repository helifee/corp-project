...
$(document).ready(function(){
	  ...
	  $('#myModal').on('hide.bs.modal', function () {
		...
		$('#companyName,#selectcompanyName').on('click', function() {
			...
			$.ajax({
				type: "post",
				url: hostUrl + "sys/org/orgnazation/queryListCompanyAndZb",
				dataType: "json",
				data:"{}",
		  ...
		  $("#supplierTitle").html("供方档案-新增");
		   ...
		   if(companyId){
			   $("input[name='companyId']").val(companyId);
				$.ajax({
			        type:'get',
			        url:hostUrl+'sys/org/orgnazation/get/'+companyId+'?time='+Math.random(),
...
function getBaseSupplierEcho(id){
	$.ajax({
        type:'get',
        url:hostUrl+'sys/base/baseSupplier/get/'+id+'?time='+Math.random(),
        success: function(data) {
        	if(data.success){
...
function getAppSystem(supplierAppData){
	   $.ajax({
	        type:'post',
	        url:hostUrl+'sys/res/appSystem/queryListToSupplier',
	        contentType:'application/json',
	        data:"{}",
	        success: function(data) {
	        	if(data.success){
...
/*function getcompany(){
	$.ajax({
        type:'post',
        url:hostUrl+'sys/org/orgnazation/queryListCompany',
        dataType:'json',
        async:false,
        contentType:'application/json',
        data:"{}",
	...
	var baseSupplierDto={};
		...
		if(type=="add"){
			baseSupplierDto.messageType=0;
			   $.ajax({
			       url:hostUrl+"sys/base/baseSupplier/save?time="+Math.random(),
			...
			var supplierId=$("#baseSupplierForm").find("input[name='id']").val();
			$.ajax({
			       url:hostUrl+"sys/base/baseSupplier/update/"+supplierId,
			       data:JSON.stringify(baseSupplierDto),
			       type:'PUT',
			       contentType:'application/json',
			       dataType:'JSON',
