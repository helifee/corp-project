	...
	function syncData() {
		...
		if(idsVal&&idsVal!="") {
			if(idsVal.length>1) {
				...
				$.ajax({
					url:hostUrl+"sys/sync/syncData/syncData/"+rowId,
					type:'POST',
					dataType:'JSON',
 ...
 */
	function pageInit(){
	  jQuery("#list").jqGrid(
	      {
	        url : hostUrl+'sys/sync/syncData/page',//创建完成之后请求数据的url
	...
	function addDataSync(){
	 ...
	 function changeState(status){
		 ...
		 if(idsVal&&idsVal!="") {
	         ...
	         $.ajax({
	        	 url:hostUrl+"sys/sync/syncData/update/"+idVal,
	        	 type:'PUT',
	        	 dataType:'JSON',
	        	 contentType:'application/json',
	 ...
	 function delDataSync(){
		 ...
		 if(idsVal&&idsVal!="") {
			 $.xljUtils.confirm("blue", "确认要删除这【" +idsVal.length + "】条数据吗？",function(){
				   $.ajax({
			        	 url:hostUrl+"sys/sync/syncData/deletePseudoBatch/"+idsVal,
// 			        	 url:hostUrl+"/sys/tend/syncData/deleteBatch/"+idsVal,
			        	 type:'DELETE',
			        	 dataType:'JSON',
			        	 contentType:'application/json',
			        	 data:JSON.stringify({}),
