...
function queryAndShowToDoSum(){
	var queryUrlText = "/platform-app/flow/sysNoticeMsg/queryTwoSumData";
	$.ajax({
...
function getToDoData(startPosition, isClearUl, index){
    ...
    queryUrlText = "/platform-app/flow/sysNoticeMsg/searchDataByKeyword"
    if(index==0){
 ...
 var showListFragment = function(dataList,index) {
     ...
     for (var idx = 0; idx < dataList.length; idx++) {
         ...
         str += "<input type='hidden' name='ifMobile' value='"+ dataItem.ifBussnissObjectLock +"'/>";
         $(li).html(str);
         $(fragment).append(li);
     }
...
var keywords = ['2017年度考核', '2017年考核'];
 mui(".mui-table-view").on('tap','.mui-table-view-cell',function(){
     ...
     var ifBussnissObjectLock = $(this).find("input[name='ifMobile']").val();
     if(ifBussnissObjectLock && ifBussnissObjectLock == "false"){
         alert("请在pc端查看");
         return;
     }
