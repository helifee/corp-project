...
$(function () {
    ...
    $("#deletePortalBtn").on('click', function () {
        ...
        $.xljUtils.confirm('blue', tipText, function () {
            ...
            if (ids && ids != '') {
                $.ajax({
                    url: hostUrl + "oa/portal/portalPage/deleteBatch/" + ids,
                    type: 'DELETE',
                    dataType: 'JSON',
    ...
    $("#authorizationPortalBtn").on('click', function () {
        ...
        if (idVal) {
            ...
            //查找站点对应权限
            $.ajax({
                url: hostUrl + 'oa/portal/portalPagePermision/queryList?_t=' + new Date().getTime(),
                type: 'POST',
    ...
    function showRoleTree($ele) {
        $ele.xljMultipleSelector({
            ...
            saveCallback: function (data, ele) {
                ...
                //保存数据
                $.ajax({
                    url: hostUrl + 'oa/portal/portalPagePermision/saveBatchForPortalPermission/' + $(ele).data('portalPageId'),
                    type: 'PUT',
                    contentType: 'application/json',
    ...
    $("#authorizationBtn").on('click', function () {
        ...
        if (portalPagePermisions.length > 0) {
            $.ajax({
                url: hostUrl + 'oa/portal/portalPagePermision/saveBatch',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(portalPagePermisions),
                dataType: 'JSON',
    ...
    function changePortalPageStatus(flag) {
        ...
        $.ajax({
            type: 'POST',
            url: hostUrl +'oa/portal/portalPage/updatePortalStatus',
            dataType: 'json',
            contentType: 'application/json',
...
function initPortalGrid() {
    ...
    $.xljUtils.initJqGrid({
        ...
        url: hostUrl + 'oa/portal/portalPage/page',
