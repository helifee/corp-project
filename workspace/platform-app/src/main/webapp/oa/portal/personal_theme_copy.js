$(function () {
    ...
    function initPortalPage() {
        $.ajax({
            type: 'GET',
            url: hostUrl+'oa/portal/portalPage/get/'+urlParam.portalPageId+'?_time='+new Date().getTime(),
                ...
                success: function (resultData) {
                    if (resultData) {
                        ...
                        if (successFlag) {
                            ...
                            window.location.href = hostUrl + 'oa/portal/personal_theme_copy.html?portalPageId='+result[0].portalPageId+'&_time='+new Date().getTime();
                        } else {
        ...
        if(originPortalPageId!=''){
            ...
            $.xljUtils.confirm('blue', '确定要恢复系统默认首页吗？', function () {
                if (portalPageId && portalPageId != '') {
                    $.ajax({
                        url: baseUrl + "oa/portal/portalPage/delete/" + portalPageId,
                        type: 'DELETE',
                        dataType: 'JSON',
                        success: function (resultData) {
                            if (resultData && resultData.success) {
                                ...
                                window.location.href = hostUrl + 'oa/portal/personal_theme_copy.html?portalPageId='+originPortalPageId+'&_time='+new Date().getTime();
                            }
                        }
                    });
                }
    ...
    function initPortalPageComponents() {
        $.ajax({
            type: "POST",
            url: baseUrl + "oa/portal/portalComponentPosition/queryList?_t=" + new Date().getTime(),
