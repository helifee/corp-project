...
//		targetName:'parentName',//选择的数据的Name存储input域
    ...
    if (oper == "add") {
        ...
        $('#forumTypeForm').attr('action', baseUrl + 'oa/bbs/forumType/save');
        //初始化UUID
        $.ajax({
            type: "GET",
...
function editForumType(id) {
    ...
    $.ajax({
        type: 'GET',
        url: baseUrl + "oa/bbs/forumType/get/" + id + '?time=' + Math.random(),
        //data: JSON.stringify({}),
        dataType: "json",
    ...
    $('#forumTypeForm').attr('action', baseUrl + 'oa/bbs/forumType/update/' + id);
